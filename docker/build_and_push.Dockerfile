...
# Copy langflow-core metadata since it is a workspace member. The full image
# does not install this sibling distribution, but uv must discover every
# workspace member before resolving the root project.
