    ...
    injectHtmlTags() {
      ...
      return {
        headTags: [
          ...
          // TrustArc Consent Wrapper Script
          {
            tagName: 'script',
            attributes: {
          ...
          // Initialize Segment with TrustArc Wrapper
          {
            tagName: 'script',
            innerHTML: `
              (function() {
                function initSegmentWithConsent() {
                  if (typeof TrustArcWrapper !== 'undefined' && window.analytics) {
                    TrustArcWrapper.withTrustArc(window.analytics).load('${segmentPublicWriteKey}');
                  } else {
                    console.error('TrustArc consent wrapper failed to load. Analytics will not be initialized to ensure consent compliance.');
                  }
                }

                // Wait for both TrustArc wrapper and analytics to be ready
                if (document.readyState === 'complete') {
                  initSegmentWithConsent();
                } else {
                  window.addEventListener('load', initSegmentWithConsent);
                }
              })();
            `,
          },
