...
> (4) **Agentic MCP server over HTTP** — the `langflow-agentic` server (previously stdio-only; supersedes the stdio-only framing in ADR-MCP-007) is now also mounted at `POST/GET/DELETE /api/v1/agentic/mcp` (streamable-http, stateless; `api/v1/agentic_mcp.py`) with the same `CurrentActiveMCPUser` auth as the flows MCP endpoints, gated by `LANGFLOW_AGENTIC_EXPERIENCE` (404 when off). `user_id` is stripped from advertised tool schemas and always overwritten with the authenticated user — a caller cannot impersonate another tenant. The stdio path is untouched.
> (5) **`run_assistant` progress streaming** — the tool accepts a FastMCP-injected `Context` (not in the tool schema; signature/return backward compatible) and forwards each assistant `progress` SSE event as an MCP progress notification (`send_progress_notification` with `related_request_id`, so the stateless streamable-http transport delivers it) plus a `ctx.info` log line. Forwarding failures are swallowed — progress can never break the tool.
