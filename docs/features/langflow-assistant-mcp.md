...
> (1) **`add_connection` can now express the Loop feedback edge** — when the target port is an `allows_loop=True` OUTPUT (e.g. `Loop.item`), `lfx/graph/flow_builder/connect.py` emits the output-shaped targetHandle `{dataType, id, name, output_types}` the runtime's loop branch requires, with accepted types mirroring the canvas (`[selected ?? types[0], ...loop_types]`). Template inputs keep priority on name collisions; structural idempotency, `remove_connection`, and `list_connections` all recognize loop edges. Every builder tool inherits this: `connect_components`, `create_flow_from_spec`, `build_flow`, `update_flow_from_spec`. `add_connection` also pins the source node's `output_type` tab to the wired output (`sync_dropdown_selected_outputs`), which previously ran only in `build_flow_from_spec`. Tab-driven components (TypeConverter) rebuild their whole `outputs` list from that tab on re-hydration, so an edge leaving `data_output` while the tab still read `Message` lost its source output on the next load and the edge was dropped and persisted — destroying any loop wired through one. Pinning at the connect layer covers the incremental edit tools and the external MCP tools, which never reach the spec builder (LE-1776).
...
>
> (5b, LE-2324) The pinned default is now also settable per deployment via **`LANGFLOW_ASSISTANT_ITERATIONS`** (precedence: per-request `/iterations N` > env > pin, clamped `[1,200]`), because `/iterations N` only tunes one browser session. And exhausting the budget no longer discards the turn's work: the terminal error branch drains pending flow events, emits them as `flow_update`, and says the partial flow is kept on the canvas — so an interrupted multi-stage build is resumable instead of a full stop.
...
| **DismissedPlanStash** | Hook-local ref (`dismissedPlanMarkdownRef`) holding the last dismissed plan markdown. One-shot: consumed by the first `handleSend` that carries it (so a turn that never replans cannot resend a stale plan), and cleared by `handleResetPlan`, by the next `propose_plan` event, and on `handleClearHistory` / `loadSession`. | `use-assistant-chat.ts` |
...
| **`GenerateComponent`** | MCP tool that re-enters the **full component validation pipeline** mid-loop, registers the resulting user component, and returns its `class_name` so a subsequent `SearchComponentTypes` finds it. The single-loop equivalent of the standalone `generate_component` intent, callable as a step inside a compound build. Its `spec` is **assistant-authored, not a user turn**, so it re-enters with `trusted_source=True`: the injection guardrail already ran on the user turn at the door, and re-running it here rejected in-scope builds whose spec names the attacks a guardrail component must catch (LE-2323). Content safety and normalization still apply. | `GenerateComponent` (class) in `src/lfx/src/lfx/mcp/flow_builder_tools/` |
...
- **Invariants**:
  ...
  - The first `handleSend` after Dismiss prepends the stashed markdown via `buildRefinementInput()` and consumes the stash in the same call — a refinement turn that never replans cannot re-prepend a stale plan. Also cleared by `handleResetPlan`, by the next `propose_plan` event, and by `handleClearHistory` / `loadSession`.
