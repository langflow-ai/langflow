...
| **ModelUnavailableMarker** | Substring (case-insensitive) used by `is_model_unavailable_error` to identify model_not_found-class errors: `"model_not_found"`, `"no longer available to new users"`, `"does not have access to model"`, `"model is not available"`, `"the model does not exist"`, `"model not available"`, `"no access to model"`, `"not found (status code: 404)"`, `"requires a subscription"`. `"no longer available to new users"` covers a model a provider retires for newly issued keys while still listing it, so only the call fails; it is matched as the full phrase because a bare `"no longer available"` also describes retired non-LLM resources (a knowledge base's embedding model, a withdrawn tool) that the model-fallback chain cannot rescue | `_MODEL_UNAVAILABLE_MARKERS` in `helpers/error_handling.py` |
