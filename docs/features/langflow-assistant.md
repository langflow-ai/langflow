...
| **ModelUnavailableMarker** | Substring (case-insensitive) used by `is_model_unavailable_error` to identify model_not_found-class errors: `"model_not_found"`, `"no longer available"`, `"does not have access to model"`, `"model is not available"`, `"the model does not exist"`, `"model not available"`, `"no access to model"`, `"not found (status code: 404)"`, `"requires a subscription"`. `"no longer available"` covers a model a provider retires for newly issued keys while still listing it, so only the call fails | `_MODEL_UNAVAILABLE_MARKERS` in `helpers/error_handling.py` |
...
2. On `FlowExecutionError`, if `is_model_unavailable_error(e.original_error_message)` matches a curated denylist of substrings (`"model_not_found"`, `"no longer available"`, `"does not have access to model"`, `"model is not available"`, `"the model does not exist"`, `"model not available"`, `"no access to model"`, `"not found (status code: 404)"`, `"requires a subscription"`) AND a provider is known, the orchestrator picks the next candidate from `get_provider_model_candidates(provider)`, logs `assistant.model_fallback from=... to=... provider=... tried_so_far=[...]`, swaps `model_name`, sets `swap_requested = True`, and re-runs THIS attempt — without consuming a slot from the outer validation-retry budget.
...
`ASSISTANT_PREFERRED_MODELS` (in `agentic/services/provider_service.py`) declares an ordered preference per provider; `get_default_model` picks the first name the provider actually offers, constrained to installed models for live providers (Ollama/WatsonX/OpenRouter). Providers not listed — and names that no longer exist — fall back to the catalog default. Every entry must be a name the catalog still offers: deprecated names are filtered out before the lookup, so listing one silently shortens the preference list instead of acting as a fallback (LE-2310, where Google's second entry `gemini-1.5-pro` was deprecated). A guard test mirrors the frontend's `classifyModelStrength` and fails if any provider's default ever regresses to a weak model; a second guard asserts every preferred name is still offered.
