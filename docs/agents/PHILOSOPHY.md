...
4. **`lfx` is the runtime, `langflow-base` is the platform, `langflow-core` is the service-complete profile, and `langflow` is the full distribution.** Built-in core components and the graph engine live in `lfx` (`src/lfx/src/lfx/components/`); provider components live in independently installable packages under `src/bundles/`. API, auth, persistence, and multi-user concerns live in `langflow-base`. `langflow-core` combines `langflow-base[complete]` with the product-aligned CLI while remaining free of provider bundles. The full `langflow` distribution consumes core and adds the curated bundles. New code goes in the lowest layer that can host it. Package dependencies are enforced one-way: `langflow` → `langflow-core` → `langflow-base` → `lfx`. See [ARCHITECTURE.md](./ARCHITECTURE.md).
