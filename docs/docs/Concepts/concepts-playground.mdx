...
If your flow has a **Chat Input** component, enter a prompt to trigger the flow and start a chat session.

If your flow requires another type of input, such as a webhook or file upload, [use the Langflow API to trigger the flow](/api-flows-run), and then open the **Playground** to review the chat history.

![Playground window](/img/playground.png)


:::tip
If there is no input field in the Playground, make sure your flow has an [Input component](/components-io) that is connected to the **Input** port of another component.

This is because the **Playground** is designed for flows that use an LLM in a query-and-response format, such as chatbots and agents.
To be fully supported by the **Playground**'s chat interface, the flow must have **Chat Input**, **Language Model**, and **Chat Output** components.
:::
...
### Review Agent logic in the Playground
...
1. Click **Controls** in the [component's header menu](/concepts-components#configure-a-component).
2. Enable **Session ID**.
3. Click **Close**.
...
If your flow requires image data, the **Playground** supports base64 images in the following formats:
...
You can provide images to the **Playground** in the visual editor or through the Langflow API.
...
When you trigger a flow with the Langflow API, use the `files` parameter to attach the base64-encoded image data to the request payload:
