...
OAuth integration allows your Langflow MCP server to authenticate users and applications through any OAuth 2.0 compliant service. When users or applications connect to your MCP server, they'll be redirected to your chosen OAuth provider to authenticate, and upon successful authentication, they'll be granted access to your flows as MCP tools.
...
During this setup, you'll register your Langflow server as an OAuth client and obtain the required credentials.
The example values use [GitHub OAuth](https://github.com/settings/developers), but your deployment may vary.

<details closed>
<summary>Where do I find the OAuth values?</summary>
...
3. To complete MCP server setup, click **Save**.
Your MCP server's **JSON** code snippets and **Auto install** configuration are automatically updated with OAuth values.

4. After enabling OAuth on your MCP server, you need to update your MCP client configuration to use the new OAuth settings.
