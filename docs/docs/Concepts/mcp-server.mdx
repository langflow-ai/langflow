...
import PartialOauthSuccess from '@site/docs/_partial-oauth-success.mdx';
...
1. Install [Cursor](https://docs.cursor.com/get-started/installation), [Claude](https://claude.ai/download), or [Windsurf](https://windsurf.com/download/editor) local to your Langflow server.

    :::info
    The auto install option is available only for specific MCP clients.
    The client must be installed locally to your Langflow server.
...
Each Langflow [Project](/concepts-flows#projects) has its own isolated MCP server with authentication configured per project.

You can selectively expose individual flows within your project as tools, choosing which specific flows to make available to the MCP client.

Langflow MCP servers support these authentication types:

- **API Key**: Use this to authenticate using a Langflow API key.
- **OAuth**: Use this to integrate with enterprise identity providers and OAuth 2.0 services for enhanced security.
- **None**: No authentication required - Use this for development or when your Langflow server is running in a trusted environment.

To configure authentication, in the **MCP Server** tab, click <Icon name="Fingerprint" aria-hidden="true"/> **Add Auth** and follow the steps below.
...
Your MCP server's JSON file is updated with values for OAuth.
...
Only use this option in development or in trusted environments.
