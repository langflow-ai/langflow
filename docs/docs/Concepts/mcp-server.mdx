...
If your Langflow server requires authentication ([`LANGFLOW_AUTO_LOGIN=false`](/environment-variables#LANGFLOW_AUTO_LOGIN)), then you must supply a [Langflow API key](/api-keys-and-authentication) in your MCP client configuration.
