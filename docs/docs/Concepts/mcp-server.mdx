...
You can selectively expose flows as tools, choosing which specific flows to make available.

Langflow MCP servers support these authentication types:

- **API Key**: Use this when you want to authenticate using a Langflow API key.
- **OAuth**: Use this to integrate with enterprise identity providers and OAuth 2.0 services for enhanced security.
- **None**: No authentication required (public endpoint) - Use this for development or when your Langflow server is running in a trusted environment without authentication enabled.
...
OAuth authentication allows integration with enterprise identity providers and OAuth 2.0 services.
When OAuth is enabled, Langflow automatically starts an [MCP Composer](https://pypi.org/project/mcp-composer) instance for your project, creating a secure proxy between MCP clients and your flows.

To configure OAuth authentication:

1. In the **MCP Server** tab, click <Icon name="Settings2" aria-hidden="true"/> **Add Auth**.
2. Select **OAuth** as the authentication type.
3. Configure the OAuth settings with the values from your OAuth deployment:
