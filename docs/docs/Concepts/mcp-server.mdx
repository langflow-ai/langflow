...
When authenticating your MCP server with an Langflow API key, your project's MCP server **JSON** code snippets automatically include the `--headers` and `x-api-key` arguments.
