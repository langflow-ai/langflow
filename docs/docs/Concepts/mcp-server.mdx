...
When OAuth is enabled, Langflow automatically starts an MCP Composer [MCP Composer](https://pypi.org/project/mcp-composer) instance for your project, creating a secure client-side proxy between MCP clients and the `mcp-proxy` on your server.
...
4. If you already installed your Langflow MCP server in your MCP client, you must update your MCP client configuration to use the new OAuth settings after enabling OAuth on your MCP server.
    ...
    If your application doesn't display the OAuth popup, try restarting the MCP client to retrieve the updated configuration.
</TabItem>
