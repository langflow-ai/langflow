...
The `LanguageModel` type is a specific data type can be produced by language model components and accepted by components that use an LLM.
...
Tools can be other components where you enabled **Tool Mode**, they can be the dedicated **MCP Tools** component, or other components that only support **Tool Mode**.
