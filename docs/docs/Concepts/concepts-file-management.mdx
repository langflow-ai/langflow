...
3. In the **File** component's tweaks, enable the **Expose Input** toggle to expose the **Files** field as an API input.
4. The **API access** pane's code snippet is automatically updated to include the tweak to the file component.
Langflow will now accept files uploaded to the **File component**.
