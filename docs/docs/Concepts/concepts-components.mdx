...
1. To select multiple components, use one of the following methods:  
   – Hold <kbd>Shift</kbd>, then click and drag to draw a selection box. Components must be entirely within the selection area to be selected.  
Alternatively, to select components for merging individually, hold <kbd>Cmd</kbd>, and then select the components.
2. Once selected, click **Group** to merge the components into a single, group component.
