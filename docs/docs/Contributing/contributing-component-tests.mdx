...
The inherited `test_latest_version` test runs every output of the component with `default_kwargs` and fails if an output returns `None`. It runs offline, so an output that opens a network connection fails the test. If an output can't run offline, mock the external call. If mocking isn't practical, override the optional `skipped_outputs` fixture to name the output and the reason:
