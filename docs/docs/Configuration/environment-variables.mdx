...
| `LANGFLOW_SKIP_AUTH_AUTO_LOGIN` | Boolean | False | See [Authentication variables](/api-keys-and-authentication#authentication-environment-variables). |
...
| `LANGFLOW_CORS_ALLOW_HEADERS` | List[String] or String | `*` | The allowed headers for CORS requests. Can be a comma-separated string or list of headers like `Content-Type,Authorization`. Use `*` to allow all headers. |
| `LANGFLOW_CORS_ALLOW_METHODS` | List[String] or String | `*` | The allowed HTTP methods for CORS requests. Can be a comma-separated string or list of methods like `GET,POST,PUT,DELETE`. Use `*` to allow all methods. |
| `LANGFLOW_CORS_ORIGINS` | String | `*` | Comma-separated list of allowed CORS origins. Use `*` for wildcard, or specify specific origins like `https://yourdomain.com,https://anotherdomain.com`. |
