...
For information about authentication and security environment variables, see [API keys and authentication](/api-keys-and-authentication).
...
| `LANGFLOW_DISABLE_TRACK_APIKEY_USAGE` | Boolean | False | Whether to track API key usage (`total_uses` and `last_used_at`) in the database. If true, API key usage tracking is disabled, which can help avoid database contention during periods of high concurrency. |
...
| `LANGFLOW_MAX_VERTEX_BUILDS_PER_VERTEX` | Integer | `2` | Maximum number of builds to keep per vertex. Older builds are deleted. |

### Flow and project management {#flow-and-project-management}
...
| `LANGFLOW_BUNDLE_URLS` | List[String] | `[]` | A list of URLs from which to load component bundles and flows. Supports GitHub URLs. If `LANGFLOW_AUTO_LOGIN=True`, flows from these bundles are loaded into the database. |
| `LANGFLOW_LOAD_FLOWS_PATH` | String | Not set | Path to a directory containing flow JSON files to be loaded on startup. Note that this feature only works if `LANGFLOW_AUTO_LOGIN=True`. |
...
| `LANGFLOW_REMOVE_API_KEYS` | Boolean | False | Set to true to automatically remove API keys and tokens from flow data before saving a flow to the database. If a field is marked as a password field _and_ it contains `api`, `key`, or `token` in its name, then the field's value is set to `null` before the flow is saved. This prevents credentials from being stored in the database. |

### Global variables and environment variables interaction

For detailed information about the relationship between Langflow global variables and environment variables, as well as environment variables that control handling of global variables, see [Global variables](/configuration-global-variables).
...
| `LANGFLOW_DOWNLOAD_WEBHOOK_URL` | String | Not set | Webhook URL for Langflow store "download" events. |
| `LANGFLOW_LIKE_WEBHOOK_URL` | String | Not set | Webhook URL for Langflow store "like" events. |
