...
See [Memory management options](/memory).

### File and data management

| Variable | Format | Default | Description |
|----------|--------|---------|-------------|
| `LANGFLOW_CONFIG_DIR` | String | Varies | Set the Langflow configuration directory where files and logs are stored. Default path depends on your installation. If `LANGFLOW_SAVE_DB_IN_CONFIG_DIR=True` then the [Langflow database](/memory) is also stored at this path. |
...
See [Configure log options](/logging#log-storage) and [Logs endpoints](/api-logs).
