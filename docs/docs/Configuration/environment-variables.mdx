...
Langflow supports many environment variables for configuration. They are organized by category below for easier reference.
...
For detailed information about authentication configuration, see [API keys and authentication](/api-keys-and-authentication#authentication-configuration-variables).
...
| Variable | Format | Default | Description |
|----------|--------|---------|-------------|
| `LANGFLOW_LOG_LEVEL` | String | `INFO` | Set the logging level for Langflow. Possible values: `DEBUG`, `INFO`, `WARNING`, `ERROR`, `CRITICAL`. See [Logging configuration](/logging#configure-log-options). |
| `LANGFLOW_LOG_FILE` | String | Not set | Path to the log file. If this option isn't set, logs are written to stdout. See [Logging configuration](/logging#configure-log-options). |
| `LANGFLOW_LOG_ENV` | String | `default` | Controls log output and formatting. See [Logging configuration](/logging#configure-log-options). |
| `LANGFLOW_LOG_ROTATION` | String | `1 day` | Controls when the log file is rotated, either based on time or file size. See [Logging configuration](/logging#configure-log-options). |
...
| `LANGFLOW_ALEMBIC_LOG_FILE` | String | `alembic/alembic.log` | Path to the Alembic log file for SQLAlchemy migrations. |

For detailed information about logging configuration, see [Logs](/logging#configure-log-options).
...
For detailed information about file management configuration, see [Manage files](/concepts-file-management#file-management-configuration).
...
For detailed information about flow configuration, see [Build flows](/concepts-flows#flow-configuration).
...
### Global variables and environment integration
...
For MCP server environment variables, see [Use Langflow as an MCP server](/mcp-server#mcp-server-configuration).
