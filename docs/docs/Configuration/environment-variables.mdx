...
| `LANGFLOW_CORS_ALLOW_METHODS` | List[String] or String | `*` | Allowed HTTP methods for CORS requests. Can be a comma-separated string or list of methods like `GET,POST,PUT,DELETE`. Use `*` for all methods. |
| `LANGFLOW_CORS_ALLOW_HEADERS` | List[String] or String | `*` | Allowed headers for CORS requests. Can be a comma-separated string or list of headers like `Content-Type,Authorization`. Use `*` for all headers. |
