...
| <Link id="LANGFLOW_ENABLE_SUPERUSER_CLI"/><span class="env-prefix">LANGFLOW_</span>ENABLE_SUPERUSER_CLI | Boolean | `true` | Allow creation of superusers via CLI. Set to `false` in production for security.<br/>See [`superuser` command](./configuration-cli.mdx#superuser). |
