...
| `LANGFLOW_KNOWLEDGE_BASES_DIR` | String | `~/.langflow/knowledge_bases` | Set the directory to store knowledge bases. The default path depends on your operating system. See [Knowledge base storage locations](/components-kb#knowledge-base-storage-locations). |
...
LANGFLOW_CONFIG_DIR=/path/to/config/
LANGFLOW_DATABASE_URL=postgresql://user:password@localhost:5432/langflow
LANGFLOW_KNOWLEDGE_BASES_DIR=/path/to/knowledge/bases
LANGFLOW_DEV=False
LANGFLOW_FALLBACK_TO_ENV_VAR=False
LANGFLOW_HEALTH_CHECK_MAX_RETRIES=5
LANGFLOW_HOST=localhost
