...
| `LANGFLOW_TRANSACTIONS_STORAGE_ENABLED` | Boolean | True | Whether to enable transaction logging to the database. If false, disables the storage of transaction logs to reduce database load and avoid potential deadlocks under high concurrency. |
