...
| `LANGFLOW_CORS_ALLOW_CREDENTIALS` | Boolean | False | Whether to allow credentials (cookies, authorization headers) in CORS requests. Only works when `LANGFLOW_CORS_ORIGINS` is set to specific origins (not `*`). |
