...
| `LANGFLOW_COMPONENTS_PATH` | String | Not set | Path to the directory containing custom components. See [`--components-path`](./configuration-cli.mdx#run-components-path). |
...
| `LANGFLOW_FRONTEND_PATH` | String | Not set | Path to the frontend directory containing build files. This is for development purposes only. See [`--frontend-path`](./configuration-cli.mdx#run-frontend-path). |
...
| `LANGFLOW_LOG_LEVEL` | String | `critical` | Set the logging level for Langflow. Possible values: `debug`, `info`, `warning`, `error`, `critical`. |
| `LANGFLOW_LOG_FILE` | String | `logs/langflow.log` | Path to the log file. If this option is not set, logs are written to stdout. |
...
| `LANGFLOW_STORE` | Boolean | `true` | Enable the Langflow Store features. See [`--store`](./configuration-cli.mdx#run-store). |
...
| `LANGFLOW_USE_NOOP_DATABASE` | Boolean | `false` | Use a no-op database, which avoids database connections and operations. Useful for running flows without a database. |
| `LANGFLOW_WEBHOOK_POLLING_INTERVAL` | Integer | `5000` | Set the polling interval for the webhook in milliseconds. |
| `LANGFLOW_DESKTOP` | Boolean | `false` | Indicates if Langflow is running in desktop mode. |
| `LANGFLOW_FEATURE_MVP_COMPONENTS` | Boolean | `false` | Enable MVP components feature flag. |
| `LANGFLOW_FEATURE_MCP_COMPOSER` | Boolean | `false` | Enable MCP composer feature flag. |
