...
1. **Langflow CLI options** override all other sources (highest priority)
2. **`.env` file** overrides system environment variables
3. **System environment variables** are used only if not set elsewhere (lowest priority)
...
When using Docker, the `-e` flag sets system environment variables, which follow the same precedence rules.
