...
* [Authentication environment variables](#authentication-configuration-values): These environment variables configure how Langflow handles user authentication and authorization.

## Langflow API keys

You can use Langflow API keys to interact with Langflow programmatically.

The API key has the same permissions and access as the user who created it. This means your API key can only access your own flows, components, and database. You cannot access other users' resources with your own Langflow API keys.
...
If you create a key as a regular user, it will have regular user privileges.
...
By default, this variable is set to `False`, and requires new users to be activated by a superuser.
```text
...
This section shows you how to configure Langflow with proper authentication using the variables described in [Authentication environment variables](/api-keys-and-authentication#authentication-environment-variables).

Once you are logged in as a Langflow superuser, create a new user on your server and [Create a Langflow API key](#create-a-langflow-api-key) for secure programmatic access.

With authentication enabled, users must log in with valid credentials, and you'll need to create Langflow API keys for programmatic access to the API. This configuration is recommended for any deployment where multiple users will access the system or when exposing Langflow to a network.
...
2. Optionally, generate a secret key for encrypting sensitive data.
Langflow creates a secret key automatically.
...
The default values are `langflow` and `langflow`. The defaults are only used if you don't set the environment variables.
...
You have started a Langflow server with authentication enabled.