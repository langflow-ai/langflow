...
Cross-Origin Resource Sharing (CORS) configuration affects how authentication credentials are handled when your Langflow frontend and backend are served from different origins (domains, ports, or protocols).

The default settings below allow all origins (`*`) and disable credentials for security.
...
- Authentication cookies and authorization headers are not sent with requests
- API requests must include the `x-api-key` header for authentication

For production deployments where your frontend and backend are on different origins, configure CORS to allow specific origins and enable credentials:
...
- Only your specified domains can make requests to the server
- Cookies and authorization headers are sent with requests
- Supports session-based authentication in addition to API key authentication

With CORS credentials enabled, you can use JWT authentication for your backend requests with a JWT token from your authentication service.

```bash
curl -X POST \
  "https://your-langflow-backend.com/api/v1/run/your-flow-id" \
  -H "Content-Type: application/json" \
...
For more information about CORS environment variables, see [Environment variables](/environment-variables#supported-variables).
