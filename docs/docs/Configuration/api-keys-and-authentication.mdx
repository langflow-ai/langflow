...
In Langflow version 1.6 and later, the `LANGFLOW_SKIP_AUTH_AUTO_LOGIN` option has been removed.
When `LANGFLOW_AUTO_LOGIN=True`, all Langflow API requests require a valid Langflow API key for authentication.
The only exceptions are the MCP endpoints (`/v1/mcp`, `/v1/mcp-projects`, and `/v2/mcp`), which inherit authentication settings from your Langflow projects' [MCP servers](/mcp-server).
</details>
