...
- Authentication cookies and authorization headers aren't sent with requests.
- API requests must include the `x-api-key` header for authentication.

Consider specifying exact origins in `LANGFLOW_CORS_ORIGINS` for production deployments.
...
In a future release, when using wildcard origins, credentials will be automatically disabled.
