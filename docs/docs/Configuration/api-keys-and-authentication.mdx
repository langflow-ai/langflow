...
Controls whether new users are automatically activated when they register.

* If `LANGFLOW_NEW_USER_IS_ACTIVE=True`, new users are automatically activated and can immediately access the Langflow server.
* If `LANGFLOW_NEW_USER_IS_ACTIVE=False`, new users must be manually activated by a superuser before they can access the server.

This setting is only relevant when authentication is enabled (`LANGFLOW_AUTO_LOGIN=False`).
...
|----------|--------|---------|-------------|
| `LANGFLOW_AUTO_LOGIN` | Boolean | True | Controls whether authentication is required to access your Langflow server. See [`LANGFLOW_AUTO_LOGIN`](#langflow-auto-login). |
| `LANGFLOW_SKIP_AUTH_AUTO_LOGIN` | Boolean | True | Temporary bypass for backwards compatibility. See [`LANGFLOW_AUTO_LOGIN`](#langflow-auto-login). |
| `LANGFLOW_SECRET_KEY` | String | Automated | Secret encryption key for encrypting sensitive data. See [`LANGFLOW_SECRET_KEY`](#langflow-secret-key). |
| `LANGFLOW_SUPERUSER` | String | `langflow` | Username for the Langflow server's superuser. See [`LANGFLOW_SUPERUSER` and `LANGFLOW_SUPERUSER_PASSWORD`](#langflow-superuser). |
| `LANGFLOW_SUPERUSER_PASSWORD` | String | `langflow` | Password for the Langflow server's superuser. See [`LANGFLOW_SUPERUSER` and `LANGFLOW_SUPERUSER_PASSWORD`](#langflow-superuser). |
| `LANGFLOW_ENABLE_SUPERUSER_CLI` | Boolean | True | Controls the availability of the `langflow superuser` command. See [`LANGFLOW_ENABLE_SUPERUSER_CLI`](#langflow-enable-superuser-cli). |
| `LANGFLOW_NEW_USER_IS_ACTIVE` | Boolean | False | Controls whether new users are automatically activated. See [`LANGFLOW_NEW_USER_IS_ACTIVE`](#langflow-new-user-is-active). |
...
For more information about all available environment variables, see [Environment Variables](/environment-variables#authentication-and-security).