...
By default, Langflow API endpoints require authentication with a Langflow API key.
...
This variable controls whether authentication is required to access your Langflow server, including the visual editor and Langflow CLI:
...
If false, the Langflow team recommends that you also set [`LANGFLOW_SUPERUSER` and `LANGFLOW_SUPERUSER_PASSWORD`](#langflow-superuser) to avoid using the insecure default values.

* If `LANGFLOW_AUTO_LOGIN=True` (default), Langflow bypasses authentication for the visual editor and the Langflow CLI, but not for Langflow API requests.
If you don't have user management enabled, all users are effectively superusers in the visual editor and Langflow CLI.
All users access the same visual editor environment without password protection.
If you also want to bypass authentication for Langflow API requests, see [`LANGFLOW_SKIP_AUTH_AUTO_LOGIN`](/api-keys-and-authentication#langflow-skip-auth-auto-login).
...
This enforces authentication for Langflow API requests, the Langflow CLI, and the visual editor.
...
At that point, Langflow will strictly enforce API key authentication for API requests, and you can manually disable authentication for the CLI and visual editor by setting `LANGFLOW_AUTO_LOGIN=True`.

<details>
<summary>Authentication enforcement in earlier versions</summary>

Langflow version 1.5 was the first version that could enforce Langflow API key authentication for Langflow API requests, even if `LANGFLOW_AUTO_LOGIN=True`.
As a temporary bypass for backwards compatibility, this version also added the `LANGFLOW_SKIP_AUTH_AUTO_LOGIN` environment variable.
Setting both `LANGFLOW_AUTO_LOGIN` and `LANGFLOW_SKIP_AUTH_AUTO_LOGIN` to true reverted to the fully unauthenticated, all-superuser behavior from earlier versions.

In Langflow versions earlier than 1.5, Langflow API requests didn't require authentication.
Additionally, the default setting of `LANGFLOW_AUTO_LOGIN=True` bypassed authentication for the Langflow CLI and visual editor, automatically granting all users superuser privileges.
</details>
...
### LANGFLOW_CORS_ORIGINS and LANGFLOW_CORS_ALLOW_CREDENTIALS {#cors-configuration-for-authentication}
...
The current default configuration allows all origins (`*`) with credentials enabled, which can be a security risk in production environments.
...
The default settings enable the following:
- Any website can make requests to your Langflow API.
- Any website can include credentials in cross-origin requests, including authentication cookies and authorization headers.

Production deployments should specify exact origins in `LANGFLOW_CORS_ORIGINS`.
...
In a future release, if you configure CORS with a wildcard origin using `LANGFLOW_CORS_ORIGINS=*`, the Langflow server will automatically disable credentialed requests such as cookies, authorization headers, and TLS client certificates.
This change prevents insecure configurations where credentials could be exposed to all origins.

For more information about all Langflow environment variables, see [Environment variables](/environment-variables#supported-variables).
