...
1. In your `.env` file, make sure to set the `LANGFLOW_ENABLE_SUPERUSER_CLI` [environment variable](./environment-variables.mdx) to `false`.
    Superuser privileges are required to create API keys with the Langflow CLI. If `LANGFLOW_ENABLE_SUPERUSER_CLI` is `false`, you can't create superusers with the Langflow CLI, and you can't create API keys if no superuser exists.
2. Set `AUTO_LOGIN=FALSE`, and then set superuser credentials for your server.

3. To confirm your superuser status, call [`GET /users/whoami`](/api-users#get-current-user), and then check that the response contains `"is_superuser": true`:
