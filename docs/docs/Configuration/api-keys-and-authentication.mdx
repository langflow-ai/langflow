...
:::tip
* Tracking of API key usage is controlled by the [`LANGFLOW_DISABLE_TRACK_APIKEY_USAGE`](/environment-variables#database-configuration) environment variable.

* To require API key authentication for webhook endpoints, use the [`LANGFLOW_WEBHOOK_AUTH_ENABLE`](/webhooks#require-authentication-for-webhooks) environment variable.
:::
...
To omit API keys and tokens from flow data in your Langflow database, set `LANGFLOW_REMOVE_API_KEYS=True`.
For more information, see [Flow and project management environment variables](/environment-variables#flow-and-project-management).
