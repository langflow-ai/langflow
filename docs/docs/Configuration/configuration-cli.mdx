...
Test mode is the default option. To run it, run `langflow migration`, with no options.
...
Always run `langflow migration` first to test the migrations.
...
| Option | Default | Type | Description |
|--------|---------|--------|-------------|
| `--fix` | `false` | Boolean | Apply migrations (destructive operation). Without the `--fix` flag, migrations are only tested. |
...
For boolean options, the default shows the actual CLI flag that is enabled by default, with the boolean value in parentheses. For example, `--auto-saving` (true) means the `--auto-saving` flag is enabled by default, while `--no-backend-only` (false) means the `--no-backend-only` flag is enabled by default, which disables backend-only mode..

| Option | Default | Type | Description |
|--------|---------|--------|-------------|
| `--auto-saving` | `--auto-saving` (true) | Boolean | Enable flow auto-saving. Use `--no-auto-saving` to disable flow auto-saving. |
| `--auto-saving-interval` | `1000` | Integer | Set the interval for flow auto-saving in milliseconds. |
| `--backend-only` | `--no-backend-only` (false) | Boolean | Only run Langflow's backend server (no frontend). |
| `--cache` | `async` | String | Type of cache to use as one of `async`, `redis`, `memory`, or `disk`. |
| `--components-path` | Not set | String | Path to the directory containing custom components. |
| `--dev` | `--no-dev` (false) | Boolean | Run in development mode (may contain bugs). |
...
| `--log-level` | `critical` | String | Set the logging level as one of `debug`, `info`, `warning`, `error`, or `critical`. |
...
| `--open-browser` | `--no-open-browser` (false) | Boolean | Open the system web browser on startup. Use `--open-browser` to enable opening the system web browser on startup. |
...
| `--remove-api-keys` | `--no-remove-api-keys` (false) | Boolean | Remove API keys from the projects saved in the database. |
...
| `--store` | `--store` (true) | Boolean | Enable the Langflow Store features. Use `--no-store` to disable the Langflow Store features. |
...
Only commonly used and non-sensitive configuration options are exposed as CLI parameters. Other configuration options are available as environment variables.

For information about environment variables, see [Supported environment variables](/environment-variables#supported-variables).
...
The CLI command always prompts for username and password, so they are required. These values are distinct from the system auto-login defaults (`LANGFLOW_SUPERUSER` and `LANGFLOW_SUPERUSER_PASSWORD` environment variables), which default to `langflow`/`langflow`.
