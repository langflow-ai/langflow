...
If you need to upgrade to a custom image based on a Langflow release, such as to add `uv` in `1.8.x`, first build a derived image from the official image, and then follow the same steps above.
Set the custom image in your compose file or `docker run`, and then pull and restart.

For a minimal Dockerfile that adds `uv` to the 1.8.x image, see the [release notes](/release-notes) (“Docker image no longer includes uv or uvx”).