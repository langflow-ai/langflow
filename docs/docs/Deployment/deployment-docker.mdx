...
Set `LANGFLOW_SUPERUSER_PASSWORD` to a strong password in your shell or `.env` file before startup. The Compose file forwards this value without providing a default password. For trusted local development, you can explicitly set `LANGFLOW_AUTO_LOGIN=true` to use automatic login instead. The legacy password `langflow` is not allowed when auto-login is disabled.

To override the other values in the table, edit `docker/dev.docker-compose.yml` directly.
