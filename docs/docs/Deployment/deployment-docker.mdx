...
The image sets `LANGFLOW_AUTO_LOGIN=false` and the [component hardening defaults](#docker-image-security-defaults), so a superuser password is required unless you override those environment variables.
...
These image values match the [multi-tenant component hardening](/api-keys-and-authentication#multi-tenant-component-hardening) recommendation.
...
For more information, see [Multi-tenant component hardening](/api-keys-and-authentication#multi-tenant-component-hardening) and [Block custom components](/deployment-block-custom-components).
