...
3. Set `LANGFLOW_SUPERUSER_PASSWORD` in a `.env` file in the `docker_example` directory. The Compose file requires this value because official images disable automatic login.

   ```shell
   echo 'LANGFLOW_SUPERUSER_PASSWORD=SUPERUSER_PASSWORD' > .env
   ```
...
Set `LANGFLOW_SUPERUSER_PASSWORD` in the repository root `.env` file before startup. Optionally set `LANGFLOW_SUPERUSER` there to change the username.

To change `LANGFLOW_DATABASE_URL` or `LANGFLOW_CONFIG_DIR`, edit `docker/dev.docker-compose.yml`. Those values are set in the Compose file and override `.env`.
...
As of Langflow 1.11.x, official Langflow Docker images also set `LANGFLOW_AUTO_LOGIN=false` at image build time, including DEV/CDK images.

To enable automatic login in a container, set `LANGFLOW_AUTO_LOGIN=true`:
