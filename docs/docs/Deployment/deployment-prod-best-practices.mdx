...
* Multi-worker memory optimization on Linux and macOS.

    If you run Langflow with multiple Gunicorn workers, you can test `LANGFLOW_GUNICORN_PRELOAD=True` to preload the FastAPI app in the Gunicorn master before workers fork.
    This can reduce per-worker memory usage through copy-on-write sharing, but you should validate it in staging before making it part of your default production configuration.
...
* **Resource usage**: Track CPU, memory, and disk usage of Langflow instances. Use Prometheus and Grafana for real-time metrics collection and monitoring in Kubernetes.
    ...
    In multi-worker Gunicorn deployments, the built-in Prometheus endpoint is per-worker and isn't aggregated across workers.
    For complete metrics, prefer one worker per pod or use an external metrics aggregation strategy.
