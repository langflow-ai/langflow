...
* **Resource usage**: Track CPU, memory, and disk usage of Langflow instances. Use Prometheus and Grafana for real-time metrics collection and monitoring in Kubernetes.
    ...
    With Prometheus enabled, Langflow also exports background job metrics prefixed with `langflow_bg_`: counts of queued, running, and suspended jobs, the age of the oldest queued job, cumulative started, completed, and failed totals, and p50 and p95 run durations over the last five minutes.
    Only the process that serves the Prometheus port collects them, by querying the job table every 15 seconds.
    To change the interval, set `LANGFLOW_BACKGROUND_METRICS_INTERVAL` to a number of seconds greater than zero.
