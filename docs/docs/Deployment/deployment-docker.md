...
* The [Quickstart](#quickstart-with-a-sqlite-database) option starts a Docker container with default values. Langflow's default SQLite database is stored in the container's filesystem, which means all data is lost when the container is stopped since no persistent volume is configured.
