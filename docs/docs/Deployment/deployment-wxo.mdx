...
Instead, Langflow packages your selected flow and flow version, and then publishes it to IBM watsonx Orchestrate as a tool that an IBM watsonx Orchestrate agent can execute.
...
5. Enter a **Type**, **Agent Name**, **Model**, and **Description**, and then click **Next**.
...
6. In the **Attach Flows** pane, in the **Available Flows** list, select a flow and flow version to deploy.
...
11. In your IBM watsonx Orchestrate deployment, navigate to **Agent Chat** > **Manage Agents**, and confirm that a new agent is listed with your Langflow flow attached as a tool.
12. To test the agent in watsonx Orchestrate, click **Talk to agent**, and then enter a question.
    The agent responds using your Langflow flow as a tool connected to watsonx Orchestrate.
13. To deploy the watsonx Orchestrate agent to a live environment, click **Deploy**.
14. Optionally, click **Activate agent monitoring**.
15. To view your agent's metrics, click **Agent chat** > **Agent analytics**, and then select your agent.
    This page includes metrics for your agent, such as Total messages, Failed messages, and Latency average.
    To view traces for each agent request, click **Trace Detail**.
...
* **Deployments**:
   ...
   A **Deployment** is a published IBM watsonx Orchestrate agent created from a specific Langflow flow version. Deployment details include the agent name, type, attached flows, model, and the IBM watsonx Orchestrate environment it belongs to.
...
After you deploy your flow to IBM watsonx Orchestrate, you can trigger the watsonx Orchestrate agent through the Langflow deployment run endpoints.
