...
11. Confirm the deployment values are correct, and then click <Icon name="Rocket" aria-hidden="true"/> **Deploy**.
    ...
    :::tip
    If you get an error that the tool name already exists on your deployment, click the <Icon name="Pencil" aria-hidden="true"/> icon to change the tool name.
    :::
...
The standard Langflow endpoint is `/run`, but for flows deployed to IBM watsonx Orchestrate, use `POST /api/v1/deployments/{deployment_id}/runs` to start a run, and `GET /api/v1/deployments/{deployment_id}/runs/{run_id}` to check its status.
