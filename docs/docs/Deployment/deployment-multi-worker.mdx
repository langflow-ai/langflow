...
A single-worker process is fine for development, but doesn't scale when you run more than one worker.
...
:::tip
If you are using Podman Desktop, the default machine may not have enough resources to run this stack. Before starting, increase the machine's CPU and memory allocation:
...
To verify that cancellation works across workers, trigger a build with the API to capture the `job_id` and then cancel it.
   ...
   1. Use the API to build a flow by its `flow_id`:
    ...
    For a full description of all fields, see [Monitor the job queue](#monitor-the-job-queue).

## Troubleshoot

For troubleshooting, see [Multi-worker deployments ](/troubleshooting#multi-worker-deployments).
...
| `LANGFLOW_REDIS_QUEUE_HOST` | `LANGFLOW_REDIS_HOST` | Redis host for the job queue. Falls back to the general Redis host setting. No auth or TLS support — use `URL` instead for secured instances. |
...
| `LANGFLOW_REDIS_QUEUE_STARTUP_GRACE_S` | `30.0` | Seconds a consumer waits for the producer's first write before treating a missing stream as end-of-stream. Increase this if your workers have slow cold-starts. Set to `0` to disable. |
...
| `LANGFLOW_REDIS_QUEUE_CANCEL_MARKER_TTL` | `60` | TTL in seconds for the cancel-marker key. The marker closes a race where a cancel signal is published before the target worker's dispatcher has subscribed. Must be greater than `0`. |
