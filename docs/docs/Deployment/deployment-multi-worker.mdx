...
| `LANGFLOW_BACKGROUND_RETENTION_DAYS` | `0` | Days to keep finished background runs, with their events, before deleting them. `0` keeps every row forever. Queued, running, and paused runs are never deleted. |
