...
:::note
The Redis job queue covers the v1 build endpoints (`/api/v1/build/*`). It has no effect on background runs of the v2 workflows API, which are configured separately with `LANGFLOW_BACKGROUND_BACKEND`. See [Scale background runs](#scale-background-runs).
:::
...
## Run multiple workers without Redis

By default, Langflow refuses to start when `LANGFLOW_WORKERS` is greater than 1 and `LANGFLOW_JOB_QUEUE_TYPE` is still `asyncio`, because the in-memory job queue is per-process.

If your deployment is headless — it only calls the Langflow API and never opens the visual editor — you can opt out of that check:
...
Langflow then starts and logs a warning listing everything that changes. The flag does not make multi-worker safe; it only stops Langflow from refusing to boot.

:::warning
This configuration is **not supported with the Langflow UI** and **not supported with MCP over SSE**.

- The visual editor and the playground do not work. They use the v1 `/build` endpoints, whose queues live in one worker's memory, so the follow-up `GET /api/v1/build/<job_id>/events` request fails whenever the load balancer routes it to a different worker.
- MCP over SSE does not work, because the SSE stream and the POST message endpoint that feeds it must run in the same process. Use the Streamable HTTP transport instead.

The following behaviors silently degrade rather than fail:

- Reattaching to a background run polls the durable event log every 0.5 seconds instead of receiving in-process events, so the first event after a reattach arrives more slowly.
- Rate limits are counted per worker while `LANGFLOW_RATE_LIMIT_STORAGE_URI` is in-memory (`memory://`), so the effective limit is multiplied by the worker count. Point it at Redis for a shared counter.
- The startup orphan sweep is guarded by a file lock, which is node-local. One worker per host or pod runs the reconcile, rather than one per deployment.
- Webhook feedback in the UI is per worker, because the webhook event manager keeps its listener registry in memory. A webhook only surfaces live events when it happens to land on the worker holding the browser's connection.

For a supported multi-worker deployment, set `LANGFLOW_JOB_QUEUE_TYPE=redis` instead.
:::
...
## Scale background runs {#scale-background-runs}

Background runs are runs submitted to `POST /api/v2/workflows` with `"mode": "background"`. The request returns a job ID right away, and you poll, stream, stop, or resume the run with that ID later. The visual editor also uses background mode for runs that pause for human input.

Background runs are configured with `LANGFLOW_BACKGROUND_BACKEND`. They do not depend on `LANGFLOW_JOB_QUEUE_TYPE`.

### Default backend

With the default backend (`LANGFLOW_BACKGROUND_BACKEND=default`), a background run executes inside the API worker that accepted it, up to `LANGFLOW_BACKGROUND_MAX_CONCURRENCY` runs per worker.

The run's status, result, and events are stored in the database, so any worker can serve them. A worker that is not running the job reads new events from the database every 0.5 seconds. This works with multiple workers without extra configuration.

### Scaled backend

With the scaled backend (`LANGFLOW_BACKGROUND_BACKEND=scaled`), API workers only queue background runs, and separate `langflow worker` processes execute them. The queue is the Langflow database itself, so no additional infrastructure is required.

Use the scaled backend when background runs need to scale independently of the API, or when they should not compete with API requests for CPU and memory.

Each `langflow worker` process runs one background job at a time. To add capacity, run more worker processes.

The scaled backend requires the following:

- Every API process and every worker uses the same database. Use PostgreSQL when processes run on more than one host. SQLite only works when every process runs on the same host.
- Every process uses the same `LANGFLOW_SECRET_KEY`, because workers decrypt stored credentials and request overrides.
- Workers run the same Langflow version, with the same components and bundles installed, as the API. Workers build and run the flows themselves.
- At least one worker is running. Without a worker, background runs stay `queued`. The API logs a reminder at startup when the scaled backend is enabled.

:::tip
Set `LANGFLOW_BACKGROUND_RETENTION_DAYS` on long-lived deployments. With the scaled backend, the job table is the work queue, so without a retention window it grows without bound. Retention is off by default because it deletes run history.
:::

To enable the scaled backend, set the following environment variables on the API and on every worker:

```text
LANGFLOW_DATABASE_URL=postgresql://langflow:langflow@your-postgres-host:5432/langflow
...
`langflow worker` accepts `--env-file` to load the variables from a file and `--log-level` to set the logging level.

The scaled backend changes the following behaviors:

- Event streams for background runs carry durable milestones, such as build progress, messages, and the final result. They do not carry token-by-token output.
- Stopping a run reaches its worker at the next component boundary.
- If a worker crashes mid-run, its job is reconciled after the lease expires (`LANGFLOW_BACKGROUND_LEASE_TTL_S`, 45 seconds by default), so a run is never left stuck as in progress.
...
| `LANGFLOW_DANGEROUSLY_ALLOW_MULTI_WORKER_WITHOUT_SHARED_QUEUE` | `False` | Start with more than one worker while the job queue is still `asyncio`. **Unsupported with the Langflow UI and with MCP over SSE.** See [Run multiple workers without Redis](#run-multiple-workers-without-redis). |
...
These settings control background runs of the v2 workflows API. For details, see [Scale background runs](#scale-background-runs).

| Variable | Default | Description |
|---|---|---|
| `LANGFLOW_BACKGROUND_BACKEND` | `default` | `default` runs background jobs inside API workers. `scaled` queues them in the database for `langflow worker` processes to run. |
| `LANGFLOW_BACKGROUND_MAX_CONCURRENCY` | `5` | With the default backend, the number of background runs each API worker executes at once. |
| `LANGFLOW_BACKGROUND_POLL_INTERVAL_S` | `0.5` | With the scaled backend, how often in seconds an event stream checks the database for new events. |
| `LANGFLOW_BACKGROUND_LEASE_TTL_S` | `45` | How long in seconds a running job can go without a heartbeat before it is considered lost. Keep it well above `LANGFLOW_BACKGROUND_HEARTBEAT_INTERVAL_S`. |
| `LANGFLOW_BACKGROUND_HEARTBEAT_INTERVAL_S` | `15` | How often in seconds a running job refreshes its heartbeat. |
| `LANGFLOW_BACKGROUND_WATCHDOG_INTERVAL_S` | `15` | With the scaled backend, how often in seconds each worker checks for jobs whose worker has died. |
| `LANGFLOW_BACKGROUND_CLAIM_CANDIDATES` | `5` | With the scaled backend, how many of the oldest queued jobs a worker considers on each claim attempt. If workers sit idle while jobs are queued, raise it to roughly your worker count. |
| `LANGFLOW_BACKGROUND_RETENTION_DAYS` | `0` | Days to keep finished background runs, with their events, before deleting them. `0` keeps every row forever. Only v2 background runs are deleted: queued, running, and paused runs are never deleted at any age, and neither are runs from the v1 build endpoints or knowledge-base ingestion. Age is measured from when a run finished. |
