...
This is equivalent to installing `lfx` plus `lfx-bundles[all-no-torch]` (the full provider set minus the torch-pulling `cuga` and `codeagents` providers, so the default install is torch-free).
...
There is intentionally no `lfx[all]` extra. `pip install langflow` is the batteries-included install except for torch-gated extras, which are opt-in: `pip install "lfx-bundles[all]"` for the `cuga`/`codeagents` components, and `pip install "langflow[docling]"` for the File component's docling conversion path.
