...
There is intentionally no `lfx[all]` extra; use `langflow` for the server plus default curated bundles, and install `lfx-bundles` explicitly for the long-tail providers.
