...
The same flow keeps working in the visual editor, in the **Playground**, and through the Langflow API, because those calls are authenticated.

To let publicly shared flows run custom component code, set:
...
|---|---|---|
| `true` (default) | `false` (default) | The server's own component code runs. Custom components are refused. |
| `true` | `true` | The code stored in the flow runs, for every node in the flow. |
| `false` | Either | The server's own component code runs. Custom components are refused. |

Before you enable this, understand what it changes. With `LANGFLOW_ALLOW_CUSTOM_COMPONENTS` at its default `true`, this setting doesn't only allow the custom component you added. It turns off code substitution for the entire public path, so every node in a public flow runs the code stored in that flow. Any user who can author a flow and enable its **Shareable Playground** can then have unauthenticated visitors execute that code on your server.

Enable it when the people authoring flows on your server are trusted, such as a single-team or single-tenant deployment. Leave it disabled in multi-tenant deployments, or where flow authors and server operators aren't the same people.
