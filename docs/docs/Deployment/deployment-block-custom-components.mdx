...
## Multi-user deployments and code execution{#multi-user-code-execution}

Custom component code runs on the Langflow server at flow-build time. In a single-user deployment (`LANGFLOW_AUTO_LOGIN=true`, the default) the only account is the superuser, so this is code you already trust.

In a multi-user deployment (`LANGFLOW_AUTO_LOGIN=false`), any **active non-admin user** can author component code and have the server execute it — for example by saving a flow with custom code and triggering it through the [webhook](/webhook) or [`/run`](/api-flows-run) endpoints. This is expected behavior of the custom-component feature, not a defect, but it means every regular user effectively has code execution on the host unless you restrict it.

When a deployment runs in multi-user mode with custom components still enabled for everyone, Langflow logs a warning at startup:

```text
SECURITY: Langflow is running in multi-user mode (LANGFLOW_AUTO_LOGIN=false) with custom components enabled for every user ...
```

To close this exposure, apply one of the two lockdowns below:

- Set `LANGFLOW_CUSTOM_COMPONENT_ADMIN_ONLY=true` to allow only superusers to author component code, while regular users keep using built-in and existing components. See [Restrict custom component creation to superusers](#restrict-custom-components-to-superusers).
- Set `LANGFLOW_ALLOW_CUSTOM_COMPONENTS=false` to run only the server's built-in component code, as described above.

[`LANGFLOW_SANDBOX_BACKEND`](./sandbox-backends.mdx) is not a substitute for either lockdown. It isolates the **Python Interpreter** and legacy **Python REPL** components, which run code supplied as a component input at runtime. Custom-component code is compiled and executed in the Langflow server process at flow-build time, before any sandbox backend is consulted, so a sandbox backend does not contain it. For untrusted or multi-tenant deployments, apply one of the lockdowns above as the primary control, and enable a sandbox backend in addition to cover the interpreter components.
