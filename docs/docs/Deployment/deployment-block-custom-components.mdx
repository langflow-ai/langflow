...
The allow-list behavior above assumes the admin controls which env vars are set at startup. In deployments where that assumption doesn't hold — for example, where operators want a hard guarantee that only built-in server templates execute — set:
