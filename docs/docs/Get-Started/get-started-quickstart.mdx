...
2. In the **Agent** component, enter your OpenAI API key directly or use a <Icon name="Globe" aria-hidden="true"/> [global variable](/configuration-global-variables).
    ...
    If your preferred provider or model isn't listed, select the **Custom** provider, and then connect an [additional language model](/components-models#additional-language-models).
