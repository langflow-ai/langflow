...
3. In the **Agent** component, select your configured model from the **Language Model** dropdown.
    ...
    If you want to use a different provider, configure it in the **Models** pane, and then select it from the **Agent** component's dropdown.
    If your preferred provider or model isn't listed, set **Model Provider** to **Connect other models** in the **Language Model** dropdown, and then connect any [language model component](/components-models#additional-language-models).
...
3. For example, to change the LLM provider from OpenAI to Groq, configure Groq in the **Models** pane, and then select the values **Model Providers** and **Model** in the **Input Schema** pane.
