...
To run the complete Langflow UI and API without provider component bundles, install the `langflow-core` distribution instead:
...
`langflow-core` includes the built-in core component catalog and does not install any `lfx-*` extension distribution, such as `lfx-openai`, `lfx-bundles`, or `lfx-ibm`. You can add individual bundles later. Treat `langflow` and `langflow-core` as alternative distributions because both provide the `langflow` command.
