...
### Install the provider-bundle-free core profile

The full `langflow` distribution consumes the service-complete `langflow-core`
profile and adds curated provider bundles. Install the core profile directly
when you want to run the complete Langflow UI and API while selecting provider
bundles yourself:
...
`langflow-core` includes the built-in component catalog and does not install any
`lfx-*` provider extension distribution, such as `lfx-openai`, `lfx-bundles`,
or `lfx-ibm`. You can add individual bundles later. The core profile owns the
`langflow` command used by both core and full installations; it also exposes
`langflow-core` as an alias.

`langflow-core` is not the slimmest Langflow layer. For a smaller, modular
platform whose service dependencies you select through extras, install
`langflow-base` instead.

Install `langflow-core[audio]` when you need the optional voice mode dependencies.
