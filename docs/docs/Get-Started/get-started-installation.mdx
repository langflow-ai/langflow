...
3. In your virtual environment, install Langflow:
    ...
    This installs the runnable Langflow application and its curated provider
    extensions. To install the same UI and API without provider extensions,
    install `langflow-base` instead:

    ```bash
    uv pip install langflow-base
    ```

    Both packages use the `langflow` command. In Langflow 1.12,
    `langflow-base` does not provide a `langflow-base` executable.

4. Set `LANGFLOW_SUPERUSER_PASSWORD` to a strong password in your environment or `.env` file, then start Langflow:
    ...
    Official release packages disable automatic login by default. Sign in with the username `langflow` (or your configured `LANGFLOW_SUPERUSER`) and the password you set.
    For an existing auto-login installation, [prepare the existing account for password sign-in](/api-keys-and-authentication#upgrade-from-auto-login) before upgrading.
    For trusted local development, you can explicitly set `LANGFLOW_AUTO_LOGIN=true` to enable automatic login.
...
:::note Langflow 1.12 package migration
The new runnable `langflow-base` package starts at version `1.12.0` and stays
version-aligned with `langflow`. Requirements such as
`langflow-base~=0.12` remain on the previous package line and don't select the
1.12 application.
:::
