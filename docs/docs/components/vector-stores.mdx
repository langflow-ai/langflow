...
- **search_kwargs:** The parameter is used to specify additional search options or filters when querying the database. It allows you to customize the search behavior and retrieve more specific results. Defaults to `{}`.
...
- **index_name:** The index_name parameter allows you to give a unique identifier to the index, which can be useful when working with multiple indexes or when saving and loading indexes from disk.
- **search_kwargs:** The parameter is used to specify additional search options or filters when querying the database. It allows you to customize the search behavior and retrieve more specific results. Defaults to `{}`.
...
- **db_name:** It refers to the name of the database that you want to connect to or perform operations on. When working with MongoDB Atlas, you need to specify the name of the database in order to access its collections and perform CRUD (Create, Read, Update, Delete) operations.

- **index_name:** This parameter refers to the name given to the vector search index that is created on your MongoDB cluster. It is used to uniquely identify the index and is used when performing operations such as inserting documents with their embeddings and performing similarity searches.
...
- **search_kwargs:** This parameter is used to specify additional search options or filters when querying the database. It allows you to customize the search behavior and retrieve more specific results. Defaults to `{}`.
...
Pinecone is a vector database with broad functionality. It is used for creating and managing vector search indexes. The Pinecone package allows you to create a VectorStore and seed it with data.
...
- **api_key:**This parameter is used to secure your deployment and prevent unauthorized access to your Qdrant cluster. When using a managed version of Qdrant, such as Qdrant Cloud, you need to provide an API key to authenticate and access your deployment. This API key acts as a security measure to ensure that only authorized users can interact with your Qdrant cluster.

- **collection_name:** It is used to identify and organize the data stored in Qdrant. When creating a new collection or accessing an existing one, you need to specify the collection_name to perform operations on that specific collection. It allows you to manage and query the vectors and metadata within that collection.


- **content_payload_key:** This parameter refers to the key used to store the page content or text of a document in the payload structure. By default, the key is set to "page_content", but you have the option to customize it and use a different key if desired. This is useful if you already have a collection with a different key for the page content and want to reuse it.
...
- **grpc_port:** This refers to the port number used for gRPC communication with the Qdrant server. gRPC is a high-performance, open-source framework that allows remote procedure calls between client and server applications. By specifying the grpc_port, you can establish a connection to the Qdrant server using the gRPC protocol. Defaults to `6334`.
...
- **metadata_payload_key:** The parameter refers to the key used to store metadata in the payload structure of a document. If you want to use a different key for the metadata, you can set the "metadata_payload_key" to a desired value. This allows you to store and retrieve specific metadata associated with each document in the Qdrant vector store. Defaults to `metadata`.
...
- **prefix:** It allows you to add a prefix to the REST URL path when making API requests. By specifying a prefix, you can customize the URL structure for the REST API endpoints. For example, if you set the prefix parameter to "service/v1", the resulting URL for the REST API would be:
```HTML
"http://localhost:6333/service/v1/{qdrant-endpoint}"
```
This can be useful for organizing and categorizing different API endpoints within your application.

- **search_kwargs:** This parameter is used to specify additional search options or filters when querying the database. It allows you to customize the search behavior and retrieve more specific results. Defaults to {}.  Defaults to `{}`.
...
- **url:** This parameter specify the URL of the Qdrant service that you want to connect to. It is required to provide the URL in order to establish a connection with the Qdrant server. The URL should point to the location where the Qdrant service is running, whether it is a local instance, an on-premise server, or a Qdrant Cloud cluster. The URL parameter allows you to connect to the desired Qdrant instance and access its functionality through the API.
...
- **client_kwargs:** The parameter is used to pass additional keyword arguments to the `Weaviate` client. It allows you to customize the behavior of the client by providing specific arguments. Defaults to `{}`.

- **search_kwargs:**This parameter is used to specify additional search options or filters when querying the database. It allows you to customize the search behavior and retrieve more specific results. Defaults to {}.

- **weaviate_url:** This parameter is used to specify the URL of the `Weaviate` server that you want to connect to. It allows you to establish a connection to the specific `Weaviate` instance where your data is stored. By providing the URL, you can ensure that the `Weaviate` client knows where to send requests and retrieve data from. Defaults to `(http://localhost:8080)`.
