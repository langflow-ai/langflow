...
The `CohereEmbeddings` is used to load the Cohere Embedding module. To use this tool, the user needs to provide an API key. Once it has the class loaded and ready to go, the user can start using it to analyze text documents. It can take a piece of text, like a sentence or a paragraph, and convert it into a special representation called an embedding. This representation captures the meaning and context of the text in a way that computers can understand.
...
- **truncate:** Is used to specify whether or not to truncate the input text. Truncation is useful when dealing with long texts that exceed the model's maximum input length. By truncating the text, the user can ensure that it fits within the model's constraints.

### HuggingFaceEmbeddings

Wrapper around sentence_transformers embedding models.

The `HuggingFaceEmbeddings` helps the user works with text embedding models provided by Hugging Face. The main purpose of this class is to create vector representations of text. When the user gives the class a piece of text, like a sentence or a paragraph, it will convert it into a numerical representation called a vector. This vector captures the meaning and context of the text in a way that computers can understand and work with. The user can use these vectors to search for similar or related documents based on a given query.
...
- **model_kwargs:** Is used to customize the behavior of the model, such as specifying the model architecture, the tokenizer, or any other model-specific configuration options. By using model_kwargs, the user can configure the Hugging Face model according to the specific needs and preferences.
...
- **model:** Is used to specify the specific text embedding model to use for embedding. It allows the user to choose a pre-trained model that best suits it's needs. Defaults to `text-embedding-ada-002`.

- **openai_api_base:** It refers to the base URL for the Azure OpenAI resource. It is used to configure the API to connect to the Azure OpenAI service. The base URL can be found in the Azure portal under the user Azure OpenAI resource. It is typically in the format:

  ```HTML
  https://the user-resource-name.openai.azure.com
  ```
...
- **request_timeout:** Is used to specify the maximum amount of time, in milliseconds, that the Langflow will wait for a response from the OpenAI API when generating embeddings for a given text. _Optional[Union[float, Tuple[float, float]]] = None_.

- **tiktoken_model_name:** Is used to count the number of tokens in documents to constrain them to be under a certain limit. By default, when set to None, this will be the same as the embedding model name. However, there are some cases where the user may want to use this Embedding class with a model name not supported by tiktoken. This can include when using Azure embeddings or when using one of the many model providers that expose an OpenAI-like API but with different models.
