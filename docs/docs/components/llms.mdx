...
It is a wrapper for interacting with GGML (Generative Graded Models in LangChain) models. It is part of the CTransformers library, which is a Python library that provides bindings for GGML models. The LLM CTransformers allows you to use language models (LLMs) within LangChain and generate text using these models. It provides a unified interface for all models and supports models hosted on the Hugging Face Hub. To use, you should have the ctransformers python package installed. See [https://github.com/marella/ctransformers]

**config:** Configuration for the Transformer models. See [https://github.com/marella/ctransformers#config]. Defaults to
...
- **cohere_api_key:** This parameter specify the URL of the Cohere API requests. 
- **max_tokens:** Denotes the number of tokens to predict per generation. Defaults to `256`.
- **temperature:** A non-negative float that tunes the degree of randomness in generation. Defautls to `0.75`.
...
- **last_n_tokens_size:** The number of tokens to look back when applying the repeat_penalty. Defaults to `64`
...
- **use_mlock:** Force system to keep model in RAM. Defaults to `False`
...
- **max_tokens:** The maximum number of tokens to generate in the completion. -1 returns as many tokens as possible given the prompt and the models maximal context size. Defaults to `256`
- **model_kwargs:** Holds any model parameters valid for create call not explicitly specified.
- **model_name:** Model name to use.
...
- **temperature:** Sampling temperature, it controls the degree of randomness in token selection. Defaults to `0`
