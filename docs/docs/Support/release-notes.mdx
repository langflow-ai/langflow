...
-   `langflow-base` dependency structure refactoring**

    The `langflow-base` package has been refactored to use granular optional dependency groups. Many dependencies that were previously included in the base `langflow-base` installation have been moved to optional extras.

    If you installed Langflow with `uv pip install langflow`, this is not a breaking change. Installing `langflow` automatically installs `langflow-base[complete]`, which includes all optional dependencies, maintaining the same functionality as before.

    If you installed Langflow with `uv pip install langflow-base` without specifying extras, **this is a breaking change**. If you install `langflow-base` directly, you will now only install the [core base dependencies](https://github.com/langflow-ai/langflow/blob/main/src/backend/base/pyproject.toml). Dependencies that were previously included by default are now available only through optional extras.
