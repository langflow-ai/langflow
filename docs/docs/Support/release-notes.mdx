...
- MCP stdio interpreter hardening

    Direct Python, Node.js, and shell entrypoints in MCP stdio configurations
    are now blocked by default. Trusted single-tenant deployments that require
    legacy commands such as `python -m mcp_weather_server` can set
    `LANGFLOW_MCP_SERVER_INTERPRETER_HARDENING=false`; do not use this opt-out
    when MCP configurations can be supplied by untrusted users. See [MCP
    interpreter hardening](/api-keys-and-authentication#mcp-interpreter-hardening).
