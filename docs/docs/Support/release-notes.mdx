...
- Centralized **Language Model** and **Embedding Model** components
    ...
    The single-provider components are still available for your flows in the **Components** menu in the [**Bundles**](/components-bundle-components) section, and you can use them to replace the **Language Model** and **Embedding Model** core components, or connect them to the **Agent** component with the <Icon name="Plus" aria-hidden="true"/> **Connect Other Models** provider option.
