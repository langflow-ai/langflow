...
- DataFrame Operations component enhancements
    ...
    Use it to clean data by keeping only records that don't contain specific values, such as filtering out invalid email addresses that don't contain `@`.

- Data Operations component enhancements

    The [**Data Operations** component](components-processing#data-operations) now includes two new operations for advanced JSON data manipulation.
    The Path Selection operations extracts values from nested JSON structures, and the JQ Expression operation uses the [jq](https://jqlang.org/) query language to perform advanced JSON filtering, projections, and transformations.
