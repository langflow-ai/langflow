...
- Manage dependencies in the UI

  Packages can now be installed and uninstalled in Langflow's **Package Manager** pane, found in the **Settings** page.
  If you experience a conflict with your installed packages, use the **Restore Langflow** option to remove your installed packages and revert Langflow to the dependencies listed in `uv.lock`.
...
- LFX engine
  ...
  The LFX engine runs flows in headless mode with reduced dependencies and no local database.
  This allows you to run flows with just a JSON file and a PyPI package, making it easier to integrate Langflow into CI/CD pipelines or scaling deplpoyments.
  For more information, see Run flows with the Langflow Executor (LFX).

- Additional structured output for Agent component

  In addition to the `Message` output, the Agent component now includes a structured output.
