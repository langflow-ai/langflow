...
- Playground refactor
    ...
    The **Playground** now appears on the right side of the visual builder to provide a more integrated testing experience. The full screen option is still available by clicking the <Icon name="Fullscreen" aria-hidden="true"/> **Fullscreen** icon. For more information, see [Test flows in the Playground](/concepts-playground).
