...
-   Docker image no longer includes `uv` or `uvx`
    ...
    If you're using Langflow 1.8.0, follow the steps above to add `uv` and `uvx` in a derived image.
    :::

    In Langflow 1.8.0, the Langflow Docker image no longer includes `uv` or `uvx` in the runtime image.
