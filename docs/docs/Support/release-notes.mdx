...
- Docker images enable component hardening by default

    Official Langflow Docker images now include stricter defaults and include the following environment variables:
    ...
    Existing Langflow containers that rely on custom components, code interpreters, or absolute local file paths must override these variables.
    Keep the image defaults when users who can build flows on the server are not fully trusted.
...
- Ongoing breaking changes: bundle separation
    ...
    In Langflow 1.11.x and future releases, bundle separation extends to more component providers.
