...
- Bundle providers split

    `uv pip install langflow` no longer installs all provider bundles by default.
    About 45 provider bundles, such as Chroma, Ollama, and Mistral, have moved to the `lfx-bundles` metapackage.

    Basic LLM support for OpenAI, Anthropic, Amazon Bedrock, and Cohere is still included in all Langflow installations through the core [Language Model](/components-models) component.
    The standalone bundles add enhanced components with additional provider-specific parameters.

    Saved flows that reference these components open normally.
    Langflow migrates component references automatically when you open a flow.
    However, if the required bundle is not installed in Langflow's environment, those components will be unavailable in the visual editor.

    To install the long-tail bundles, do the following:

    1. In your Langflow environment, run `uv pip install lfx-bundles`.
    2. Restart Langflow.
    3. To confirm the bundles loaded, run `lfx extension list`.
