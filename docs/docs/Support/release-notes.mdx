...
-   Docker image no longer includes `uv` or `uvx`

    In Langflow 1.8.x, the Langflow Docker image no longer includes `uv` or `uvx` in the runtime image.
    ...
    If you use MCP from within a Langflow 1.8.x Docker image, you must install `uv` in an image derived from the official `langflowai/langflow` image.
