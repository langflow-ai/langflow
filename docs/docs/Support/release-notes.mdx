...
- SDKs
    ...
    Python and TypeScript/JavaScript SDKs are now available. These SDKs provide client libraries that interact with Langflow's REST APIs. For more information, see the [Langflow SDK README](https://github.com/langflow-ai/sdk/README.md).
...
- Developer API: `/workflows` synchronous endpoints (Beta)
    ...
    The Developer API now includes `/workflows` endpoints for executing flows with enhanced error handling, timeout protection, and structured responses.
    The synchronous execution endpoint is available at `POST /api/v2/workflows`.
    For more information, see [Synchronous Workflows API endpoints](/workflows-api-synchronous).
