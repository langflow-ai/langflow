...
- DataFrame Operations component enhancements
    ...
    The [**DataFrame Operations** component](components-processing#dataframe-operations) now includes an operator for `does not contains` for filtering data.
    With this operator, you can remove invalid email addresses from your mailing list by filtering records that don't contain `@`.
