...
Database URLs that refer to local TLS certificate or key files, including PostgreSQL `sslrootcert`, `sslcert`, and `sslkey` parameters, are also blocked when local file access is restricted. Administrators using file-based TLS credentials should review affected flows before upgrading. A [directory-scoped exception for administrator-provided files](https://github.com/langflow-ai/langflow/issues/15402) is planned; until then, these URLs require a trusted single-tenant deployment with `LANGFLOW_RESTRICT_LOCAL_FILE_ACCESS=false`.
