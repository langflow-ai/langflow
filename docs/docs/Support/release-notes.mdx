...
- Langflow MCP Client for Bob and Claude Code

    The **Settings** page now includes <Icon name="Terminal" aria-hidden="true"/> **Langflow MCP Client**, with instructions to connect **Bob (IBM)** and **Claude Code** to this Langflow instance using `lfx-mcp` (`uvx --from lfx lfx-mcp`) and your Langflow URL and API key.
    For more information, see [Connect coding agents to Langflow (Langflow MCP Client)](/mcp-client#langflow-mcp-client-settings).

- MCP Tools: register servers first, and then add from the MCP sidebar

    **MCP Tools** is no longer added from the **Agents** component list. Register each MCP server under **Settings** > **MCP Servers*, or from the flow editor **MCP** sidebar.
