...
- Official packages require authentication by default

    Official Python packages, tagged source archives, and Docker images now set `LANGFLOW_AUTO_LOGIN=false`.
    ...
    Development branches and local builds from those branches still default to automatic login.
    For trusted local development with a release package or Docker image, set `LANGFLOW_AUTO_LOGIN=true`.

    For more information, see [API keys and authentication](/api-keys-and-authentication#langflow-auto-login).

## 1.12.x

For 1.12.x release notes, see the [1.12.x documentation](https://docs.langflow.org/1.12.0/release-notes).
