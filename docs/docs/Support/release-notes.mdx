...
- Removed `LANGFLOW_SKIP_AUTH_AUTO_LOGIN` environment variable
    ...
    The `LANGFLOW_SKIP_AUTH_AUTO_LOGIN` environment variable has been removed in version 1.6.
    This means that when `LANGFLOW_AUTO_LOGIN=True`, API requests still require a valid Langflow API key for authentication.
