...
- Removed `LANGFLOW_SKIP_AUTH_AUTO_LOGIN` environment variable

    Continuing the stricter Langflow API authentication requirements introduced in version 1.5, this release removes the `LANGFLOW_SKIP_AUTH_AUTO_LOGIN` environment variable, which was a temporary bypass for backwards compatibility.
    Starting with version 1.6, all Langflow API requests require a valid Langflow API key for authentication, regardless of the `LANGFLOW_AUTO_LOGIN` setting.
