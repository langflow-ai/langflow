...
- Tools components are redistributed
    ...
    Tools that performed the same function were combined into single components that support multiple providers, such as the [**Web Search** component](/web-search), which now includes web search, news search, and RSS reader functionality.
