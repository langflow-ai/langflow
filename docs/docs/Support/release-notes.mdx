...
- File System component
    ...
    The new **File System** component gives agents sandboxed read/write access to files on disk via five tools: `read_file`, `write_file`, `edit_file`, `glob_search`, and `grep_search`.
    An optional `read_only` mode restricts the agent to read and search operations only.

    For more information, see [File System](/file-system).
