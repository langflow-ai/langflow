...
- Voice-to-voice audio streaming removed

    The voice-to-voice feature and its associated `/ws/flow_as_tool/{flow_id}` WebSocket endpoint have been removed.
    The speech-to-text microphone button in the **Playground** and the `/ws/flow_tts/{flow_id}` endpoint remain available.
