...
### Windows Desktop update issue {#windows-desktop-update-issue}

:::warning
Windows users of Langflow Desktop should **not** use the in-app update feature to upgrade to Langflow version 1.6.0.
Only Windows Desktop users migrating to Langflow version 1.6.0 are affected.
:::

The **Update** button in Langflow Desktop will not work for Windows users, and may result in data loss.

Instead, Windows users must download a fresh installation from the [Langflow website](https://langflow.org) when a new version is available.
...
8. Import your flows to the version 1.6.0 Langflow installation's `data` directory.
9. Start version 1.6.0 of Langflow, and confirm your flows behave as expected.
10. **If flows are missing**, restore your flows from your manual backup by doing the following:
    1. Close Langflow.
