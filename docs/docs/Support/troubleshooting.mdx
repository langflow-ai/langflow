...
### Streaming or polling reverts to `direct` delivery

This happens when `LANGFLOW_WORKERS > 1` and `LANGFLOW_JOB_QUEUE_TYPE` is still set to `asyncio` (the default). Langflow automatically forces `LANGFLOW_EVENT_DELIVERY=direct` in this case and logs a warning that names the `LANGFLOW_JOB_QUEUE_TYPE` environment variable. Set `LANGFLOW_JOB_QUEUE_TYPE=redis` to restore streaming and polling delivery.
