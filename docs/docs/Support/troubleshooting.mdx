...
This means that `LANGFLOW_AUTO_LOGIN` is set to `false`, and since only superusers can create and activate non-superuser accounts, you are locked out of Langflow.
...
For more information, see [Start Langflow Server with authentication enabled](/api-keys-and-authentication).
