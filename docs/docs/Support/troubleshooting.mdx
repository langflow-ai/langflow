...
If you encounter issues with Langflow Desktop and need to access startup logs for debugging, follow these steps for your operating system.
...
2. List the contents to find log files:
   ```bash
   ls -la
   ```

3. Locate the `langflow.log` file and send it to support for debugging assistance.
...
3. Locate the `Langflow.log` file in the opened folder and send it to support for debugging assistance.
...
When running multiple Langflow instances locally on different ports (for example, one on `localhost:7860` and another on `localhost:7861`), you might see authentication errors in the logs:

```text
[07/22/25 10:57:07] INFO     2025-07-22 10:57:07 - INFO     - utils - User not found or inactive.
```

To resolve this error, use separate browser instances or browser profiles for each Langflow instance.
...
### Langflow Desktop shows "latest version" but is outdated

If Langflow Desktop shows you're on the latest version (for example, version `1.3.3`) but a newer version (like `1.5.0`) is available, this is likely because you're running a version prior to `1.4.2`.

The automatic update feature through the graphical interface was introduced in version `1.4.2`.
Versions prior to `1.4.2` don't include the necessary functionality to perform updates.

To resolve, uninstall Langflow Desktop and download the latest version from the [official website](https://langflow.org/desktop).
