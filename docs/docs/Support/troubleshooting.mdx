...
If you're running Langflow in a Docker container and encounter errors when using components that handle files or images, such as the [**Read File** component](/read-file) or [**Docling** components](/bundles-docling), you may need to install additional system dependencies.
...
For more information about customizing Docker images, see [Customize the Langflow Docker image with your own code](/deployment-docker#customize-the-langflow-docker-image-with-your-own-code).
