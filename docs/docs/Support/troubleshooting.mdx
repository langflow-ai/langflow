...
## Langflow Assistant issues {#langflow-assistant}

The following issues can occur when using [Langflow Assistant](/langflow-assistant).

### The agent ran out of steps before finishing

If **Langflow Assistant** stops with `The agent ran out of steps before finishing. Try again, or break the request into smaller parts.`, the request needed more steps than the Assistant's step limit allows.

To resolve this error, raise the limit for your session with the `/iterations` command, such as `/iterations 150`, and then send the request again.
The default limit is `100`, and the maximum is `200`.
To raise the default for every user of your Langflow server, set [`LANGFLOW_ASSISTANT_ITERATIONS`](/environment-variables).
You can also split the request into smaller prompts.

For more information, see [The agent ran out of steps before finishing](/langflow-assistant#the-agent-ran-out-of-steps-before-finishing).
...
If the deployment is headless and never uses the visual editor or MCP over SSE, you can set `LANGFLOW_DANGEROUSLY_ALLOW_MULTI_WORKER_WITHOUT_SHARED_QUEUE=true` to start anyway. This is unsupported with the Langflow UI and with MCP over SSE, and several subsystems become per-worker. Read [Run multiple workers without Redis](./deployment-multi-worker#run-multiple-workers-without-redis) before enabling it.
...
### Background runs stay queued

With `LANGFLOW_BACKGROUND_BACKEND=scaled`, the API only queues background runs, and `langflow worker` processes execute them. If no worker is running against the same database, runs stay `queued` indefinitely.

Start at least one `langflow worker` with the same `LANGFLOW_DATABASE_URL` and `LANGFLOW_SECRET_KEY` as the API. For more information, see [Scale background runs](./deployment-multi-worker#scale-background-runs).

### Background runs fail with `worker_lost`

The worker running the job stopped sending heartbeats for longer than `LANGFLOW_BACKGROUND_LEASE_TTL_S` (45 seconds by default), so the run was marked failed. This usually means the worker process crashed or was killed, for example by the out-of-memory killer.

Check the worker logs around the time of the failure. If workers are healthy but under heavy load, increase `LANGFLOW_BACKGROUND_LEASE_TTL_S`, keeping it well above `LANGFLOW_BACKGROUND_HEARTBEAT_INTERVAL_S`.

## Voice mode {#voice-mode}

Voice mode is deprecated as of Langflow 1.10.
The Playground microphone now provides speech-to-text only.

For setup, remaining WebSocket endpoints, and current behavior, see [Use voice mode](/concepts-voice-mode).
