...
### Claude for Desktop not using MCP server tools correctly
...
### MCP browser-based flows won't open browser on Windows

If you're using MCP Tools with browser navigation actions such as Playwright on Windows and the agent executes the tools successfully but no browser tabs open, this is a known limitation.
