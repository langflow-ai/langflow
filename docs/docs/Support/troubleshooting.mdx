...
The Langflow image pre-creates `/app/langflow` with that user as the owner, so a new
`langflow-data` named volume inherits the correct ownership.
If you ran an earlier image build that did **not** pre-create this directory, the named
volume was initialized as `root:root` and the non-root container user cannot
write `secret_key` into it, and the error `PermissionError: [Errno 13]` on `/app/langflow/secret_key` appears.
...
To preserve the `langflow-postgres` data while resetting only the
`langflow-data` configuration volume, remove the `langflow-data` volume by name instead:
