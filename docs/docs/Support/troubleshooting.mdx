...
If you're using a flow from an version of Langflow before v1.7.2 and the **MCP Tools** component loses its **Tool Mode** setting, do the following:
...
3. In the component's template structure, locate the `tool_mode` field under `inputs`.
   The field may be missing or set to `false`.
