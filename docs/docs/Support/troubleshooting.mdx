...
2. Open the log contents to view the log files:
   ```bash
   open .
   ```
...
3. Locate the `Langflow.log` file, and then send it to support for debugging assistance.
...
When running multiple local Langflow instances on different ports, such as `localhost:7860` and `localhost:7861`, you might see authentication errors in the logs.
For example:
