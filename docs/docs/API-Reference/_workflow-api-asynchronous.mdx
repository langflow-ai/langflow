---
title: Asynchronous Workflow API endpoints
...
For synchronous execution (receiving complete results immediately), see [Synchronous Workflow API endpoints](/workflow-api-synchronous).

## Prerequisites

Before using the Asynchronous Workflow API, you need:
...
```bash
curl -N -X GET \
  "$LANGFLOW_SERVER_URL/api/v2/workflows?job_id=job_id_1234567890&stream=true&sequence_id=42" \
  -H "Accept: text/event-stream" \
  -H "x-api-key: $LANGFLOW_API_KEY"
```
