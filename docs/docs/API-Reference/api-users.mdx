...
<CodeBlock language="python">{examplePythonApiUsersSetUserPassword}</CodeBlock>

</TabItem>
<TabItem value="JavaScript" label="JavaScript">

<CodeBlock language="javascript">{exampleJavascriptApiUsersSetUserPassword}</CodeBlock>

</TabItem>
<TabItem value="curl" label="curl">

<CodeBlock language="bash">{exampleApiUsersSetUserPassword}</CodeBlock>
...
This route is self-service. Authenticate as the target user, send that user's `id` in the path, and include `current_password` plus the new `password`.
