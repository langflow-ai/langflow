...
To be compatible with Langflow's OpenAI Responses API endpoint, you flow and request must adhere to the following requirements:
...
- **Tools**: The `tools` parameter isn't supported. Returns an error if provided.
