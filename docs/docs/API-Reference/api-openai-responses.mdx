...
This request is using the same flow, but the LLM has not yet been introduced to Alice in this session.
...
Variables are only available during this specific request execution and are not persisted.
...
If your flow components reference variables that aren't provided via headers and don't exist in the database, the flow will fail, unless the `FALLBACK_TO_ENV_VARS` environment variable is `true`, in which case the flow will use the variable set in the `.env` file.

In the above example, `OPENAI_API_KEY` will fall back to the database variable if not provided in the header, and `USER_ID` and `ENVIRONMENT` will fall back to environment variables (if `FALLBACK_TO_ENV_VARS` is enabled) or cause the flow to fail if not found.