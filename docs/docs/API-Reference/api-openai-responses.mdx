...
Langflow includes an OpenAI Responses API-compatible endpoint at `POST /api/v1/responses`.

This endpoint allows you to use existing OpenAI client libraries with minimal code changes - just replace the `model` name, such as `gpt-4`, with your `flow_id`.
Flow IDs can be found on the code snippets on the [**API access** pane](/concepts-publish#api-access) or in a flow's URL.

For more information on the OpenAI Responses API, see the [OpenAI documentation](https://platform.openai.com/docs/api-reference/responses).
