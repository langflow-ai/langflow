...
The `api_key` parameter can have any value, such as `"api-dummy-key"` in the client examples, as the actual authentication is handled through the `default_headers` configuration.
