...
While this endpoint is designed to be compatible with OpenAI's API, there are some differences when using OpenAI client libraries.
The `api_key` parameter is required by the OpenAI SDK but is not used by Langflow, so requests must include a dummy API key value.
You can set it to any value, such as `"api-dummy-key"` in the client examples, as the actual authentication is handled through the `default_headers` configuration.

Langflow uses `x-api-key` headers for authentication, while OpenAI uses `Authorization: Bearer` headers.
When sending requests to Langflow with OpenAI client libraries, you'll need to configure custom headers and include an `api_key` configuration.

Replace the values for `LANGFLOW_SERVER_URL`, `LANGFLOW_API_KEY`, and `FLOW_ID` with values from your deployment.
<Tabs groupId="client">
