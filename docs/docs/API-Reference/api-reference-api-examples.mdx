...
:::tip
You can also manage MCP servers and clients in the Langflow visual editor:

* To manage all connections to external MCP servers, click <McpIcon aria-hidden="true"/> **MCP servers** when editing a flow, or click your profile icon, select **Settings**, and then click **MCP Servers**.

* To manage the MCP servers for your Langflow projects, click **MCP Server** on the [**Projects** page](/concepts-flows#projects), or, when editing a flow, click **Share**, and then select **MCP Server**.
:::
