...
* **Base**
  ...
  * POST `/v1/run/{flow_id_or_name}`: Run a flow (optionally stream tokens with `?stream=true`). See [Run flow](/api-flows-run#run-flow).
...
* **Files (v1)**. See [Files endpoints](/api-files#filesv1-endpoints)
...
* **Files (v2)**. See [Files endpoints](/api-files#filesv2-endpoints)
...
* **MCP (v2 servers)**
  * GET `/v2/mcp/servers`: List configured MCP servers (optionally include tool counts with `action_count=true`).
  * GET `/v2/mcp/servers/{server_name}`: Get a specific server configuration.
  * POST `/v2/mcp/servers/{server_name}`: Add a new server configuration.
  * PATCH `/v2/mcp/servers/{server_name}`: Update an existing server configuration.
  * DELETE `/v2/mcp/servers/{server_name}`: Delete a server configuration.
