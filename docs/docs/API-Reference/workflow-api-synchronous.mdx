---
title: Synchronous Workflow API endpoints
slug: /workflow-api-synchronous
---
...
The Synchronous Workflow API provides programmatic access to execute Langflow workflows synchronously, receiving complete results immediately upon completion. This API is part of the Langflow v2 API and offers enhanced workflow execution capabilities compared to the v1 `/run` endpoint.

For asynchronous execution modes (background jobs and streaming), see [Asynchronous Workflow API endpoints](/workflow-api-asynchronous).

## Prerequisites

Before using the Synchronous Workflow API, you need:
...
## Execute workflow endpoint (synchronous)
