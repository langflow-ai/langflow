...
| `expose_graph_state` | `boolean` | No | `true` | AG-UI only. When `false`, the stream carries only the conversation and omits the flow's graph state. See [Stream only the conversation](#stream-only-the-conversation). |
...
The field is accepted on `POST /api/v2/workflows` and `POST /api/v2/workflows/public`, and is ignored when `stream_protocol` is not `agui`.
