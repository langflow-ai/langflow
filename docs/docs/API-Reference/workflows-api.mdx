...
| `expose_graph_state` | `boolean` | No | `true` | AG-UI only. When `false`, the stream carries only the conversation and omits the flow's graph state. Not accepted by `POST /api/v2/workflows/public`. See [Stream only the conversation](#stream-only-the-conversation). |
...
##### Stream only the conversation {#stream-only-the-conversation}

The `STEP_*` and `STATE_*` events describe the flow itself: every component id that will run, and each component's own output, including intermediate results that are not the flow's answer. The Langflow canvas renders node status from them, so they are on by default.

Set `"expose_graph_state": false` when the consumer is another client, such as CopilotKit, a custom chat UI, or an embedded widget. The stream then carries only `RUN_*`, `TEXT_MESSAGE_*`, `TOOL_CALL_*`, and the message-lifecycle and `langflow.warning` `CUSTOM` events. `STATE_SNAPSHOT`, `STATE_DELTA`, `STEP_STARTED`, `STEP_FINISHED`, and `langflow.log` are never emitted, so they are never persisted for a background run either, and a re-attached client receives the same narrowed stream.
...
The field is ignored when `stream_protocol` is not `agui`.

`POST /api/v2/workflows/public` does not accept the field and always behaves as if it were `false`. An anonymous visitor to a shared flow has no reason to receive its topology, and a request that sends `expose_graph_state` is rejected with `422` rather than silently ignored.
