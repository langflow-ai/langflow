...
| `expose_graph_state` | `boolean` | No | `false` for `agui`, `true` for `langflow` | Whether the stream may carry the flow's graph state (per-component events and component outputs). Not accepted by `POST /api/v2/workflows/public`, which always runs without it. See [Graph state](#stream-only-the-conversation). |
...
| `warning` | Non-terminal notice (`data.message`), such as server code replacing saved component code. The run continues. |
...
`CUSTOM` events named `langflow.warning` carry a non-terminal notice in `value.message`, such as server code replacing saved component code. Display the notice and continue processing events until `RUN_FINISHED` or `RUN_ERROR`.

##### Graph state: what the stream says about the flow {#stream-only-the-conversation}

Some events describe the flow itself rather than the conversation. On `agui` those are `STATE_SNAPSHOT` and `STATE_DELTA` (every component id that will run, and each component's own output), `STEP_STARTED` / `STEP_FINISHED`, and `langflow.log`. On `langflow` they are `vertices_sorted`, the per-vertex `build_start`, `end_vertex`, and `log`. A component's output here is whatever it produced, including intermediate results that are not the flow's answer, such as retrieved documents or a rendered prompt.

`expose_graph_state` controls them:
...
**Changed in 1.12.3:** when the field is omitted, `agui` now defaults to `false`, since its consumers are usually third-party clients that read the conversation. `langflow` still defaults to `true`, so callers parsing the passthrough shape are unaffected. Set the field explicitly to pin the behavior on either protocol. A server logs one notice per process the first time an `agui` run relies on the new default.

With graph state off, the stream carries `RUN_*`, `TEXT_MESSAGE_*`, `TOOL_CALL_*`, and the `CUSTOM` events tied to the conversation: `langflow.content.*`, `langflow.message.removed`, `langflow.run.cancelled`, `langflow.human_input_required`, and `langflow.warning`. Messages no longer name the component that produced them: `properties.source` is emptied, which also removes the model name an LLM component reports there. The `output` event on `langflow` is the flow's answer, so it is still sent, and it carries the id of the output component that produced it. With graph state on, `langflow` reports an `output` for every terminal component, matching what sync returns; with it off, only real output components report, since a component left dangling is terminal without being the answer and its event would carry its own content. Human-in-the-loop is unaffected: a `request_id` embeds the paused component's id because resume needs it.

Suppressed events are never generated, so a background run never persists them and a re-attached client receives the same narrowed stream.

`POST /api/v2/workflows/public` does not accept the field and always runs without graph state, on every protocol. An anonymous visitor to a shared flow has no reason to receive its topology, and a request that sends `expose_graph_state` is rejected with `422` rather than silently ignored.
...
| `warnings` | Non-fatal notices, such as server code replacing saved component code. Empty when none apply; warnings do not change `status` or `has_errors`. Also included when polling completed results. |
...
Component substitution notices are returned to authenticated workflow callers. Anonymous public Playground runs do not expose these server policy details.
...
| `422 Unprocessable Entity` | Unknown `stream_protocol`, unsupported field for the selected mode, or refused `tweaks` with `TWEAKS_REFUSED`. |
...
For a refused tweak, `detail` is an object with `code` `TWEAKS_REFUSED`. See [Refused tweaks with `TWEAKS_REFUSED`](/deployment-tweaks-policy#tweaks-refused).
