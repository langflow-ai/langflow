...
Set `"expose_graph_state": false` when the consumer is another client, such as CopilotKit, a custom chat UI, or an embedded widget. The stream then carries only `RUN_*`, `TEXT_MESSAGE_*`, `TOOL_CALL_*`, and the message-lifecycle `CUSTOM` events. `STATE_SNAPSHOT`, `STATE_DELTA`, `STEP_STARTED`, `STEP_FINISHED`, and `langflow.log` are never emitted, so they are never persisted for a background run either, and a re-attached client receives the same narrowed stream.
