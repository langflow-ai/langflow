...
To sort the results, use `order_by` to select the field and `order` to select the direction.
The `order_by` parameter defaults to `timestamp`, and `order` accepts `ASC` or `DESC` and defaults to `ASC`.
`order_by` and `order` set the display order of the returned page; they don't change which page is selected.

The endpoint always returns a bounded page anchored at the most recent messages, so a flow with a long history never serializes in full.
Use `limit` to set the page size, which defaults to 100 and is clamped to a server-side maximum of 500.
Use `offset` to page backwards into older history.
With the defaults, the endpoint returns the 100 most recent messages, oldest first within that page.

This example retrieves up to 100 messages sent by `Machine` and `AI` in a given chat session (`session_id`), ordered from oldest to newest by timestamp.
