...
The six lists of restrictions are saved in one policy bundle.
...
| Allow only some integrations | `approved_integration_provider_ids` | `["google", "slack"]`. `[]` means every integration. |
| Block one integration action | `blocked_integration_action_keys` | `integrations.google.drive.delete` |
...
An integration outside `approved_integration_provider_ids`, or whose every action is
in `blocked_integration_action_keys`, disappears from the palette and from
`GET /api/v1/integrations`. Running a saved flow that still uses it fails before the
provider is contacted, and creating or authorizing a connection for it returns `403`.
...
Integration providers use their manifest `provider_id`, in lowercase.

```json
"approved_integration_provider_ids": ["google", "slack"]
```

Integration actions use the `policy_keys` a bundle declares for each capability.
A key is always `integrations.<provider_id>.<action>`; missing provider or action
segments and other malformed syntax are rejected with `422` before anything is written.
Keys are trimmed and case-folded. A syntactically valid key for a provider that
is not installed is accepted, so operators can configure policy before installing
a bundle. This does not verify that the provider or action exists; use the loaded
catalog to check the keys when the bundle is installed.
`GET /api/v1/integrations?include_blocked=true` lists every loaded provider with the
`policy_keys` of each of its actions, so you can copy the exact key to block.

```json
"blocked_integration_action_keys": ["integrations.google.drive.delete"]
```

Blocking one key of an action blocks the whole action.
A component stays visible while at least one of its actions is still allowed.
At execution, an action-picker bundle declares the selected capabilities through
`Component.select_integration_capabilities`. Blocking `delete` therefore does not
block an invocation selecting `search`. Bundles that do not implement selection
must be allowed to use every capability they declare.
Custom toolsets that override `_get_tools()` or `to_toolkit()` must also be allowed
to use every declared capability before construction, because their tools do not
pass through the standard per-invocation action checks. Their connection leases
cover that full set even if they implement the selection hook.
An empty or undeclared action selection returns HTTP `422` with
`error_code: "action-unsupported"`; it is not a governance denial.
Blocked executions return HTTP `403` with `error_code: "policy-blocked"` and an
administrator-action hint. Session-backed graph builds record the denial as an error
message in flow history.
...
5. To see the integration policy that applies to you, `GET /api/v1/integrations/policy/effective`.

    It returns the caller's allowed providers and whether a plugin owns
    the policy instead of the bundle. Only superusers receive the operator's
    blocked action keys and the complete loaded provider list. Other users
    receive an empty `blocked_action_keys` list and only allowed providers in
    `loaded_provider_ids`; use `GET /api/v1/integrations` to list usable actions.
    Check `unrestricted` before interpreting an empty approved provider list:
    an externally managed empty ceiling can deny every provider.
    Set `approved_integration_provider_ids` and `blocked_integration_action_keys` with
    `PUT /api/v1/policy-bundle`.

:::warning Installing an integration policy plugin

An empty persisted `approved_integration_provider_ids` list means unrestricted.
Installing a plugin must preserve that meaning. Denying all providers requires
an explicit external policy configuration; a plugin changing existing policy
semantics must provide an operator-visible migration or configuration step before
enforcement changes. This API does not perform that migration automatically.
Before enabling external enforcement, verify the intended provider set and check
`managed_externally` and `unrestricted` on the effective-policy endpoint.
