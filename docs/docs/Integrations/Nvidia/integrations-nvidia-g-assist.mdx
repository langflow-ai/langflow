...
The **NVIDIA G-Assist** component requires an NVIDIA GPU on a Windows operating system.

It uses the `gassist.rise` package, which is installed with all Langflow versions that include this component.

## Use the NVIDIA G-Assist component in a flow

1. Create a flow with a **Chat Input** component, **NVIDIA G-Assist (System-Assist)** component, and **Chat Output** components.
...
2. Connect the **Chat Input** component to the **NVIDIA G-Assist** component's **Prompt** input.
...
3. Connect the **NVIDIA G-Assist** component's output to the **Chat Output** component.
...
For example, `"What is my current GPU temperature?"`.
    ...
    The **NVIDIA G-Assist** component queries your GPU based on the prompt, and then prints the response to the **Playground**.
