...
Once you're authenticated with Composio, you must authenticate with the individual services, such as Gmail, through the Composio platform.
This is a two-step process:

* **Composio Authentication**: Use your Composio API key to connect Langflow to the Composio platform.
* **Service Authentication**: Authenticate with the specific service through Composio's interface.

The Composio API key handles the connection to Composio, while the individual service authentication is managed through Composio's platform for each service you want to use.
...
4. To authenticate your key and connect Langflow to Composio, click <Icon name="Unlink" aria-hidden="true"/> **Connect**.
A successful connection will turn the icon green.
5. In the Composio **Gmail** component, enable **Tool Mode**.
The **Actions** field populates with the actions available to the Agent, which it will choose from based on your request.
6. Connect the **Agent** component's **Tools** port to the Composio **Gmail** component's **Tools** port.
...
9. To test a specific tool or function, tell the agent to perform an action like writing a draft email:
...
11. Optional: To add more Composio tools, add more Composio components to your flow, enable **Tool Mode**, and then connect them to the **Agent** component's **Tools** port.
