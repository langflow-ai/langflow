...
Copy the script below into that file, then update the placeholder values with the details you collected in the previous step.
...
Open your Collector’s `config.yaml` file, copy in the configuration shown below, and replace the placeholder values with the ones that match your setup or requirements.
