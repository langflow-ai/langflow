...
To copy everything from SQLite into PostgreSQL, stop Langflow and run `langflow convert-sqlite-to-postgres`:

```bash
langflow convert-sqlite-to-postgres \
  --source sqlite:////path/to/langflow.db \
  --target postgresql://user:password@host:5432/langflow
```

The SQLite database must already be on the schema of the Langflow version you run the command with, so start that version against it once first.
If you copy the SQLite file before converting, use `sqlite3 langflow.db ".backup copy.db"` or `VACUUM INTO`. Langflow runs SQLite in WAL mode, so a plain file copy can come out empty.

The command creates the PostgreSQL schema, copies every row in one transaction, and checks each table's row count before committing.
If anything is refused, such as a value PostgreSQL does not accept or a target database that already belongs to another instance, nothing is written and the command lists the problems.
Keep the same `LANGFLOW_SECRET_KEY` when you point Langflow at the new database, or stored credentials can't be decrypted.

To move only your flows instead, do the following:
