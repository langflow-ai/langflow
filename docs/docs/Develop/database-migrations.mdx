...
Most Langflow users never need to run database migration.
...
To migrate your database, do the following:
    ...
    2. Back up the database.
         ...
         If you're using SQLite, copy the `langflow.db` file at `src/backend/base/langflow/langflow.db`.

         If you're using PostgreSQL, make a logical backup with `pg_dump`, or use your database provider's snapshot. For more information, see [enterprise backup guidance](/enterprise-database-guide).
...
Alembic output is written to a log file by default at `alembic/alembic.log`.
...
| `LANGFLOW_ALEMBIC_LOG_FILE` | Absolute or relative path for the Alembic log file when not logging to `stdout`. |
| `LANGFLOW_MIGRATION_LOCK_TIMEOUT_S` | Seconds to wait for the PostgreSQL migration advisory lock. Default: `300`. |
| `LANGFLOW_MIGRATION_LOCK_NAMESPACE` | Optional shared namespace for the Alembic PostgreSQL lock. Use the same value on all instances that share the database. |

## Troubleshooting

For troubleshooting database migrations, see [Troubleshoot Langflow](/troubleshoot).
