...
| `lfx-openai-compatible` | [OpenAI Compatible](./bundles-openai-compatible) model provider for any OpenAI-compatible endpoint (provider-only; no components) |
| `lfx-vllm` | [vLLM](/bundles-vllm) model provider for a self-hosted vLLM server (provider-only; no components) |
