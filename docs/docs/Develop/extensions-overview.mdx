...
* **`uv pip install langflow`** — the Langflow server plus the default curated component packages below as regular pip dependencies.
...
The long-tail `lfx-bundles` metapackage remains available for explicit installation, but is not installed by default with `langflow`. Install it when you need providers that still live there, such as `tavily`, `qdrant`, or `ollama`.
...
Code that imports components by their old in-tree path, such as `from lfx.components.qdrant import QdrantVectorStoreComponent`, keeps working as long as the bundle that now ships the provider is installed.
