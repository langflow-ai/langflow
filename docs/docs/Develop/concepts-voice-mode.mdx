...
* Optional: A [Gandr](https://gandr.ai) API key to enable Gandr voices for the LLM's response. The free tier is 50,000 tokens.
...
8. Optional: Add an ElevenLabs or Gandr API key to enable more voices for the LLM's response.
Langflow saves these keys as global variables.
...
The Langflow API's websocket endpoint requires an [OpenAI API key](https://platform.openai.com/docs/overview) for authentication, and supports optional [ElevenLabs](https://elevenlabs.io) and [Gandr](https://gandr.ai) integrations with the matching API key.
...
Gandr is an OpenAI compatible text-to-speech provider.
To use it, store your Gandr API key in the `GANDR_API_KEY` global variable or environment variable, and send the following message over the websocket connection:
...
The available voices are `gandr-mia`, `gandr-ava`, `gandr-jenny`, `gandr-dane`, `gandr-leo`, and `gandr-lewis`.
Gandr supports 23 languages, and every render is watermarked.
Keys are available at [gandr.ai](https://gandr.ai), and the free tier is 50,000 tokens.
