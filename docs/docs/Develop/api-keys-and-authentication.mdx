...
### LANGFLOW_ENABLE_PUBLIC_SIGNUP {#langflow-enable-public-signup}

When `LANGFLOW_ENABLE_PUBLIC_SIGNUP=True` (default), unauthenticated callers can create user accounts through the public users endpoint.
Account activation is still controlled by [`LANGFLOW_NEW_USER_IS_ACTIVE`](#langflow-new-user-is-active).

When `LANGFLOW_ENABLE_PUBLIC_SIGNUP=False`, only authenticated superusers can create user accounts.
Use this setting to prevent anonymous callers from filling the database with inactive signup records.

```text
LANGFLOW_ENABLE_PUBLIC_SIGNUP=False
```
