...
To run permitted executions behind a kernel boundary instead of blocking them outright, set a [sandbox backend](./environment-variables.mdx). Use `exec-sandbox` for a QEMU microVM on this host, or [`createos`](../Deployment/sandbox-createos.mdx) for a Firecracker microVM on a remote host when this one has no hypervisor.
