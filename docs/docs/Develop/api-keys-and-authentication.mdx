...
The default is `True` for development branches and local builds from those branches. It is `False` for official release packages, source checkouts/downloads of official release tags, and Docker images, including DEV/CDK images.
Official tags point to a release-only commit that disables auto-login, so GitHub's automatic source ZIP/TAR downloads use the same default as the wheels published to PyPI and GitHub Releases.
For trusted local development with a release package or Docker image, explicitly set `LANGFLOW_AUTO_LOGIN=True` to enable automatic login.
For more information about containers, see [Docker image defaults](/deployment-docker#docker-image-security-defaults).

#### Upgrade an existing auto-login installation {#upgrade-from-auto-login}

Before installing a release that disables auto-login, prepare the existing account for password sign-in. Keep the same database, configuration directory, encryption key, and superuser username so that your flows and stored credentials remain available.

1. Back up the database and configuration directory. Keep the instance restricted to trusted local access during the transition.
2. While signed in to the existing installation, identify the current account with the authenticated [Get current user API](/api-users#get-current-user). If you already know its password, verify that password with `POST /api/v1/login` and continue to step 4.
3. If the account uses an unknown generated bootstrap password, use an existing administrator session or API key with `PATCH /api/v1/users/{user_id}` to set a strong password on **that same account**. Send only the `password` field, as described in [Update user](/api-users#update-user), then verify password sign-in. The self-service `reset-password` endpoint requires the old password and is not suitable for an unknown bootstrap password.
4. Set `LANGFLOW_AUTO_LOGIN=false`, set `LANGFLOW_SUPERUSER` to the existing account's username, and set `LANGFLOW_SUPERUSER_PASSWORD` before restarting or upgrading. Startup requires this setting, but it does **not** overwrite a non-legacy password on an existing account. Use the password established in the previous steps.
5. Start the new release with the existing database and configuration directory. In a fresh browser session, sign in with the established password and verify that your existing flows are present.

Do not delete the existing account, database, or encryption key to regain access. Creating a different superuser does not transfer ownership of the existing account's flows. If you upgraded before preparing the account, use an existing administrator credential to update that account's password; if none is available, complete the preparation on a backed-up previous installation restricted to local access before upgrading again.

An explicit `LANGFLOW_AUTO_LOGIN=true` in an existing environment or `.env` file takes precedence over the new default. Remove that override or set it to `false` as part of the transition. Disabling auto-login prevents new anonymous sessions; it does not revoke previously issued sessions or API keys.
...
Anonymous public-flow links are direct-link capabilities, not discovery grants. Langflow accepts a canonical `PUBLIC` authorization share and preserves existing links derived from `Flow.access_type=PUBLIC` or an A2A folder with `auth_type=none`. Public shares never appear in general flow or shared-with-me listings.

A canonical `PUBLIC` share takes precedence over both compatibility grants. For flows, the shares API accepts only `permission_level=execute` at `PUBLIC` scope; execution includes the scrubbed direct-link read needed to load the playground. The API rejects `read`, `write`, and `admin` because there is no read-only, writable, or administrative anonymous flow surface. Deleting the share row removes the canonical grant and any still-set legacy setting becomes the effective grant again, so revoke access entirely by clearing the legacy setting as well.

An anonymous link grants only resource read and flow execution. It never grants create, write, delete, deploy, or administration actions, and callers cannot submit flow data or component tweaks. Public execution uses a stable non-user principal, so owner credentials, variables, files, and knowledge bases are unavailable unless a dependency has its own public authorization path. Errors are sanitized, and A2A HITL resumes recheck the current grant. Revocation blocks subsequent starts and resumes; it does not terminate work that is already running.

When `LANGFLOW_AUTHZ_ENABLED=true`, anonymous access also requires an authorization plugin that opts into public principals, maps request hosts through a deployment-owned allowlist, resolves a tenant from that mapping and server-derived domain context, and allows the action. The OSS public-principal plugin seam defaults to deny and never performs cross-tenant discovery. Plugins must not treat the raw `Host` header as a trusted tenant identity.

For A2A, `auth_type=none` applies this contract to both the direct agent card and execution routes. API-key and OAuth agent cards remain anonymously readable by direct link for A2A protocol discovery, but their JSON-RPC execution routes retain owner-key admission and owner execution.
...
The connection and integration endpoints count requests per signed-in user instead of per IP address, so users who reach Langflow through the same NAT or proxy do not share a limit. The unauthenticated OAuth callback is the exception and is still counted per IP address.
...
| `LANGFLOW_PUBLIC_FLOW_RATE_LIMIT_PER_MINUTE` | Integer | `20` | Maximum number of unauthenticated public-flow runs allowed per minute from a single IP address. Public flows consume resources under an isolated anonymous principal, so they are rate-limited separately from the `/login` endpoint. |
| `LANGFLOW_CONNECTION_METADATA_RATE_LIMIT_PER_MINUTE` | Integer | `60` | Maximum number of connection and integration metadata reads allowed per minute for each signed-in user. These endpoints return no credential material and make no outbound provider call, and the Connections UI polls the connection listing every two seconds while an OAuth consent is pending, so they are limited separately from and more generously than the `/login` endpoint. Connection writes have their own limit, and credential tests, health checks, and the OAuth endpoints keep their own tighter buckets. |
| `LANGFLOW_CONNECTION_WRITE_RATE_LIMIT_PER_MINUTE` | Integer | `30` | Maximum number of connection creates, updates, revokes, and deletes allowed per minute for each signed-in user. The four operations share one limit, sized for someone managing connections in the Connections UI rather than for login attempts. Credential tests, health checks, and starting an OAuth consent use `LANGFLOW_RATE_LIMIT_PER_MINUTE`, each with its own counter. |
...
| `LANGFLOW_KB_ALLOWED_HOSTS` | List[String] | Not set | An **exclusive** allowlist of destination hosts for the network knowledge base backends (OpenSearch and Chroma Cloud). Accepts the same patterns as `LANGFLOW_SSRF_ALLOWED_HOSTS`. Unlike that variable, which is an exception list that still admits any unlisted public host, this one denies by default: a destination a user supplies is refused unless its host is listed here. A destination Langflow reads from a server environment variable is already operator-controlled and does not need an entry, so setting `OPENSEARCH_URL` on the server keeps working unchanged. Chroma Cloud's default host is also unaffected. See the note below. |

#### Model provider endpoints {#provider-allowed-hosts}

Custom model provider endpoints require HTTPS unless the operator explicitly permits HTTP through `LANGFLOW_SSRF_ALLOWED_HOSTS`.
This allowlist accepts bare hostnames, IP addresses, `*.domain` wildcards, and CIDR ranges, without schemes or ports.
For a URL containing an IP address, a covering CIDR grants the HTTP exception: for example, `172.16.0.0/12` permits `http://172.17.0.3:8080/v1`.
For a DNS hostname, use its name or a wildcard; the HTTPS check does not resolve hostnames to match CIDRs.

An API key sourced from the server environment also requires the custom destination to be approved in `LANGFLOW_PROVIDER_CREDENTIAL_ALLOWED_HOSTS`.
That separate allowlist accepts bare hosts, `host:port`, and `*.domain` wildcards, but not CIDRs.
Prefix an entry with `http://` to explicitly permit sending that credential over HTTP.
For example, a trusted internal gateway at `http://172.17.0.3:8080/v1` using a server-provisioned key needs both settings:

```bash
export LANGFLOW_SSRF_ALLOWED_HOSTS=172.16.0.0/12
export LANGFLOW_PROVIDER_CREDENTIAL_ALLOWED_HOSTS=http://172.17.0.3:8080
```

#### Knowledge base destinations {#kb-allowed-hosts}

The OpenSearch and Chroma Cloud knowledge base backends need a stricter rule than the other connectors.

The SSRF variables above validate a hostname and then hand the original URL to a vendor SDK, which resolves DNS again when it connects.
For the **API Request** component Langflow closes that gap by pinning the validated IP address, but neither knowledge base SDK exposes a place to do that: `chromadb` builds its own HTTP client inside `CloudClient` and connects while the client is still being constructed, and `OpenSearchVectorSearch` passes one set of arguments to both its synchronous and asynchronous clients, so a single pinned transport cannot serve both.
A hostname whose DNS answer changes between validation and connection can therefore still reach an internal address.

`LANGFLOW_KB_ALLOWED_HOSTS` closes that by requiring the destination to be one you chose:

```bash
export LANGFLOW_KB_ALLOWED_HOSTS=search.corp.example,*.opensearch.internal
```

A destination is permitted when either of the following is true:

* Langflow read it from a server environment variable, such as `OPENSEARCH_URL` set on the server process. Only an operator can set one, so no allowlist entry is needed.
* Its host appears in `LANGFLOW_KB_ALLOWED_HOSTS`.

Everything else is refused, including hosts that resolve to ordinary public addresses.
A cluster URL a user saved as a Langflow variable in the UI, and a `cloud_host` sent in a request body, both need an entry.
Chroma Cloud's fixed default host, `api.trychroma.com`, takes no user input and is always permitted.

This is a second check rather than a replacement: an approved host must still satisfy `LANGFLOW_SSRF_PROTECTION_ENABLED` and `LANGFLOW_SSRF_ALLOWED_HOSTS`, so an environment variable pointing at a cloud metadata address is still refused.
Both checks share the `LANGFLOW_CONNECTOR_SSRF_VALIDATION_ENABLED` switch.
...
| `LANGFLOW_RESTRICT_LOCAL_FILE_ACCESS` | Boolean | `True` | By default, built-in file-reading components, including the [**File**](/read-file), [**Directory**](/legacy-core-components#legacy-data-components), [**JSON/CSV-to-Data**](/legacy-core-components#legacy-data-components), and the [**CSV Agent**](/bundles-langchain#csv-agent), [**JSON Agent**](/bundles-langchain#legacy-langchain-components), and [**OpenAPI Agent**](/bundles-langchain#openapi-agent) can only read and write paths inside the [`LANGFLOW_CONFIG_DIR`](/memory) storage directory. This also blocks `sqlite` and `duckdb` dialects in the [**SQL Database**](/sql-database) components and local-filesystem Git clones. **Security warning:** setting this to `false` allows any user who can build flows to read arbitrary files on the server (including Langflow's own secret keys) through file components and `sqlite:////path` database URLs. Set it to `false` only in single-tenant deployments where reading local server files by absolute path is an intentional feature. |
...
* `LANGFLOW_RESTRICT_LOCAL_FILE_ACCESS=true`: confines built-in file access to the upload sandbox. This is the default; keep it enabled when untrusted users can build flows.
...
The flags above decide *whether* code is allowed to run. They do not constrain what it does once it runs, so they are authorization controls rather than an isolation boundary.

To run permitted executions behind a kernel boundary instead of blocking them outright, set a [sandbox backend](./environment-variables.mdx). The built-in `exec-sandbox` backend runs code in a QEMU microVM on this host. Trusted third-party packages can register other backends.

A sandbox backend covers the [**Python Interpreter**](/python-interpreter) and the legacy Python REPL tool only — the other surfaces listed above still need the blocking flags.
...
5. Activate the account so the user can sign in.
    ...
    To reset another user's password, `PATCH /api/v1/users/{id}` with `"password": "<new password>"`. That path does not require the current password. `PATCH /api/v1/users/{id}/reset-password` is self-service: it must be called as that user and requires `current_password`. See [Update user](./api-users#update-user).
