...
### Local file access restriction {#local-file-access-restriction}

For multi-tenant deployments, enable the following setting together with
`LANGFLOW_ALLOW_CUSTOM_COMPONENTS=false`:

| Variable | Format | Default | Description |
|----------|--------|---------|-------------|
| `LANGFLOW_RESTRICT_LOCAL_FILE_ACCESS` | Boolean | `False` | When `true`, built-in file components may read and write only within the authenticated user's or executing flow's storage directory. Other tenants' storage, server secrets, the Langflow SQLite database, traversal paths, and escaping symlinks are rejected. This setting also blocks local-file database dialects such as `sqlite` and `duckdb` in the SQL Database components. |
