...
:::note Multi-tenant recommendation
For a deployment where mutually-untrusted users build flows, set `LANGFLOW_ALLOW_CUSTOM_COMPONENTS=false`, `LANGFLOW_BLOCK_CODE_INTERPRETER_COMPONENTS=true`, `LANGFLOW_RESTRICT_LOCAL_FILE_ACCESS=true`, and `LANGFLOW_MCP_SERVER_ALLOWED_PACKAGES` together. The first blocks user-authored component code; the second blocks the built-in code-execution components that would otherwise be an equivalent escape hatch; the third confines built-in file access to the current user or flow's upload storage; and the package allowlist prevents tenants from publishing and executing arbitrary npm/Python packages through MCP. Pair these with the SSRF settings above.
:::
