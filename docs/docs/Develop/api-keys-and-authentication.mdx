...
import PartialAutoLoginDefaults from '@site/docs/_partial-auto-login-defaults.mdx';
...
To connect Langflow to an upstream identity provider, see [SSO and external authentication](./external-authentication).
For role-based access control, see [Authorization](./authorization).
...
* If `LANGFLOW_AUTO_LOGIN=False`, automatic login is disabled. Users must sign in to the visual editor, authenticate as a superuser to run certain Langflow CLI commands, and use a Langflow API key or session token for Langflow API requests. Session tokens can be sent in the `access_token_lf` cookie or `Authorization: Bearer` header.
...
API requests still require a Langflow API key or a session token unless you also set [`LANGFLOW_SKIP_AUTH_AUTO_LOGIN`](#langflow-skip-auth-auto-login) to `true`.

<PartialAutoLoginDefaults />

#### LANGFLOW_SKIP_AUTH_AUTO_LOGIN {#langflow-skip-auth-auto-login}

When `LANGFLOW_AUTO_LOGIN=true`, set `LANGFLOW_SKIP_AUTH_AUTO_LOGIN=true` to also skip authentication for API requests.
Unauthenticated requests then run as the configured superuser.
The default is `false`, so a Langflow API key or a session token is still required even when auto-login is enabled.
...
When `LANGFLOW_AUTO_LOGIN=True`, `LANGFLOW_SUPERUSER` selects the auto-login account username and `LANGFLOW_SUPERUSER_PASSWORD` is optional. If the password isn't set, or if it is the legacy value `langflow`, Langflow generates a bootstrap password for the auto-login account.
...
| Variable | Format | Default | Description |
|----------|--------|---------|-------------|
| `LANGFLOW_ALLOW_CUSTOM_COMPONENTS` | Boolean | `True` | When `false`, blocks user-authored component code and registered built-in code-execution components at runtime. See [Block custom components](/deployment-block-custom-components). |
...
With authentication enabled, all users must sign in to the visual editor with valid credentials, and API requests require a Langflow API key or session token.
...
* [SSO and external authentication](./external-authentication)
