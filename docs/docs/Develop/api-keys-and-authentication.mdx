...
| `LANGFLOW_MCP_SERVER_INTERPRETER_HARDENING` | Boolean | `False` | When `true`, MCP stdio configurations cannot execute tenant-selected Python modules/scripts or Node.js files. Langflow's authenticated internal agentic MCP module remains available. Published Langflow images enable this by default. |

:::note Multi-tenant recommendation
For a deployment where mutually-untrusted users build flows, set `LANGFLOW_ALLOW_CUSTOM_COMPONENTS=false`, `LANGFLOW_BLOCK_CODE_INTERPRETER_COMPONENTS=true`, `LANGFLOW_RESTRICT_LOCAL_FILE_ACCESS=true`, `LANGFLOW_MCP_SERVER_INTERPRETER_HARDENING=true`, and `LANGFLOW_MCP_SERVER_ALLOWED_PACKAGES` together. These controls block user-authored and built-in code execution, confine local files to the current user or flow's upload storage, reject tenant-selected Python/Node MCP entrypoints, and restrict MCP package runners to operator-approved packages. Pair these with the SSRF settings above.
:::
