...
In a multi-tenant deployment where mutually-untrusted users build flows, keep `LANGFLOW_CONNECTOR_SSRF_VALIDATION_ENABLED=true` and `LANGFLOW_SSRF_PROTECTION_ENABLED=true`, and set `LANGFLOW_CONNECTOR_SSRF_ALLOW_LOOPBACK=false`, so a tenant cannot point a vector store, SQL database, model-provider proxy, Ollama/LM Studio/Home Assistant URL, or Glean/AstraDB tool at a loopback or internal service or the cloud-metadata endpoint. Allowlist your own internal hosts with `LANGFLOW_SSRF_ALLOWED_HOSTS`.
:::
...
| `LANGFLOW_RESTRICT_LOCAL_FILE_ACCESS` | Boolean | `False` | When `true`, built-in file-reading components (File, Directory, JSON/CSV-to-Data, and the CSV/JSON/OpenAPI agents) may only read paths in the authenticated user's or executing flow's storage subdirectory, and `save_file` writes are confined there too. Paths in another user's or flow's storage are rejected, as are absolute server paths (`/etc/passwd`, the SQLite DB, secrets), traversal strings, and escaping symlinks. This setting also blocks local-file database dialects (`sqlite`, `duckdb`) in the SQL Database components and local-filesystem Git clones. |
| `LANGFLOW_MCP_SERVER_DOCKER_HARDENING` | Boolean | `False` | When `true`, MCP Docker transports are limited to isolated `docker run` invocations. Host file/API/device access, host or container namespace sharing, named networks, published ports, custom runtimes, restart persistence, and sandbox-downgrading security options are rejected. Published Langflow images enable this by default. |
| `LANGFLOW_MCP_SERVER_ALLOWED_PACKAGES` | String | Not set | A comma-separated allowlist of exact package names that MCP `npx` and `uvx` stdio servers may download and execute. Version pins are permitted, but direct URL/path references and package-injection options are rejected. Set this in multi-tenant deployments; an empty value blocks all package runners. Published Langflow images allow only `mcp-proxy` and `lfx` by default. |
...
This hardening narrows `LANGFLOW_RESTRICT_LOCAL_FILE_ACCESS=true` from the entire `config_dir` to the authenticated user's or executing flow's storage subdirectory. Before upgrading a deployment that already enables this setting, move any component-managed files stored directly under `config_dir` into the appropriate user or flow storage subdirectory and update saved paths. Files left directly under `config_dir` are no longer readable or appendable through components.
:::

:::note Multi-tenant recommendation
For a deployment where mutually-untrusted users build flows, set `LANGFLOW_ALLOW_CUSTOM_COMPONENTS=false`, `LANGFLOW_BLOCK_CODE_INTERPRETER_COMPONENTS=true`, `LANGFLOW_RESTRICT_LOCAL_FILE_ACCESS=true`, `LANGFLOW_MCP_SERVER_DOCKER_HARDENING=true`, `LANGFLOW_MCP_SERVER_INTERPRETER_HARDENING=true`, and `LANGFLOW_MCP_SERVER_ALLOWED_PACKAGES` together. These controls block user-authored and built-in code execution, confine local files to the current user or flow's upload storage, isolate MCP Docker transports, reject tenant-selected interpreter/shell MCP entrypoints, and restrict MCP package runners to operator-approved packages. Pair these with the SSRF settings above.
:::
