...
A canonical `PUBLIC` share takes precedence over both compatibility grants. When one exists, its permission level alone decides the action, so a share at `read` denies anonymous execution even while `Flow.access_type=PUBLIC` or `auth_type=none` is still set — use it to narrow an existing public link without editing the legacy setting. Deleting the share row removes the canonical grant and any still-set legacy setting becomes the effective grant again, so revoke access entirely by clearing the legacy setting as well.
