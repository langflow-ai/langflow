...
As of Langflow 1.11.x, Langflow application defaults remain permissive for trusted single-user installs, while official Docker images enable the hardened values. For more information, see [Docker image security defaults](/deployment-docker#docker-image-security-defaults).
...
:::note When to enable these flags
If untrusted users can build flows on this server, set the following together:
...
Pair these with the SSRF settings above to reduce common escape hatches; they are not full user isolation. See [Security](/security).
:::

:::warning Shared tracing backends
External tracing integrations (LangSmith, Langfuse, Phoenix, Arize, Opik, and similar) are process-wide, not per user.
On a shared server, every user's flow inputs and outputs go to the same tracing project.
Do not enable a shared SaaS tracer when users are untrusted; use built-in local tracing instead.
:::
