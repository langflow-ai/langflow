...
| `LANGFLOW_MCP_SERVER_INTERPRETER_HARDENING` | Boolean | `True` | When `true`, blocks tenant-controlled Python, Node.js, and shell MCP stdio entrypoints while retaining approved package runners and Langflow's authenticated internal MCP server. Set to `false` only for an explicitly trusted single-tenant deployment that requires legacy interpreter commands; this re-enables host process execution from MCP configurations. |
...
* `LANGFLOW_MCP_SERVER_INTERPRETER_HARDENING=true`: keeps direct interpreter and shell MCP entrypoints blocked (the default). Leave it enabled for shared deployments.
...
### MCP interpreter hardening {#mcp-interpreter-hardening}

MCP stdio configurations can start processes on the Langflow host. Interpreter hardening is enabled by default and blocks direct Python, Node.js, and shell scripts supplied through those configurations. Use `LANGFLOW_MCP_SERVER_INTERPRETER_HARDENING=false` only for a trusted single-tenant deployment that intentionally relies on legacy commands such as `python -m mcp_weather_server`; never use this opt-out for shared or untrusted users.
