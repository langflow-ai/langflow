...
By default, Langflow validates API keys against keys stored in the database.
However, you can configure Langflow to validate API keys against an environment variable instead.
...
This variable controls how Langflow validates API keys passed in the `x-api-key` header or query parameter.
...
#### Use environment-based API key validation {#env-based-api-key}

When `LANGFLOW_API_KEY_SOURCE=env`, Langflow validates the `x-api-key` header against the value of the `LANGFLOW_API_KEY` environment variable instead of checking the database.

This mode is ideal for:

* **Kubernetes deployments**: Inject API keys through Kubernetes Secrets without needing to create database entries.
* **CI/CD pipelines**: Use a fixed API key for automated testing and deployment workflows.
* **Containerized environments**: Pre-configure authentication without post-deployment setup.
* **Single-key scenarios**: When you need a simple, centralized API key managed through infrastructure configuration.

To enable environment-based API key validation:

1. Set the API key source to `env`:

    ```text
    LANGFLOW_API_KEY_SOURCE=env
    ```

2. Set the API key value:
...
:::warning
When using `LANGFLOW_API_KEY_SOURCE=env`, all authenticated requests use the same API key, and successful authentication grants superuser privileges.
This mode is designed for single-tenant deployments or automated systems, not multi-user environments where different users need different access levels.
:::

<details>
<summary>Kubernetes deployment example</summary>

The following example shows how to configure environment-based API key validation in a Kubernetes deployment using a Secret:
...
2. Reference the Secret in your Langflow deployment:
...
The following example shows how to configure environment-based API key validation with Docker Compose:
...
Then set the API key in your `.env` file or environment:
...
#### Database vs. environment API key validation

| Feature | `LANGFLOW_API_KEY_SOURCE=db` | `LANGFLOW_API_KEY_SOURCE=env` |
|---------|------------------------------|-------------------------------|
| Key storage | Langflow database | Environment variable |
| Key management | Langflow UI or CLI | Infrastructure configuration |
| Multiple keys | Yes | No (single key) |
| Per-user keys | Yes | No (superuser access) |
| Usage tracking | Yes (`total_uses`, `last_used_at`) | No |
| Key rotation | Through Langflow UI/CLI | Update environment variable and restart |
| Best for | Multi-user environments | Automated systems, Kubernetes, CI/CD |
