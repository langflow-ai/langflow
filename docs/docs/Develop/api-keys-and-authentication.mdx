...
Anonymous public-flow links are direct-link capabilities, not discovery grants. Langflow accepts a canonical `PUBLIC` authorization share and preserves existing links derived from `Flow.access_type=PUBLIC` or an A2A folder with `auth_type=none`. The compatibility grants are revoked by changing the corresponding legacy setting; a canonical grant is revoked by deleting its share row. Public shares never appear in general flow or shared-with-me listings.
