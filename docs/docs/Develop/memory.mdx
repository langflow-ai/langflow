...
- `pool_size`: Maximum number of database connections to keep in the pool. Default: 20 seconds.
- `max_overflow`: Maximum number of connections that can be created beyond the pool_size. Default: 30 seconds.
...
**Separate Connection Settings**:
- `LANGFLOW_DB_CONNECT_TIMEOUT`: Maximum number of seconds to wait when establishing a new database connection. This is separate from the connection pool settings and is primarily used for SQLite connections. Default: 30 seconds.
