...
For more information and an example, see [Configure an external PostgreSQL database](/configuration-custom-database).

To fine-tune your database connection pool and timeout settings, use the `LANGFLOW_DB_CONNECTION_SETTINGS` environment variable to define the JSON configuration for database connection pool settings:
...
This variable accepts a JSON object containing the database connection pool parameters:
