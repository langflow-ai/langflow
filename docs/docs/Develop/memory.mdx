...
* `LANGFLOW_MIGRATION_LOCK_NAMESPACE`: Optional namespace for PostgreSQL advisory locks used during database migrations. If not provided, a hash of the database URL is used. Useful when multiple Langflow instances share the same PostgreSQL database and need coordinated migration locking. This setting only applies when using PostgreSQL as your database backend.
