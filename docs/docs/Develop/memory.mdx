...
<!-- TODO: Message History not reqd with Agent. Message history alone vs external chat storage -->

To store and retrieve messages in local Langflow memory, use **Message History** components in your flow.

To store or retrieve chat messages from external memory, use both **Message History** and third-party chat memory components in your flow.
If external storage is connected to the **Message History** component, then no chat messages are stored in local Langflow memory.

For more information and examples, see the [**Message History** component](/components-helpers#message-history) reference and [**Memory chatbot** template](/memory-chatbot).
