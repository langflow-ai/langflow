...
This includes essential Langflow database tables, file management, caching, and chat memory.
...
The following tables are stored in Langflow's application database.
This list covers the main product tables.
On OSS, the default is the SQLite file `langflow.db`. Desktop uses `database.db`. You can also use an [external PostgreSQL database](#configure-external-memory).
...
• **A2ATask** and **A2ACheckpoint**: Persist Agent-to-Agent (A2A) protocol tasks and human-in-the-loop graph checkpoints so A2A runs survive restarts and can resume across workers.

• **Deployment**: Tracks deployments of Langflow flows to external providers, including the deployment name, provider resource key, and associated project and provider account.
...
• **MemoryBase**: Stores memory base configurations for a flow, including display name, linked `kb_name`, embedding model, ingestion threshold, auto-capture, and optional preprocessing settings. Vector-store `backend_type` / `backend_config` are not columns on this table; they live on the linked **KnowledgeBase** row. For more information, see [Manage memory bases](/memory-bases).

• **MemoryBasePreprocessingOutput**: Captures LLM-distilled preprocessing output before vector-store writes. Each row represents one preprocessing batch and tracks the status (`processed`, `ingested`, or `skipped`) to support resumable two-phase commit (LLM call → knowledge base write).
...
Additional tables support authorization, SSO, and governance, including **AuthzRole**, **AuthzRoleAssignment**, **AuthzShare**, **AuthzTeam**, **AuthzAuditLog**, **CasbinRule**, **SSOConfig**, **SSOSettings**, **SSOUserProfile**, **CatalogPolicyRule**, **ModelProviderPolicy**, and policy bundle tables.
For more information, see [Authorization](/authorization) and [External authentication](/external-authentication).
...
* `LANGFLOW_MIGRATION_LOCK_NAMESPACE`: Optional namespace for PostgreSQL advisory locks used during database migrations. If unset, Alembic uses the static lock key `11223344`, which coordinates every Langflow instance on the same PostgreSQL server. If set, Alembic hashes the namespace to derive the lock key. Set this variable to isolate different Langflow deployments that share one PostgreSQL server, and use the same value on every instance in that deployment. This setting only applies when using PostgreSQL as your database backend.
...
| `LANGFLOW_CACHE_SIGNING_KEY` | String | Not set | Shared secret used to sign Redis cache payloads. Set the **same value on every replica** that shares a Redis instance. When unset, each deployment derives a key from its own config directory, so separate replicas sign with different keys and cannot verify each other's entries — valid entries are then discarded as misses and the cache never hits across replicas. Workers of a single deployment share a config directory and agree on a key without this. |

:::warning
If you run more than one Langflow replica against one Redis instance, set `LANGFLOW_CACHE_SIGNING_KEY` to the same value on all of them.
Cache entries carry an integrity tag, and a replica can only verify entries signed with its own key.
Without a shared key each replica generates its own, so cross-replica reads are discarded as cache misses.
:::
...
For semantic retrieval of past conversations across sessions, use a [memory base](/memory-bases) instead of chronological chat memory.
The **Memory Base** component enables **Filter by Session** by default, so retrieval stays in the current session. Disable **Filter by Session** to search across all sessions ingested into the memory base.
...
* [Configure vector database providers](/knowledge#configure-vector-database-providers)
