...
3. Validate the extension — see [Validate an extension](#validate-extension).
...
## Validate an extension {#validate-extension}

Run the static checker from your extension root before `lfx extension dev` or `pip install`:

```bash
lfx extension validate .
```

The validator parses the manifest, checks bundle paths, and walks Python source files. It reports the same structured errors as the loader — `code`, `message`, `location`, `content`, and `hint` — so you can fix problems without restarting Langflow.

Imports are **not** executed by default, so validation is safe on extensions you just downloaded. Pass `--execute-imports` only when you trust the source; that flag runs an import probe in a subprocess.
