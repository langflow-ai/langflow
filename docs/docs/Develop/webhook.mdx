...
If the input is not valid JSON, the **Webhook** component wraps it in a `payload` object so that it can be processed and trigger the flow.
...
Use the following information to help address common issue that can occur with the **Webhook** component.
