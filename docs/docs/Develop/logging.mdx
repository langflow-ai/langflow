...
| `LANGFLOW_NATIVE_TRACING` | Boolean | `true` | Enables Langflow’s built-in tracer to record execution traces directly in the Langflow database for use in Trace View. Set to `false` to disable tracing. |
