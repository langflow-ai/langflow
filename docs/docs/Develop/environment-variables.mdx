...
The `LF_CONNECTION__*` family and `LANGFLOW_REQUEST_VARIABLES` carry credentials to headless runtimes without a database. See [Resolve connections in headless LFX](/lfx-connections).
