...
| `LANGFLOW_LOAD_FLOWS_OVERWRITE_ON_NAME_MATCH` | Boolean | `True` | When a flow file in `LANGFLOW_LOAD_FLOWS_PATH` shares a name with an existing DB row but has a different `id`, controls whether to overwrite the existing row. `True` (default) preserves the prepackaged-flows-are-source-of-truth behavior for CI/CD pipelines where UUIDs regenerate between builds. Set to `False` to skip-with-warning and preserve user edits made in the UI on restart. |
