...
| `LANGFLOW_JOB_QUEUE_TYPE` | String | `asyncio` | Job queue backend for the v1 build endpoints (`/api/v1/build/*`). Use `redis` for multi-worker deployments. Does not affect background runs of the v2 workflows API; see `LANGFLOW_BACKGROUND_BACKEND`. See [Deploy Langflow with multiple workers](./deployment-multi-worker#configuration-reference). |
| `LANGFLOW_BACKGROUND_BACKEND` | String | `default` | Backend for background runs of the v2 workflows API. `default` runs them inside API workers. `scaled` queues them in the database for separate `langflow worker` processes to run. See [Scale background runs](./deployment-multi-worker#scale-background-runs). |
| `LANGFLOW_BACKGROUND_RETENTION_DAYS` | Integer | `0` | Days to keep finished background runs, with their events, before deleting them, measured from when the run finished. `0` disables retention and keeps every row forever. Only v2 background runs are deleted: queued, running, and paused runs are never deleted at any age, and neither are runs from the v1 build endpoints or knowledge-base ingestion. See [Background run settings](./deployment-multi-worker#background-run-settings). |
