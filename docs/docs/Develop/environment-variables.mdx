...
| `LANGFLOW_AUDIT_ENABLED` | Boolean | `False` | Record an append-only audit event for audited Flow and Project operations in the `audit_events` table. A committed operation and its event share one transaction. Events never contain Flow graphs, component settings, secrets, or request bodies. |
