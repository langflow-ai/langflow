...
### Sandbox backends {#langflow-sandbox}

These variables control optional execution backends for the [**Python Interpreter** component](/python-interpreter#sandbox) and the legacy Python REPL tool.
The `exec-sandbox` backend is not included in a default `uv pip install langflow` installation; install `langflow[sandbox]` to use it.

| Variable | Format | Default | Description |
|----------|--------|---------|-------------|
| `LANGFLOW_SANDBOX_BACKEND` | String | `none` | Name of the execution backend for the code-execution components (Python Interpreter and the legacy Python REPL tool). Any registered backend name is accepted — see [Sandbox backends](../Deployment/sandbox-backends.mdx). Set to `exec-sandbox` to run user code in a dedicated QEMU microVM per execution on this host. Requires `uv pip install 'langflow[sandbox]'`, Python 3.12 or later, and QEMU 8+ with hardware virtualization (KVM on Linux, HVF on macOS). Set to `createos` to run each execution in a throwaway Firecracker microVM on the [CreateOS](../Deployment/sandbox-createos.mdx) control plane instead, which needs no hypervisor on this host but does need `CREATEOS_SANDBOX_API_KEY`. Under either backend, a sandbox that is configured but unavailable fails with an error instead of falling back to in-process execution. |
| `LANGFLOW_SANDBOX_TIMEOUT_SECONDS` | Integer | `30` | Wall-clock limit for one sandboxed execution (1-300 seconds). The selected backend can declare a lower cap, and a value above that cap is refused even inside this range — see [Sandbox backends](../Deployment/sandbox-backends.mdx). Only used when `LANGFLOW_SANDBOX_BACKEND` is not `none`. |
| `LANGFLOW_SANDBOX_MEMORY_MB` | Integer | `192` | Guest VM memory for sandboxed executions, in MB (minimum 128). Under `exec-sandbox` this is the VM size. Under `createos` it is only a **floor**: a CreateOS shape fixes vCPU as well as memory, so the VM defaults to the `s-2vcpu-4gb` shape and this value can only raise it, never lower it. Pin an exact shape with `CREATEOS_SANDBOX_SHAPE`. Only used when `LANGFLOW_SANDBOX_BACKEND` is not `none`. |
| `LANGFLOW_SANDBOX_ALLOW_NETWORK` | Boolean | `False` | Whether sandboxed code may access the network. The default blocks every routable destination (under `createos`, link-local `169.254.0.0/16` remains reachable — see [Run code in CreateOS microVMs](../Deployment/sandbox-createos.mdx)). The two backends differ when this is enabled without `LANGFLOW_SANDBOX_ALLOWED_DOMAINS`: `exec-sandbox` reaches only its package-registry defaults (PyPI), while `createos` grants **unrestricted** egress because the platform has no equivalent built-in list. On `createos`, always set `LANGFLOW_SANDBOX_ALLOWED_DOMAINS` alongside this. Only used when `LANGFLOW_SANDBOX_BACKEND` is not `none`. |
| `LANGFLOW_SANDBOX_ALLOWED_DOMAINS` | String | Not set | Comma-separated list of domains sandboxed code may reach when network access is enabled. Treat like an SSRF allow-list: prefer narrow, fully-qualified domains. Under `createos`, each entry must be `host`, `*.host`, an IPv4 address, or a CIDR block, each optionally with `:port`; IPv6 and `*:port` are rejected because the platform cannot enforce them, and a malformed entry refuses the run rather than being sent. A public DNS resolver is also added so names resolve at all — that resolver answers for any name, so DNS remains a low-bandwidth exfiltration channel even though connections to other hosts are blocked. Only used when `LANGFLOW_SANDBOX_BACKEND` is not `none`. |
| `LANGFLOW_SANDBOX_ACCEPT_EGRESS_EXCEPTIONS` | Boolean | `False` | Whether to run when the selected backend cannot block every destination. A backend declares what it cannot block. If you restrict egress — `LANGFLOW_SANDBOX_ALLOW_NETWORK` is false, or `LANGFLOW_SANDBOX_ALLOWED_DOMAINS` is set — a backend with any such exception is refused unless this is `true`. Under `createos` the exception is `169.254.0.0/16`, which carries the VM metadata service and stays reachable from the guest whatever the policy says. Set this to `true` only after you decide those destinations are acceptable. Only used when `LANGFLOW_SANDBOX_BACKEND` is not `none`. |
| `LANGFLOW_SANDBOX_ALLOW_SOFTWARE_EMULATION` | Boolean | `False` | Permit the sandbox to run without a hardware hypervisor, letting QEMU fall back to TCG software emulation. Upstream documents TCG as not security-supported, so enable only for trusted/development workloads. Applies to `exec-sandbox` only; it has no effect under `createos`, where the hypervisor is on the CreateOS host. Only used when `LANGFLOW_SANDBOX_BACKEND` is not `none`. |
| `LANGFLOW_SANDBOX_BACKEND_PLUGINS` | String | Not set | Comma-separated names of out-of-tree sandbox backends that may load, from the `lfx.sandbox_backends` entry point group — see [Sandbox backends](../Deployment/sandbox-backends.mdx). Empty (default) means no third-party backend code is imported at all. Loading a plugin imports its code into the Langflow process on the path that decides whether user code is isolated, and a backend's declared capabilities are configuration metadata rather than proof it enforces them, so naming one here is a statement that you trust that package. A name already used by a built-in backend is refused rather than replaced. |
| `CREATEOS_SANDBOX_API_KEY` | String | Not set | CreateOS API key, required when `LANGFLOW_SANDBOX_BACKEND=createos`. Get one at [createos.nodeops.network/profile](https://createos.nodeops.network/profile). The key is sent as a request header, so the control-plane URL must use `https`. Execution is refused when this is unset. |
| `CREATEOS_SANDBOX_BASE_URL` | String | `https://api.sb.createos.sh` | CreateOS control-plane URL. Must use `https` unless it points at loopback (`localhost`, `127.0.0.1`, `::1`) for local development against a self-hosted control plane. |
| `CREATEOS_SANDBOX_ROOTFS` | String | `devbox:1` | CreateOS rootfs image for the guest. Because the sandbox has no network by default, whatever Python packages this image ships are the only ones available to sandboxed code. To add packages, build a CreateOS template and put its name here — see [Run code in CreateOS microVMs](../Deployment/sandbox-createos.mdx). |
| `CREATEOS_SANDBOX_SHAPE` | String | Not set | Pin an exact CreateOS shape (for example `s-4vcpu-8gb`) instead of the `s-2vcpu-4gb` default. Run `curl https://api.sb.createos.sh/v1/shapes` for the catalog. A shape that is unknown, or smaller than `LANGFLOW_SANDBOX_MEMORY_MB`, refuses the run. |
| `LANGFLOW_SANDBOX_SESSION_MODE` | String | `off` | Whether one guest VM is reused across executions of the same flow. `off` gives every execution a fresh guest. `flow` shares one guest between executions of the same flow by the same user within one worker process, until it goes idle. Variables, imports and files then carry from one execution to the next, which removes the VM creation cost from every execution after the first and lets an agent build up state across tool calls. Reuse is a deliberate weakening of isolation **between executions** — code from one execution can read what earlier code in the same flow left behind — so read [Sessions](../Deployment/sandbox-createos.mdx) before enabling it. A load-balanced flow can have one independent guest per worker, and those guests do not share state. An execution that times out destroys its guest rather than handing it to the next run. Honored only by backends that report session support; `exec-sandbox` does not, and runs cold either way. |
| `LANGFLOW_SANDBOX_SESSION_IDLE_SECONDS` | Integer | `600` | How long a reused guest may sit idle before it is destroyed. Bounds both the cost of a forgotten session and how long its state stays readable. Only used when `LANGFLOW_SANDBOX_SESSION_MODE` is not `off`. |
| `LANGFLOW_SANDBOX_COLLECT_ARTIFACTS` | Boolean | `False` | Whether files written to `/workspace/artifacts` in the guest are read back after a run. The component then returns them alongside stdout, so code that saves a chart or a CSV can hand it to the rest of the flow. Costs one extra round trip per execution. Refused at run time when the configured backend cannot read files out of the guest, rather than silently returning nothing. `exec-sandbox` cannot; `createos` can. |
| `LANGFLOW_SANDBOX_MAX_ARTIFACT_BYTES` | Integer | `5242880` | Total size of collected artifacts allowed per execution, in bytes (5 MiB). Guest code chooses what it writes, so this caps what a runaway or hostile program can pull into the Langflow process. Artifacts travel base64-encoded inside the component output, so raise this only when the deployment can carry payloads of that size. The same number bounds the compressed archive on the wire, and at most 256 entries are read, so an archive of many empty files cannot cost unbounded work. Only used when `LANGFLOW_SANDBOX_COLLECT_ARTIFACTS` is `True`. |
| `CREATEOS_SANDBOX_METRICS` | Boolean | `False` | Log the guest's CPU, memory and bandwidth use after each execution. Off by default because it costs one extra control-plane request per execution. |
...
| `LANGFLOW_ALLOW_PUBLIC_CUSTOM_COMPONENTS` | Boolean | `False` | Whether flows shared through the **Shareable Playground** can run custom component code. By default, an unauthenticated public flow build runs only the server's own copy of each component, and a flow containing a custom component is refused with `This flow cannot be executed.` Set to `true` to build public flows from the database as their owner. This requires `LANGFLOW_ALLOW_CUSTOM_COMPONENTS=true`, and it lets every node in a public flow run the code stored in that flow, not only the custom ones. To run a custom component in a shared **Playground** without relaxing that, install the component on the server with `LANGFLOW_COMPONENTS_PATH` instead. For more information, see [Custom components in publicly shared flows](/deployment-block-custom-components#public-flow-custom-components). |
