...
3. Define [Langflow environment variables](#supported-variables) in the `.env` file.
    ...
    LANGFLOW_AUTO_SAVING_INTERVAL=2000
...
Environment="LANGFLOW_AUTO_SAVING_INTERVAL=2000"
...
| `LANGFLOW_SANDBOX_ALLOW_NETWORK` | Boolean | `False` | Whether sandboxed code may access the network. The default runs the microVM fully offline. When enabled without `LANGFLOW_SANDBOX_ALLOWED_DOMAINS`, exec-sandbox reaches only its package-registry defaults (PyPI). Only used when `LANGFLOW_SANDBOX_BACKEND` is not `none`. |
...
| `LANGFLOW_AUTO_SAVING_INTERVAL` | Integer | `2000` | Set the auto-save interval in milliseconds if `LANGFLOW_AUTO_SAVING=True`. |
...
| `LANGFLOW_RESTRICT_LOCAL_FILE_ACCESS` | Boolean | `True` | By default, confines built-in file components to the upload storage directory and blocks local-file SQL dialects and local Git clones. Setting this to `false` allows authenticated flow builders to read arbitrary server files — only do so in trusted single-tenant deployments. See [Component hardening for untrusted users](/api-keys-and-authentication#multi-tenant-component-hardening). |
...
| `LANGFLOW_AUDIT_ENABLED` | Boolean | `False` | Record an append-only audit event for audited Flow and Project operations in the `audit_events` table, including who ran each Flow and whether the run succeeded or failed. A committed operation and its event share one transaction. Events never contain Flow graphs, component settings, secrets, request bodies, run inputs, or run results. |
| `LANGFLOW_AUDIT_RETENTION_DAYS` | Integer | `90` | Number of days to keep audit events. Cleanup deletes by event time only, never because a Project, Flow, user, or API key was deleted. Set to `0` to keep events indefinitely. |
| `LANGFLOW_AUDIT_EXCLUDE_EVENTS` | List[String] | Not set | Comma-separated audit actions that are never recorded, written as they appear in the event `action` field: an exact action such as `project:delete`, every action on a resource such as `flow:*`, or one action on every resource such as `*:delete`. An excluded action records no event for any outcome, including denials. Entries that match no audited action are ignored with a startup warning and exclude nothing. Example: `flow:execute,project:delete` stops recording flow runs and Project deletions. |
