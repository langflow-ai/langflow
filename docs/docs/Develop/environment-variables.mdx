...
3. Define [Langflow environment variables](#supported-variables) in the `.env` file.
    ...
    LANGFLOW_AUTO_SAVING_INTERVAL=5000
...
Environment="LANGFLOW_AUTO_SAVING_INTERVAL=5000"
...
| `LANGFLOW_AUTO_SAVING_INTERVAL` | Integer | `5000` | Set the auto-save interval in milliseconds if `LANGFLOW_AUTO_SAVING=True`. Saving waits for this long after the last edit, and continuous editing defers it by at most three times this value. |
...
| `LANGFLOW_FLOW_AUDIT_ENABLED` | Boolean | `False` | Record who changed each flow's graph and what they changed. Off by default: the trail is durable storage a deployment opts into. Read it at `GET /api/v1/flows/{flow_id}/audit/`. Secret field values are never recorded. |
| `LANGFLOW_MAX_FLOW_AUDIT_ENTRIES_PER_FLOW` | Integer | `200` | Max audit entries kept per flow. The oldest are pruned when a new editing session opens, so a burst of editing never pays for it. Only applies when `LANGFLOW_FLOW_AUDIT_ENABLED=True`. |
| `LANGFLOW_FLOW_AUDIT_SESSION_WINDOW_SECONDS` | Integer | `300` | How long one person's consecutive edits to a flow fold into a single audit entry. An entry per save is unreadable, so a session extends until the person stops editing for this long. Only applies when `LANGFLOW_FLOW_AUDIT_ENABLED=True`. |
