...
| `LANGFLOW_AGENTIC_EXPERIENCE` | Boolean | `False` | Enables [Langflow Assistant](/langflow-assistant). When `false`, the Assistant's endpoints return `404` and its `run_assistant` MCP tool is unavailable. The rest of the MCP toolkit at `/api/v1/agentic/mcp` is unaffected. Disabled by default because the Assistant generates and runs component code on your Langflow server. |
