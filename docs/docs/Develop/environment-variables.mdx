...
| `LANGFLOW_SANDBOX_SESSION_MODE` | String | `off` | Whether one guest VM is reused across executions of the same flow. `off` gives every execution a fresh guest. `flow` shares one guest between executions of the same flow by the same user, until it goes idle. Variables, imports and files then carry from one execution to the next, which removes the VM creation cost from every execution after the first and lets an agent build up state across tool calls. Reuse is a deliberate weakening of isolation **between executions** — code from one execution can read what earlier code in the same flow left behind — so read [Sessions](../Deployment/sandbox-createos.mdx) before enabling it. One guest is shared per (flow, user) across workers and replicas that use the same CreateOS account, so a load-balanced flow does not fork its state. Two executions of one session that overlap ACROSS processes are not serialized, so let them overlap only if the flow tolerates it. An execution that times out destroys its guest rather than handing it to the next run. Honored only by backends that report session support; `exec-sandbox` does not, and runs cold either way. |
