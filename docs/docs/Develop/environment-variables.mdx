...
| `LANGFLOW_SANDBOX_BACKEND` | String | `none` | Execution backend for the code-execution components (Python Interpreter and the legacy Python REPL tool). Set to `exec-sandbox` to run user code in a dedicated QEMU microVM per execution instead of in-process. Requires `pip install 'langflow[sandbox]'`, Python 3.12 or later, and QEMU 8+ on the host. When configured but unavailable, execution fails with an error instead of falling back to in-process execution. |
...
| `LANGFLOW_SANDBOX_ALLOW_NETWORK` | Boolean | `False` | Whether sandboxed code may access the network. The default runs the microVM fully offline. Only used when `LANGFLOW_SANDBOX_BACKEND` is not `none`. |
| `LANGFLOW_SANDBOX_WARM_POOL_SIZE` | Integer | `0` | Number of pre-booted VMs kept warm to reduce sandboxed execution latency. Warm VMs consume memory while idle. Only used when `LANGFLOW_SANDBOX_BACKEND` is not `none`. |
