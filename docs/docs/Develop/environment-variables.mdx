...
| `LANGFLOW_GUNICORN_PRELOAD` | Boolean | `False` | On non-Windows deployments, preload the FastAPI application in the Gunicorn master process before forking workers. This can reduce per-worker memory usage through copy-on-write sharing, but you should validate it in staging before enabling it in production. |
