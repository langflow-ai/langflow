...
For OpenTelemetry traces, metrics, and logs that describe Langflow service health, including `LANGFLOW_OTEL_*` and `OTEL_EXPORTER_OTLP_*`, see [OpenTelemetry](/observability-opentelemetry).
...
For the full list, what Langflow does and does not export, and vendor setup, see [OpenTelemetry](/observability-opentelemetry).
