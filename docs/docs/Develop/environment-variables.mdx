...
| `LANGFLOW_AUDIT_ENABLED` | Boolean | `False` | Record who changed each resource, when, and whether it worked. Off by default: the log is durable storage a deployment opts into. Read it at `GET /api/v1/audit/{resource_type}/{resource_id}`. Field values and secrets are never recorded — only the names of the fields that changed. |
| `LANGFLOW_AUDIT_ANONYMIZE_PAYLOAD` | Boolean | `False` | Drop the payload from every audit row, keeping only who, what and when. |
