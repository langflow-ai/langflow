...
| `LANGFLOW_AUDIT_ENABLED` | Boolean | `False` | Record append-only Action events for audited Flow and Project mutations in the `audit_events` table. A committed operation and its event share one transaction. Authorization decisions remain in `authz_audit_log`; Flow runs are outside this initial release. Events never contain Flow graphs, component settings, secrets, or request bodies. |
...
| `LANGFLOW_AUDIT_EXCLUDE_EVENTS` | List[String] | Not set | Comma-separated audit actions that are never recorded, written as they appear in the event `action` field: an exact action such as `project:delete`, every action on a resource such as `flow:*`, or one action on every resource such as `*:delete`. An excluded action records no Action event for any outcome. Entries that match no audited action are ignored with a startup warning and exclude nothing. Example: `flow:write,project:delete` stops recording Flow writes and Project deletions. |
