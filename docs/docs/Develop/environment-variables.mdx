...
| `LANGFLOW_ALLOW_PUBLIC_CUSTOM_COMPONENTS` | Boolean | `False` | Whether flows shared through the **Shareable Playground** can run custom component code. By default, an unauthenticated public flow build runs only the server's own copy of each component, and a flow containing a custom component is refused with `This flow cannot be executed.` Set to `true` to build public flows from the database as their owner. This requires `LANGFLOW_ALLOW_CUSTOM_COMPONENTS=true`, and it lets every node in a public flow run the code stored in that flow, not only the custom ones. For more information, see [Custom components in publicly shared flows](/deployment-block-custom-components#public-flow-custom-components). |
