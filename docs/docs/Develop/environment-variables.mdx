...
| `LANGFLOW_ALEMBIC_LOG_TO_STDOUT` | Boolean | `False` | Whether to log Alembic database migration output to stdout instead of a log file. If `true`, Alembic logs to stdout and the default log file is ignored. For more information about database migrations, see [Memory management options](/memory#configure-external-memory). |
