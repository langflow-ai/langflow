...
For deployments with higher `LANGFLOW_WORKERS` values we recommend to set experimental `LANGFLOW_GUNICORN_PRELOAD=true`.
The Redis-backed job queue is also recommended so build and queue traffic stays consistent when requests are handled by different workers. Set `LANGFLOW_JOB_QUEUE_TYPE=redis` so queue events can be shared across workers.
A dedicated Redis DB should always be different than the one set up for the cache.
