...
| `LANGFLOW_MCP_SERVER_INTERPRETER_HARDENING` | Boolean | `True` | If `true`, blocks direct Python, Node.js, and shell MCP stdio entrypoints. Set to `false` only for a trusted single-tenant deployment that requires legacy interpreter commands. See [MCP interpreter hardening](/api-keys-and-authentication#mcp-interpreter-hardening). |
