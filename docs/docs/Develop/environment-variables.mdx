...
| `LANGFLOW_AGENTIC_EXPERIENCE` | Boolean | `False` | Enables [Langflow Assistant](/langflow-assistant). When `false`, the Assistant's endpoints and its MCP server at `/api/v1/agentic/mcp` return `404`. Disabled by default because the Assistant generates and runs component code on your Langflow server. |
