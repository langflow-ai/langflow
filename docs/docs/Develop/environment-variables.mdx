...
| `LANGFLOW_COMPONENT_LOAD_MAX_WORKERS` | Integer | Not set | Maximum number of component modules to import concurrently during startup. Set a positive integer to limit thread contention in CPU-constrained containers. When not set, Langflow uses Python's default thread-pool concurrency. Invalid or non-positive values log a warning and use the default. |
