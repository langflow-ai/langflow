...
For multi-worker tuning—including the Redis job queue and Gunicorn worker settings—see [Tune multi-worker Langflow](/deployment-multi-worker).
...
| `LANGFLOW_GUNICORN_PRELOAD` | Boolean | `False` | Enables Gunicorn `preload_app`. Non-Windows only. See [Recommended Gunicorn settings](/deployment-multi-worker#recommended-gunicorn-settings). |
| `LANGFLOW_JOB_QUEUE_TYPE` | String | `asyncio` | Job queue backend. Use `redis` for multi-worker deployments. See [Configuration reference](/deployment-multi-worker#configuration-reference). |
