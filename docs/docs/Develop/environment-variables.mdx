...
| `LANGFLOW_AUDIT_ENABLED` | Boolean | `False` | Record who changed each resource, when, and whether it worked. Off by default: the log is durable storage a deployment opts into. Covers flows and projects. Read it at `GET /api/v1/audit/{resource_type}/{resource_id}`. Field values and secrets are never recorded — only the names of the fields that changed. |
