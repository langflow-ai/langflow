...
| `LANGFLOW_BACKGROUND_RETENTION_DAYS` | Integer | `0` | Days to keep finished background runs, with their events, before deleting them. `0` disables retention and keeps every row forever. Queued, running, and paused runs are never deleted. See [Background run settings](./deployment-multi-worker#background-run-settings). |
