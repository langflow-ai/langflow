...
| `LANGFLOW_JOB_QUEUE_TYPE` | String | `asyncio` | Job queue backend. Use `redis` to share queue events across workers in a multi-worker deployment. For all Redis job queue variables, see [Deploy Langflow with multiple workers](./deployment-multi-worker#configuration-reference). |
...
For multi-worker environment variables, configuration reference, and a Docker Compose example, see [Deploy Langflow with multiple workers](./deployment-multi-worker).
