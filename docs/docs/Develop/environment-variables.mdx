...
| `LANGFLOW_BACKGROUND_RETENTION_DAYS` | Integer | `0` | Number of days to keep finished flow runs (completed, failed, cancelled, or timed out) in the job store, along with their stored events, signals, and checkpoints. Every flow run records a job, including API runs, **Playground** builds, knowledge base ingestion, and trigger runs, so the job tables grow without limit unless this is set. `0` disables cleanup and keeps every run. When set, an hourly cleanup deletes older finished runs. Queued, running, and paused runs that are waiting for human input are never deleted. After a run is deleted, the [workflow status endpoint](/workflow-api#get-workflow-status-endpoint) returns `404` for its job ID, and a background request that reuses its `idempotency_key` starts a new run. |
