...
| `LANGFLOW_REQUIRED_PLUGINS` | String | Not set | Comma-separated names from the `langflow.plugins` entry-point group that must load and register successfully. Langflow aborts startup if a required plugin is missing or fails; plugins not listed here retain best-effort loading. |
