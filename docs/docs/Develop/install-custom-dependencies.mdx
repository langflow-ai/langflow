...
These optional dependencies are listed in the [pyproject.toml](https://github.com/langflow-ai/langflow/blob/main/pyproject.toml) file under `[project.optional-dependencies]`.
...
However, the optional dependency groups for `langflow` and `langflow-base` are different and managed separately.
...
By default, installing `langflow-base` without any extras includes all dependencies listed in the `[project.dependencies]` section.
Optional dependency groups are not installed by default and must be explicitly requested.

These optional dependency groups are listed in the in the [base `pyproject.toml`](https://github.com/langflow-ai/langflow/blob/main/src/backend/base/pyproject.toml) file under `[project.optional-dependencies]`.
...
To install all optional dependencies, use the `all` extra:

```bash
uv pip install "langflow-base[all]"
```
