...
* The `main` package (`langflow`) is managed by the root-level `pyproject.toml`. It is the full end-user distribution and depends on `langflow-core` plus Langflow's curated provider bundles.
* The `core` distribution (`langflow-core`) is managed at `src/langflow-core/pyproject.toml`. It is the service-complete, provider-bundle-free profile consumed by the full distribution. It depends on `langflow-base[complete]` and owns the `langflow` and `langflow-core` commands.
* The `base` package (`langflow-base`) is managed at `src/backend/base/pyproject.toml`. It is the modular application platform and includes infrastructure such as the FastAPI web framework. Bare `langflow-base` intentionally has a smaller dependency surface; its `complete` extra adds the service integrations required by `langflow-core`. The `base` package depends on the `lfx` package.
* The `lfx` package is managed at `src/lfx/pyproject.toml`. LFX is a lightweight CLI tool for executing and serving Langflow flows. Its optional groups add executor-specific capabilities such as Cassandra support and tool guards.
...
Install `langflow` for the default provider bundle set, or install `langflow-core` directly when you want the service-complete application without provider bundles. The full distribution already installs `langflow-core` transitively and uses its `langflow` command, so adding `langflow` to a core environment adds the curated bundles without introducing a second command owner.
...
`langflow-base` is the lower-level modular platform beneath `langflow-core`. Use it when you are assembling an application with only specific service dependencies; the full `langflow` distribution reaches it transitively through core rather than depending on it directly.
...
```bash
uv pip install "langflow-base[postgresql,audio]"
```

To install the service integrations that make up the core profile's platform layer, use the `complete` extra. This is broader than bare `langflow-base`, but still does not install provider component bundles:
...
uv pip install "langflow-base[postgresql,audio]" matplotlib
```
