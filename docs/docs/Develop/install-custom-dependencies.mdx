...
* The `langflow` package is managed by the root level `pyproject.toml`, and it includes end-user features and main application code, such as Langchain and OpenAI. The `langflow` package depends on the `base` package.
...
* The `core` distribution (`langflow-core`) is managed at `src/langflow-core/pyproject.toml`. It provides the full Langflow application with the built-in component catalog, but does not install any `lfx-*` provider bundle. It depends on `langflow-base[complete]`.
...
Use `langflow` for the default provider bundle set, or `langflow-core` when you want only built-in core components. Do not co-install them because both distributions provide the `langflow` command.
