...
Langflow provides optional dependency groups and support for custom dependencies to extend Langflow functionality. This guide covers how to add dependencies for different Langflow installations, including Langflow Desktop, Langflow OSS.

The Langflow codebase uses two `pyproject.toml` files to manage dependencies, with one for `base` and one for `main`.

* The `main` package, which is managed by the root level `pyproject.toml`, includes end-user features and main application code, such as Langchain and OpenAI.
* The `base` package, which is managed at `src/backend/base/pyproject.toml`, includes core infrastructure such as the FastAPI web framework.
...
To add a dependency to the `main` package, navigate to the project root and run:
```bash
uv add matplotlib
```
