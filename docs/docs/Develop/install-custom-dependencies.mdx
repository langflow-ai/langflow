...
Install `langflow` for the default provider bundle set, or install
`langflow-core` directly when you want the service-complete application without
provider bundles. The full distribution already installs `langflow-core`
transitively and uses its `langflow` command, so adding `langflow` to a core
environment adds the curated bundles without introducing a second command
owner.
...
`langflow-base` is recommended when you want to deploy Langflow with only
specific service dependencies. It is the lower-level modular platform beneath
`langflow-core`; the full `langflow` distribution reaches it transitively
through core rather than depending on it directly.
...
To install the service integrations that make up the core profile's platform
layer, use the `complete` extra. This is broader than bare `langflow-base`, but
still does not install provider component bundles:
