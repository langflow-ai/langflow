...
opt-in standalone extensions listed in
...
version-aligned with `langflow` starting with version 1.12.0. It includes platform
services such as Redis, Chroma, MCP, Kubernetes, object storage, and
