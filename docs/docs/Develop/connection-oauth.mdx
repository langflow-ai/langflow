...
Two legacy components — **Gmail Loader** and **Google Drive Loader** — accept an optional
connection as an alternative to pasted token JSON. The Gmail Loader needs
`https://www.googleapis.com/auth/gmail.readonly`, which is a **restricted** scope: it works
only with a self-managed, customer-owned registration that includes it, never with a
Langflow-owned hosted or Desktop registration.
