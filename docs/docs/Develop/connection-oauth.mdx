...
| Shareable-playground links, the public workflow endpoint, and A2A calls admitted without authentication | an isolated anonymous principal | none: no user connection and no instance connection, whatever the flow references |
...
- **Publishing a flow does not publish its credentials.** Making a flow public, or exposing it over A2A
  without authentication, means anonymous visitors run it — and an anonymous run resolves no connection at
  all. Give such a flow an instance connection instead, or keep it authenticated.
