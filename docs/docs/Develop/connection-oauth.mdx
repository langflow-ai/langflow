...
Write the configured `scopes` in Microsoft's **short** form, such as `Mail.Read`.
Requesting `https://graph.microsoft.com/Mail.Read` makes Entra echo the long form
in the token response, and the components declare the short form; the resolver
