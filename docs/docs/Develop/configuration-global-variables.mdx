...
Model providers for language models and embedding models are configured globally in the **Models** pane, not through global variables.
Click **Models** in the header to add provider API keys and select which models to enable. These globally configured models are then available to all components in your flows.
