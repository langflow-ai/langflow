...
`LANGFLOW_REQUEST_VARIABLES` holds a JSON-encoded string of variables for the current request; a decoded value that is not an object is logged and ignored.
