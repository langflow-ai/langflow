...
## Store global variables in Kubernetes secrets {#kubernetes-secrets}

By default, Langflow stores global variables encrypted in its database at `langflow.db`.
When running Langflow in Kubernetes, you can switch to storing global variables in [Kubernetes Secrets](https://kubernetes.io/docs/concepts/configuration/secret/), to keep sensitive credentials out of the Langflow database entirely.
...
* Langflow running inside a Kubernetes cluster. See [Kubernetes deployment](/deployment-kubernetes-dev).

### Grant Langflow permission to manage Kubernetes Secrets

:::tip
If you're deploying with the [Langflow Helm chart](https://github.com/langflow-ai/langflow-helm-charts), the chart creates a `ServiceAccount` automatically. Skip the `ServiceAccount` block above, and set `subjects[0].name` in the `RoleBinding` to the name of the Helm-generated `ServiceAccount` instead. To find the `ServiceAccount` name, run `kubectl get serviceaccount -n langflow`.
:::

1. Create a manifest for the `ServiceAccount`, `Role`, and `RoleBinding` resources, and save it as `langflow-rbac.yaml`:
...
2. Apply the resources to your cluster:
   ```bash
...
4. To apply the updated Deployment, and then confirm the pod found the service account, run:
...
5. Log in to Langflow, navigate to **Settings** > **Global Variables**, and then create a global variable.

6. To confirm that Langflow created a corresponding Kubernetes Secret, run:
...
Headless runtimes have two channels that never touch Langflow's database, and both are documented in [Resolve connections in headless LFX](/lfx-connections).

`LANGFLOW_REQUEST_VARIABLES` holds a JSON object of variables for the current request.
