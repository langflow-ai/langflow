...
When using the default Chroma backend, knowledge bases are stored local to your Langflow instance.
...
:::note
Storage location settings apply to the Chroma backend only.
When using an external DB Provider such as OpenSearch, embeddings are stored in the remote cluster, not on the local filesystem.
:::
...
### Configure different vector database providers
...
To configure a different database provider, go to **Settings → DB Providers**.
A DB Provider is the vector store backend where knowledge base embeddings are stored and searched.
The selected provider applies to all new knowledge bases you create.
Existing knowledge bases keep their original backend.

#### OpenSearch

To use OpenSearch as a DB Provider, you need a running OpenSearch cluster accessible to your Langflow instance.
This example uses an OpenSearch container running locally, but you could also use a remote OpenSearch instance.

1. Start a local OpenSearch container with security disabled. This allows you to connect without a username, password, or TLS.
    ...
    Create a password that doesn't include `admin`, and contains uppercase and lowercase characters, a digit, and a special character.
    :::
...
2. Verify the cluster is reachable:
    ...
    If you get no response or a connection error, the container may still be starting. Wait a few seconds and try again.
...
9. Select an embedding model, and confirm **DB Provider** is set to **OpenSearch**.

    Optionally, add **Custom Metadata Fields** to tag every chunk with additional context. For example, if you're ingesting files from multiple teams, add a field `team` with a value of `support`. When the **Knowledge Base** component searches, you can then filter results to only return chunks where `team` equals `support` to keep results scoped to the support team's content.
10. Click **Next Step**.
11. Add your source files and configure chunking settings, then click **Next Step**.
12. In the **Review & Build** pane, preview the first chunk of your data and confirm the chunk size is appropriate for your use case. A typical chunk size is 512–1000 characters. Smaller chunks give more granular retrieval but may lose context at boundaries. Click **Create**.
