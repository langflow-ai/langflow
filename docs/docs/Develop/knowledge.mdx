...
:::important Upgrade note: OpenSearch knowledge bases created by earlier versions
Earlier versions named each OpenSearch index after the knowledge base name alone, so knowledge bases with the same name that belonged to different users shared one index.
When you upgrade, the database migration keeps existing data available without changing anything on the OpenSearch cluster:

* If only one user's knowledge bases used an index, they keep using it. The migration records the index in their `backend_config` as `index_name`, with `index_name_origin` set to `legacy_kb_name`. No action is needed.
* If knowledge bases that belong to different users used the same index, their chunks are mixed and can't be separated by owner. Those knowledge bases start with their own empty index, and the old index isn't changed or deleted. Their `backend_config` records the old index as `legacy_shared_index`, and Langflow logs a warning each time they are used. To restore their data, ingest each knowledge base's sources again, and then delete the old index from your OpenSearch cluster.
:::
