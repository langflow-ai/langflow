...
To use [Postgres with pgvector](https://github.com/pgvector/pgvector) as a database provider, you need a PostgreSQL database with the `vector` extension available, and the Langflow `pgvector` optional dependency installed.
...
3. Set the `PGVECTOR_CONNECTION_STRING` environment variable to your Postgres connection string, and then start or restart Langflow:
    ...
    Langflow creates the `vector` extension and required tables on first use.

4. To connect Postgres pgvector to Langflow, click **Settings**, and then click **DB Providers**.
5. Select **Postgres pgvector**.

    There are no credentials to enter in the UI. Postgres pgvector is configured from your server environment.

6. Click **Use Postgres pgVector**.
