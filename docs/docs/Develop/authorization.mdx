...
4. Use the [roles API](#roles-api), [role assignments API](#role-assignments-api), or [`langflow admin`](./admin-api-cli) to assign the built-in `viewer`, `developer`, or `admin` roles to your users, or create custom roles.
...
Superusers can use every administration route. An installed authorization plugin can also delegate narrow administration rights with `user:manage`, `team:manage`, and `role:manage`. The OSS pass-through service deliberately returns false for all three administration checks, even when normal resource authorization is configured to allow requests.

Delegated user administrators cannot promote or demote platform superusers. IdP-derived memberships and grants remain read-only through the manual administration API.

Use `GET /api/v1/authz/capabilities` to discover the caller's administration resources and whether the installed plugin supports team-role assignments. An installed plugin may also return `features.directory`, a caller-specific capability document whose provider and action fields describe the directory operations available to that caller. The field is omitted when no directory integration is installed; OSS never advertises it. For the directory command tree and credential-free `DirectoryState` contract, see [Administer users and teams with the API and CLI](./admin-api-cli#declarative-enterprise-directory-state).

## Authorization API reference

All authorization endpoints are under `/api/v1/authz/`. Write operations require a superuser or the matching delegated administration permission.
...
`/api/v1/authz/roles` — requires a superuser or `role:manage` for writes
...
`/api/v1/authz/role-assignments` — requires a superuser or `role:manage` for writes
...
`/api/v1/authz/teams` — requires a superuser or `team:manage` for writes
