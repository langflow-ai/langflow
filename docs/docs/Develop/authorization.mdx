...
Use `GET /api/v1/authz/capabilities` to discover the caller's administration resources and whether the installed plugin supports team-role assignments. For complete REST and CLI usage, see [Administer users and teams with the API and CLI](/admin-api-cli).
