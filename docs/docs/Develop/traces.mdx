...
Trace data is stored in the Langflow database in the `trace` and `span` tables. Trace data is presented in the **Flow Activity** page in the UI, and can be retrieved through the tracing API.
...
You can remove trace data either from the UI (where available) or through the tracing API:

- **Delete a single trace**: Use the `DELETE /monitor/traces/{trace_id}` endpoint to remove one trace and all of its spans.
- **Delete all traces for a flow**: Use `DELETE /monitor/traces?flow_id=...` to delete all traces associated with a specific flow.

These operations require authentication and only allow users to delete traces for flows they own.

## See also

- [Logs](/logging)
- [Monitor endpoints](/api-monitor)
- [Observability integrations](/integrations-langfuse)
