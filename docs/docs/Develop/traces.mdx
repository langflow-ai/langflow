...
- **Human-in-the-Loop gate spans**: For paused flows, a `Human In The Loop: Approved` or `Human In The Loop: Rejected` span records the reviewer's decision as part of the same trace as the rest of the run.
