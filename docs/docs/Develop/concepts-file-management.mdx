...
Changing `LANGFLOW_STORAGE_TYPE` to `s3` doesn't copy the files you already have.
Langflow keeps reading and writing new files in the bucket, and the previously uploaded bytes stay on local disk, where the S3 backend can't find them.

To copy them, stop Langflow and run the following command with the same AWS credentials Langflow uses:

```bash
langflow relocate-files --bucket S3_BUCKET_NAME --prefix S3_BUCKET_DIRECTORY
```
...
The command moves uploaded files and files attached to flows.
