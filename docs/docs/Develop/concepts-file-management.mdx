...
# (service principal env vars, AKS workload identity, managed identity, or az login)
...
If you don't set `AZURE_STORAGE_CONNECTION_STRING`, Langflow falls back to [`DefaultAzureCredential`](https://learn.microsoft.com/en-us/python/api/azure-identity/azure.identity.defaultazurecredential), which tries credential sources in this order: a service principal from environment variables (`AZURE_CLIENT_ID`/`AZURE_TENANT_ID`/`AZURE_CLIENT_SECRET`), AKS workload identity, managed identity, then developer tool credentials such as an existing `az login` session. You can also set `AZURE_STORAGE_ACCOUNT_URL` directly instead of `AZURE_STORAGE_ACCOUNT_NAME` if your account uses a non-default endpoint.
...
| `LANGFLOW_OBJECT_STORAGE_BUCKET_NAME` | String | `langflow-bucket` | The name of the S3 bucket, GCS bucket, or Azure Blob container to use for file storage. Only used when `LANGFLOW_STORAGE_TYPE` is `s3`, `gcs`, or `azure`; set this to a bucket/container you actually own before switching to one of those backends. |
| `LANGFLOW_OBJECT_STORAGE_PREFIX` | String | `files` | Optional prefix/folder path within the bucket or container where files will be stored. Set to an empty string to store files at the bucket/container root. |
