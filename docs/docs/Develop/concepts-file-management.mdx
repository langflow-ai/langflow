...
:::tip
**Google Drive** storage is available through the [**Read File**](/read-file) and [**Write file**](/write-file) components, but can't be configured as a storage backend through environment variables as **S3** can.
:::
