...
Files uploaded to Langflow file management are stored in your configured storage backend and are available to all of your flows.
Langflow supports two storage backends: local filesystem storage and AWS S3 storage.
Langflow uses local file storage as default. To configure Langflow to use S3 for file storage, see [configure file storage](/concepts-file-management#configure-file-storage).
...
Your AWS credentials must have the necessary S3 permissions.
Your requirements will vary depending on your use case.
...
When using S3 storage (`LANGFLOW_STORAGE_TYPE=s3`), you must also configure AWS credentials using one of the following methods:
- **Environment variables**: Set `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, and `AWS_DEFAULT_REGION`
- **AWS credentials file**: Configure in `~/.aws/credentials` or `~/.aws/config`
- **IAM roles**: Use IAM roles when running on AWS infrastructure (EC2, ECS, Lambda)
