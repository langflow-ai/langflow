...
This list includes all files in your server's file management system, but you can only select [file types that are supported by the **Read File** component](/components-files#read-file).
...
1. Add a [**Read File** component](/components-files#read-file) to your flow.
