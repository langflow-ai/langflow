...
For more information, see [Change permission for an IAM user](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_users_change-permissions.html).
...
When using S3 storage (`LANGFLOW_STORAGE_TYPE=s3`), you must also configure AWS credentials.

Langflow uses the `boto3` library to interact with S3, which follows the [AWS credential chain](https://boto3.amazonaws.com/v1/documentation/api/latest/guide/credentials.html). This means credentials are sourced in the following order:

1. **Explicit credentials**: Environment variables for `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` set in Langflow's `.env` file.
2. **AWS credentials**: Environment variables for `aws_access_key_id` and `aws_secret_access_key` in the AWS credentials file at `~/.aws/credentials`.
3. **IAM roles**: When Langflow is running on AWS infrastructure, boto3 automatically retrieves temporary credentials from the AWS metadata service. For more information, see the [AWS documentation](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles.html).

When Langflow runs on AWS infrastructure, you can use IAM roles instead of explicit credentials.
When Langflow runs elsewhere, you must use environment variables or an AWS credentials file to provide S3 access credentials.
