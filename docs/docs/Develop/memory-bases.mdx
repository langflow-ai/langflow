...
A memory base is a vector database for message history.
...
Each memory base is backed by a local [ChromaDB](https://docs.trychroma.com/docs/overview/introduction) vector store.

After a memory base is created, you can include its knowledge in a flow by adding the [**Memory Base** component](/memory-base).

### How memory bases ingest messages

One memory base is attached to a single flow.
Each time the flow completes a run, the captured messages are counted toward a configurable **threshold**.
When the number of unprocessed runs reaches the threshold, an asynchronous ingestion job writes the messages to the ChromaDB vector store.
...
To ingest manually when auto-capture is off, use the [`/flush` endpoint](/api-memories#flush-manually-trigger-ingestion).

:::tip
The flow must contain a **Chat Input** or a **Chat Output** component to write messages to the `messages` table.
:::

By default, the [**Memory Base** component](/memory-base) retrieves only chunks that match the current `session_id`.
...
The LLM distills the raw conversation into a compact summary, and the summary is written to the vector store, instead of the messages.
...
After creating a memory base, add the [**Memory Base** component](/memory-base) to your flow to retrieve relevant context from it.
For example, to connect a Memory Base to an Agent to give the agent memory of all past conversations, do the following:

1. Create a [Simple Agent starter flow](/quickstart).
...
For more information, see the [**Memory Base** component reference](/memory-base).
...
- [Memory Base component](/memory-base)
