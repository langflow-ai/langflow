...
* Every time a new email address is registered via the POST `/api/v2/registration/` endpoint.
* Once at **Langflow Desktop** startup if an email address was previously registered.
...
If telemetry is disabled with the `DO_NOT_TRACK` environment variable in Langflow Desktop, you are still prompted to enter your email address, but the email address will only be stored in your local Langflow database.
