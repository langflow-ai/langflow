...
- **DatabaseDialect**: The database engine Langflow is configured to use, such as `sqlite` or `postgresql`. Only the engine name is sent, never the database URL, host, or credentials.
