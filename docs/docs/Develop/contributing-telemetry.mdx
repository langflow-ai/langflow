...
This telemetry event is sent to track registered email addresses. The event is triggered in two cases:

1. Every time a new email address is registered via the POST `/api/v2/registration/` endpoint.
2. Once at **Langflow Desktop** startup, if an email address has been previously registered.

- **Email**: The registered email address, which helps track user registrations and understand the user base.
