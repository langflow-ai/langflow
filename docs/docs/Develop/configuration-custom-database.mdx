...
:::note
Setting [`LANGFLOW_DATABASE_URL`](#connect-langflow-to-a-local-postgresql-database) does not enable Postgres as a vector store for knowledge bases or memory bases, which instead use [DB Providers](/knowledge#configure-vector-database-providers).
:::
