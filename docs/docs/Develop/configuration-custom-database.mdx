...
4. In your `.env` file, set `LANGFLOW_DATABASE_URL` to your PostgreSQL connection string:
    ...
    To force the connection to use SSL, add the `sslmode=require` parameter to the connection string:

    ```text
    LANGFLOW_DATABASE_URL="postgresql://user:password@localhost:5432/dbname?sslmode=require"
    ```

    If your PostgreSQL server requires client certificates, add the certificate paths to your connection string:
    ...
    - **`sslmode=verify-ca`**: Requires SSL and verifies the server certificate against the Certificate Authority (CA).
    - **`sslmode=verify-full`**: Requires SSL, verifies the server certificate, and checks the request hostname against the certificate hostname. This option is the most secure.
