...
3. Click the **MCP Tools** component, and then click <Icon name="Plus" aria-hidden="true"/> **Add MCP Server**.
    ...
    :::tip
    To manage all of your MCP server connections, click <McpIcon aria-hidden="true"/> **MCP servers** in the visual editor, or click your profile icon, select **Settings**, and then click **MCP Servers**.
    :::
