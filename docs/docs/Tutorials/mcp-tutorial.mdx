...
```shell
LANGFLOW_MCP_SERVER_INTERPRETER_HARDENING=false uv run langflow run
```

Do not disable this protection on a shared or otherwise untrusted deployment. Prefer an operator-approved `uvx` or `npx` package runner when available. See [MCP interpreter hardening](/api-keys-and-authentication#mcp-interpreter-hardening).
:::
