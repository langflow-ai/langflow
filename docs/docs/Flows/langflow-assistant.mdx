---
title: Create custom components with Langflow Assistant
slug: /langflow-assistant
---

import Icon from "@site/src/components/icon";

**Langflow Assistant** is an in-app co-pilot pane within the **Playground**.

Like other co-pilots, **Langflow Assistant** can answer questions about the application and help you get more out of Langflow.

**Langflow Assistant** understands the structure of the Langflow graph, so it can create flows and components from natural language prompts.

## Prerequisites

* Start Langflow with `LANGFLOW_AGENTIC_EXPERIENCE=true` set in your `.env` file.
* Connect an LLM provider in the [Global model providers page](/components-models).
...
4. **Langflow Assistant** creates the component's code.
    <details closed>
...
6. Connect the component to a **Chat Input** and **Chat Output** components.
...
7. Open the **Playground**, and tell Langflow to check a list of URLs.
    For example:
    ```
    ...
    ```
8. Run the flow.
    The output will be similar to the following, with a Table with one row per URL, including page titles and status codes.
    ```df
    | url | title | status |
    | --- | --- | --- |
    ...
    | https://this-domain-should-not-resolve-12345.invalid |  |  |
    ```
9. To iterate further, tell **Langflow Assistant** what you want.
    For example, prompt it to `Update URLTitleExtractor to add max_urls (default 5) and skip duplicates.`.

    **Langflow Assistant** creates a new **URLTitleExtractor** component, with the new features added.
10. Replace the old component with the new component.
    Set **max_urls** to `3`.
