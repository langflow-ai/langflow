...
| `/history [N \| off]` | Sets how many past messages the Assistant remembers, from `0` to `100`. `/history off` restores the default. Without an argument, the command reports the current value. |
| `/iterations [N \| off]` | Sets the maximum number of steps the Assistant can take for each request, from `1` to `200`. `/iterations off` restores the default of `100`. Without an argument, the command reports the current value. |

If **Langflow Assistant** reports that it ran out of steps, raise the limit with `/iterations`, or split the request into smaller prompts.
A higher limit can increase token usage for long requests.
To change the default for every user of your Langflow server, set [`LANGFLOW_ASSISTANT_ITERATIONS`](/environment-variables).
