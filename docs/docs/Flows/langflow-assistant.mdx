...
* Custom component creation must be enabled and permitted for your user role:
  * `LANGFLOW_ALLOW_CUSTOM_COMPONENTS` must not be set to `false` The default setting is `true`.
  * `LANGFLOW_CUSTOM_COMPONENT_ADMIN_ONLY` must not be set to `true` unless you are a superuser.
...
Continuing from the [Create a custom component with Langflow Assistant](#create-a-custom-component-with-langflow-assistant), ask **Langflow Assistant** to build a simple agent flow that uses the **URLTitleExtractor** custom component as a tool.
