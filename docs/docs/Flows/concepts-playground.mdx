...
By default, a shared **Playground** can't run flows that contain custom components, because the run is unauthenticated and Langflow executes only its own copy of each component on that path. For more information, see [Custom components in publicly shared flows](/deployment-block-custom-components#public-flow-custom-components).
