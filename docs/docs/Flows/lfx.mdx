...
| `--flow-json`                               | Read inline flow JSON content as a string, e.g. `uv run lfx serve --flow-json '{...}'` where `{...}` is the flow JSON definition. |
| `--stdin`                                   | Read JSON flow content from `stdin`, e.g. `cat flow.json \| uv run lfx serve --stdin`.                              |
