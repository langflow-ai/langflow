...
Each flow carries a version identifier that changes whenever its graph is saved. Your editor sends the version it loaded with every save, so a save built on a version that someone else has already replaced is refused instead of overwriting their work.
...
To continue, click **Review changes**. The dialog lists your own changes, which are always included, alongside the other person's changes, which you can select individually. Selecting none keeps only your work.
...
* **Overwrite** puts the merge in the original flow. Before it does, the version being replaced is saved to the flow's version history and attributed to whoever wrote it, so the other person can still open it and recover their work. Overwriting is still checked against the server: if someone saved again while the dialog was open, it's refused rather than discarding them, and the dialog reopens comparing against the version that arrived, so you choose again with the newer changes in front of you.
