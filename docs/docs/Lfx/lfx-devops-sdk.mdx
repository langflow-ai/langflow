...
- Install the Langflow `lfx` package
   ...
   :::tip
   `uv pip install lfx` installs the core LFX executor but does not include components that ship as separate bundle packages.
   If your flows use those components, install the required bundles in the same virtual environment.
   For more information, see [About bundles](/components-bundle-components).
   :::
...
## Generate requirements.txt for flows {#generate-requirementstxt-for-flows}
