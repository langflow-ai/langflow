...
Every entry in `policy_keys` must use the governance grammar `integrations.<provider_id>.<action>`, in lowercase, where `<provider_id>` is the manifest's own `provider_id`. Operators block actions by these keys through the [policy bundle](/api-governance-policy), so a key outside that namespace would make the action ungovernable and is rejected at load time. Blocking any one key of an action blocks the whole action; a component stays visible while at least one of its actions remains allowed.
...
Provider IDs are owned identities in the installed registry: one bundle owns each provider ID. Capability IDs and policy keys cannot be claimed by another provider or bundle. A provider may group several of its own actions under one policy key. This lets discovery, connection resolution, and policy use the provider key without depending on an extension-qualified identity. Alternative SDK, Composio, and MCP bundles must use distinct provider and policy namespaces when installed together.

The registry rejects a conflicting installation with `integration-identity-conflict`, naming both extensions and the conflicting key. Startup skips the rejected bundle's components and integration metadata and emits the error; an already installed owner remains active. Hot reload checks ownership before swapping live modules or metadata, so a rejected reload preserves the previous bundle. Reloading the owning bundle is allowed, and removing it releases its keys.
...
The `Integration manifests - lfx dependency floor` CI check enforces this pairing for every bundle that declares integrations, including JSON and TOML manifests. It reads `[project].dependencies` and requires an unconditional lfx dependency whose lower bound is at least `1.13.0.dev0`; declaring `lfx.compat = ["1"]` alone is insufficient because that names the bundle API contract, not the lfx release. Bundles without integrations are exempt. The guard applies when future provider bundles add their manifests, even if the synchronization command produces no changes today.

The validator and loader check the feature's runtime floor before parsing the strict manifest schema and report `lfx-version-too-old` with the installed and required versions. Earlier lfx builds cannot inherit this diagnostic; dependency resolution and the CI guard prevent installing these manifests with older releases. Unreleased development builds that used the earlier inline integration shape must be updated to a build supporting bundle-owned references; those shapes were never a released compatibility contract.
...
| `integration-identity-conflict` | Two registered integrations claim the same provider ID, capability ID, or policy key. |
| `lfx-version-too-old` | The running lfx version predates bundle-owned integration references; upgrade lfx. |
