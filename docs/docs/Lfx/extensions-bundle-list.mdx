...
These providers also ship as standalone `lfx-*` packages, but they are **not**
included in `uv pip install langflow`. Install one package, or install all
eight together with every no-Torch long-tail bundle by using
