...
The server connects to Langflow's REST API over **stdio**: your MCP client spawns it as a subprocess and communicates over stdin and stdout. There is no HTTP port to connect to.

Flow data is never cached server-side, so every mutating tool does a GET → modify → PATCH cycle. The component registry is cached on first access per session.
...
Any client that supports stdio MCP servers can connect to `lfx-mcp`. Set the command to `lfx-mcp` (or `uvx --from lfx lfx-mcp`) and pass the following environment variables:
...
Add the following to the Claude Desktop configuration file at `~/Library/Application Support/Claude/claude_desktop_config.json`:
...
<TabItem value="uvx" label="via uvx (no install)">
...
| `configure_component` | Set parameter values on a component. Returns a `warnings` field if a server-side refresh failed for a parameter, such as an API key not yet configured on the component. |
...
| Tool | Description |
|------|-------------|
| `run_flow` | Run a flow and return the output; streams progress events when the client supports it. Accepts `input_type` (default: `"chat"`), `output_type` (default: `"chat"`), and `tweaks` (dict of component param overrides at runtime). |
...
| `notify_done` | Signal that you are done modifying a flow so the UI updates immediately. Optional `summary` string is forwarded in the `flow_settled` event payload visible in the UI. |
| `batch` | Execute multiple actions in sequence; use `$N.field` to reference results from previous steps. Cannot nest `batch` inside another `batch`. |
...
- An OpenAI API key. This example uses Langflow's Agent component with OpenAI. Add your OpenAI API key as a Global Variable in Langflow under **Settings → Global Variables** so all flows can use it automatically. If you prefer a different provider, adjust the prompt accordingly.
...
1. Add `lfx-mcp` to Claude Code. Replace the placeholder values with your actual keys:
...
4. Give Claude Code instructions. For example:
...
Given this instruction, Claude Code will typically:
...
The API key for your LLM provider is not configured. Add it as a Global Variable in Langflow under **Settings → Global Variables → Add**, and then ask Claude Code to validate the flow again.
