...
## Supply variables and credentials per request

The run and stream request bodies accept `global_vars`, a map of variables applied to a deep copy of the graph for that request only.
Use it to send an API key or a connection credential without putting it in the server's environment, where it would become an ambient default for every other caller:
...
curl -X POST http://localhost:8000/flows/$FLOW_ID/run \
  ...
  -d '{
        "input_value": "Hello, world!",
        "global_vars": {"LF_CONNECTION__GOOGLE__WORK": "{\"access_token\":\"ya29.a0-example\",\"scopes\":[\"https://www.googleapis.com/auth/drive.readonly\"]}"}
      }'
```

Request-scoped values are consulted before the process environment, so a caller's value always wins.
With the built-in environment resolver, start the server with `--no-env-fallback` to prevent process-environment fallback. Custom resolvers retain their own credential lookup behavior:

```bash
uv run lfx serve ./flows/ --no-env-fallback
```

Add `--reset-environ` to also restore `os.environ` after each run, so a flow that mutates the process environment cannot affect the next request on the same worker.

For connection references, the `LF_CONNECTION__*` key derivation, the credential JSON format, and how to plug in your own resolver, see [Resolve connections in headless LFX](./lfx-connections).
