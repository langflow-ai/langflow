...
With the default resolver, `lfx run` resolves it from the environment under the key `LF_CONNECTION__<PROVIDER>__<NAME>`.
For actions that require scopes, include the scopes actually granted to the token:
...
With the default `--check-variables`, `lfx run` rejects malformed references and checks for injected values when using the built-in environment resolver. A missing value aborts execution:
...
Custom resolvers perform their own lookup and do not require environment keys during pre-flight. Credential expiry and scope checks occur when the lease is resolved.
