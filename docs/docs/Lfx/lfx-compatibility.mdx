...
This is equivalent to installing `lfx` plus `lfx-bundles[all]`, including CUGA and Smolagents dependencies that can pull in PyTorch.
The **Code Agents** components require a [separate manual OpenDsStar installation](/bundles-codeagents#prerequisites); OpenDsStar is excluded from all managed extras.
