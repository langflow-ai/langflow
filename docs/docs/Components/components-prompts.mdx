...
| use_double_brackets | Use Double Brackets | Advanced parameter. When enabled, use Mustache syntax `{{variable}}` instead of f-string syntax `{variable}`. Useful for templates containing JSON or code. See [When to use Mustache templating](#when-to-use-mustache-templating-double-brackets) for details. |
| tool_placeholder    | Tool Placeholder    | Advanced parameter. A placeholder input for tool mode. |
