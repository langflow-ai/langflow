...
You can download the [Cleanlab Quickstart flow](/files/cleanlab_quickstart_rag_flow.json), and then [import the flow](/concepts-flows-import) to your Langflow instance. This flow takes in a user's query, does a web search to retrieve relevant context, then serves an answer.
