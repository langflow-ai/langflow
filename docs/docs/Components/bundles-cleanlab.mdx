...
You can view the respective query and all the associated scores that Cleanlab provides in the Project you created earlier. Here is what the query looks like in the Cleanlab AI Platform. Notice how the trustworthiness score has passed validation (score = 0.985)!
...
If we take a closer look at this query in the Cleanlab AI Platform, we see that the original response given was entirely incorrect and was flagged correctly (score = 0.269)!
...
You have the ability to create custom guardrails and evaluations in a few minutes. Let's add the following guardrail to the Project:

![Cleanlab add guardrail](/img/cleanlab-politics-guardrail.png)

Now, when the AI responds with something political, the guardrail is triggered, and the fallback response is returned in its place.
...
Then, when a user asks the same or sementically similar query in the future, instead of returning the unhelpful fallback answer, Cleanlab serves the newly added Expert Answer automatically.
