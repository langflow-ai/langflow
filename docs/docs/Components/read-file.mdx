...
| csv_delimiter | CSV Delimiter | Input parameter. A single character separating CSV columns in **Structured Content** output. Default: comma (`,`). Use `;` for semicolon-separated files or `\t` for tab-separated files. |
