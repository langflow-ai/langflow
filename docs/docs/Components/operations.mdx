...
The **Data Operations** component performs operations on all three of Langflow's main data types — [`JSON`](/data-types#json), [`Table`](/data-types#table), and [`Message`](/data-types#message) (text) — from a single component.
...
Those three components are now legacy. Existing flows that use them continue to work, but new flows should use the **Data Operations** component.

## Use the Data Operations component in a flow

1. Add a **Data Operations** component to your flow.
...
Connect the matching input to the component that produces that data type.
    ...
    To perform multiple operations, chain multiple **Data Operations** components together.
    :::
...
The **Data Operations** component's inputs and outputs are dynamic: they change based on the selected operation.
