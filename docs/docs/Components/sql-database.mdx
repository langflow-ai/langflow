...
:::warning Local-file dialects are restricted by default
The local-file dialects (`sqlite`, `duckdb`) are blocked unless [`LANGFLOW_RESTRICT_LOCAL_FILE_ACCESS`](/api-keys-and-authentication#multi-tenant-component-hardening) is set to `false`, because a database URL such as `sqlite:////etc/passwd` would otherwise let flow builders read arbitrary files on the server. To use a SQLite database in the **Database URL** field, either set `LANGFLOW_RESTRICT_LOCAL_FILE_ACCESS=false` (trusted single-tenant deployments only) or use a network database such as PostgreSQL or MySQL.
:::

If a network database requires a local TLS certificate or key file, an administrator can stage it under [`LANGFLOW_DATABASE_TLS_FILES_DIR`](/api-keys-and-authentication#multi-tenant-component-hardening). PostgreSQL `sslrootcert`, `sslcert`, and `sslkey` and MySQL/MariaDB `ssl_ca`, `ssl_cert`, and `ssl_key` URL options can then reference files inside that directory while local file access remains restricted. Database hostname validation still applies when connector SSRF validation is enabled.
...
| run_sql_query | Result Table | Output parameter. The query results as a [`Table`](/data-types#table). |
