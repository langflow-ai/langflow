...
```
ChatInput → While Loop → Agent Step → [Tool Calls] → Execute Tool → While Loop
                              ↓ [AI Message - done]
                         ChatOutput
```
...
![Agent Loop flow diagram showing While Loop, Agent Step, Execute Tool, and ChatOutput components connected in a loop pattern](/img/component-agent-blocks.png)
