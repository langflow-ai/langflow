...
If the remote agent URL points at a local or private host, such as another Langflow instance on `http://localhost:7862`, add that URL to the SSRF allowlist on that host before you run the A2A component:
