...
<details>
<summary>Dynamic Create Data</summary>

Replace this legacy component with the [**Data Operations** component](/data-operations).

This component dynamically creates a `Data` object with configurable fields. Define the table in the **Input Configuration** field, and the component creates corresponding input or output handles in the component.

It accepts the following parameters:

| Name | Display Name | Info |
|------|--------------|------|
| form_fields | Input Configuration | Input parameter. A table that defines the dynamic form fields. Each row creates a new input field that can connect to other components. |
