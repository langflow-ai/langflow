...
The **Routes** table defines the categories for routing. Each row contains three fields:

- **Route Name**: The category name used for both the output port name and LLM matching. This is required.
- **Route Description**: Optional description that helps the LLM understand when to use this category. The description is included in the prompt sent to the LLM.
- **Route Message**: Optional custom message to send when this route matches. If left empty, the original input text is passed through.
...
When the LLM categorizes the input text, it routes to the matching category's output port.
