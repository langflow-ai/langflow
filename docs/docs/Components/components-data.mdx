...
The parser requires the Docling dependency to be installed.
...
* pipeline: Which Docling pipeline to use. Docling chooses the pipeline based on the source document type.
* ocr_engine: Which OCR parser to use. The default is [EasyOCR](https://pypi.org/project/easyocr/).
* doc_key: The key to use for the DoclingDocument column, which holds the structured information extracted from the source document. Default: `doc`.
* md_image_placeholder: Defines the placeholder for image files in the output Markdown document.
* md_page_break_placeholder: Defines the placeholder for page breaks in the output Markdown document.
