...
| follow_redirects | Follow Redirects | Input parameter. Whether to follow HTTP redirects. The default setting is `false` to prevent SSRF bypass attacks. If `true`, HTTP redirects are followed. For more information, see [SSRF protection environment variables](/api-keys-and-authentication#ssrf-protection). |
