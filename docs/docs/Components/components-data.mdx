...
| Name | Display Name | Info |
|------|--------------|------|
| pipeline | Pipeline | Input parameter. Which Docling pipeline to use. Docling chooses the pipeline based on the source document type. Options: `standard`, `vlm`. Default: `standard`. |
| ocr_engine | OCR Engine | Input parameter. Which OCR parser to use. The default is [EasyOCR](https://pypi.org/project/easyocr/). Options: `easyocr`, `tesserocr`, `rapidocr`, `ocrmac`. |
| doc_key | Document Key | Input parameter. The key to use for the DoclingDocument column, which holds the structured information extracted from the source document. Default: `doc`. |
