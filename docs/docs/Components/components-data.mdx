...
as well as provider-specific API components in [bundles](/components-bundle-components).
...
If you upload new files to the **File** component, either through **My Files** or at runtime, those files are stored in Langflow file management.
...
Supported archive formats for bundling multiple files:
...
Alternatively, you can use the [**Directory** component](#directory) or use multiple **File** components in the same flow.

For information about working with image files, see [Upload images](/concepts-file-management#upload-images).
...
s well as provider-specific search components in [bundles](/components-bundle-components).
...
They can output the retrieved data in [DataFrame](/data-types#dataframe) format, or can be connected to an **Agent** component to be used as tools.
</details>
...
## SQL Database
    ...
    For CQL queries, see the components in the [**DataStax** bundle](/bundles-datastax).
    For vector search, see [**Vector Store** components](/components-vector-stores).
