...
As of Langflow 1.6, the File component supports advanced document parsing using the [Docling](https://docling-project.github.io/docling/) library.
Only non-tabular documents can be parsed, and only one document at a time.
The parser requires the Docling dependency to be installed. As of Langflow 1.6, the Docling dependency is included with Langflow.
For more information, see [Install custom dependencies](/install-custom-dependencies).
The individual Docling components are still available in the [**Docling bundle**](/integrations-docling).

The advanced parser includes the following advanced fields.
