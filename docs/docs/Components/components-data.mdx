...
3. Click <Icon name="SlidersHorizontal" aria-hidden="true"/> **Controls** in the [component's header menu](/concepts-components#component-menus) to configure advanced parsing parameters, which are hidden by default:
    ...
    | ocr_engine | OCR Engine | Input parameter, advanced parsing. The OCR parser to use with the `standard` pipeline. The **File** component supports only [EasyOCR](https://pypi.org/project/easyocr/). Default: None (empty string). |
