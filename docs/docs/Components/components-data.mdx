...
Advanced parsing can process a wide range of document types including PDFs, Office documents, images, and tabular data.
...
These formats are better processed using standard parsing methods.

When you enable advanced parsing, you can configure the following advanced parsing parameters:

| Name | Display Name | Info |
|------|--------------|------|
| pipeline | Pipeline | Input parameter. Which Docling pipeline to use. Docling chooses the pipeline based on the source document type. Options: `standard`, `vlm`. Default: `standard`. See [Adjust pipeline features](https://docling-project.github.io/docling/usage/advanced_options/#adjust-pipeline-features) for details. |
| ocr_engine | OCR Engine | Input parameter. Which OCR parser to use. [EasyOCR](https://pypi.org/project/easyocr/) is the supported OCR engine. For additional OCR engines, use the dedicated [**Docling** components](/integrations-docling). |
| doc_key | Document Key | Input parameter. The key to use for the DoclingDocument column, which holds the structured information extracted from the source document. Default: `doc`. See [Docling Document](https://docling-project.github.io/docling/concepts/docling_document/) for details. |
| md_image_placeholder | Markdown Image Placeholder | Input parameter. Defines the placeholder for image files in the output Markdown document. |
| md_page_break_placeholder | Markdown Page Break Placeholder | Input parameter. Defines the placeholder for page breaks in the output Markdown document. |
...
Advanced parsing is not available for multiple files.
