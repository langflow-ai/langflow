...
Data source components bring data into your flows from various sources like files, API endpoints, and URLs.
For example:

* **Load files**: Import data from a file or directory with the [**Read File** component](/read-file) and [**Directory** component](/directory).
