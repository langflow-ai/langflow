...
Most parameters are hidden by default in the visual editor.
...
Many parameters are hidden by default in the visual editor.
You can toggle parameters through the <Icon name="SlidersHorizontal" aria-hidden="true"/> **Controls** in the [component's header menu](/concepts-components#component-menus).
...
| markdown | Markdown Export | Input parameter. Export processed documents to Markdown format. Only available when advanced mode is enabled in the file component. Default is false. |
...
When you enable advanced parsing, you can configure the following advanced parsing parameters:

| Name | Display Name | Info |
|------|--------------|------|
| pipeline | Pipeline | Input parameter. Which Docling pipeline to use. The `standard` pipeline is the only supported pipeline. For additional pipeline support, use the dedicated [**Docling** components](/integrations-docling). Default: `standard`. |
| ocr_engine | OCR Engine | Input parameter. Which OCR parser to use. [EasyOCR](https://pypi.org/project/easyocr/) is the only supported OCR engine. For additional OCR engines, use the dedicated [**Docling** components](/integrations-docling). |
| doc_key | Document Key | Input parameter. The key to use for the `DoclingDocument` column, which holds the structured information extracted from the source document. See [Docling Document](https://docling-project.github.io/docling/concepts/docling_document/) for details. Default: `doc`. |
| md_image_placeholder | Markdown Image Placeholder | Input parameter. Defines the placeholder for image files in the output Markdown document. Default: `<!-- image -->`. |
| md_page_break_placeholder | Markdown Page Break Placeholder | Input parameter. Defines the placeholder for page breaks in the output Markdown document. Default: empty string. |

:::tip
For more Docling features, use the [**Docling** components](/integrations-docling).
:::
...
- **One file with advanced parsing**:
    ...
    - **Markdown**: A [`Message`](/data-types#message) containing the document exported to Markdown format with image placeholders.
    - **File Path**: A [`Message`](/data-types#message) containing the path to the file in [Langflow file management](/concepts-file-management).

- **Multiple files**: Produces a **Files** [`DataFrame`](/data-types#dataframe) containing the content and metadata of all selected files.

    Advanced parsing doesn't support multiple files.
    See [Advanced parsing limitations](#advanced-parsing-limitations).
...
For other RSS feeds, use the [**RSS Reader** component](#rss-reader), and for other searches use the [**Web Search** component](#web-search) or provider-specific <Icon name="Blocks" aria-hidden="true" /> [**Bundles**].
...
If the core **Data** components don't meet your needs, you can find provider-specific components in <Icon name="Blocks" aria-hidden="true" /> [**Bundles**](/components-bundle-components).
