...
5. Add an **Agent** component to your flow, and then enter your OpenAI API key.
    ...
    If your preferred model isn't in the **Agent** component's built-in model list, select <Icon name="Plus" aria-hidden="true"/> **Connect Other Models**, and then use a [**Language Model** component](/components-models) to attach a specific model.
