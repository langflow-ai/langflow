...
As of Langflow 1.6, the File component supports advanced document parsing using the Docling library.
Only non-tabular documents can be parsed, and only one document at a time.
The parser requires the Docling dependency to be installed. For more information, see [](/install-custom-dependencies)
