...
- ✅ `data/custom_component.py` (level 2)
- ❌ `data/tools/custom_component.py` (level 3 - not discovered)

Each first-level directory becomes an independent category in the UI.

**Advanced:** To increase the depth limit, modify `MAX_DEPTH` in `src/lfx/src/lfx/custom/directory_reader/directory_reader.py`. Subdirectories will still appear as separate categories, not nested hierarchies.
