...
For the structure of a component (class, attributes, inputs, and methods), see the [Components overview](/concepts-components#component-code), which includes examples pulled from the [Recursive Character Text Splitter](https://github.com/langflow-ai/langflow/blob/main/src/lfx/src/lfx/components/langchain_utilities/recursive_character.py) component in the repository.
