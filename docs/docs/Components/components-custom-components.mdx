...
* Internal variables for logging, such as `self.log()`, `self.status`, and `self.stop()`.

Use the [Custom component quickstart](#quickstart) to add an example component to Langflow, and then use the reference guides that follow for more advanced component customization.
...
Save the custom component in the Langflow directory where the UI will discover it.
