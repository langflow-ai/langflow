...
By default, components in Langflow that produce multiple outputs show all outputs simultaneously in the visual editor.
The component will have one output port for each output, and the user can connect zero or more outputs to other components.

This behavior is controlled by the `group_outputs` parameter:

- **`group_outputs=True`**: When a component has more than one output and `group_outputs` is `true`, the outputs are grouped in the visual editor, and the user must select one.

    Use this option when the component is expected to return only one type of output when used in a flow.

- **`group_outputs=False` (default)**: All outputs are available simultaneously in the visual editor. The component has one output port for each output, and the user can connect zero or more outputs to other components.
...
<TabItem value="false" label="False or not set">
