...
The **Message History** component can store and retrieve chat messages in either [Langflow storage](/memory) _or_ external chat memory storage like Mem0 or Redis.

For more information, see [Store chat memory](/memory#store-chat-memory).

### When to use the Message History component

Use the **Message History** component when you want to do any of the following:

* Customize chat memory settings, including options for sorting and filtering messages.
* Store chat memory in external storage, such as Mem0 or Redis.
* Access chat memories in a flow without chat, such as sentiment analysis and feedback flows that review recently stored memories.

:::tip
Chat-based flows with a **Language Model** or **Agent** component have built-in chat memory that is enabled by default.
These flows don't require a **Message History** component unless you want to customize chat memory settings or use external chat memory storage.
For more information, see [Store chat memory](/memory#store-chat-memory).
:::
...
:::important
The following example shows the flow created in the preceding steps with an optional **Type Convert** component between the **Message History** and **Prompt Template** components.
Type conversion is required only if you need to convert the memories into `Data` type because the **Message History** component can output `Message` and `DataFrame` [data types](/data-types) by default.
This is only necessary when retrieving memories outside of a chat context and you specifically need to manipulate the memories in `Data` format.
:::
...
| messages | Message | Output parameter. The retrieved memories as Message objects, including `messages_text` containing retrieved chat message text. |
