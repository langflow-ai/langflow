...
| Response | [DataFrame](/data-types#dataframe) | Output. Result of running the selected tool once. |

## Confluent Kafka Producer

This component publishes a [Message](/data-types#message), a [Data](/data-types#data) object, or every row of a [DataFrame](/data-types#dataframe) to a Kafka topic and returns the delivery report. Use it to write agent decisions, enriched records, or flow results back to a topic.
...
| Delivery Report | [Data](/data-types#data) | Output. `records_sent`, `delivered`, `failed`, and per-record `reports`. |
...
| Records | [DataFrame](/data-types#dataframe) | Output. One row per record. |
...
Tableflow tables are read-only from external engines. To change the data, publish to the source topic. To query the same tables with SQL alongside your other lakehouse data, register the Tableflow catalog in watsonx.data and use the [**IBM watsonx.data Presto** component](/bundles-ibm#ibm-watsonxdata-presto).
...
| Result Table | [DataFrame](/data-types#dataframe) | Output. Rows of the table after filter, projection, and limit. |
| Tables | [Data](/data-types#data) | Output. The tables in the namespace. |
...
Reference `langflow_flows` in `CREATE AGENT`, and the agent can invoke your flows on every event it processes. When your Langflow deployment fronts the MCP server with OAuth (MCP Composer), use the connection's OAuth options instead of the API key in the URL.

### Query Tableflow tables from watsonx.data

Register the Tableflow REST catalog as an Iceberg datasource in watsonx.data (Presto: `connector.name=iceberg`, `iceberg.catalog.type=REST`, `iceberg.rest.uri=<Tableflow catalog URI>`, `iceberg.rest.auth.type=OAUTH2`, `iceberg.rest.auth.oauth2.credential=<key>:<secret>`), then query the tables with the [**IBM watsonx.data Presto** component](/bundles-ibm#ibm-watsonxdata-presto) or any SQLAlchemy URL in the [**SQL Database** component](/sql-database). See [Integrating Confluent Tableflow in watsonx.data](https://www.ibm.com/docs/en/watsonxdata/standard/2.3.x?topic=integrations-integrating-confluent-tableflow-in-watsonxdata).

## See also

- [IBM bundle](/bundles-ibm) — watsonx.ai, watsonx.data Presto and MCP, Db2 Vector Store
