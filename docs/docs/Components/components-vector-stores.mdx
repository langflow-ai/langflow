...
Some parameters are conditional, and they only become available after you set other parameters or select specific options for other parameters.
Conditional parameters may not be visible on the **Controls** pane until you set the required dependencies.
However, all parameters are listed in the [component's code](/concepts-components#component-code).

For information about a specific provider's parameters, see the provider's documentation and the component details.
...
The **Graph RAG** component uses [DataStax Graph RAG](https://datastax.github.io/graph-rag/) for Graph RAG traversal, enabling graph-based document retrieval.

This component must be used with a component that produces `VectorStore` output, specifically the **Astra DB** and **OpenSearch** vector store components.
