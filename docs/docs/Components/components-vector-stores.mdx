...
| api_endpoint | Astra DB API Endpoint | Input parameter. Your database's Data API endpoint. Providing a Data API endpoint overrides `database_name`. |
...
| search_method | Search Method | Input parameter. The search methods to use, either `Hybrid Search` or `Vector Search`. Your collection must be configured to support the chosen option. The default depends on what your collection supports. For more information, see [Ways to find data in Astra DB Serverless](https://docs.datastax.com/en/astra-db-serverless/databases/about-search.html). |
| reranker | Reranker | Input parameter. The re-ranker model to use for hybrid search, depending on the collection configuration. |
| lexical_terms | Lexical Terms | Input parameter. A comma-separated string of keywords for hybrid search, like `features, data, attributes, characteristics`. |
...
This feature requires that you [create a collection with hybrid search enabled](https://docs.datastax.com/en/astra-db-serverless/api-reference/collection-methods/create-collection.html#example-hybrid).
Once you connect the **Astra DB** component to a collection that supports hybrid search, the component automatically populates some of the hybrid search parameters, like `reranker` and `search_method`.
