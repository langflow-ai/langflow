...
This example uses the **Vector Store RAG** template uses two subflows to load embeddings and content into a vector database, and then runs a vector search to retrieve relevant context based on a user's query:

1. Create a flow with the **Vector Store RAG** template.

2. Configure the two **Astra DB** components, or replace them with another pair of **Vector Store** components of your choice.
    ...
    Depending on your vector store provider and configuration, you might need to provide a query statement, API request, or other search parameters.

3. To configure the embeddings model, do one of the following:

    * In the **OpenAI Embeddings** components, enter your OpenAI API key.
    * Replace the **OpenAI Embeddings** components with another pair of [**Embedding Model** component](/components-embedding-models) of your choice.
    * Remove the **OpenAI Embeddings** components if you are connecting to an Astra DB vector store with a vectorize integration.
    In this case, the **Astra DB** components automatically use the vectorize integration to generate embeddings from the **Ingest Data** (in the **Load Data** subflow) and **Search Query** (in the **Retriever Flow** subflow).
...
4. In the **Language Model** component, enter your OpenAI API key, or select a different provider and model to use for the chat portion of the flow.

5. Run the **Load Data** subflow to populate your vector store.
In the **File** component, select one or more files, and then click <Icon name="Play" aria-hidden="True" /> **Run component** on the **Vector Store** component in the **Load Data** subflow.
    ...
    If your vector store already contains data, you don't need to run the **Load Data** subflow if you want to use your existing data for vector search.

6. Open the **Playground** and start chatting to run the **Retriever** subflow.
...
### Hidden and conditional parameters
...
### Search results and vector store output {#output}

Most **Vector Store** components produce `search_results` and `vector_store` output.

* **`search_results`**: The vector search results in [`Data`](/data-types#data) or [`DataFrame`](/data-types#dataframe) format.
If both types are supported, you can set the format near the component's output port in the visual editor.

    Some **Vector Store** components produce other output instead of or in addition to the `search_results`, depending on the component's purpose and available functions.
    To explore output of specific components, see the component code, documentation, and visual editor.

* **`vector_store`**: A provider-specific instance of a [LangChain vector store](https://python.langchain.com/docs/integrations/vectorstores/) that drives the underlying vector search functions.
This output is not directly returned for use in the flow, but it is created by the component during the flow run to execute component functions that read and write to the vector store.

    These instances are created because Langflow is based on LangChain.
    They are provider-specific and configured according to the component's parameters.
...
The **Graph RAG** component uses [DataStax Graph RAG](https://datastax.github.io/graph-rag/) for Graph RAG traversal and graph-based document retrieval in an Astra DB vector store.

:::tip
This component only works with the **Astra DB** vector store component because it uses a `VectorStore` port to connect to the database.
Consider using the **Astra DB Graph** component instead.
:::
...
| Name | Display Name | Info |
|------|--------------|------|
| embedding_model | Embedding Model | Input parameter. Specify the embedding model to use.
Not required if the attached vector store has an [Astra DB vectorize integration](https://docs.datastax.com/en/astra-db-serverless/databases/embedding-generation.html). |
| vector_store | Vector Store Connection | Input parameter. Connection to the vector store through a `VectorStore` port connected to a component that produces `VectorStore` output.|
