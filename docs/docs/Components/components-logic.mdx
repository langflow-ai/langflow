...
The **Smart Router** component builds on the [**If-Else** component](#if-else), but instead of string matching, the **Smart Router** uses a connected [**Language Model** component](/components-models) to categorize and route incoming messages.

To use the **Smart Router**, replace the **If-Else** component in a flow like the [Incoming messages flow](#use-the-if-else-component-in-a-flow). Instead of sorting by regex, use the **Routes** table to define the outputs for your messages, and connect a **Language Model** component to provide the sorting intelligence.
...
| Name                | Type     | Description                                                       |
|---------------------|----------|-------------------------------------------------------------------|
| llm                 | Language Model | Input parameter. The language model to use for categorization. Required. |
...
| category_N_result   | Message  | Output parameter. Dynamic outputs created for each category defined in the routes table. |
| default_result      | Message  | Output parameter. The Else output. Only available when `enable_else_output` is `true`. |
