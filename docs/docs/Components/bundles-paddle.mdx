...
The **Paddle** bundle is included in the `lfx-paddle` Extension bundle. It is
not installed by the default `uv pip install langflow` profile.

Install it directly, or include it with the other opt-in standalone and
long-tail bundles:
