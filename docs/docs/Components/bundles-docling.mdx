...
This component enables document conversion (for example, PDF to text or Markdown) using multimodal models hosted on remote APIs.
...
Use the **provider** dropdown to switch between IBM Cloud and OpenAI-compatible APIs.
The parameters in the component will automatically update based on your selection.
...
| url | String | The base URL of the Watsonx API (for example, `https://us-south.ml.cloud.ibm.com`). |
...
| Name | Type | Description |
|------|------|-------------|
| openai_base_url | String | Base URL for the OpenAI-compatible API (for example, `https://openrouter.ai/api/`). |
| openai_api_key | Secret String | API key for OpenAI-compatible endpoints (leave blank if not required). |
| openai_model | String | Model ID for OpenAI-compatible provider (for example, `gpt-4o-mini`). |
