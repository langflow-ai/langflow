...
* **Linux system dependencies (manual installation or custom Docker images)**:

    When installing Langflow manually on Linux (including custom Docker containers based on Ubuntu/Debian), you must install the following system packages for EasyOCR to work properly with image processing:
    ...
    These packages provide the OpenCV dependencies required by EasyOCR for OCR (Optical Character Recognition) on images. Without these packages, you may encounter errors like `EasyOCR is not installed` or `Could not extract content from the provided file(s)` when processing images.

    :::note
    The official Langflow Docker images (`langflowai/langflow`) include these dependencies by default. This step is only required for manual installations or custom Docker builds.
    :::
