...
The Docling dependency is required to use the Docling components in Langflow.
    ...
    * **Langflow OSS**:
        * If you installed `langflow` (`uv pip install langflow`), the **Docling** bundle is included automatically. Install local Docling parsing with `uv pip install "langflow[docling]"`.
        * If you installed `langflow-base` directly, install `uv pip install "langflow-base[docling]"` for the **Read File** component's Docling parser.
        * If you installed the standalone bundle directly, install local parsing with `uv pip install "lfx-docling[local]"`.
        * To use the **Chunk DoclingDocument** component, install the chunking extra with `uv pip install "langflow[docling-chunking]"` or `uv pip install "lfx-docling[chunking]"`.
        * To use picture-description LLM support in the **Docling** component, install `uv pip install "langflow[docling-image-description]"` or `uv pip install "lfx-docling[image-description]"`.
