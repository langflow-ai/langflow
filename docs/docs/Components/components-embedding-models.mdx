...
The Langflow **Embedding Model** core component generates text embeddings using the selected Large Language Model (LLM).

The **Embedding Model** core component supports many providers and models.
It is appropriate for most text-based embedding model use cases in Langflow.
...
## Use the Embedding Model component in a flow

This example shows how to use the **Embedding Model** component in a flow by creating a semantic search system.
...
2. Add an **Embedding Model** component, and then provide a valid OpenAI API key.
    ...
    If you want to use a different provider or model, edit the **Provider**, **Model**, and **API Key** fields accordingly.

    You can enter the API key directly or select a Langflow global variable that references your API key.
...
| embeddings | Embeddings | Output parameter. An instance for generating embeddings using the selected provider. |
