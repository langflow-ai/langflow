...
If your provider or model isn't supported by the **Embedding Model** core component, additional provider-specific **Embedding Model** components are available in <Icon name="Blocks" aria-hidden="true" /> [**Bundles**](/components-bundle-components) in the [workspace](/concepts-overview).
