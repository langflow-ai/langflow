...
Alternatively, you can use [additional embedding models](#additional-embedding-model-components) in place of the **Embedding Model** core component.
...
If your provider or model isn't supported by the **Embedding Model** core component, additional provider-specific embedding model components are available in <Icon name="Blocks" aria-hidden="true" /> [**Bundles**](/components-bundle-components) in the [visual editor](/concepts-overview).
...
The **Embedding Similarity** component is replaced by built-in similarity search functionality in most vector store components.
