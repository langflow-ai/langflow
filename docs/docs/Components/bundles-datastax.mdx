...
If you want to apply filters regardless of the LLM's input, use the **Static Filters** option.
You can enable this option and other optional parameters through the <Icon name="SlidersHorizontal" aria-hidden="true"/> **Controls** in the [component's header menu](/concepts-components#component-menus).
...
Instead, use the [Astra DB vector store component](/components-vector-stores#astra-db).
:::

Connect this component to the **Embeddings** port of the [Astra DB vector store component](/components-vector-stores#astra-db) to generate embeddings through Astra DB's vectorize feature.
