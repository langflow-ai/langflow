...
Bundled components are custom components that support specific third-party integrations with Langflow.
...
Self Query Retiever
VectorStoreInfo
VectorStoreRounterAgent

Zep - Zep Chat Memory (only component in bundle)
...
All CrewAI components are considered legacy components, including the **Hierarchical Task** component, **Sequential Task** component, and the following components:
...
This component is considered legacy.
