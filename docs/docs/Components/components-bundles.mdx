...
`uv pip install langflow` includes most provider bundles automatically.

For an installation with only built-in core components, use `uv pip install langflow-core`. This distribution does not install any `lfx-*` provider bundle. Add an individual bundle explicitly when a flow needs that provider.
