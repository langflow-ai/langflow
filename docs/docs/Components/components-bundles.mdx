...
:::tip
For the most accurate and up-to-date list of components within each bundle for your version of Langflow, check <Icon name="Blocks" aria-hidden="true" /> **Bundles** in the visual editor.
...
`uv pip install langflow` installs the service-complete `langflow-core` profile
and adds most provider bundles automatically.

To use the same application profile without provider bundles, install
`langflow-core` directly. It does not install any `lfx-*` provider distribution;
add an individual bundle explicitly when a flow needs that provider.

`langflow-core` is not a replacement for the lower-level `langflow-base`
package. Use `langflow-base` when you want the smaller modular platform and
need to select service dependencies as well as provider bundles.
...
The default `uv pip install langflow` and `uv pip install lfx[bundles]` installations do not install components that require the PyTorch dependency.
