...
`uv pip install langflow` includes the curated graduated provider set, but not the `lfx-bundles` long tail.
Install the supported torch-free full profile with `uv pip install "langflow[bundles]"`.
