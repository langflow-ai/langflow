...
The **Language Model** output must be connected to a component with a **Language Model** input, such as the [**Structured Output** component](/components-llm-operations#structured-output), that uses the attached LLM to power the receiving component's reasoning.
