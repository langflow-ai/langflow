...
- **Errors:** If Agentics is not found, run `uv pip install agentics-py` and restart Langflow. For API/key errors, set the provider’s API key as a global variable or env var. For DataFrame errors, ensure input is a list of dicts or use a DataFrame component output.
