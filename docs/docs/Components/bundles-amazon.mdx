...
| top_k | Integer | Input parameter. Limits the number of highest probability vocabulary tokens to consider. Sent only to providers that support it (Anthropic, Mistral); for the others, use `additional_model_fields` to pass the provider's own equivalent. Default: `250`. |
