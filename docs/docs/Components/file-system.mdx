...
It includes five file I/O tools for agents: `read_file`, `write_file`, `edit_file`, `glob_search`, and `grep_search`.
...
5. In the **Playground**, send a message to test the connection. For example:
...
When running Langflow in Docker, mount the sandbox directory as a persistent volume, so the pepper file used to derive per-user namespace hashes survives container restarts:
