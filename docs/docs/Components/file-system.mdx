...
Official release packages, tagged source archives, and Docker images default `LANGFLOW_AUTO_LOGIN` to `false`, so each authenticated user gets a private workspace. Development branches default to `true` and use the shared workspace.

Standalone `lfx run` and identity-free `lfx serve` executions use the stable `shared/` workspace when LFX generates their user identity. This preserves files between runs even with `LANGFLOW_AUTO_LOGIN=false`; it does not enable automatic login. An explicitly supplied or verified user identity uses private per-user storage when automatic login is disabled. Do not use identity-free serving to separate multiple users' files; configure verified identities for that purpose.
