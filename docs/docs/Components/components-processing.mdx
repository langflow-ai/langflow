...
* Perform advanced JSON queries with the [**Data Operations** component](#data-operations) using JQ Expression.
...
| mapped_json_display | JSON to Map | Input parameter. JSON structure to explore for path selection. Used by Path Selection operation. |
| query | JQ Expression | Input parameter. The jq query expression for advanced JSON filtering and transformation. Used by JQ Expression operation. |
| selected_key | Select Path | Input parameter. The JSON path expression to extract values. Used by Path Selection operation. |
...
2. In the **JQ Expression** field, enter a jq query expression to query against the **Data Operations** component's Data input.
