...
1. To use this component in a flow, connect a component that outputs [DataFrames, Data, or Messages](/data-types#data) to the **Save File** component's input.
The following example connects a **Webhook** component to two **Save File** components to demonstrate the different outputs.

![Two Save File components connected to a Webhook component](/img/component-save-to-file.png)

2. In the **Save File** component's **Input Type** field, select the expected input type.
This example expects **Data** from the **Webhook**.
3. In the **File Format** field, select the file type for your saved file.
This example uses `.md` in one **Save File** component, and `.xlsx` in another.
...
The **Split Text** component splits incoming text data into chunks based on parameters like chunk size and separator.
It's ideal for chunking data to be tokenized and embedded into vector databases.

The component accepts `Message`, `Data`, or `DataFrame`, and then outputs chunks as **Chunks** or **DataFrame**.
...
| Name | Display Name | Info |
|------|--------------|------|
| data_inputs | Input | The data to split. The component accepts [Data](/data-types#data) or [DataFrame](/data-types#dataframe) objects. |
...
| separator | Separator | The character to split on. Default: `newline`. You can enter a separator string expression, or input a `Message` from another component. |
| text_key | Text Key | The key to use for the text column. Default: `text`. |
| keep_separator | Keep Separator | Select how to handle separators in output chunks. If False, separators are omitted from output chunks. Options include `False` (remove separators), `True` (keep separators in place), `Start` (place separators at the beginning of chunks), or `End` (place separators at the end of chunks). Default: `False`. |
