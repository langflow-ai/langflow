...
However, you can also use a language model component for actions that don't emit chat output directly, such as the **Smart Function** component.
...
If your provider or model isn't supported by the **Language Model** core component, additional provider-specific models are available in <Icon name="Blocks" aria-hidden="true" /> [**Bundles**](/components-bundle-components) in the [visual editor](/concepts-overview).
...
1. In the visual editor, <Icon name="Search" aria-hidden="true" /> **Search** for your preferred model provider, and then add the provider's LLM component to your flow.
...
The **Model Provider** field changes to a `LanguageModel` input port.
