...
These integrations make agents more specialized and powerful than standalone Large Language Models (LLMs).
...
For more information about these settings, see [Use Langflow agents](/agents).
...
| tools | List | Input parameter. The list of tools available for the agent to use. This field is optional and can be empty. For more information, see [Configure tools for agents](/agents-tools). |
