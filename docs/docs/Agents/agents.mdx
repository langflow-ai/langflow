...
Many optional **Agent** input parameters are hidden by default in the visual editor.
...
The **Agent** component outputs a **Response** (`response`), which is [`Message` data](/data-types#message) containing the agent's raw response to the query.
