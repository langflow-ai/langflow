...
* **Output Format Instructions** (`format_instructions`): System message that guides the agent's LLM's approach to data extraction and JSON formatting. Works with the Output Schema to ensure consistent, properly structured responses. Valid only with Structured response mode.
* **Output Schema** (`output_schema`): Schema validation table that defines the structure and data types for structured output. The LLM uses this schema along with the format instructions to validate and structure its JSON responses. No validation occurs if no output schema is provided.
...
7. Inspect the Playground's output.
