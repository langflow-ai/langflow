...
* **Structured Response** (`structured_response`): The agent's reply formatted as structured [`Data`](/data-types#data) according to the **Output Schema** you define.
    ...
    4. Connect the **Structured Response** port to a downstream component that accepts [`JSON`](/data-types#json) input, such as a **Parser** or [**Operations** component](./operations).
