...
Langflow MCP servers support both the **streamable HTTP** protocol and **Server-Sent Events (SSE)** as a fallback.
...
| sse_url | String | Input parameter. HTTP/SSE mode only. The URL for an MCP server. Supports both streamable HTTP and SSE protocols. Default for Langflow Desktop: `http://localhost:7868/`. Default for other installations: `http://localhost:7860/api/v1/mcp/project/PROJECT_ID/streamable` or `http://localhost:7860/api/v1/mcp/streamable` |
