...
11. Optional: To override headers or add additional headers to the **MCP Tools** component, click <Icon name="SlidersHorizontal" aria-hidden="true"/> **Controls** in the [component's header menu](/concepts-components#component-menus), enable **Show** for the **Headers** parameter, and then add header key values. Headers configured in the component take precedence over the headers configured in the MCP server settings.
