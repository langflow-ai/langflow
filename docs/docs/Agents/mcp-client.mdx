...
4. In the **MCP Server** field on the **MCP Tools** component, select the server you registered.
    ...
    New servers are registered in **Settings** > **MCP Servers**, or in the <McpIcon /> **MCP** sidebar > <Icon name="Plus" aria-hidden="true"/> **Add MCP Server**.
...
- [Langflow MCP Client](/langflow-mcp-client)
