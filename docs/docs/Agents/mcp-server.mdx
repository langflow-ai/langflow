...
1. Per-component timeout is set directly on an MCP Tools component in your flow. To configure per-component timeout in the [**MCP Server** component](/mcp-server), open the advanced settings tab and enter a value in the **Tool Execution Timeout (seconds)** field. A value of `0` disables the per-component override and falls back to the global setting.

2. Global timeout is set with the `LANGFLOW_MCP_TOOL_EXECUTION_TIMEOUT` environment variable. The default is 180 seconds.

3. Langflow enforces a minimum floor of 180 seconds for tool execution, regardless of lower values set at the component or global level.

To support tool calls that take longer than 180 seconds, set `LANGFLOW_MCP_TOOL_EXECUTION_TIMEOUT` or the per-component **Tool Execution Timeout (seconds)** field to a value greater than `180`.
