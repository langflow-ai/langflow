...
- More configuration options for JWT-based session authentication
    ...
    Langflow 1.8 offers additional configuration options for JWT algorithms, including support for RS256/RS512 algorithms, configurable keys, and token lifetimes. For more information, see [JWT authentication](/1.8.0/jwt-authentication).
...
- New integrations, bundles, and components:
  ...
  New filter operator for **DataFrame Operations** component
    ...
    The [**DataFrame Operations** component](/1.8.0/dataframe-operations) now includes a `not contains` filter operator.
  ...
  New JSON operations for **Data Operations** component
    ...
    The [**Data Operations** component](/1.8.0/data-operations) now includes two operations for advanced JSON data manipulation.
