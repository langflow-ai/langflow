...
For example, if you are looping over a `DataFrame`, you could use multiple [**DataFrame Operations** components](/1.8.0/dataframe-operations) to conditionally filter data, and then run separate loops on each set of filtered data.
