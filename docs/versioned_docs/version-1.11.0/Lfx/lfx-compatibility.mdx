...
`uv pip install langflow` instead depends on `lfx-bundles[all-no-torch]` plus the individual `lfx-*` packages, so Langflow stays torch-free by default.
...
There is intentionally no `lfx[all]` extra, because that is equivalent to `uv pip install langflow`.
