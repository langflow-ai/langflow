...
For exposing fields in generated API snippets and related UI steps, see [Tweaks (API inputs)](/concepts-publish#input-schema).
For large or stable values that should not depend on component ids, prefer [request-level global variables](#request-level-global-variables).
