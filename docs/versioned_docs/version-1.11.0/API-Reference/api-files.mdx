...
2. Use the returned `file_path` to send the image file to other components that can accept file input. Where you specify the file path depends on the component type.
    ...
    :::tip
    For help with tweaks, expose fields in the component [**Parameters** panel](/concepts-publish#input-schema), then open **Share > API access** to copy snippets with the required component IDs.
    :::
