...
By default, a shared **Playground** can't run a flow that contains a custom component, because the run is unauthenticated and Langflow executes only its own copy of each component on that path. To run one anyway, install the component on the server with `LANGFLOW_COMPONENTS_PATH`. For more information, see [Custom components in publicly shared flows](/deployment-block-custom-components#public-flow-custom-components).
