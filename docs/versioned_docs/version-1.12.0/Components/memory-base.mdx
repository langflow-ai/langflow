...
The Memory Base component retrieves context from a [memory base](./memory-bases) attached to the current flow using semantic search.

Retrieval uses the [DB Provider](/knowledge#configure-vector-database-providers) selected when the memory base was created.

The most relevant conversation chunks are returned as a [DataFrame](/data-types#table).
...
For more information, see [Manage memory bases](./memory-bases).
