...
By default, knowledge bases use Chroma as a local vector store, but you can configure an external vector database provider such as Chroma Cloud, OpenSearch, or Postgres pgvector.
Knowledge bases and [memory bases](/memory-bases) share the same [DB Providers](/knowledge#configure-vector-database-providers).
...
* **Local storage by default**: Langflow knowledge bases use Chroma local storage by default, unless you configure another DB Provider such as Postgres pgvector.
