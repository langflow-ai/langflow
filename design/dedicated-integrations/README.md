...
Status: gate open pending role-owner sign-offs. Existing release-owner decisions remain accepted; the new
broad-content-read decision and cross-provider messaging/portability requirements are proposed for owner review.
The structural checker passes with proposed records; gate-close mode (`--require-accepted`) requires accepted
decisions and completed Name, Date and PR cells in both aggregate and record sign-off tables. Wave 1 remains
Google 5, Microsoft 8, Slack 7 actions.
Last updated: 2026-09-09 (scope reach controls, evidence freshness, consent/identity surfaces and portability)
...
| 2 | Every scope classified; restricted and broad content-read grants have written decisions | scope entries in the matrices; `decisions/google-restricted-scopes.md`, `decisions/broad-content-read.md` | classification present and sourced; every scope on an included action tagged `required`, `optional`, or `alternative`; conditional rows carry a structured predicate naming a real action input; at least one scope is required; restricted scopes need a `restricted_scope_decisions` entry; `content_read_reach: cross_resource` needs a `scope_risk_decisions` entry regardless of classification, including optional grants | Google avoidance accepted; broad-read conditional acceptance proposed, pending owner signatures |
...
| 5 | Frontend surface list | `frontend-surfaces.md` | none | drafted 2026-09-01; 14 extend + 14 new, including the operator governance surface; MVP/defer split; includes opt-in, identity, policy/tenant consent and Drive empty states |
| 6 | Trigger/webhook track recorded as deferred | `triggers-deferred.md` | none | governing-plan findings folded in; **amended 2026-09-04 (release owner decision)**: the track is re-opened as 1.13 scope and the transport and delivery discovery it deferred is recorded in `../dedicated-integrations-triggers/`; its gate remains open. The boundary this criterion fixed still binds wave-1 actions |
| 7 | Re-issued estimate | `estimate.md` | none | re-issued 2026-09-01, amended 2026-09-02 and 2026-09-04: 51.75 engineer-weeks under the confirmed decisions (INT-9 restored to 1.13); 2026-09-09 UX additions require owner sizing before gate close |
...
| + | Instance-connection referenceability (added during INT-6, 2026-09-05) | `decisions/instance-connection-referenceability.md` | none | proposed, pending recorded owner approval: any non-anonymous principal may resolve an instance row in 1.13; narrowing goes through a policy hook, no `referenceable` column; no estimate delta |
...
|---|---|---|---|---|
| lfx owner | `connection-contract.md`, `ga-swap-procedure.md`, `security-review.md`, `decisions/substrate-google.md`, `decisions/substrate-microsoft.md`, `decisions/substrate-slack.md`, `decisions/kb-oauth-connector-adoption.md`, `cross-provider-capabilities.md` | | | |
| langflow-base owner | `connection-contract.md`, `security-review.md`, `decisions/substrate-google.md`, `decisions/substrate-microsoft.md`, `decisions/substrate-slack.md`, `decisions/google-restricted-scopes.md`, `decisions/kb-oauth-connector-adoption.md`, `decisions/desktop-oauth-ownership.md`, `decisions/broad-content-read.md`, `decisions/instance-connection-referenceability.md` | | | |
| Enterprise owner | `connection-contract.md`, `security-review.md`, `decisions/substrate-google.md`, `decisions/substrate-microsoft.md`, `decisions/substrate-slack.md`, `decisions/google-restricted-scopes.md`, `decisions/broad-content-read.md`, `decisions/instance-connection-referenceability.md` | | | |
| frontend owner | `connection-contract.md` (section 12.d), `frontend-surfaces.md`, `decisions/palette-naming.md`, `decisions/broad-content-read.md`, `cross-provider-capabilities.md` | | | |
| hosted-app owner | `decisions/google-restricted-scopes.md`, `decisions/substrate-google.md`, `decisions/substrate-microsoft.md`, `decisions/substrate-slack.md`, `decisions/desktop-oauth-ownership.md`, hosted and desktop rows of every matrix, `decisions/broad-content-read.md` | | | |
| product owner | `decisions/palette-naming.md`, `decisions/broad-content-read.md`, `cross-provider-capabilities.md` | | | |
...
Evidence dates on the matrix and every source must be no more than **30 days old** (UTC); future dates are
rejected too. This applies in normal and gate-close modes, including deferred substrate evidence. A fresh matrix
date cannot hide a stale source. Historical signatures do not expire. CI checks this whenever its path filters
run; the release owner also runs the checker before gate close and release validation. Refresh a date only after
re-reading the URL and reviewing affected scope, GA, license and reach claims. No network fetch is performed by
the checker: HTTPS syntax, schema validity and recent dates do not prove a URL exists or a scope spelling/claim
is correct. Human source review remains required; this gate records no live-tenant validation.
...
decisions/broad-content-read.md    proposed controls for broad content grants across providers
cross-provider-capabilities.md    product differences and deployment portability requirements
...
`restricted_scope_decisions` (one entry per restricted scope any included or deferred action carries),
`scope_risk_decisions` (one entry per broad content-read scope on included/deferred actions), `sources`,
...
| `consent` | `user`, `admin`, or `both`: provider default, not tenant approval; with notes and a source |
| `tenant_consent` | Required per included action: sourced deployment validation requirement. INT-14 must validate the actual tenant/app/scope set; B12 owns approval/denial UI |
| `scopes[].content_read_reach` | Provider-granted content read: `none`, `selected_resources`, or `cross_resource`. This dimension is independent of scope classification and action input filters |
...
```json
{
  ...
  "scopes": [ { "scope": "https://www.googleapis.com/auth/gmail.send", "classification": "sensitive", "provider_classification": "...", "source": "gmail-scopes", "role": "required", "content_read_reach": "none" } ],
  "consent": "user",
  "consent_source": "...",
  "tenant_consent": { "status": "deployment_validation_required", "requirement": "Validate the actual tenant registration and scopes before rollout", "source": "workspace-admin-app-access" },
