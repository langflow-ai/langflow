...
class ConnectionPoolMonitor:
    ...
    def detect_leaked_engines(self, max_age_seconds: float = 3600) -> list[str]:
        ...
        # List comprehension is generally faster for filtered lists
        return [
            engine_id
            for engine_id, metrics in self.engines.items()
            if (current_time - metrics.created_at) > max_age_seconds
        ]
    ...
    def get_health_report(self) -> ConnectionPoolHealth:
        ...
        # Avoid recomputing .values() and sum by zipping for one iteration over engines
        # Use local variable to avoid attribute lookup in loops
        engines_values = list(self.engines.values())
        health.total_engines = len(engines_values)
        total_connections = 0
        for m in engines_values:
            total_connections += m.total_connections
        health.total_connections = total_connections

        # Detect potential leaks
        leaked_engine_ids = self.detect_leaked_engines()
        leaked_engines_count = len(leaked_engine_ids)
        health.leaked_engines = leaked_engines_count

        # Use generator expression instead of list for slightly improved memory usage
        leaked_connections = 0
        # Pre-store self.engines as local for faster lookups in loop
        engines_dict = self.engines
        for eid in leaked_engine_ids:
            if eid in engines_dict:
                leaked_connections += engines_dict[eid].total_connections
        health.leaked_connections = leaked_connections
