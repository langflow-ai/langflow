...
def docling_worker(file_paths: list[str], queue, pipeline: str, ocr_engine: str):
    ...
    try:
        ...
        results = []
        batch_log_size = 20 if total_files > 40 else 1
        for i, file_path in enumerate(file_paths):
            # Check for shutdown before processing each file
            check_shutdown()

            # Reduce debug log frequency for large batches
            if batch_log_size == 1 or i % batch_log_size == 0 or i == total_files - 1:
        ...
        # Efficient processed_data construction and success count in one pass
        processed_data = []
        success_count = 0
        for res in results:
