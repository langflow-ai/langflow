...
class ProgressIndicator:
    ...
    def _animate_step(self, step_index: int) -> None:
        ...
        # The title doesn't change during animation; store it up front.
        title = step["title"]
        animation_chars = self._animation_chars
        animation_chars_len = len(animation_chars)

        # Locally cache frequently accessed attributes to reduce attribute lookups inside the loop
        running = self.running
        stop_animation = self._stop_animation
        shutdown_requested = self._shutdown_requested

        while (
            self.running and step["status"] == "running" and not self._stop_animation and not self._shutdown_requested
        ):
            # Compute animated character and line in locals
            animation_char = animation_chars[self._animation_index]
            display_line = f"\r{animation_char} {title}..."

            # Single write call for carriage return + animation
            sys.stdout.write(display_line)
            sys.stdout.flush()

            self._animation_index = (self._animation_index + 1) % animation_chars_len

            # Sleep with early exit check before each interval for quickest shutdown
            for _ in range(15):
