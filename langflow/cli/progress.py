...
class ProgressIndicator:
    ...
    def print_summary(self) -> None:
        ...
        # Use tuple for "status" membership test (faster than list)
        completed_steps = [s for s in self.steps if s["status"] in ("completed", "failed")]
        if not completed_steps:
            return

        # Avoid repeated lookups by pulling out fields with local var
        # Minimize generator overhead by using sum and inline logic
        total_time = 0.0
        ...
        # Merge two echo calls to a single call for faster stdout operation
        # (saves interpreter and terminal overhead)
        click.echo("\n" + click.style(f"Total initialization time: {total_time:.2f}s", fg="bright_black"))
