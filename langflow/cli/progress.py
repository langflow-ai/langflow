...
class ProgressIndicator:
    ...
    def print_shutdown_summary(self) -> None:
        ...
        # Pre-resolve the statuses set for faster lookup.
        _done_status = {"completed", "failed"}

        # Merge the two filtering loops and avoid list creation until necessary
        completed_steps = []
        total_time = 0.0
        for s in self.steps:
            if s["status"] in _done_status:
        ...
        # Minimize extra function calls to click.echo
        msg = click.style(f"Total shutdown time: {total_time:.2f}s", fg="bright_black")
        click.echo()
        click.echo(msg)
