...
class Settings(BaseSettings):
    ...
    """The maximum number of flow version entries to keep per flow.

    Older entries are deleted with a confirmation prompt.
    If this value is reduced below the current entry count for a flow,
    the oldest entries are deleted when the next entry is created.
    """
