...
# --- FastAPI route-detail compatibility patch (FastAPI >=0.137 lazy include_router) ---------
#
# FastAPI 0.137 changed include_router to *lazy* inclusion: instead of eagerly flattening a
# sub-router's routes onto the parent, the top-level app.routes now contains _IncludedRouter
# wrappers. Those wrappers are matchable BaseRoute objects but intentionally carry no .path.
#
# opentelemetry-instrumentation-fastapi names every request span by walking app.routes and
# reading route.path. Its Match.FULL branch already guards the missing-path case, but the
# Match.PARTIAL branch does not, so a partial match (an OPTIONS/CORS preflight against a
# GET-only route) raises AttributeError mid-request and turns into a 500.
#
# Recovering the template is not cosmetic: under the stable conventions this value becomes
# http.route, which is a *metric label*, so the raw request path would mint a series per flow
# id, file id and session id. Everything mounts under /api, so on FastAPI >=0.137 that is
