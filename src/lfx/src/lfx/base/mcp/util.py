...
def _server_uri_for_trust_check(client) -> str:
    """Extract the server URI for the trust hook, or "" for non-URL transports (e.g. stdio)."""
    conn = getattr(client, "_connection_params", None)
    return conn.get("url", "") if isinstance(conn, dict) else ""
        ...
        try:
            ...
            if trust_verifier is not None:
                await run_trust_check(
                    trust_verifier,
                    tool_name,
                    _server_uri_for_trust_check(client),
                    arguments,
