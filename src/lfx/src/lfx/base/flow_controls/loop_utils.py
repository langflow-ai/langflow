...
async def execute_loop_body(
    ...
    Args:
        ...
        loop_id: ID of the Loop component used to tag per-iteration execution events
    ...
    # Existing saved flows embed an older Loop component implementation that
    # calls this helper without the newer ``loop_id`` argument. The outgoing
    # Loop -> body edge is available in both old and new flows, so use its
    # source vertex as a backwards-compatible authoritative fallback.
    effective_loop_id = loop_id or (getattr(start_edge, "source_id", None) if start_edge else None)

    async def run_iteration(item, iteration_index, trace_result=None):
        async with graph.create_subgraph(loop_body_vertex_ids) as iteration_subgraph:
            ...
            if trace_result is not None:
                trace_result.set_outputs({"result": output})
            return output

    iteration_count = len(data_list)
    tracing_service = graph.tracing_service
    for iteration_index, item in enumerate(data_list):
        if effective_loop_id is not None and getattr(tracing_service, "supports_loop_iteration_tracing", False) is True:
            async with tracing_service.trace_loop_iteration(
                loop_id=str(effective_loop_id),
                iteration_index=iteration_index,
                iteration_count=iteration_count,
                inputs={"item": item},
            ) as trace_result:
                output = await run_iteration(item, iteration_index, trace_result)
        else:
            output = await run_iteration(item, iteration_index)
