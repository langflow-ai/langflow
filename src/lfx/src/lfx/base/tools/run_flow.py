...
from lfx.inputs.inputs import BoolInput, DropdownInput, InputTypes, MessageInput, StrInput
...
class RunFlowBaseComponent(Component):
    def __init__(self, *args, **kwargs):
        ...
        self._cache_flow_dispatcher: dict[str, Callable[..., Any]] = {
            ...
            "delete": getattr(self._shared_component_cache, "delete", lambda _: None),
    ...
    ################################################################
    @property
    def normalized_session_id(self) -> str | None:
        if not hasattr(self, "__normalized_session_id"):
            self.__normalized_session_id = self.session_id.text
        return self.__normalized_session_id
    ...
    async def get_graph(
        ...
        if flow := self._flow_cache_call("get", flow_name=flow_name_selected, flow_id=flow_id_selected):
            return flow

        flow = await self.get_flow(flow_name_selected, flow_id_selected)
        if not flow:
            msg = "Flow not found"
            raise ValueError(msg)
    ...
    async def get_required_data(self) -> tuple[str, list[dotdict]] | None:
        ...
        """
        graph = await self.get_graph(self.flow_name_selected, self.flow_id_selected)
    ...
    async def resolve_selected_flow_output(self):
        """Extract vertex id and output name from the current output node and resolve the value."""
        current = (
        ...
        if not current or self.IOPUT_SEP not in current:
            return None
        vertex_id, output_name = current.split(self.IOPUT_SEP, 1)
    ...
    async def update_outputs(self, frontend_node: dict, field_name: str, field_value: Any) -> dict:
        ...
        graph = await self.get_graph(
            ...
            flow_id_selected=frontend_node.get("template", {})
    ...
    def _format_flow_outputs(self, graph: Graph) -> list[Output]:
        ...
        The method is set to resolve_selected_flow_output which uses the unique name
    ...
    async def _run_flow_with_cached_graph(
        ...
        *,
        ...
        return await run_flow(
            ...
            user_id=user_id,
            session_id=self.normalized_session_id,
            output_type=output_type,
            graph=graph,
        )
    ...
    def _get_cached_flow(self, *, flow_name: str | None, flow_id: str | None = None) -> Graph | None:
        cache_key = self._build_flow_cache_key(flow_name=flow_name, flow_id=flow_id)
    ...
    def _set_cached_flow(self, *, flow: Graph) -> None:
        ...
        flow_name = flow.flow_name or graph_dump.get("name")
        payload = {
        ...
        cache_key = self._build_flow_cache_key(flow_name=flow_name, flow_id=flow.flow_id)
        self._shared_component_cache.set(cache_key, payload)

    def _build_flow_cache_key(self, *, flow_name: str | None, flow_id: str | None = None) -> str | None:
        if not self.user_id or not (flow_name or flow_id):
            msg = "Failed to build cache key: Flow name or id, and user ID are required"
            raise ValueError(msg)
        return f"run_flow:{self.user_id}:{flow_id or 'missing_id'}:{flow_name or 'missing_name'}"
    ...
    def _build_graph_from_dict(self, *, cache_entry: dict[str, Any]) -> Graph | None:
        ...
        graph = Graph.from_payload(
            ...
            flow_name=cache_entry.get("flow_name") or graph_dump.get("name"),
