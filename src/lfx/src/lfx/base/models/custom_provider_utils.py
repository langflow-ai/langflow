...
def get_custom_provider_options(user_id: UUID | str | None) -> list[dict[str, Any]]:
    ...
    if not user_id:
        return []
    try:
        return run_until_complete(_fetch_options(user_id))
    except Exception:  # noqa: BLE001
        logger.debug("Could not load custom providers")
        return []


async def _fetch_options(user_id: UUID | str) -> list[dict[str, Any]]:
    try:
...
def get_custom_provider_credentials(
    ...
    try:
        return run_until_complete(_fetch_credentials(custom_provider_id, user_id))
    except Exception:  # noqa: BLE001
        logger.debug(f"Could not fetch custom provider credentials for {custom_provider_id}")
        return None


async def _fetch_credentials(custom_provider_id: str, user_id: UUID | str) -> tuple[str, str] | None:
    try:
