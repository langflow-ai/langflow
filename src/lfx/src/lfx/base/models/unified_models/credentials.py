...
def validate_model_provider_key(provider: str, variables: dict[str, str], model_name: str | None = None) -> None:
    ...
    try:
        if provider == "OpenAI":
            ...
            # Probe with the model the caller selected (or the first catalog
            # model) so a key scoped to a subset of models still validates.
            probe_model = validation_model or "openai/gpt-4o-mini"
            try:
