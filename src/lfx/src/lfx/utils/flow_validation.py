...
async def prepare_public_flow_build(target: Mapping[str, Any] | Any | None) -> dict | None:
    ...
    if settings_service.settings.allow_public_custom_components:
        ...
        # The opt-in permits tenant component *code*; it does not permit spawning an OS
        # process. The MCP stdio command lives in a field value, so the trusted-code
        # substitution below never touches it and this branch returns before the standard
        # public validator runs.
        opt_in_data = _extract_flow_data(target)
        if isinstance(opt_in_data, Mapping):
            opt_in_nodes = opt_in_data.get("nodes", [])
            if isinstance(opt_in_nodes, list) and opt_in_nodes:
                _raise_if_public_flow_launches_mcp_stdio(opt_in_nodes)
        return None
...
def _raise_if_public_flow_launches_mcp_stdio(nodes: list) -> None:
    """Refuse a public build whose graph selects the MCP stdio (subprocess) transport.

    Shared by both public entry points: the standard validator and the
    ``allow_public_custom_components`` opt-in branch, which returns before that validator
    runs. Remote MCP transports spawn nothing and stay allowed.
    """
    ...
    mcp_stdio += _collect_blocked_components(
        ...
        blocked_hashes=_blocked_code_hashes(MCP_STDIO_COMPONENT_TYPES),
    )
    if mcp_stdio:
...
def validate_public_flow_no_code_execution(target: Mapping[str, Any] | Any | None) -> None:
    ...
    * MCP **stdio** server configuration — a node that launches an operating-system
    ...
    _raise_if_public_flow_launches_mcp_stdio(nodes)
