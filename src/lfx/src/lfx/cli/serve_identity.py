...
The edge gateway (Kong key-auth, oauth2-proxy) already authenticated the caller
and forwards that fact downstream. This module lets ``lfx serve`` *consume* that
identity and thread it into flow execution as ``user_id`` — but only as a
cryptographically verifiable assertion by default.
