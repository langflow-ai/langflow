...
# --- Cached compile path for eval_custom_component_code -------------------
#
# create_class() above is the correctness-preserving reference implementation
# and stays untouched (it is exercised directly by tests and other callers).
# eval_custom_component_code() runs the same source through every vertex of
# every flow build, so it gets its own path here that caches the immutable
# parse/compile output and re-execs it fresh on every call.
#
# What's cached (in _ComponentCompileArtifacts, built by
# _compile_component_artifacts): the extracted class name, and two compiled
# code objects. Code objects are immutable, and compiling an ast tree never
# mutates it, so these are safe to share across calls.
#
# What's deliberately NOT cached: exec_globals. It's a fresh dict built and
# populated on every call, and the returned constructor closes over it and
# writes into it (see build_class_constructor) -- sharing it would defeat
# isolation immediately. The import/from-import specs needed to rebuild it
# are cached as plain (str, str | None) tuples rather than the AST nodes
# themselves, so nothing mutable-by-reference outlives the compile step.
#
# The isolation guarantee this preserves: exec()-ing a `class` statement's
# code object always creates a brand-new class object, no matter how many
# times that same code object has been exec'd before. Caching the code object
# is safe; caching the class it produces (the reverted approach) is not. See
# test_component_class_isolation.py.


@dataclass(frozen=True)
class _ComponentCompileArtifacts:
    """Immutable output of parsing + compiling one component source string."""

    class_name: str
    class_code_obj: CodeType
    definitions_code_obj: CodeType | None
    plain_imports: tuple[tuple[str, str | None], ...]
    plain_import_froms: tuple[tuple[str, tuple[tuple[str, str | None], ...]], ...]


def _find_component_class_name(module: ast.Module, code_for_error: str) -> str:
    """Find the first Component subclass in an already-parsed module.

    Same matching rule as extract_class_name, factored out so the cached path
    below doesn't need a second, separate ast.parse of the source to find it.
    """
    for node in module.body:
        ...
        for base in node.bases:
            if isinstance(base, ast.Name) and any(pattern in base.id for pattern in ["Component", "LC"]):
                return node.name
    msg = f"No Component subclass found in the code string. Code snippet: {code_for_error[:100]}"
    raise TypeError(msg)


@lru_cache(maxsize=512)
def _compile_component_artifacts(code: str) -> _ComponentCompileArtifacts:
    """Parse and compile component source once; cache the immutable result.

    Mirrors create_class's parse/categorize/compile steps exactly (including
    the same backwards-compat import rewrites and DEFAULT_IMPORT_STRING
    prefix), but stops before anything is exec'd -- nothing exec'd yet means
    nothing here can be a class object or any other mutable, shared artefact.
    lru_cache never caches a raised exception, so invalid source is simply
    re-validated on every call, same as before.
    """
    if not hasattr(ast, "TypeIgnore"):
        ast.TypeIgnore = create_type_ignore_class()

    processed = code.replace("from langflow import CustomComponent", "from langflow.custom import CustomComponent")
    processed = processed.replace(
        "from langflow.interface.custom.custom_component import CustomComponent",
        "from langflow.custom import CustomComponent",
    )
    processed = DEFAULT_IMPORT_STRING + "\n" + processed

    module = ast.parse(processed)
    class_name = _find_component_class_name(module, code)
    ...
    # tuples -- no AST node is retained past this function returning.
    plain_imports = tuple((alias.name, alias.asname) for node in imports for alias in node.names)
    plain_import_froms = tuple(
        (node.module, tuple((alias.name, alias.asname) for alias in node.names)) for node in import_froms
    )

    class_code = next(node for node in module.body if isinstance(node, ast.ClassDef) and node.name == class_name)
    class_code_obj = compile_class_code(class_code, future_imports)

    definitions_code_obj = None
    if definitions:
        combined_module = ast.Module(body=future_imports + definitions, type_ignores=[])
        definitions_code_obj = compile(combined_module, "<string>", "exec")

    return _ComponentCompileArtifacts(
        class_name=class_name,
        class_code_obj=class_code_obj,
        definitions_code_obj=definitions_code_obj,
        plain_imports=plain_imports,
        plain_import_froms=plain_import_froms,
    )


def _resolve_plain_imports(plain_imports, exec_globals) -> None:
    """Populate exec_globals from cached `import x[.y][ as z]` specs.

    Faithful transcription of prepare_global_scope's `imports` loop, reading
    from cached (module_name, asname) tuples instead of AST nodes.
    """
    for module_name, asname in plain_imports:
        module_obj = None
        for name in _get_module_fallbacks(module_name):
            try:
                module_obj = importlib.import_module(name)
        ...
        if module_obj is None:
            if sys.platform == "win32":
                variable_name = asname or module_name.split(".")[0]
                exec_globals[variable_name] = _MissingModulePlaceholder(module_name)
                logger.debug("Module '%s' unavailable on Windows — inserted placeholder", module_name)
                continue
            module_obj = importlib.import_module(module_name)
        ...
        if asname:
            exec_globals[asname] = module_obj
        else:
            variable_name = module_name.split(".")[0]
            exec_globals[variable_name] = sys.modules.get(variable_name, module_obj)


def _resolve_plain_import_froms(plain_import_froms, exec_globals) -> None:
    """Populate exec_globals from cached `from x import y[ as z]` specs.

    Faithful transcription of prepare_global_scope's `import_froms` loop,
    reading from cached (module, names) tuples instead of AST nodes.
    """
    for module_name_original, names in plain_import_froms:
        module_names_to_try = _get_module_fallbacks(module_name_original)
        ...
        for module_name in module_names_to_try:
            try:
                ...
                for name, asname in names:
                    key = asname or name
                    exec_globals[key] = _resolve_attribute(imported_module, module_name, name)
                success = True
                break
            except ModuleNotFoundError as e:
                last_error = e
        ...
        if not success:
            if last_error:
                raise last_error
            msg = f"Module {module_name_original} not found. Please install it and try again"
            raise ModuleNotFoundError(msg)


def _instantiate_component_class(artifacts: _ComponentCompileArtifacts) -> type:
    """Exec a fresh class from cached, immutable compile artefacts.

    Must run in full on every call: exec_globals is a brand-new dict each
    time (the returned constructor closes over it and writes into it -- see
    build_class_constructor), and exec()-ing a `class` statement's code
    object always builds a new class object regardless of how many times
    that code object has been exec'd before. That's what keeps every call
    isolated even though class_code_obj itself is shared. See
    test_component_class_isolation.py.
    """
    exec_globals = globals().copy()
    _resolve_plain_imports(artifacts.plain_imports, exec_globals)
    _resolve_plain_import_froms(artifacts.plain_import_froms, exec_globals)

    if artifacts.definitions_code_obj is not None:
        exec(artifacts.definitions_code_obj, exec_globals)

    return build_class_constructor(artifacts.class_code_obj, exec_globals, artifacts.class_name)
