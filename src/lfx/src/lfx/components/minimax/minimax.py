...
class MiniMaxModelComponent(LCModelComponent):
    ...
    inputs = [
        *LCModelComponent.get_base_inputs(),
        IntInput(
            ...
            range_spec=RangeSpec(min=0, max=204800),
        ),
        DictInput(
            name="model_kwargs",
        ...
        StrInput(
            ...
            advanced=True,
            info="Base URL for API requests. Defaults to https://api.minimax.io/v1",
            value="https://api.minimax.io/v1",
        ),
        SecretStrInput(
            name="api_key",
            display_name="MiniMax API Key",
            info="The MiniMax API Key.",
            advanced=False,
            required=True,
        ),
        SliderInput(
            ...
            info="Controls randomness in responses. Must be in (0.0, 1.0].",
            value=0.9,
            range_spec=RangeSpec(min=0.01, max=1, step=0.01),
            advanced=True,
        ),
        IntInput(
            name="seed",
    ...
    def _get_exception_message(self, e: Exception):
        """Get message from MiniMax API exception."""
        try:
