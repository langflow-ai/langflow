...
class SemanticMap(Component):
    ...
    inputs = [
        ...
        BoolInput(
            name="aggregate_rows",
            display_name="Aggregate Multiple Rows",
            info="When enabled, reduces multiple rows into aggregated output",
            value=False,
        ),
    ]
    ...
    async def semantic_map(self) -> DataFrame:
        ...
        transduction_type = TRANSDUCTION_AREDUCE if self.aggregate_rows else TRANSDUCTION_AMAP
        target = AG(
            atype=atype,
