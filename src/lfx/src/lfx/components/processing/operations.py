...
components into a single component. A single "Operation" picker lists every
operation across all three Langflow data types; selecting one reveals the
matching input (JSON / Table / Text), its operation-specific fields, and the
...
from lfx.inputs import DictInput, SortableListInput
from lfx.io import (
    BoolInput,
    DataFrameInput,
    DataInput,
...
# Operations grouped by the Langflow data type they act on. The sets are the
# single source of truth used for input/output routing and dispatch.
JSON_OPERATIONS = {
    "Select Keys",
    "Literal Eval",
    "Combine",
...
# Text operations whose output is a Message (the generic text-out group).
# "Word Count" (JSON out) and "Text to DataFrame" (Table out) are handled
# separately because their output type differs.
TEXT_MESSAGE_OPERATIONS = {
    "Case Conversion",
    "Text Replace",
    "Text Extract",
    "Text Head",
...
class OperationsComponent(Component):
    ...
    description = "Perform operations on JSON, Tables, and Text from a single component."
    ...
    # The three main inputs (one per data type) and the operation selector are
    # never treated as operation-specific fields.
    MAIN_INPUTS = ("data", "df", "text_input")

    # Every operation-specific field (union of all the lists above), used to
    # hide and reset fields whenever the operation changes.
    ...
    inputs = [
        ...
        SortableListInput(
            ...
            info="Select the operation to perform. The matching input and fields appear once you choose one.",
            options=[
                # JSON
    ...
    def _category(operation: str) -> str:
        """Return the data-type category ('json', 'table', 'text') for an operation."""
    ...
    def update_build_config(self, build_config: dotdict, field_value: Any, field_name: str | None = None) -> dotdict:
        ...
        # Hide and reset every operation-specific field.
        ...
        # Reset all main inputs to hidden + optional.
        ...
        # No operation selected: surface all three inputs so the component is
        # discoverable as handling JSON, Table, and Text.
        if not operation:
            for inp in self.MAIN_INPUTS:
                if inp in build_config:
                    build_config[inp]["show"] = True
            return build_config

        # Show only the input that matches the operation's data type.
        category = self._category(operation)
        main_input = {"json": "data", "table": "df", "text": "text_input"}[category]
        if main_input in build_config:
        ...
        # Reveal the operation-specific fields.
    ...
    def update_outputs(self, frontend_node: dict, field_name: str, field_value: Any) -> dict:
        """Create the output(s) appropriate to the selected operation."""
        if field_name != "operation":
