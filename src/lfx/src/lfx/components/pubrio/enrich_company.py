...
class PubrioEnrichCompanyComponent(Component):
    display_name = "Pubrio Enrich Company"
    description = "Enrich a company with full firmographic data (uses credits)."
    icon = "building"
    name = "PubrioEnrichCompany"

    inputs = [
        SecretStrInput(name="api_key", display_name="Pubrio API Key", required=True),
        MessageTextInput(name="query", display_name="Query", info="JSON with lookup_type and value, or a domain name.", tool_mode=True),
