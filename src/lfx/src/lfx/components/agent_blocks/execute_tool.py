...
class ExecuteToolComponent(Component):
    ...
    async def execute_tools(self) -> DataFrame:
        ...
        # This allows CallModel to continue updating the same message
        ai_message_id = None
        if self.ai_message is not None:
            try:
        ...
        # If ai_message has an ID from CallModel, we reuse it so updates go to the same message
        agent_message = self._get_or_create_agent_message()

        # Always send the message to update the frontend with the new content_blocks structure
        # Even if we have an existing ID, the frontend needs to receive the message with
        # the "Agent Steps" content_blocks to display tool execution updates
        agent_message = await self._send_tool_event(agent_message)

        tool_count = 0
        overall_start_time = perf_counter()
        for tc in raw_tool_calls:
            result, agent_message = await self._execute_single_tool_call(
                tc, tools_by_name, agent_message, overall_start_time
            )
            overall_start_time = perf_counter()  # Reset for next tool

            # Extract tool info and build result row using shared function
        ...
        # Build the AI message row with content_blocks AFTER tool execution
        # This ensures the content_blocks with tool steps are passed through the loop
        # to the next CallModel iteration, which will preserve them in the UI
    ...
    async def _execute_single_tool_call(
        ...
        """Execute a single tool call with real-time UI updates via send_message."""
        # Extract tool call info using shared function
        ...
        # Emit tool start via content_blocks
        duration_ms = int((perf_counter() - start_time) * 1000)
        agent_message, tool_content = await self._emit_tool_start(agent_message, tool_name, tool_args, duration_ms)
        tool_start_time = perf_counter()

        # Execute the tool using shared function
        try:
            result = await execute_tool(matching_tool, tool_args)
            duration_ms = int((perf_counter() - tool_start_time) * 1000)

            # Emit tool end via content_blocks
            agent_message = await self._emit_tool_end(
                agent_message, tool_content, str(result) if result else "", duration_ms
            )
            ...
            # Emit tool error via content_blocks
            agent_message = await self._emit_tool_error(agent_message, tool_content, str(e), duration_ms)
