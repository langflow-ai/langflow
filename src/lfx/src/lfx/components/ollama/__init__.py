# lfx-bundles-shim
"""Compatibility shim: lfx.components.ollama moved to lfx-bundles.
...
try:
    sys.modules[__name__] = importlib.import_module("lfx_bundles.ollama")
except ModuleNotFoundError as exc:
    if exc.name is not None and (exc.name == "lfx_bundles" or exc.name.startswith("lfx_bundles.")):
        msg = (
            "The 'ollama' components moved to the 'lfx-bundles' distribution. Install it with: pip install lfx-bundles."
        )
