...
class ChatOllamaComponent(LCModelComponent):
    ...
    async def update_build_config(self, build_config: dict, field_value: Any, field_name: str | None = None):
        ...
        if field_name in {"model_name", "base_url", "tool_model_enabled", "api_key"}:
            if field_name == "base_url":
                normalized_field_value = field_value.rstrip("/").removesuffix("/v1") if field_value else ""
                # API key is required for cloud Ollama
                if normalized_field_value != DEFAULT_OLLAMA_API_URL:
                    build_config["api_key"]["advanced"] = True
                    build_config["api_key"]["required"] = False
                    build_config["api_key"]["value"] = None
                else:
                    build_config["api_key"]["advanced"] = False
