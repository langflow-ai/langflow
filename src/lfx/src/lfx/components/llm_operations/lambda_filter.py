...
# The Smart Transform lambda is generated by the model and historically passed
# straight to ``eval()``. A model can be steered (e.g. via prompt injection in
# the data being transformed) into emitting an expression with side effects such
# as ``lambda x: __import__("os").system(...)``. To keep the feature working for
# legitimate data-shaping lambdas while removing the arbitrary-code-execution
# primitive, the generated lambda is parsed and validated against an AST
# allowlist and then evaluated with a restricted ``__builtins__`` namespace
# instead of the full interpreter builtins.

# Builtins that are safe to expose to a generated data-transformation lambda.
# Deliberately excludes anything that can import modules, access the
# filesystem/process, introspect objects, or compile/execute further code
# (e.g. ``__import__``, ``eval``, ``exec``, ``compile``, ``open``, ``input``,
# ``globals``, ``locals``, ``vars``, ``getattr``, ``setattr``, ``delattr``).
_SAFE_LAMBDA_BUILTINS: dict[str, Any] = {
    ...
    # Some builtins may be unavailable in restricted environments; skip any that
    # are not present rather than failing at import time.
...
# Attribute names that are never allowed on a generated lambda. Dunder access is
# the standard escape hatch out of a restricted ``eval`` (e.g.
# ``().__class__.__bases__[0].__subclasses__()``), so any dunder attribute is
# rejected outright.


def _attribute_is_allowed(attr: str) -> bool:
    """Return ``False`` for dunder attribute access used to escape the sandbox."""
...
class LambdaFilterComponent(Component):
    ...
    def _assert_lambda_ast_is_safe(self, tree: ast.Expression) -> None:
        """Reject AST nodes that allow escaping the restricted lambda sandbox.

        The generated lambda is meant only to reshape data. Imports, attribute
        access to dunders, and other code-execution primitives are rejected so a
        model-emitted expression cannot reach the filesystem, process, or
        interpreter internals. Raises ``ValueError`` on the first unsafe node.
        """
        if not (isinstance(tree, ast.Expression) and isinstance(tree.body, ast.Lambda)):
            msg = "Generated expression is not a single lambda."
            # ValueError (not TypeError): this is a policy rejection, and callers
            # plus flow validation rely on ValueError semantics for lambda errors.
            raise ValueError(msg)  # noqa: TRY004

        for node in ast.walk(tree):
            # Block import statements/expressions outright.
            if isinstance(node, (ast.Import, ast.ImportFrom)):
                msg = "Imports are not allowed in a generated lambda."
                raise ValueError(msg)  # noqa: TRY004 - policy rejection, ValueError is intentional
            # Block dunder attribute access (the classic sandbox-escape vector).
            if isinstance(node, ast.Attribute) and not _attribute_is_allowed(node.attr):
                msg = f"Access to '{node.attr}' is not allowed in a generated lambda."
                raise ValueError(msg)
            # Block dunder name references (e.g. ``__import__``, ``__builtins__``)
            # and any explicitly forbidden builtin name (e.g. ``open``, ``eval``).
            if isinstance(node, ast.Name) and (
                not _attribute_is_allowed(node.id) or node.id in _FORBIDDEN_LAMBDA_NAMES
            ):
                msg = f"Reference to '{node.id}' is not allowed in a generated lambda."
                raise ValueError(msg)

    def _compile_lambda(self, lambda_text: str) -> Callable[[Any], Any]:
        """Safely compile a generated lambda string into a callable.

        Replaces the previous ``eval`` of model-generated text. The expression is
        parsed, checked against an AST allowlist (no imports / no dunder access),
        and evaluated with a restricted ``__builtins__`` namespace so that only
        safe data-shaping operations are possible.
        """
        try:
            tree = ast.parse(lambda_text, mode="eval")
        except SyntaxError as exc:
            msg = f"Could not parse generated lambda: {exc}"
            raise ValueError(msg) from exc

        self._assert_lambda_ast_is_safe(tree)

        compiled = compile(tree, filename="<lambda_filter>", mode="eval")
        # No module globals are exposed; only the curated safe builtins.
        restricted_globals: dict[str, Any] = {"__builtins__": _SAFE_LAMBDA_BUILTINS}
        return eval(compiled, restricted_globals, {})  # noqa: S307 - AST-validated, restricted builtins
    ...
    def _parse_lambda_from_response(self, response_text: str) -> Callable[[Any], Any]:
        ...
        return self._compile_lambda(lambda_text)
