            ...
            try:
                ...
                if model_name in embedding_by_model:
                    ...
                    logger.warning(
                        ...
                        f"Attempting dynamic model switching (may cause dimension mismatch)."
                    )
                    emb_obj = embeddings_list[0]
                    original_model_attr = getattr(emb_obj, "model", None)
                    original_deployment_attr = getattr(emb_obj, "deployment", None)
                    original_dimensions_attr = getattr(emb_obj, "dimensions", None)
