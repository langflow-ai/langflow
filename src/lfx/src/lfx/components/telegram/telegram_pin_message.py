...
class TelegramPinMessage(Component):
    display_name = "Telegram Закрепить Сообщение"
    description = "Закрепляет сообщение в Telegram чате используя Bot API."
    documentation: str = "https://core.telegram.org/bots/api#pinchatmessage"
    icon = "Pin"
    name = "TelegramPinMessage"
    ...
    inputs = [
        ...
        IntInput(
            ...
            required=True,
            info="Уникальный идентификатор сообщения для закрепления.",
            tool_mode=True,
        ),
        BoolInput(
            name="disable_notification",
            display_name="Отключить уведомления",
            value=False,
            required=False,
            advanced=True,
            info="Если включено, Telegram не будет отправлять уведомление о закреплении сообщения участникам чата.",
        ),
    ]
    ...
    async def build_output(self) -> Data:
        ...
        payload: dict[str, object] = {"chat_id": self.chat_id, "message_id": int(self.message_id)}
