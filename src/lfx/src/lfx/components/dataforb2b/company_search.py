"""DataForB2B — LinkedIn Company / Account Search component for Langflow."""
...
from ._client import COMPANY_COLUMNS, DataForB2BClient, build_filters
from ._filter_inputs import common_search_inputs, filter_slot_inputs, read_slots


class DataForB2BCompanySearchComponent(Component):
    display_name = "Search Companies"
    description = (
        "Search companies and accounts by structured filters — industry, "
        "headcount/size, location, funding, keywords, LinkedIn URL — using "
        "DataForB2B's database. Build target-account lists for B2B sales and "
        "account-based marketing. Accepts LinkedIn URLs as identifiers."
    )
    documentation = "https://docs.dataforb2b.ai"
    icon = "DataForB2B"
    name = "DataForB2BCompanySearch"
    ...
    inputs = [
        ...
        *filter_slot_inputs(COMPANY_COLUMNS),
    ...
    def _run(self) -> dict:
        ...
        data = DataForB2BClient(self.api_key).search_companies(payload)
        self._cached = data
        return data
