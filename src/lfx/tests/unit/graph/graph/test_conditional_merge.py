...
def test_conditional_router_excludes_branch_and_merge_node_proceeds():
    ...
    router = ConditionalRouterComponent(
        _id="router", default_route="false_result"
    )
    router.set(
    ...
    assert "text_output" in result_ids, (
        f"Merge node blocked; results: {result_ids}"
    )
    ...
    assert "HELLO_TRUE" in str(combined), (
        f"Expected true-branch message in output, got: {combined!r}"
    )
...
async def test_component_vertex_get_result_returns_none_for_excluded_vertex():
    ...
    result = await text_input_vertex._get_result(
        requester=requester, target_handle_name="input_value"
    )

    assert result is None, (
        f"Expected None for conditionally excluded vertex, got {result!r}"
    )
