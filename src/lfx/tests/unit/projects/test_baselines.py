...
@pytest.mark.parametrize(
    "text", ["Cite primary sources.", 'Use {current_date}. Return {"sources": []}. Keep {{literal}}.']
)
async def test_baseline_resolves_the_registered_contract_and_executes(text):
    contract = get_slot("Instructions")
    baseline = build_slot_baseline(contract.default_flow_ref, text)
    assert baseline["data"]["nodes"][0]["data"]["node"]["display_name"] == "Prompt Template"
    ...
    assert terminal.custom_component.get_output("prompt").value.text == text
...
def test_whitespace_instructions_are_not_an_eligible_output():
    ...
    baseline["data"]["nodes"][0]["data"]["node"]["template"]["template"]["value"] = "   "
