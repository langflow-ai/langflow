...
# Current generated starter project: tests/unit/upgrade/test_cli_gate.py -> parents[4] == src/
_STARTER_PROJECTS = Path(__file__).parents[4] / "backend" / "base" / "langflow" / "initial_setup" / "starter_projects"
...
def test_check_defaults_to_bundled_registry_for_clean_flow():
    """End-to-end regression: the current Basic Prompting flow must PASS --upgrade-flow=check.
    ...
    """
    raw = json.loads((_STARTER_PROJECTS / "Basic Prompting.json").read_text(encoding="utf-8"))
