...
class TestWhileLoopWithAgentBlocks:
    ...
    def test_whileloop_accepts_dataframe_from_execute_tool(self):
        """Test that WhileLoop can accept DataFrame input (from ExecuteTool)."""
        from lfx.base.agents.tool_execution import build_ai_message_row, build_tool_result_row

        # Simulate what ExecuteTool produces: AI message row + tool result rows
        ai_row = build_ai_message_row(
            text="Let me calculate",
            tool_calls=[{"name": "calc", "id": "123", "args": {"expression": "2+2"}}],
            message_id="msg_123",
        )
        tool_result_row = build_tool_result_row(
            tool_name="calc",
            tool_call_id="123",
            result="4",
        )

        # Create DataFrame like ExecuteTool would
        result_df = DataFrame([ai_row, tool_result_row])

        # The DataFrame should have rows: ai, tool_result
        assert len(result_df) == 2

        # WhileLoop should accept this DataFrame
        comp = WhileLoopComponent()
        converted = comp._to_dataframe(result_df)
        assert isinstance(converted, DataFrame)
...
class TestWhileLoopInitialState:
    ...
    def test_agent_flow_with_message_history(self):
        """Test the full agent flow pattern with MessageHistory.

        Flow: MessageHistory → WhileLoop.initial_state
              ChatInput → WhileLoop.input_value
              WhileLoop → AgentStep.messages
        """
        from lfx.components.agent_blocks import AgentStepComponent

        # Simulate past conversation from MessageHistory
        history = DataFrame(
            [
        ...
        # WhileLoop combines history + new message
        while_loop = WhileLoopComponent()
        while_loop.initial_state = history
        while_loop.input_value = Message(text="Now multiply by 3", sender="User")

        combined_df = while_loop.loop_output()

        # Should have 3 messages: 2 history + 1 new
        assert len(combined_df) == 3
        assert combined_df.iloc[0]["text"] == "What is 2+2?"
        assert combined_df.iloc[1]["text"] == "The answer is 4."
        assert combined_df.iloc[2]["text"] == "Now multiply by 3"

        # AgentStep can convert this to LangChain messages
        ...
        assert lc_messages[2].__class__.__name__ == "HumanMessage"
