...
class TestALTKAgentContextOrdering:
    ...
    def test_conversation_context_chronological_order(self):
        ...
        assert contents == expected_order, (
            f"Conversation context not in chronological order. "
            f"Expected: {expected_order}, Got: {contents}"
        )
        ...
        for msg in context:
            assert isinstance(msg, HumanMessage), f"Expected HumanMessage, got {type(msg)}"
    ...
    def test_conversation_context_no_current_input(self):
        ...
        chat_history = [
            Message(text="old message", sender="User", sender_name="User")
        ]
