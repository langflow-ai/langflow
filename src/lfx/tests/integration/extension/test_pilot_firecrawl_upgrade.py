...
def test_legacy_bare_name_flow_upgrades(migration_table, class_name: str, module_stem: str) -> None:  # noqa: ARG001
    ...
    expected = f"ext:firecrawl:{class_name}@official"
    flow = _saved_flow(_saved_flow_node(f"firecrawl-bare-{class_name}", class_name))
...
def test_legacy_import_path_flow_upgrades(migration_table, class_name: str, module_stem: str) -> None:
    ...
    expected = f"ext:firecrawl:{class_name}@official"
    legacy = f"lfx.components.firecrawl.{module_stem}.{class_name}"
    flow = _saved_flow(_saved_flow_node(f"firecrawl-full-{class_name}", legacy))
    report = migrate_flow_payload(flow, table=migration_table)
    assert report.rewritten_count == 1
    assert flow["data"]["nodes"][0]["data"]["type"] == expected
...
def test_short_import_path_flow_upgrades(migration_table, class_name: str, module_stem: str) -> None:  # noqa: ARG001
    ...
    expected = f"ext:firecrawl:{class_name}@official"
    legacy = f"lfx.components.firecrawl.{class_name}"
    flow = _saved_flow(_saved_flow_node(f"firecrawl-short-{class_name}", legacy))
    report = migrate_flow_payload(flow, table=migration_table)
    assert report.rewritten_count == 1
    assert flow["data"]["nodes"][0]["data"]["type"] == expected
...
def test_lfx_firecrawl_distribution_is_importable() -> None:
    ...
    try:
        import lfx_firecrawl  # type: ignore[import-not-found]
    except ImportError:
        pytest.skip("lfx-firecrawl not installed in this test environment")

    for class_name, _module_stem in BUNDLE_CLASSES:
        klass = getattr(lfx_firecrawl, class_name, None)
        assert klass is not None, f"lfx_firecrawl does not re-export {class_name!r}"
