"""Integration test: legacy icosacomputing flows upgrade cleanly.
...
BUNDLE_CLASSES: tuple[tuple[str, str], ...] = (("CombinatorialReasonerComponent", "combinatorial_reasoner"),)
...
def test_legacy_bare_name_flow_upgrades(migration_table, class_name: str, module_stem: str) -> None:  # noqa: ARG001
    ...
    expected = f"ext:icosacomputing:{class_name}@official"
    flow = _saved_flow(_saved_flow_node(f"icosacomputing-bare-{class_name}", class_name))
...
def test_legacy_import_path_flow_upgrades(migration_table, class_name: str, module_stem: str) -> None:
    ...
    expected = f"ext:icosacomputing:{class_name}@official"
    legacy = f"lfx.components.icosacomputing.{module_stem}.{class_name}"
    flow = _saved_flow(_saved_flow_node(f"icosacomputing-full-{class_name}", legacy))
    report = migrate_flow_payload(flow, table=migration_table)
    assert report.rewritten_count == 1
    assert flow["data"]["nodes"][0]["data"]["type"] == expected
...
def test_short_import_path_flow_upgrades(migration_table, class_name: str, module_stem: str) -> None:  # noqa: ARG001
    ...
    expected = f"ext:icosacomputing:{class_name}@official"
    legacy = f"lfx.components.icosacomputing.{class_name}"
    flow = _saved_flow(_saved_flow_node(f"icosacomputing-short-{class_name}", legacy))
    report = migrate_flow_payload(flow, table=migration_table)
    assert report.rewritten_count == 1
    assert flow["data"]["nodes"][0]["data"]["type"] == expected
...
@pytest.mark.integration
def test_lfx_icosacomputing_distribution_is_importable() -> None:
    """The bundle is importable in the development workspace."""
    try:
        import lfx_icosacomputing  # type: ignore[import-not-found]
    except ImportError:
        pytest.skip("lfx-icosacomputing not installed in this test environment")

    for class_name, _module_stem in BUNDLE_CLASSES:
        klass = getattr(lfx_icosacomputing, class_name, None)
        assert klass is not None, f"lfx_icosacomputing does not re-export {class_name!r}"
...
@pytest.mark.integration
def test_lfx_icosacomputing_ships_manifest() -> None:
    """``importlib.metadata`` can find ``extension.json`` for the installed dist."""
    try:
        dist = importlib_metadata.distribution("lfx-icosacomputing")
    ...
    if _is_editable_install(dist):
        ...
        package_dir = Path(lfx_icosacomputing.__file__).parent
    ...
    assert manifest["id"] == "lfx-icosacomputing"
    assert manifest["lfx"]["compat"] == ["1"]
    assert any(b["name"] == "icosacomputing" for b in manifest["bundles"])
