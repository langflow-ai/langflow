"""Integration test: legacy google_bigquery flows upgrade cleanly.
...
BUNDLE_CLASSES: tuple[tuple[str, str], ...] = (("BigQueryExecutorComponent", "google_bq_sql_executor"),)
...
def test_legacy_bare_name_flow_upgrades(migration_table, class_name: str, module_stem: str) -> None:  # noqa: ARG001
    ...
    expected = f"ext:google_bigquery:{class_name}@official"
    flow = _saved_flow(_saved_flow_node(f"google_bigquery-bare-{class_name}", class_name))
...
def test_legacy_import_path_flow_upgrades(migration_table, class_name: str, module_stem: str) -> None:
    ...
    flow = _saved_flow(_saved_flow_node(f"google_bigquery-full-{class_name}", legacy))
    report = migrate_flow_payload(flow, table=migration_table)
    assert report.rewritten_count == 1
    assert flow["data"]["nodes"][0]["data"]["type"] == expected
...
def test_short_import_path_flow_upgrades(migration_table, class_name: str, module_stem: str) -> None:  # noqa: ARG001
    ...
    flow = _saved_flow(_saved_flow_node(f"google_bigquery-short-{class_name}", legacy))
    report = migrate_flow_payload(flow, table=migration_table)
    assert report.rewritten_count == 1
    assert flow["data"]["nodes"][0]["data"]["type"] == expected
...
@pytest.mark.integration
def test_google_bigquery_distribution_is_importable() -> None:
    """The bundle is importable in the development workspace."""
    try:
        import lfx_google_bigquery  # type: ignore[import-not-found]
    except ImportError:
        pytest.skip("lfx-google-bigquery not installed in this test environment")

    for class_name, _module_stem in BUNDLE_CLASSES:
        klass = getattr(lfx_google_bigquery, class_name, None)
        assert klass is not None, f"lfx_google_bigquery does not re-export {class_name!r}"
...
@pytest.mark.integration
def test_google_bigquery_ships_manifest() -> None:
    """``importlib.metadata`` finds ``extension.json`` for the installed dist."""
    try:
        dist = importlib_metadata.distribution("lfx-google-bigquery")
    ...
    if _is_editable_install(dist):
        ...
        package_dir = Path(lfx_google_bigquery.__file__).parent
    ...
    assert manifest["id"] == "lfx-google-bigquery"
    assert manifest["lfx"]["compat"] == ["1"]
    assert any(b["name"] == "google_bigquery" for b in manifest["bundles"])
