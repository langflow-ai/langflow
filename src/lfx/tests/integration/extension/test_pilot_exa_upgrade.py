"""Integration test: legacy exa flows upgrade cleanly.
...
BUNDLE_CLASSES: tuple[tuple[str, str], ...] = (("ExaSearchToolkit", "exa_search"),)
...
def test_legacy_bare_name_flow_upgrades(migration_table, class_name: str, module_stem: str) -> None:  # noqa: ARG001
    ...
    expected = f"ext:exa:{class_name}@official"
    flow = _saved_flow(_saved_flow_node(f"exa-bare-{class_name}", class_name))
...
def test_legacy_import_path_flow_upgrades(migration_table, class_name: str, module_stem: str) -> None:
    ...
    expected = f"ext:exa:{class_name}@official"
    legacy = f"lfx.components.exa.{module_stem}.{class_name}"
    flow = _saved_flow(_saved_flow_node(f"exa-full-{class_name}", legacy))
    report = migrate_flow_payload(flow, table=migration_table)
    assert report.rewritten_count == 1
    assert flow["data"]["nodes"][0]["data"]["type"] == expected
...
def test_short_import_path_flow_upgrades(migration_table, class_name: str, module_stem: str) -> None:  # noqa: ARG001
    ...
    expected = f"ext:exa:{class_name}@official"
    legacy = f"lfx.components.exa.{class_name}"
    flow = _saved_flow(_saved_flow_node(f"exa-short-{class_name}", legacy))
    report = migrate_flow_payload(flow, table=migration_table)
    assert report.rewritten_count == 1
    assert flow["data"]["nodes"][0]["data"]["type"] == expected
...
def test_lfx_exa_distribution_is_importable() -> None:
    ...
    try:
        import lfx_exa  # type: ignore[import-not-found]
    except ImportError:
        pytest.skip("lfx-exa not installed in this test environment")

    for class_name, _module_stem in BUNDLE_CLASSES:
        klass = getattr(lfx_exa, class_name, None)
        assert klass is not None, f"lfx_exa does not re-export {class_name!r}"
