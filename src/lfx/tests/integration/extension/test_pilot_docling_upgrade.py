"""Integration test: legacy docling flows upgrade cleanly.
...
def test_legacy_bare_name_flow_upgrades(migration_table, class_name: str, module_stem: str) -> None:  # noqa: ARG001
    ...
    expected = f"ext:docling:{class_name}@official"
    flow = _saved_flow(_saved_flow_node(f"docling-bare-{class_name}", class_name))
...
def test_legacy_import_path_flow_upgrades(migration_table, class_name: str, module_stem: str) -> None:
    ...
    expected = f"ext:docling:{class_name}@official"
    legacy = f"lfx.components.docling.{module_stem}.{class_name}"
    flow = _saved_flow(_saved_flow_node(f"docling-full-{class_name}", legacy))
    report = migrate_flow_payload(flow, table=migration_table)
    assert report.rewritten_count == 1
    assert flow["data"]["nodes"][0]["data"]["type"] == expected
...
def test_short_import_path_flow_upgrades(migration_table, class_name: str, module_stem: str) -> None:  # noqa: ARG001
    ...
    expected = f"ext:docling:{class_name}@official"
    legacy = f"lfx.components.docling.{class_name}"
    flow = _saved_flow(_saved_flow_node(f"docling-short-{class_name}", legacy))
    report = migrate_flow_payload(flow, table=migration_table)
    assert report.rewritten_count == 1
    assert flow["data"]["nodes"][0]["data"]["type"] == expected
...
def test_lfx_docling_distribution_is_importable() -> None:
    ...
    try:
        import lfx_docling  # type: ignore[import-not-found]
    except ImportError:
        pytest.skip("lfx-docling not installed in this test environment")

    for class_name, _module_stem in BUNDLE_CLASSES:
        klass = getattr(lfx_docling, class_name, None)
        assert klass is not None, f"lfx_docling does not re-export {class_name!r}"
...
@pytest.mark.integration
def test_lfx_docling_ships_manifest() -> None:
    """``importlib.metadata`` can find ``extension.json`` for the installed dist."""
    try:
        dist = importlib_metadata.distribution("lfx-docling")
    ...
    if _is_editable_install(dist):
        ...
        package_dir = Path(lfx_docling.__file__).parent
    ...
    assert manifest["id"] == "lfx-docling"
    assert manifest["lfx"]["compat"] == ["1"]
    assert any(b["name"] == "docling" for b in manifest["bundles"])
