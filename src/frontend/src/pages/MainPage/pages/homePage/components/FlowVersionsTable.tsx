  ...
  const rows = useMemo(() => {
    return flows.map((flow) => {
      ...
      const deployedEntryCount = hasLoadedHistory
        ? entries.filter((entry) => (deploymentCounts[entry.id] ?? 0) > 0).length
    ...
    <div className="px-5 pb-5 pt-7">
      ...
      <div className="mt-1 border-y border-border/70">
        {rows.map(
          ...
          return (
            <div key={flow.id} className="border-b border-border/60 last:border-b-0">
              <div
              ...
              >
                ...
                <span
                  className={cn(
                    ...
                    flowStatus === "Deployed" ? "text-foreground" : "text-muted-foreground",
                  )}
                >
                  <span
                    className={cn(
                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                        ...
                        navigate(`${targetPath}?historyId=${encodeURIComponent(entry.id)}`);
                      }}
                    >
                      <span className="flex justify-center">
