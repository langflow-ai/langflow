  ...
  const renderedDescription = useMemo(() => {
    ...
    return (
      <MemoizedMarkdown
        linkTarget="_blank"
        className={cn(
          "markdown text-mmd prose flex w-full flex-col leading-5 word-break-break-word [&_pre]:whitespace-break-spaces [&_pre]:!bg-code-description-background [&_pre_code]:!bg-code-description-background",
          mdClassName,
        )}
      >
        {String(description)}
      ...
      {editNameDescription ? (
        <>
          <Textarea
            maxLength={charLimit}
            className={cn(
              "nowheel text-mmd h-full w-full p-0 focus:border-primary focus:ring-0",
              inputClassName,
            )}
            autoFocus
            style={style}
        ...
        <div
          ...
          ref={overflowRef}
          className={cn(
            "nodoubleclick generic-node-desc-text text-mmd h-full cursor-grab text-muted-foreground word-break-break-word",
