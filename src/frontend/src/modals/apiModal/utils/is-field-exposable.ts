 ...
 *
 * Preconditions:
 * - tweakable at all (`isFieldTweakable`) — the type carries an editable
 *   literal value and the backend does not refuse the field;
 ...
 * - not the code field nor an internal (`_`-prefixed) field (both in
 *   `isFieldTweakable`).
 */
export const isFieldExposable = (
  ...
  if (!isFieldTweakable(node.data?.type, name, template)) return false;
