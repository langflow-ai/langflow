...
export default function SandboxInfoModal({
  ...
  const [sandboxInfo, setSandboxInfo] = useState<SandboxInfoResponse | null>(
    null,
  );
  ...
  const fetchSandboxInfo = async () => {
    ...
    try {
      ...
      const sortedComponents = (response.data.components || []).sort(
        (a: SandboxSupportedComponent, b: SandboxSupportedComponent) =>
  ...
  const ComponentCard = ({
    ...
    component: SandboxSupportedComponent;
  }) => (
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center space-x-2 mb-1">
            ...
            {component.force_sandbox && (
              <Badge
                variant="primary"
                className="text-xs px-2 py-0.5 border-orange-200 bg-orange-100 text-orange-800 dark:bg-purple-800 dark:border-purple-900 dark:text-purple-200"
              >
            ...
            <div className="space-y-3 text-sm text-muted-foreground">
              <ul className="space-y-2 ml-4">
                <li className="flex items-start space-x-2">
                  ...
                  />
                  <span>
                    <strong>Unmodified components</strong> run with full system
                    access when their code matches known signatures
                  </span>
                </li>
                <li className="flex items-start space-x-2">
                  <IconComponent
                  ...
                  />
                  <span>
                    <strong>Sandbox-compatible components</strong> can have
                    their code modified, but will be considered untrusted, and
                    will run with security restrictions
                  </span>
                </li>
                <li className="flex items-start space-x-2">
                  <IconComponent
                    name="Lock"
                    className="w-4 h-4 text-orange-500 dark:text-purple-400 mt-0.5 flex-shrink-0"
                  />
                  <span>
                    <strong>Forced-sandboxing</strong> is applied to sensitive
                    components (e.g. Custom Components)
                  </span>
                </li>
                <li className="flex items-start space-x-2">
                  <IconComponent
                  ...
                  />
                  <span>
                    <strong>Incompatible components</strong> cannot be modified
                    from the frontend and will be blocked by server if otherwise
                    manipulated
                  </span>
                </li>
              </ul>
