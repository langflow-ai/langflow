...
export default function PromptEditorModal({
  ...
  readonly = false,
  strategy,
}: PromptEditorModalProps): JSX.Element {
  ...
  function checkVariables(valueToCheck: string): void {
    setWordsHighlight(strategy.extractVariables(valueToCheck));
  }
  ...
  function validatePrompt(closeModal: boolean): void {
    ...
    postValidatePrompt(
      strategy.buildValidatePayload(field_name, inputValue, nodeClass!),
      {
        onSuccess: (apiReturn) => {
          if (field_name === "") {
  ...
  return (
    ...
    >
      ...
      <BaseModal.Header
        description={
          strategy.headerDescriptionKey
            ? t(strategy.headerDescriptionKey)
            : undefined
        }
      >
        ...
        <div className={classNames("flex h-full w-full rounded-lg border")}>
          {isEdit && !readonly ? (
            ...
            <SanitizedHTMLWrapper
              ref={previewRef}
              className={getClassByNumberLength() + strategy.previewClassName}
