...
export function createEnhancedSourceHandle(
  ...
  if (!outputField) {
    console.error(
      `Output field ${sourceFieldName} not found in node ${nodeType}`,
    );
    return null;
  }

  // Replace NODE_ID with actual ID in the handle format
  const handleFormat = outputField.connectionFormat.handleFormat.replace(
    "NODE_ID",
    sourceNodeId,
  );
  return handleFormat;
...
export function createEnhancedTargetHandle(
  ...
  if (!inputField) {
    console.error(
      `Input field ${targetFieldName} not found in node ${nodeType}`,
    );
    return null;
  }

  // Replace NODE_ID with actual ID in the handle format
  const handleFormat = inputField.connectionFormat.handleFormat.replace(
    "NODE_ID",
    targetNodeId,
  );
  return handleFormat;
...
export async function connectNodesWithRegistry(
  ...
  console.log(
    `Connecting nodes with registry: ${sourceNodeId} → ${targetNodeId}`,
  );

  // Get compatible connections
  const connections = await findCompatibleNodeConnections(
    sourceNodeId,
    targetNodeId,
  );
