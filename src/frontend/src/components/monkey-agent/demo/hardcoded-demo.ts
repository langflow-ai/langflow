...
export const processCommand = (
  ...
  // Check for connection commands
    ...
    if (promptNodes.length === 0 || chatNodes.length === 0) {
      return {
        success: false,
        message: "I need both a PromptTemplate and ChatOpenAI node on the canvas to connect them. Please add them first.",
      ...
      if (connectNodes(sourceNode.id, targetNode.id, onConnect)) {
        ...
        return {
          success: false,
          message: "I tried to connect the nodes but encountered a technical issue. Please try again.",
          action: "connection_failed",
  ...
  // Check for flow creation commands
    ...
    if (flow) {
      return {
        success: true,
        message: "I've created a simple QA flow with a PromptTemplate connected to a ChatOpenAI node.",
        action: "created_flow",
        details: { flowType: "qa", nodes: [flow.promptId, flow.chatId], connected: flow.connected }
      };
    } else {
      return {
        success: false,
