...
export const processExistingNodeCommand = (
  ...
  if (message.toLowerCase().includes("add text input")) {
    ...
    if (nodeId) {
      return {
        success: true,
        message:
          "I've added a Text Input node to your canvas using Langflow's native system. It's identical to what you'd get by manually placing one.",
        action: "added_node",
      ...
      return {
        success: false,
        message:
          "I couldn't add the Text Input node. The node template might not be available.",
        action: "error",
  ...
  // Command to add a Chat Input node
    ...
    if (nodeId) {
      return {
        success: true,
        message:
          "I've added a Chat Input node to your canvas. This node allows interactive chat input in your flow.",
        action: "added_node",
      ...
      return {
        success: false,
        message:
          "I couldn't add the Chat Input node. The node template might not be available.",
        action: "error",
        details: { reason: "template_not_found" },
      };
    }
  ...
  // Command to add OpenAI Embeddings node
    ...
    message.toLowerCase().includes("add openai embeddings") ||
    message.toLowerCase().includes("add embeddings")
  ) {
    const nodeId = addExistingNode(
      "OpenAIEmbeddings",
      { x: 200, y: 200 },
      setNodes,
    );
    ...
    if (nodeId) {
      return {
        success: true,
        message:
          "I've added an OpenAI Embeddings node to your canvas. This node generates vector embeddings for text using OpenAI's models.",
        action: "added_node",
      ...
      return {
        success: false,
        message:
          "I couldn't add the OpenAI Embeddings node. The node template might not be available.",
        action: "error",
        details: { reason: "template_not_found" },
      };
    }
  ...
  // Command to add OpenAI node (the basic text generation one, not embeddings)
    ...
    !message.toLowerCase().includes("embeddings")
  ) {
    ...
    if (openAITextNodes.length > 0) {
      ...
      console.log(
        "Using specific OpenAI text generation node:",
        openAINodeType,
      );
    }
    // If not found, check for LLMs category
      ...
      // Next try other variants that contain "OpenAI" in the name but EXCLUDE embeddings
        ...
        const openAIVariants = llmNodes.filter(
          (node) =>
    ...
    if (openAINodeType && templates[openAINodeType]) {
      ...
      if (
        ...
        console.log(
          "Rejecting node as it appears to be an embeddings node:",
          template.description,
        );
    ...
    return {
      success: false,
      message:
        "I couldn't find the OpenAI text generation node. Available LLM nodes: " +
        llmNodes.join(", "),
      action: "error",
  ...
  // Command to connect existing nodes
    ...
    message.toLowerCase().includes("connect existing nodes") ||
    ...
    if (!sourceNodeId || !targetNodeId) {
      console.log(
        "No specific node IDs provided, searching for TextInput and OpenAI nodes",
      );

      // Find a TextInput node
      const textInputNodes = nodes.filter(
        (node) =>
      ...
      // Find an OpenAI node
      const openAINodes = nodes.filter(
        (node) =>
    ...
    if (!sourceNodeId || !targetNodeId) {
      return {
        success: false,
        message:
          "I couldn't find suitable nodes to connect. Please specify the node IDs or ensure you have a Text Input and OpenAI node in your flow.",
        action: "error",
    ...
    console.log("Target node data:", {
      ...
      templateFields: targetNode.data.node?.template
    ...
    if (
      targetNode.data.node?.template &&
      !targetNode.data.node.template[targetInputField]
    ) {
      console.warn(
        `Target node does not have '${targetInputField}' field. Available fields:`,
        Object.keys(targetNode.data.node.template),
      );

      // Try to find a suitable alternative input field
      const inputFieldPriority = [
        "input",
        "text",
        "message",
        "content",
      ...
      for (const field of inputFieldPriority) {
        if (targetNode.data.node.template[field]) {
          console.log(
            `Using alternative field '${field}' instead of '${targetInputField}'`,
          );
    ...
    if (connected) {
      ...
      return {
        success: false,
        message:
          "I wasn't able to connect the nodes. Please try connecting them manually.",
        action: "error",
        details: {
  ...
  // Command to add Text Input + OpenAI and connect them
    ...
    message.toLowerCase().includes("add text and openai") ||
    message.toLowerCase().includes("connect text to openai")
  ) {
    // First add Text Input
    const textNodeId = addExistingNode(
      "TextInput",
      { x: 200, y: 200 },
      setNodes,
    );
    ...
    // If we found a valid node type, add it to the canvas
    if (openAINodeType && templates[openAINodeType]) {
      const openAINodeId = addExistingNode(
        openAINodeType,
        { x: 500, y: 200 },
        setNodes,
      );
      ...
      if (openAINodeId) {
        ...
        setTimeout(() => {
          ...
          if (sourceNode && targetNode) {
            ...
            console.log(
              "Target node template fields:",
              Object.keys(targetTemplate),
            );
            ...
            if (!inputFieldName) {
              ...
              inputFieldName =
                templateKeys.length > 0 ? templateKeys[0] : "input_value";
            }
        ...
        return {
          success: false,
          message:
            "I couldn't find the OpenAI text generation node to connect with.",
          action: "error",
  ...
  // Command to add two nodes and connect them using the enhanced registry knowledge
    ...
    message.toLowerCase().includes("add document to embeddings flow") ||
    ...
    const documentLoaderId = addExistingNode(
      "DocumentLoader",
      documentLoaderPosition,
      setNodes,
    );
    ...
    if (!documentLoaderId) {
      return {
        success: false,
        message:
          "I couldn't add the Document Loader node. The node template might not be available.",
        action: "error",
        details: { reason: "template_not_found" },
      };
    }
    ...
    const embeddingsId = addExistingNode(
      "OpenAIEmbeddings",
      embeddingsPosition,
      setNodes,
    );
    ...
    return {
      success: true,
      message:
        "I've added a Document Loader node and an OpenAI Embeddings node to your canvas, and connected them.",
      action: "added_connected_nodes",
  ...
  // Enhanced Registry Commands - Using knowledge from the backend registry
    ...
    console.log(
      "Starting document embeddings flow with enhanced registry knowledge",
    );
    ...
    const documentNodeId = addExistingNode(
      documentLoaderType,
      documentPosition,
      setNodes,
    );
    ...
    // Add embeddings node
    const embeddingsPosition = { x: 500, y: 200 };
    const embeddingsNodeId = addExistingNode(
      embeddingsType,
      embeddingsPosition,
      setNodes,
    );
  ...
  return {
    ...
    message:
      ...
      message +
      '". Currently, I can add these nodes: Text Input, Chat Input, OpenAI, or OpenAI Embeddings. Try saying "add text input", "add chat input", "add openai", "add openai embeddings", or "connect text to openai".',
...
function findNodeByTypeDescription(typeDescription: string) {
  ...
  if (typeNameMappings[lowerKey]) {
    for (const nodeName of typeNameMappings[lowerKey]) {
      if (templates[nodeName]) {
        console.log(
          `Found mapped template match for ${typeDescription}: ${nodeName}`,
        );
  ...
  if (matchingTemplates.length > 0) {
    console.log(
      `Found ${matchingTemplates.length} display name matches for ${typeDescription}`,
    );
  ...
  if (descriptionMatches.length > 0) {
    console.log(
      `Found ${descriptionMatches.length} description matches for ${typeDescription}`,
    );
...
function connectExistingNodes(
  ...
  try {
    ...
    if (sourceNodeOutputs && sourceNodeOutputs.length > 0) {
      const output = sourceNodeOutputs.find(
        (o) => o.name === sourceOutputField,
      );
    ...
    const targetInputTypes =
      targetNode.data.node?.template?.[targetInputField]?.input_types;
    if (targetInputTypes && targetInputTypes.length > 0) {
      inputType = targetInputTypes[0];
    }

    console.log(
      `Source output type: ${outputType}, Target input type: ${inputType}`,
    );
