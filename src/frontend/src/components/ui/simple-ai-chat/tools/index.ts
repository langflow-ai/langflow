...
export class FlowCreationTool implements AIAssistantTool {
  ...
  description: string = "Create a QA flow with a PromptTemplate connected to a ChatOpenAI node.";
  ...
  execute(flowStore: FlowStore): string {
    try {
      ...
      // Add nodes to canvas
      flowStore.setNodes((oldNodes) => [...oldNodes, promptNode as any, chatNode as any]);
