...
jest.mock("@/stores/alertStore", () => ({
  __esModule: true,
  default: {
    getState: () => ({ setErrorData: setErrorDataMock }),
  },
}));
...
describe("useExtensionEvents", () => {
  ...
  it("should become active and switch to fast polling when events arrive", async () => {
    ...
    apiGetMock.mockResolvedValueOnce({
      data: {
        events: [
          { type: "bundle_reloaded", timestamp: Date.now() / 1000, payload: { bundle: "a" } },
        ],
        settled: false,
      },
    });
  ...
  it("should call setErrorData on bundle_reload_failed", async () => {
    ...
    expect(setErrorDataMock).toHaveBeenCalledWith(
      expect.objectContaining({ list: expect.arrayContaining(["Reload blew up"]) }),
    );
  });
  ...
  it("should fall back to event type in error message when payload has no message", async () => {
    ...
    apiGetMock.mockResolvedValueOnce({
      data: {
        events: [
          { type: "extension_error", timestamp: Date.now() / 1000, payload: {} },
        ],
        settled: false,
      },
    });
    ...
    expect(setErrorDataMock).toHaveBeenCalledWith(
      expect.objectContaining({
        list: expect.arrayContaining(["extension_error: check server logs for details"]),
      }),
    );
  });
  ...
  it("clearEvents resets the events list", async () => {
    ...
    apiGetMock.mockResolvedValueOnce({
      data: {
        events: [{ type: "bundle_reloaded", timestamp: Date.now() / 1000, payload: {} }],
        settled: false,
      },
    });
