...
export const usePostComponent: useMutationFunctionType<
  IPostComponentProps
> = (options) => {
  ...
  const postComponent = async (
    payload: IPostComponentProps,
  ): Promise<any> => {
    const { publicFlow, tags, newFlow: { name, data, description, is_component, parent, last_tested_version } } = payload;
    const newComponent = {
        name: name,
        data: data,
