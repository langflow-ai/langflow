...
export const SIDEBAR_BUNDLES = [
  ...
  { display_name: "Memories", name: "memories", icon: "Cpu" },
  { display_name: "Merge", name: "merge", icon: "Merge" },
...
export const getNodeIcon = async (name: string): Promise<CachedIcon> => {
  const cacheAndReturn = (icon: CachedIcon): CachedIcon => {
  ...
  if (eagerLoadedIconsMap[iconName || name]) {
    return cacheAndReturn(eagerLoadedIconsMap[iconName || name] as CachedIcon);
  }

  if (isFontAwesomeIcon(iconName || name)) {
    return cacheAndReturn(fontAwesomeIcons[iconName || name] as CachedIcon);
  }
