...
export const addOpenAiInputKey = async (page: Page) => {
  const numberOfOpenAiFields = await page.getByTestId("popover-anchor-input-openai_api_key").count();

  for (let i = 0; i < numberOfOpenAiFields; i++) {
    const openAiInput = page.getByTestId("popover-anchor-input-openai_api_key").nth(i);
