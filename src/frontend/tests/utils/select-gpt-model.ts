...
export const selectGptModel = async (page: Page) => {
  ...
  for (let i = 0; i < gptModelDropdownCount; i++) {
    ...
    const gptOMiniOption = await page
      .getByTestId("gpt-4o-mini-option")
      .count();
    ...
    await page.getByPlaceholder("Add API key").fill(process.env.OPENAI_API_KEY!);
    await page.waitForSelector("text=OpenAI Api Key Saved", { timeout: 30000 });
    await page.getByTestId("llm-toggle-gpt-4o-mini").click();
    await page.getByText("Close").last().click();
    }
    else{
        const isChecked = await page.getByTestId("llm-toggle-gpt-4o-mini").isChecked();
    if (!isChecked) {
      await page.getByTestId("llm-toggle-gpt-4o-mini").click();
    }
    await page.getByText("Close").last().click();
