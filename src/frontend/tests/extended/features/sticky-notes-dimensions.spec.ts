...
test(
  ...
  async ({ page }) => {
    ...
    const longText = "This is a very long text that should not change the note dimensions because we have fixed sizing with overflow handling. ".repeat(10);
...
test(
  ...
  async ({ page }) => {
    ...
    await page.locator(".generic-node-desc-text").last().dblclick();
    const veryLongText = "Line of text that will create vertical overflow. ".repeat(20);
    ...
    expect(["auto", "scroll", "hidden"].includes(textareaOverflow.overflowY)).toBe(true);
    expect(["auto", "scroll", "hidden"].includes(contentOverflow.overflowY)).toBe(true);
    
    // Content should be constrained (either by max-height or overflow)
    const hasOverflowControl = contentOverflow.scrollHeight > contentOverflow.clientHeight || 
...
test(
  ...
  async ({ page }) => {
    ...
    if (enlargedSize.width > initialSize.width || enlargedSize.height > initialSize.height) {
