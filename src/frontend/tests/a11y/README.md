...
widget (Radix, AG Grid, cmdk) and can't be fixed from the feature's markup. In
that case:

- Add its rule ID to the spec's `ignoreRules` array **with a comment** that
  explains — grounded in the report DOM — exactly why it's suppressed. The
  `KB_IGNORE_RULES` block in
  [`knowledge-bases.a11y.spec.ts`](knowledge-bases.a11y.spec.ts) sets the bar for
  that justification.
- Keep real, tracked gaps that must be fixed elsewhere (for example, theme-level
  contrast) visible in the comment — never silently ignore them.
