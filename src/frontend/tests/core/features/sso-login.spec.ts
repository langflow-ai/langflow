...
const validateEnvVars = () => {
  ...
  if(missingEnvVars.length > 0) {
    throw new Error(`The following environment variables are required for this test: ${missingEnvVars.join(', ')}`);
  }
};
...
const preformLogin = async (page, user: SSOUser) => {
  console.log(`Attempting SSO login with ${user.type} as user:${user.username}`);
  ...
  try  {
    ...
    await page.screenshot({ path: `sso-login-failure-${user.type}-${new Date().toISOString()}.png` });
...
const preformLogout = async (page) => {
  try {
    ...
    await page.screenshot({ path: `sso-logout-failure-${new Date().toISOString()}.png` });
...
test.describe("SSO Authentication", () => {
  ...
  test('Validate Admin SSO User role', async ({ page }) => {
    ...
    await page.waitForSelector('button:has-text("New User")', { timeout: 30000 });
    ...
    const adminUserRow = userTableRows.filter({has: page.getByRole('cell', { name: `${user.username}` })});
