...
test("shareable playground: auto-login user can send message and get response", {
...
test("shareable playground: streaming works", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page, context }) => {
  skipIfMissing.openAiKey();
  loadDotenvIfLocal(__dirname);
  ...
  await expect(playgroundPage.getByTestId(TID.buttonSend).last()).toBeVisible();

  await playgroundPage.close();
});

test("shareable playground: session management works", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page, context }) => {
  skipIfMissing.openAiKey();
  loadDotenvIfLocal(__dirname);
