...
function deterministicText(body) {
  ...
  if (prompt.includes("sentiment")) {
    return conversation.includes("LOOPBACK_YOUTUBE_COMMENTS_USED")
  ...
  if (conversation.includes("LOOPBACK_YOUTUBE_TRANSCRIPT_USED")) {
    return (
...
function requestedToolCall(body) {
  ...
  const calculatorRequest = /^\s*2\s*\+\s*2\s*\??\s*$/.test(prompt)
    ? body.tools.find((tool) => /calculat/i.test(tool?.function?.name ?? ""))
    : null;
  const searchRequest = (body.messages ?? [])
    .map((message) => flattenContent(message.content))
    .join("\n")
    .match(/(?:search first|web search|using google search|travel plan)/i)
    ? body.tools.find((tool) =>
        /(?:search|perform_search)/i.test(tool?.function?.name ?? ""),
      )
    : null;
  const transcriptRequest = (body.messages ?? [])
    .map((message) => flattenContent(message.content))
    .join("\n")
    .match(/comment analysis[\s\S]*video url|video url[\s\S]*comment analysis/i)
    ? body.tools.find((tool) =>
        /(?:youtube|transcript)/i.test(tool?.function?.name ?? ""),
      )
    : null;
  const requested = match
    ? body.tools.find((tool) => tool?.function?.name === match[1])
    : (calculatorRequest ?? searchRequest ?? transcriptRequest);
    ...
    function: {
      ...
      arguments:
        ...
        (calculatorRequest
          ? '{"expression":"2+2"}'
          : transcriptRequest
            ? '{"url":"https://www.youtube.com/watch?v=langflow-loopback"}'
