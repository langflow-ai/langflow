"""Unit tests for the Oracle Vector Store component (lfx-oracle bundle).

The component used to live at ``lfx.components.oracledb.oraclevs`` and was
tested under ``src/backend/tests/unit/components/vectorstores/`` using the
in-tree ``ComponentTestBaseWithoutClient`` helper.  It has since been
extracted into a standalone bundle; these tests now travel with the bundle
and import the public bundle entry point.  The base-class scaffolding
(which parametrised version-compat tests against a saved-fixture
directory) is dropped: ``OracleVectorStoreComponent`` is new in 1.11.0 and
has no historical schema fixtures to validate against
(``file_names_mapping`` was empty).
"""
...
from lfx_oracle.components.oracle import OracleVectorStoreComponent
from lfx_oracle.components.oracle.connection import build_connection_params, split_credentialized_dsn
...
def test_connection_params_rejects_sensitive_keys() -> None:
    with pytest.raises(
        ...
        build_connection_params(
            {
                "connection_string": "user/password@host/service",  # pragma: allowlist secret
...
def test_connection_params_rejects_normalized_sensitive_keys() -> None:
    with pytest.raises(ValueError, match="connection_params contains sensitive keys"):
        build_connection_params(
            {"password ": "secret", "wallet-password": "wallet-secret"},  # pragma: allowlist secret
            dsn="host/service",
        )
...
class TestOracleVectorStoreComponent:
    @pytest.fixture
    ...
    def default_kwargs(self, request) -> dict[str, Any]:
        ...
        if connection_params:
            from lfx_oracle.components.oracle import OracleEmbeddingsComponent
    ...
    def test_create_db(self, component_class: type[OracleVectorStoreComponent], default_kwargs: dict[str, Any]) -> None:
        ...
        with (
            patch("lfx_oracle.components.oracle.oraclevs.oracledb.connect", return_value=MagicMock()) as mock_connect,
            patch("lfx_oracle.components.oracle.oraclevs.OracleVS", return_value=vector_store) as mock_oracle_vs,
            patch("lfx_oracle.components.oracle.oraclevs.create_index") as mock_create_index,
            patch("lfx_oracle.components.oracle.oraclevs.version", return_value="1.1.0", create=True),
        ):
    ...
    def test_build_vector_store_generates_index_defaults_and_serializes_metadata(
        ...
        with (
            patch("lfx_oracle.components.oracle.oraclevs.oracledb.connect", return_value=MagicMock()) as mock_connect,
            patch(
                "lfx_oracle.components.oracle.oraclevs.OracleVS.from_documents",
    ...
    def test_build_vector_store_defaults_none_index_params(
        ...
        with (
            patch("lfx_oracle.components.oracle.oraclevs.oracledb.connect", return_value=MagicMock()),
            patch("lfx_oracle.components.oracle.oraclevs.OracleVS", return_value=vector_store),
            patch("lfx_oracle.components.oracle.oraclevs.create_index") as mock_create_index,
            patch("lfx_oracle.components.oracle.oraclevs.version", return_value="1.1.0", create=True),
        ):
            assert component.build_vector_store() is vector_store
    ...
    def test_build_vector_store_omits_mutate_on_duplicate_for_langchain_1_0(
        ...
        with (
            patch("lfx_oracle.components.oracle.oraclevs.oracledb.connect", return_value=MagicMock()) as mock_connect,
            patch("lfx_oracle.components.oracle.oraclevs.OracleVS", return_value=vector_store) as mock_oracle_vs,
            patch("lfx_oracle.components.oracle.oraclevs.version", return_value="1.0.2", create=True),
        ):
            assert component.build_vector_store() is vector_store
    ...
    def test_build_vector_store_closes_opened_connection_when_construction_fails(
        ...
        with (
            patch("lfx_oracle.components.oracle.oraclevs.oracledb.connect", return_value=connection),
            patch("lfx_oracle.components.oracle.oraclevs.OracleVS", side_effect=RuntimeError("construct boom")),
            patch("lfx_oracle.components.oracle.oraclevs.version", return_value="1.1.0", create=True),
    ...
    def test_build_vector_store_closes_opened_connection_when_index_creation_fails(
        ...
        with (
            patch("lfx_oracle.components.oracle.oraclevs.oracledb.connect", return_value=connection),
            patch("lfx_oracle.components.oracle.oraclevs.OracleVS", return_value=vector_store),
            patch("lfx_oracle.components.oracle.oraclevs.create_index", side_effect=RuntimeError("index boom")),
            patch("lfx_oracle.components.oracle.oraclevs.version", return_value="1.1.0", create=True),
