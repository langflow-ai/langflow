...
def test_oracle_doc_loader_component_integration(
    ...
    if not connection_params:
        with (
            patch(
                "lfx_oracle.components.oracle.oracledb_loaders.oracledb.connect",
                return_value=MagicMock(),
            ) as mock_connect,
            patch("lfx_oracle.components.oracle.oracledb_loaders.OracleDocLoader") as mock_loader,
        ):
...
def test_oracle_autonomous_database_loader_component_load_documents(
    ...
    if not connection_params:
        with patch("lfx_oracle.components.oracle.oracledb_loaders.OracleAutonomousDatabaseLoader") as mock_loader:
