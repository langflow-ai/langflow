"""Lazy component re-exports for the ``huggingface`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.huggingface`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.huggingface.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_huggingface.components.huggingface.<Class>``.
"""
