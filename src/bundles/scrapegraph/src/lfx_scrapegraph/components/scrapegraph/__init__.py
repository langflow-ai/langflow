"""Lazy component re-exports for the ``scrapegraph`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.scrapegraph`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.scrapegraph.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_scrapegraph.components.scrapegraph.<Class>``.
"""
