"""Unit tests for the Firecrawl extension bundle (``lfx-firecrawl``).

The components used to live at ``lfx.components.firecrawl``; they have since
been extracted into a standalone bundle, so these tests travel with the
bundle and import the public bundle entry point.

They verify v2 SDK usage, the v1->v2 parameter mapping, response
...
import pytest
from lfx_firecrawl import FirecrawlCrawlApi, FirecrawlMapApi, FirecrawlScrapeApi, FirecrawlSearchApi
...
class TestFirecrawlScrapeApi:
    def test_scrape_calls_v2_and_serializes(self, mock_firecrawl):
        ...
        mock_firecrawl.assert_called_once_with(api_key="test-key")  # pragma: allowlist secret
