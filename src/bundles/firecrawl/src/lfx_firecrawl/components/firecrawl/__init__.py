"""Lazy component re-exports for the ``firecrawl`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.firecrawl`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.firecrawl.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_firecrawl.components.firecrawl.<Class>``.
"""
