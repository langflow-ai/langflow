"""Lazy component re-exports for the ``maritalk`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.maritalk`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.maritalk.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_maritalk.components.maritalk.<Class>``.
"""
