"""Lazy component re-exports for the ``mongodb`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.mongodb`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.mongodb.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_mongodb.components.mongodb.<Class>``.
"""
