"""Lazy component re-exports for the ``faiss`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.faiss`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.faiss.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_faiss.components.faiss.<Class>``.
"""
