"""Unit tests for the Exa Search extension bundle (``lfx-exa``).

Adapted from the pre-bundle-split suite contributed in
langflow-ai/langflow#13001; the component now ships as a standalone
bundle, so these tests travel with the bundle and import the public
bundle entry point.  ``exa_py`` is a hard runtime dep of the bundle, so
the client class is patched at the component module rather than mocked
at import time.  No network access is required.
"""
...
CLIENT_PATCH_TARGET = "lfx_exa.components.exa.exa_search.Exa"


@pytest.fixture
def component() -> ExaSearchToolkit:
    """Build an ExaSearchToolkit with sensible defaults set."""
    c = ExaSearchToolkit()
    c.exa_api_key = "test-key"  # pragma: allowlist secret
    c.metaphor_api_key = ""
    c.search_type = "auto"
...
def test_resolved_api_key_prefers_exa_key(component):
    ...
    component.metaphor_api_key = "legacy"  # pragma: allowlist secret
...
def test_build_client_sets_integration_header(component):
    ...
    with patch(CLIENT_PATCH_TARGET) as mock_exa:
        mock_exa.return_value.headers = {}
        client = component._build_client()
        mock_exa.assert_called_once_with(api_key="test-key")  # pragma: allowlist secret
