"""Lazy component re-exports for the ``deepseek`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.deepseek`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.deepseek.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_deepseek.components.deepseek.<Class>``.
"""
