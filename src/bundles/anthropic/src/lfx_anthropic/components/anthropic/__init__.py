"""Lazy component re-exports for the ``anthropic`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.anthropic`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.anthropic.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_anthropic.components.anthropic.<Class>``.
"""
