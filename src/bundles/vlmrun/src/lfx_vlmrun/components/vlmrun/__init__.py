"""Lazy component re-exports for the ``vlmrun`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.vlmrun`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.vlmrun.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_vlmrun.components.vlmrun.<Class>``.
"""
