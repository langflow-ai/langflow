"""Lazy component re-exports for the ``chroma`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.chroma`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.chroma.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_chroma.components.chroma.<Class>``.
"""
