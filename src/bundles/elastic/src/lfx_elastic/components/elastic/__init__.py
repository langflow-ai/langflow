"""Lazy component re-exports for the ``elastic`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.elastic`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.elastic.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_elastic.components.elastic.<Class>``.
"""
...
if TYPE_CHECKING:
    ...
    from .opensearch_multimodal import OpenSearchVectorStoreComponentMultimodalMultiEmbedding

_dynamic_imports = {
    "ElasticsearchVectorStoreComponent": "elasticsearch",
    "OpenSearchVectorStoreComponent": "opensearch",
    "OpenSearchVectorStoreComponentMultimodalMultiEmbedding": "opensearch_multimodal",
}
