"""Lazy component re-exports for the ``pinecone`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.pinecone`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.pinecone.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_pinecone.components.pinecone.<Class>``.
"""
