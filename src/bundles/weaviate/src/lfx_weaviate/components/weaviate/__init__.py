"""Lazy component re-exports for the ``weaviate`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.weaviate`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.weaviate.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_weaviate.components.weaviate.<Class>``.
"""
