"""Lazy component re-exports for the ``supabase`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.supabase`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.supabase.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_supabase.components.supabase.<Class>``.
"""
