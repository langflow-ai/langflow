"""Lazy component re-exports for the ``upstash`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.upstash`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.upstash.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_upstash.components.upstash.<Class>``.
"""
