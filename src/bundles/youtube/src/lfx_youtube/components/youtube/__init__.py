"""Lazy component re-exports for the ``youtube`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.youtube`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.youtube.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_youtube.components.youtube.<Class>``.
"""
