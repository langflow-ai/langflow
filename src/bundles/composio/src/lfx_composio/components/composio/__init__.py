"""Lazy component re-exports for the ``composio`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.composio`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.composio.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_composio.components.composio.<Class>``.
"""
