"""Lazy component re-exports for the ``aiml`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.aiml`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.aiml.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_aiml.components.aiml.<Class>``.
"""
