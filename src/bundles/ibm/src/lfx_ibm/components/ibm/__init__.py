"""Lazy component re-exports for the ``ibm`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.ibm`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.ibm.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_ibm.components.ibm.<Class>``.
"""
