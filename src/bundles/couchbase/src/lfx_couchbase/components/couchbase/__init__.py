"""Lazy component re-exports for the ``couchbase`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.couchbase`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.couchbase.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_couchbase.components.couchbase.<Class>``.
"""
