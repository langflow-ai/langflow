"""Lazy component re-exports for the ``altk`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.altk`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.altk.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_altk.components.altk.<Class>``.
"""
