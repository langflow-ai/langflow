"""Lazy component re-exports for the ``qdrant`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.qdrant`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.qdrant.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_qdrant.components.qdrant.<Class>``.
"""
