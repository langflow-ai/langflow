"""Lazy component re-exports for the ``nvidia`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.nvidia`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.nvidia.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_nvidia.components.nvidia.<Class>``.
"""
...
_dynamic_imports = {
    ...
    "NvidiaSystemAssistComponent": "system_assist",
}
