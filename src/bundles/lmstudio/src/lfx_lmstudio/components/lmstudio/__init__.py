"""Lazy component re-exports for the ``lmstudio`` bundle.

Mirrors the pre-extraction layout of ``lfx.components.lmstudio`` so saved
flows that referenced the module-level class
(e.g. ``lfx.components.lmstudio.<Class>``) keep resolving via the
migration table after rewrite to
``lfx_lmstudio.components.lmstudio.<Class>``.
"""
