        ...
        try:
            ...
            try:
                ...
                with shutdown_progress.step(2):
                    ...
                    # Shut down the code-execution sandbox VMs here in addition
                    # to the module's atexit hook: QEMU only reaps guests on
                    # parent death from 10.2+, so an uvicorn worker recycling
                    # without a full interpreter exit could otherwise leave
                    # microVMs running (issue #12029 backend).
                    try:
