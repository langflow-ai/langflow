...
async def run_flow(
    ...
    logger.warning(
        f"[BUG LOG] Starting flow execution: flow_id={flow_id}, flow_name={flow_name}, run_id={run_id}, session_id={session_id}"
    )
    ...
    if graph is None:
        ...
        try:
            ...
            for i, vertex in enumerate(graph.vertices):
                logger.warning(
                    f"[BUG LOG] Vertex {i + 1}/{len(graph.vertices)}: id={vertex.id}, type={vertex.vertex_type}, is_active={vertex.is_active()}"
                )

            # Log edge details for debugging
            for i, edge in enumerate(graph.edges):
                logger.warning(
                    f"[BUG LOG] Edge {i + 1}/{len(graph.edges)}: source={edge.source_id}, target={edge.target_id}"
                )

        except Exception as e:
            logger.warning(f"[BUG LOG] Error loading flow: {e!s}", exc_info=True)
            raise
    ...
    try:
        ...
        elapsed_time = time.time() - start_time
        logger.warning(f"[BUG LOG] Error executing flow after {elapsed_time:.2f} seconds: {e!s}", exc_info=True)
