...
class LCAgentComponent(Component):
    ...
    async def run_agent(self, agent: AgentExecutor) -> Text:
        ...
        result = await agent.ainvoke(input_dict, config={"callbacks": [AgentAsyncHandler(self.log)] + self.get_langchain_callbacks()})
        self.status = result
...
class LCToolsAgentComponent(LCAgentComponent):
    ...
    async def run_agent(
        ...
        result = await runnable.ainvoke(input_dict, config={"callbacks": [AgentAsyncHandler(self.log)] + self.get_langchain_callbacks()})
        self.status = result
        if "output" not in result:
            raise ValueError("Output key not found in result. Tried 'output'.")
