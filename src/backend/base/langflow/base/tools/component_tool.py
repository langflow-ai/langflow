...
def build_description(component: Component, output: Output) -> str:
    ...
    args = ", ".join(
        ...
        for input_name in sorted(output.required_inputs or [])
    )
    ...
    def get_tools(
        ...
        for output in self.component.outputs:
            ...
            if flow_mode_inputs:
                ...
                args_schema = create_input_schema(tool_mode_inputs or self.component.inputs)
        ...
        if len(tools) == 1 and (tool_name or tool_description):
            ...
            for tool in tools:
                tool.name = _format_tool_name(f"{tool_name}_{tool.name}") or tool.name
                tool.description = f"{tool_description} Output details: {tool.description}" or tool.description
                tool.tags = [tool.name]
