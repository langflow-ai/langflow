                ...
                for output in latest_node["outputs"]:
                    ...
                    if node_data_output:
                        ...
                        if old_selected:
                            ...
                            # Only set selected if it's in the available types
                            if migrated_selected in output.get("types", []):
                                output["selected"] = migrated_selected
                            # Otherwise, don't set selected and let it default to types[0]
...
def scape_json_parse(json_string: str) -> dict:
    # Fast path for None and dict
    if json_string is None:
        return {}
    if isinstance(json_string, dict):
        return json_string
    # Use str.replace only if needed (œ in string)
    if "œ" in json_string:
...
def update_edges_with_latest_component_versions(project_data):
    ...
    # Optimize deepcopy for dicts with only lists and dicts
    project_data_copy = deepcopy(project_data)

    # Pre-create node_type_map and id_to_node for O(1) lookups
    nodes = project_data_copy.get("nodes", [])
    ...
    # Cache the original nodes for reconciliation
    orig_nodes = project_data.get("nodes", [])
    orig_id_to_node = {}
    for n in orig_nodes:
        nid = n.get("id")
        if nid is not None:
            orig_id_to_node[nid] = n

    for edge in project_data_copy.get("edges", []):
        # Extract and parse source and target handles (fast path for dicts)
        ...
        # Fast id lookups for nodes
        source_id = edge.get("source")
        target_id = edge.get("target")
        source_node = orig_id_to_node.get(source_id)
        target_node = orig_id_to_node.get(target_id)
        ...
        if source_node is None and source_handle and "dataType" in source_handle:
            ...
            node_ids = node_type_map.get(node_type)
            if node_ids:
                new_node_id = node_ids[0]
                logger.info(f"Reconciling missing source node: replacing {source_id} with {new_node_id}")
                edge["source"] = new_node_id
                source_handle["id"] = new_node_id
                source_node = orig_id_to_node.get(new_node_id)
                # Update edge ID, assuming id formats
                old_id_prefix = edge.get("id", "").split("{")[0]
                if old_id_prefix:
                    new_id_prefix = old_id_prefix.replace(str(source_id), str(new_node_id))
                    edge["id"] = edge.get("id", "").replace(old_id_prefix, new_id_prefix)

        # Try to reconcile missing target node by type
        if target_node is None and target_handle and "id" in target_handle:
            ...
            if id_parts:
                ...
                if node_ids:
                    new_node_id = node_ids[0]
                    logger.info(f"Reconciling missing target node: replacing {target_id} with {new_node_id}")
                    edge["target"] = new_node_id
                    target_handle["id"] = new_node_id
                    target_node = orig_id_to_node.get(new_node_id)
                    old_id_suffix = edge.get("id", "").split("}-")[1] if "}-" in edge.get("id", "") else ""
                    if old_id_suffix:
                        new_id_suffix = old_id_suffix.replace(str(target_id), str(new_node_id))
        ...
        if source_node and target_node:
            ...
            # Use dict lookup for outputs by 'name' and 'display_name'
            outputs_lst = source_node_data.get("outputs", [])
            sh_name = source_handle.get("name")
            output_data = None
            for o in outputs_lst:
                if o.get("name") == sh_name:
                    output_data = o
                    break
            if not output_data:
                for o in outputs_lst:
                    if o.get("display_name") == sh_name:
                        output_data = o
                        source_handle["name"] = o.get("name")
                        break

            # Compute new_output_types
            if output_data:
                types = output_data.get("types", [])
                if len(types) == 1:
                    new_output_types = types
                elif len(types) > 1 and output_data.get("selected"):
                    selected = output_data.get("selected")
                    migrated_selected = selected
                    if selected == "Data":
                        migrated_selected = "JSON"
                    elif selected == "DataFrame":
                        migrated_selected = "Table"
                    if migrated_selected in types:
            ...
            templ_dict = target_node_data.get("template", {})
            input_types_new = []
            if field_name in templ_dict:
                input_types_new = templ_dict.get(field_name, {}).get("input_types", [])

            if target_handle.get("inputTypes", []) != input_types_new:
                edge_changes_log[target_node_data.get("display_name", "unknown")].append(
                    {
                        "attr": "inputTypes",
                        "old_value": target_handle.get("inputTypes", []),
                        "new_value": input_types_new,
                    }
                )
                target_handle["inputTypes"] = input_types_new

            # Use escape_json_dump once per edge, supply pre-serialized dict if possible
            escaped_source_handle = escape_json_dump(source_handle)
            escaped_target_handle = escape_json_dump(target_handle)

            # Avoid extra json.loads+dump if original handle already dict or str
            try:
                src_val = edge.get("sourceHandle", "{}")
                if isinstance(src_val, dict):
                    old_escape_source_handle = escape_json_dump(src_val)
                else:
                    old_escape_source_handle = escape_json_dump(json.loads(src_val))
            except (json.JSONDecodeError, TypeError):
                old_escape_source_handle = edge.get("sourceHandle", "")

            try:
                tgt_val = edge.get("targetHandle", "{}")
                if isinstance(tgt_val, dict):
                    old_escape_target_handle = escape_json_dump(tgt_val)
                else:
                    old_escape_target_handle = escape_json_dump(json.loads(tgt_val))
            except (json.JSONDecodeError, TypeError):
...
def log_node_changes(node_changes_log) -> None:
    # Same implementation; minimal room for optimization
    formatted_messages = []
