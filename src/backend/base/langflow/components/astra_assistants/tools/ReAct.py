...
class ReActThoughtGenerator(BaseModel):
    ...
    """
    thought: str = Field(..., description="Your current stream of consciousness step by step thoughts as you analyze this question.")
    action: str = Field(..., description="Actions that need to be taken to complete the thought.")
    answer: Optional[str]= Field(..., description="A string that represents the final answer to the question. Only provide this if you are sure of the answer.")
        ...
        schema_extra = {
            "example": [
                ...
                },
                {
                    ...
                    "action": "None",
                    "answer": "The final refactored code is: `squared_numbers = [x ** 2 for x in numbers]`. This replaces the `for` loop in a more concise manner."
                }
            ]
        }
