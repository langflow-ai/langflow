...
class TotogiPlanAI(LCToolComponent):
    display_name: str = "PlanAI"
    description: str = "Totogi PlanAI"
    ...
    class TotogiPlanAISchema(BaseModel):
        pass
    ...
    def build_tool(self) -> Tool:
        logger.info(f"Building Totogi PlanAI tool: {self.display_name}")
        return StructuredTool.from_function(
            name="totogi_charging_detail_get",
            description="Retrieve details from PlanAI.",
            func=self._get_product_detail,
            args_schema=self.TotogiPlanAISchema,
        )
