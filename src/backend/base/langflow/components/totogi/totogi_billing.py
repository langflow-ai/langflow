...
class TotogiBilling(LCToolComponent):
    display_name: str = "Billing"
    description: str = "Totogi Billing"
    ...
    class TotogiBillingSchema(BaseModel):
        pass
    ...
    def build_tool(self) -> Tool:
        logger.info(f"Building Totogi Billing tool: {self.display_name}")
        return StructuredTool.from_function(
            name="totogi_billing_detail_get",
            description="Retrieve billing details from Totogi Billing.",
            func=self._get_product_detail,
            args_schema=self.TotogiBillingSchema,
        )
