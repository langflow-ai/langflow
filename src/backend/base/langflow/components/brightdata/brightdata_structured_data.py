...
class BrightDataStructuredDataEnhancedComponent(Component):
    ...
    def _detect_website_type(self, url: str) -> tuple[str | None, int, dict[str, Any]]:
        ...
        for dataset_name, config in datasets.items():
            ...
            base_score = (
                score_breakdown["domain_score"] + score_breakdown["pattern_score"] + score_breakdown["keyword_score"]
            )
    ...
    def _calculate_specificity_bonus(self, dataset_name: str, url: str, domain: str, path: str, query: str) -> int:
        ...
        if "linkedin.com" in domain:
            if (dataset_name == "linkedin_person_profile" and re.search(r"/in/[^/]+/?$", path)) or (
                dataset_name == "linkedin_company_profile" and "/company/" in path
            ):
                bonus += 20
            elif (dataset_name == "linkedin_job_listings" and "/jobs/" in path) or (
                dataset_name == "linkedin_posts" and ("/posts/" in path or "/feed/update/" in path)
            ):
                bonus += 15
        ...
        # Instagram specific bonuses
            ...
            if (dataset_name == "instagram_posts" and "/p/" in path) or (
        ...
        # TikTok specific bonuses
            ...
            if (
                (dataset_name == "tiktok_posts" and "/video/" in path)
                or (dataset_name == "tiktok_profiles" and re.search(r"/@[^/]+/?$", path))
                or (dataset_name == "tiktok_shop" and "/shop/" in path)
            ):
                bonus += 20

        # E-commerce platform bonuses
        elif ("ebay.com" in domain and dataset_name == "ebay_product" and "/itm/" in path) or (
            "etsy.com" in domain and dataset_name == "etsy_products" and "/listing/" in path
        ):
            bonus += 20
        elif "walmart.com" in domain:
            if (dataset_name == "walmart_product" and "/ip/" in path) or (
                dataset_name == "walmart_seller" and "/seller/" in path
            ):
                bonus += 20

        # App store bonuses
        elif (
            ("play.google.com" in domain and dataset_name == "google_play_store" and "/store/apps/" in path)
            or ("apps.apple.com" in domain and dataset_name == "apple_app_store" and "/app/" in path)
            or ("zillow.com" in domain and dataset_name == "zillow_properties_listing" and "/homedetails/" in path)
            or ("booking.com" in domain and dataset_name == "booking_hotel_listings" and "/hotel/" in path)
        ):
            bonus += 25
    ...
    def extract_structured_data(self) -> Data:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Invalid JSON in additional_params: {e!s}, using empty dict")
                additional_params = {}
            ...
            if self.auto_detect:
                ...
                if not detected_type:
                    ...
                    error_msg = (
                        f"Could not automatically detect website type for domain: {parsed_url.netloc}\n"
                ...
                try:
                    ...
                    if snapshot_response.status_code == HTTP_OK:
                        try:
                            ...
                            logger.warning(f"Failed to parse snapshot response as JSON: {e!s}")
                            time.sleep(5)
                            continue

                        # Handle snapshot response format - could also be dict or list
                        status = None
                        final_data: dict[str, Any] | list[Any] | None = None
                    ...
                    logger.warning(f"Polling attempt {attempts} failed: {e!s}")
            ...
            error_msg = f"Unexpected error during data extraction: {e!s}"
            logger.error(error_msg)
            raise ValueError(error_msg) from e
