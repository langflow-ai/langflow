...
class FileComponent(BaseFileComponent):
    ...
    def _process_docling_in_subprocess(self, file_path: str) -> Data | None:
        ...
        if settings.storage_type == "s3":
            local_path, should_delete = asyncio.run(self._get_local_file_for_docling(file_path))
        else:
    ...
    def _process_docling_subprocess_impl(self, local_file_path: str, original_file_path: str) -> Data | None:
        ...
        self.log(f"Starting Docling subprocess for file: {original_file_path}")
        self.log(args)
