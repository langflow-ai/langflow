...
from langflow.io import MessageTextInput, Output, DropdownInput, MultiselectInput
import git
import os
import tempfile
import shutil
...
class GitFileComponent(Component):
    ...
    def get_branches(self, repo_url):
        try:
            ...
            self.log(f"Error getting branches: {str(e)}")
            return ["main", "master"]
        finally:
    ...
    def update_build_config(self, build_config: dict, field_value: str, field_name: str | None = None) -> dict:
        ...
        if self.branch and self.branch not in ["Enter repository URL first"] and field_name in {"repository_url", "branch", "selected_files"}:
            try:
                temp_dir = tempfile.mkdtemp()
    ...
    def get_files_content(self) -> list[Data]:
        ...
        try:
            ...
            error_msg = f"Error getting files content: {str(e)}"
