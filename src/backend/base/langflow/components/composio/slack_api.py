...
class SlackAPIComponent(LCToolComponent):
    ...
    MessageTextInput(
        ...
        display_name="Cursor",
        info="Paginate through collections of data by setting the `cursor` parameter to a `next_cursor` attribute returned by a previous request's `response_metadata`. Default value fetches the first `page` of the collection. See [pagination](https://slack.dev) for more detail. ",
        show=False,
    ),
    BoolInput(
    ...
    BoolInput(
        ...
        info="Pass true to post the message as the authed user, instead of as a bot. Defaults to false. See [authorship](https://slack.dev) below. ",
        show=False,
    ),
    MessageTextInput(
    ...
    MessageTextInput(
        ...
        info="Channel, private group, or IM channel to send message to. Can be an encoded ID, or a name. See [below](https://slack.dev) for more details. ",
        show=False,
        required=True,
    ),
    MessageTextInput(
        ...
        info="Emoji to use as the icon for this message. Overrides `icon_url`. Must be used in conjunction with `as_user` set to `false`, otherwise ignored. See [authorship](https://slack.dev) below. ",
        show=False,
    ),
    MessageTextInput(
        name="SLACK_SENDS_A_MESSAGE_TO_A_SLACK_CHANNEL-icon_url",
        display_name="Icon Url",
        info="URL to an image to use as the icon for this message. Must be used in conjunction with `as_user` set to false, otherwise ignored. See [authorship](https://slack.dev) below. ",
        show=False,
    ),
    BoolInput(
    ...
    MessageTextInput(
        ...
        display_name="Text",
        info="How this field works and whether it is required depends on other fields you use in your API call. [See below](https://slack.dev) for more detail. ",
        show=False,
    ),
    MessageTextInput(
    ...
    MessageTextInput(
        ...
        info="Set your bot's user name. Must be used in conjunction with `as_user` set to false, otherwise ignored. See [authorship](https://slack.dev) below. ",
        show=False,
    ),
    MessageTextInput(
        name="SLACK_UPDATES_A_SLACK_MESSAGE-as_user",
        display_name="As User",
        info="Pass true to update the message as the authed user. [Bot users](https://slack.dev) in this context are considered authed users. ",
        show=False,
    ),
    MessageTextInput(
    ...
    MessageTextInput(
        ...
        display_name="Blocks",
        info="A JSON-based array of [structured blocks](https://slack.dev), presented as a URL-encoded string. If you don't include this field, the message's previous `blocks` will be retained. To remove previous `blocks`, include an empty array for this field. ",
        show=False,
    ),
    MessageTextInput(
    ...
    MessageTextInput(
        ...
        display_name="Text",
        info="New text for the message, using the [default formatting rules](https://slack.dev). It's not required when presenting `blocks` or `attachments`. ",
        show=False,
    ),
    MessageTextInput(
    ...
    BoolInput(
        ...
        display_name="As User",
        info="Pass true to post the message as the authed user, instead of as a bot. Defaults to false. See [chat.postMessage](chat.postMessage#authorship). ",
        show=False,
    ),
    MessageTextInput(
    ...
    MessageTextInput(
        ...
        display_name="Channel",
        info="Channel, private group, or DM channel to send message to. Can be an encoded ID, or a name. See [below](https://slack.dev) for more details. ",
        show=False,
    ),
    BoolInput(
    ...
    MessageTextInput(
        ...
        display_name="Parse",
        info="Change how messages are treated. Defaults to `none`. See [chat.postMessage](chat.postMessage#formatting). ",
        show=False,
    ),
    MessageTextInput(
