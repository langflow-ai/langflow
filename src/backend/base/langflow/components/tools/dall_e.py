...
class DallEWrapper(BaseModel):
        ...
        if response and response.data:
            ...
            if self.result_format in ["base64", "base64_json"] and hasattr(image_data, "b64_json"):
                ...
                if self.result_format == "url":
                    ...
                    download_path = self._download(image_data.url)
                    if not isinstance(download_path, Path):
                        msg = f"Expected a Path object, got {type(download_path)}"
                        raise TypeError(msg)
                    result = str(download_path)  # type: ignore[assignment]
            else:
    ...
    def _download(self, url: str, response: httpx.Response | None = None) -> Path:
        ...
        # Get MD5 or ETag, with a fallback to an empty string
        md5_header = response.headers.get("Content-MD5")
        etag_header = response.headers.get("ETag")

        # Decode bytes to string, if necessary
        if isinstance(md5_header, bytes):
            md5_header = md5_header.decode("utf-8", errors="replace")
        if isinstance(etag_header, bytes):
            etag_header = etag_header.decode("utf-8", errors="replace")

        # Ensure ETag is stripped of quotes
        etag_header = etag_header.strip('"') if etag_header else ""

        # Use MD5 if available, otherwise fall back to ETag
        unique_id = md5_header or etag_header
