...
class AstraDBVectorStoreComponent(LCVectorStoreComponent):
    ...
    def get_api_endpoint(self):
        if not self._cached_api_endpoint:
            self._cached_api_endpoint = self.get_api_endpoint_static(
                token=self.token,
                environment=self.environment,
                api_endpoint=self.api_endpoint,
                database_name=self.database_name,
            )
        return self._cached_api_endpoint

    def get_keyspace(self):
        if self._cached_keyspace is None:  # Check if caching is yet to be done.
            self._cached_keyspace = self.keyspace.strip() if self.keyspace else None
        return self._cached_keyspace
