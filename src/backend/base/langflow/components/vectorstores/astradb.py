...
class AstraDBVectorStoreComponent(LCVectorStoreComponent):
    ...
    def _initialize_database_options(self):
        if self._cached_database_options is not None:
            return self._cached_database_options

        try:
            self._cached_database_options = [
                {
                    "name": name,
    ...
    def _initialize_collection_options(self):
        if self._cached_collection_options is not None:
            return self._cached_collection_options

        database = self.get_database_object()
        if database is None:
            self._cached_collection_options = []
            return self._cached_collection_options
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._cached_database_options = None
        self._cached_collection_options = None
