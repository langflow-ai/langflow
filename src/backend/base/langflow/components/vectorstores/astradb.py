...
class AstraDBVectorStoreComponent(LCVectorStoreComponent):
    ...
    def reset_database_list(self, build_config: dict):
        ...
        # Prepare the dropdown options and metadata
        build_config["database_name"]["options"] = list(map(lambda db: db["name"], database_options))
        build_config["database_name"]["options_metadata"] = [
            {"collections": db["collections"], "api_endpoint": db["api_endpoint"]} for db in database_options
        ]

        # Reset the selected database if not in the new list
        selected_database = build_config["database_name"]["value"]
        if selected_database not in build_config["database_name"]["options"]:
