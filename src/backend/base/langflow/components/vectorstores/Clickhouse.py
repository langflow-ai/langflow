...
from langflow.io import HandleInput, IntInput, StrInput, SecretStrInput, DataInput, DropdownInput, MultilineInput, DictInput
...
class ClickhouseVectorStoreComponent(LCVectorStoreComponent):
    ...
    inputs = [
        ...
        DropdownInput(name="index_type", display_name="index_type", options=['annoy', 'vector_similarity'], info="Type of the index.", value="annoy", advanced=True),
        DropdownInput(name="metric", display_name="metric", options=['angular', 'euclidean', 'manhattan', 'hamming', 'dot'], info="Metric to compute distance.", value="angular", advanced=True),
        BoolInput(name="secure", display_name="Use https/TLS. This overrides inferred values from the interface or port arguments.", value=False, advanced=True),
    ...
    def build_vector_store(self) -> Clickhouse:
        ...
        settings = ClickhouseSettings(table=self.table, database=self.database, host=self.host,
                                      index_type=self.index_type, metric=self.metric,
                                      password=self.password, port=self.port, secure=self.secure,
