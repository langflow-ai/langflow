...
class VectaraRag(CustomComponent):
    ...
    def build_config(self) -> dict:
        ...
        SUMMARIZER_PROMPTS = ["vectara-summary-ext-24-05-sml",
    ...
    def build(self, vectara_customer_id: str, vectara_corpus_id: str, vectara_api_key: str, search_query: str, lexical_interpolation: float=0.005,
             filter: str='', reranker: str='mmr', reranker_k: int=50, diversity_bias: float=0.2, max_results: int=7,
             response_lang: str='eng', prompt: str='vectara-summary-ext-24-05-sml') -> Message:
        ...
        summary_config = SummaryConfig(is_enabled=True, max_results=max_results, response_lang=response_lang, prompt_name=prompt)
        config = VectaraQueryConfig(lambda_val = lexical_interpolation, filter=filter, summary_config=summary_config, rerank_config=rerank_config)
