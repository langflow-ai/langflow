...
class DataPurgeService(Service):
    ...
    def __init__(self, database_service, settings_service):
        ...
        self.cleanup_interval: timedelta | None = None
        ...
        try:
            ...
            if cleanup_interval_str:
                ...
                if self.cleanup_interval:
                    logger.info(
                        f"Data purge service initialized with interval: {cleanup_interval_str} ({self.cleanup_interval})"
                    )
                    await self.start_periodic_cleanup()
                    logger.info("Data purge periodic task started successfully")
                else:
                    logger.warning(
                        f"Invalid purge interval format: {cleanup_interval_str}. Expected format: '1m', '2h', '1d', etc."
                    )
            else:
    ...
    async def purge_old_data(self) -> int:
        ...
        try:
            ...
            async with self.database_service.with_session() as session:
                ...
                if total_deleted > 0:
                    logger.info(
                        f"Successfully purged {total_deleted} records - VertexBuilds: {vertex_deleted}, Messages: {message_deleted}, Transactions: {transaction_deleted}"
                    )
                else:
