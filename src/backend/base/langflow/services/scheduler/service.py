...
class SchedulerService(Service):
    ...
    async def create_scheduler(self, *, session: AsyncSession, scheduler: SchedulerCreate) -> SchedulerRead:
    ...
    async def get_scheduler(self, *, session: AsyncSession, scheduler_id: UUID) -> SchedulerRead:
    ...
    async def get_schedulers(self, *, session: AsyncSession, flow_id: UUID | None = None) -> list[SchedulerRead]:
    ...
    async def delete_scheduler(self, *, session: AsyncSession, scheduler_id: UUID) -> None:
        """Delete a scheduler."""
        scheduler_query = select(Scheduler).where(Scheduler.id == scheduler_id)
        scheduler = await session.exec(scheduler_query)
        scheduler = scheduler.first()
    ...
    async def _create_job(self, scheduler: Scheduler) -> None:
        ...
        # Log job creation
        logger.info(
            f"Creating scheduled job for flow_id={scheduler.flow_id} with {schedule_desc} (scheduler_id={scheduler.id})"
        )
        ...
        async def run_flow():
            ...
            try:
                ...
                logger.info(
                    f"Starting scheduled flow execution for flow_id={scheduler.flow_id} (scheduler_id={scheduler.id})"
                )
                ...
                if job:
                    logger.info(
                        f"Job configuration: max_instances={job.max_instances}, next_run_time={job.next_run_time}"
                    )
                ...
                for attempt in range(1, max_retries + 1):
                    try:
                        logger.warning(
                            f"[BUG LOG] Attempt {attempt}/{max_retries} to run scheduled flow {scheduler.flow_id}"
                        )
                        async with httpx.AsyncClient(timeout=settings.scheduler_timeout) as client:
                            ...
                            if response.status_code == 200:
                                ...
                                try:
                                    ...
                                    logger.warning(
                                        f"[BUG LOG] Successfully ran scheduled flow {scheduler.flow_id} in {elapsed_time:.2f} seconds"
                                    )
                                    logger.warning(
                                        f"[BUG LOG] Flow execution result: {json.dumps(result_data, indent=2)}"
                                    )
                                ...
                                return
                            logger.warning(
                                f"[BUG LOG] Error running flow: HTTP {response.status_code} - {response.text}"
                            )
                        ...
                        elapsed_time = time.time() - start_time
                        logger.warning(
                            f"[BUG LOG] Timeout running scheduled flow {scheduler.flow_id} after {elapsed_time:.2f} seconds"
                        )
                        if attempt < max_retries:
                            logger.warning(f"[BUG LOG] Retrying in {retry_delay} seconds...")
                            await asyncio.sleep(retry_delay)
                            ...
                            logger.warning(
                                f"[BUG LOG] Failed to run scheduled flow after {max_retries} attempts due to timeout"
                            )
                    except Exception as e:
                        logger.warning(
                            f"[BUG LOG] Error on attempt {attempt}/{max_retries} running scheduled flow {scheduler.flow_id}: {e}",
                            exc_info=True,
                        )
                        if attempt < max_retries:
                            logger.warning(f"[BUG LOG] Retrying in {retry_delay} seconds...")
                ...
                if start_time:
                    ...
                    elapsed_str = ""
                logger.warning(
                    f"[BUG LOG] Error running scheduled flow {scheduler.flow_id}{elapsed_str}: {e}", exc_info=True
                )
        ...
        logger.info(
            f"Scheduled job for flow_id={scheduler.flow_id} created successfully. Next run time: {next_run_time}"
        )
        ...
        if job_id:
            try:
                ...
                if job:
                    next_run = job.next_run_time.strftime("%Y-%m-%d %H:%M:%S") if job.next_run_time else "Not scheduled"
                    logger.info(
                        f"Removing scheduled job {job_id} for scheduler_id={scheduler_id}. Next run was: {next_run}"
                    )
            ...
            result.append(
                {
                    ...
                    "schedule": scheduler.cron_expression
                    if scheduler.cron_expression
                    else f"{scheduler.interval_seconds} seconds",
                }
            )
