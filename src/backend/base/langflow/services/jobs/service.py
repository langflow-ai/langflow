...
class JobService(Service):
    ...
    async def get_job_by_job_id(self, job_id: UUID | str) -> list[Job]:
        """Get jobs for a specific flow with pagination.
    ...
    async def create_job(self, job_id: str, flow_id: UUID) -> Job:
    ...
    async def update_job_status(self, job_id: str, status: str, *, finished_timestamp: bool = False) -> Job | None:
    ...
    async def with_status_updates(self, job_id: UUID, flow_id: UUID, run_graph_func, *args, **kwargs):
        ...
        try:
            ...
            await self.update_job_status(job_id, JobStatus.IN_PROGRESS.value)
            ...
            await self.update_job_status(job_id, JobStatus.FAILED.value, finished_timestamp=True)
            raise

        except asyncio.TimeoutError:
            # Handle timeout specifically
            await self.update_job_status(job_id, JobStatus.TIMED_OUT.value, finished_timestamp=True)
            raise

        except asyncio.CancelledError:
            # Handle cancellation
            await self.update_job_status(job_id, JobStatus.CANCELLED.value, finished_timestamp=True)
            raise
