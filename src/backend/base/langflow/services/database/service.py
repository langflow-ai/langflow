    ...
    def _run_migrations(self, should_initialize_alembic, fix) -> None:
        ...
        with self.alembic_log_path.open("w", encoding="utf-8") as buffer:
            ...
            if should_initialize_alembic:
                try:
                    logger.debug("Initializing alembic for first time")
                    self.init_alembic(alembic_cfg)
                    logger.debug("Alembic initialized successfully")
                except Exception as exc:
                    msg = "Error initializing alembic"
                    logger.exception(msg)
                    raise RuntimeError(msg) from exc
            else:
                logger.debug("Alembic already initialized")

            # Check for schema differences and auto-upgrade if needed
            try:
                buffer.write(f"{datetime.now(tz=timezone.utc).astimezone().isoformat()}: Checking migrations\n")
                logger.debug("Checking for schema differences")
                command.check(alembic_cfg)
                logger.debug("No schema differences detected")
            except util.exc.AutogenerateDiffsDetected as exc:
                logger.info(f"Schema differences detected: {exc}")
                logger.info("Auto-upgrading database schema to latest version")
                try:
                    buffer.write(f"{datetime.now(tz=timezone.utc).astimezone().isoformat()}: Auto-upgrading schema\n")
                    command.upgrade(alembic_cfg, "head")
                    logger.info("Database schema auto-upgrade completed successfully")
                    # Give the database a moment to settle
                    time.sleep(3)

                    # Verify upgrade was successful
                    buffer.write(f"{datetime.now(tz=timezone.utc).astimezone().isoformat()}: Verifying schema after upgrade\n")
                    command.check(alembic_cfg)
                    logger.debug("Schema verification successful after auto-upgrade")
                except Exception as upgrade_exc:
                    msg = f"Failed to auto-upgrade database schema: {upgrade_exc}"
                    logger.exception(msg)
                    raise RuntimeError(msg) from upgrade_exc
            except util.exc.CommandError as exc:
                logger.error(f"Migration command error: {exc}")
                if "AutogenerateDiffsDetected" in str(exc) or "revision" in str(exc).lower():
                    logger.info("Attempting auto-upgrade due to migration command error")
                    try:
                        command.upgrade(alembic_cfg, "head")
                        logger.info("Auto-upgrade completed after command error")
                        time.sleep(3)
                    except Exception as upgrade_exc:
                        msg = f"Failed to auto-upgrade after command error: {upgrade_exc}"
                        logger.exception(msg)
                        raise RuntimeError(msg) from upgrade_exc
                else:
                    raise
            except Exception as exc:
                logger.error(f"Unexpected error during migration check: {exc}")
                raise

            # If fix mode is enabled, try more aggressive migration strategies
            if fix:
                logger.debug("Fix mode enabled, attempting downgrade/upgrade strategy")
