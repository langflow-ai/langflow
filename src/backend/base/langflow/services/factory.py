...
def import_all_services_into_a_dict():
    ...
    for service_type in ServiceType:
        try:
            ...
            # Iterate the module namespace directly to avoid the overhead of inspect.getmembers
            for name, obj in module.__dict__.items():
                # Keep isinstance checks to correctly handle class-like objects
                if not isinstance(obj, type):
                    continue
                # Skip the base Service class itself quickly to avoid unnecessary issubclass calls
                if obj is Service:
                    continue
