...
class OIDCConfig(BaseModel):
    ...
    redirect_uri: str = Field(..., description="OAuth callback URL (e.g., https://langflow.example.com/api/v1/auth/callback)")
...
class LDAPConfig(BaseModel):
    ...
    user_search_filter: str = Field(
        ...
        description="LDAP filter for user lookup"
    )
...
class SSOConfig(BaseModel):
    ...
    def get_provider_config(self) -> OIDCConfig | SAMLConfig | LDAPConfig:
        ...
        if self.provider == AuthProvider.OIDC and self.oidc:
            return self.oidc
        elif self.provider == AuthProvider.SAML and self.saml:
            return self.saml
        elif self.provider == AuthProvider.LDAP and self.ldap:
            return self.ldap
        else:
