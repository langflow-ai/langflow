...
from .sandbox_context import SandboxConfig, SandboxExecutionContext, SandboxExecutionResult
...
class SandboxManager:
    ...
    def __init__(self, config: SandboxConfig | None = None, db_session: Session | None = None):
    ...
    async def execute_component(
        ...
        code: str,
        component_path: str,
        component_instance: CustomComponent | None = None,
    ...
    def _extract_value_data(self, value):
        ...
        if hasattr(value, "data") and not callable(value.data):
            # Object has a data attribute (like Data objects)
            return self._extract_value_data(value.data)
        if hasattr(value, "get_result") and callable(value.get_result):
        ...
        try:
            ...
            if component_class_name == "TypeConverterComponent":
                ...
                if hasattr(component_instance, "input_data"):
                    input_data = component_instance.input_data
            ...
            if hasattr(component_instance, "_vertex") and component_instance._vertex:
                ...
                logger.info("SANDBOX_MANAGER: No vertex found on component")
            ...
            if secrets_env and profile.allow_secrets_for_untrusted:
                ...
                logger.warning("Secrets requested but not allowed by profile (allow_secrets_for_untrusted=False)")
            ...
            return SandboxExecutionResult(
                execution_id=context.execution_id,
                success=False,
                error=f"Runtime execution failed: {e!s}",
                stdout="",
                stderr="",
                execution_time=0.0,
    ...
    async def _run_in_nsjail(
        ...
        try:
            ...
            return SandboxExecutionResult(
                ...
                success=False,
                stdout="",
                stderr=f"nsjail execution failed: {e!s}",
                execution_time=0.0,
    ...
    def _analyze_sandbox_failure(self, return_code: int, stderr: str, stdout: str, execution_time: float, context: SandboxExecutionContext) -> tuple[str, str]:
        ...
        if "import" in stderr.lower() and ("not allowed" in stderr.lower() or "blocked" in stderr.lower()):
            return (
                "Import restriction violation. Component tried to import blocked packages. "
                "import_blocked"
            )
        ...
        if "permission denied" in stderr.lower() and ("file" in stderr.lower() or "directory" in stderr.lower()):
            return (
                ...
                "Component tried to access files outside allowed paths.",
                "file_access_denied"
            )
        ...
        if "nsjail" in stderr.lower():
            ...
            if "exec failed" in stderr.lower():
                return (
...
def get_sandbox_manager(db_session: Session | None = None) -> SandboxManager:
