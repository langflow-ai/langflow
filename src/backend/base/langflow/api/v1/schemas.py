...
class SpecificationSummary(BaseModel):
    """Lightweight specification summary for frontend lists"""
    id: UUID
    name: str
    version: str
    status: str
    ...
    reusability_score: float | None = None
...
class FlowWithSpecification(FlowRead):
    """Flow model with specification metadata included"""
    specification: SpecificationSummary | None = None


class SpecificationListResponse(BaseModel):
    """Paginated response for specification lists"""
    items: list[SpecificationSummary]
    total: int
    page: int
    page_size: int
    has_more: bool
...
class BulkOperationResponse(BaseModel):
    ...
    failed: list[dict]  # List of {id: UUID, error: str}
    total_processed: int


class BulkCreateSpecificationRequest(BaseModel):
    """Request for bulk creating specifications"""
    specifications: list[dict]  # List of specification data


class BulkUpdateSpecificationRequest(BaseModel):
    """Request for bulk updating specifications"""
    updates: list[dict]  # List of {id: UUID, data: dict}


class BulkDeleteSpecificationRequest(BaseModel):
    """Request for bulk deleting specifications"""
    specification_ids: list[UUID]


class SpecificationExportResponse(BaseModel):
    """Response for specification export"""
...
class SpecificationImportRequest(BaseModel):
    """Request for specification import"""
    format: str  # "yaml" or "json"
    content: str
    overwrite_existing: bool = False


class SpecificationImportResponse(BaseModel):
    """Response for specification import"""
    imported_count: int
    skipped_count: int
    failed_count: int
    errors: list[str] = []
...
class SpecificationTemplateResponse(BaseModel):
    """Response for specification templates"""
    templates: list[dict]
    categories: list[str]


class ConversionResultResponse(BaseModel):
    """Response for flow-to-specification conversion"""
    success: bool
...
class FlowSpecificationMetadata(BaseModel):
    """Metadata about flow-specification relationship"""
    flow_id: UUID
    specification_id: UUID | None = None
