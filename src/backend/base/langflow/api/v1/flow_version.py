...
from langflow.services.database.models.flow_version.crud import (
    ...
    get_flow_version_counts_by_version_ids,
...
async def list_flow_versions(
    ...
    entries = await get_flow_version_list(
        session,
        flow_id,
        current_user.id,
        limit,
    ...
    version_ids = [entry.id for entry in entries]
    counts_by_version_id = await get_flow_version_counts_by_version_ids(
        session,
        flow_id=flow_id,
    ...
    deployment_counts = {str(version_id): 0 for version_id in version_ids}
    deployment_counts.update({str(version_id): count for version_id, count in counts_by_version_id.items()})
