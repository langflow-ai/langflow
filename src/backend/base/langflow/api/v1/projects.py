...
def _is_deployment_project_slug(name: str) -> bool:
    return len(name) <= 63 and _DEPLOYMENT_PROJECT_SLUG_RE.fullmatch(name) is not None
...
async def _lock_replacement_operation(session: DbSession, project_id: UUID, operation_id: UUID) -> str:
    ...
    if dialect == "postgresql":
        ...
        await session.exec(
            update(Folder).where(Folder.id == project_id).values(description=Folder.description)
        )
    else:
        raise HTTPException(
...
async def replace_project_operation(
    *,
    ...
    try:
        ...
        can_widen_project_fetch = await authorization_service.supports_cross_user_fetch() and await (
            authorization_service.is_enabled()
        )
        ...
        for flow in request_body.flows:
            if flow.folder_id is not None and flow.folder_id != project_id:
                raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Flow folder_id does not match project")
        ...
        if requested_ids:
            other_flows = list(
                (
                    await session.exec(select(Flow).where(Flow.id.in_(requested_ids)))
                ).all()
            )
        ...
        if len(set(flow_names)) != len(flow_names) or len(set(endpoint_names)) != len(endpoint_names):
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Flow names and endpoint names must be unique")
        ...
        ).first() if endpoint_names else None
        ...
        await araise_if_deployment_guard_error_or_skip(exc, log_message=f"op=replace_project_operation project_id={project_id}")
