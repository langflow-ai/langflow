...
async def simple_workflow_flow(
    ...
    *,
    ...
    This function is similar to simple_run_flow but returns clean JSON
    directly instead of wrapping it in a RunResponse object.
    """
    import json
    import time
    ...
    try:
        ...
        # Check if any output looks like API Response JSON and extract it directly
        if task_result:
            for run_output in task_result:
                for output_data in run_output.outputs:
                    if output_data and hasattr(output_data, "results"):
                        # Look for Message objects that contain JSON from API Response
                        results = output_data.results
                        if isinstance(results, dict):
                            for key, value in results.items():
                                if hasattr(value, "text"):
                                    # Try to parse the JSON from the Message text
                                    try:
                                        parsed_json = json.loads(value.text)
                                        if isinstance(parsed_json, dict) and "output" in parsed_json and "metadata" in parsed_json:
                                            # This looks like an API Response component output - return it directly
                                            return parsed_json
                                    except (json.JSONDecodeError, AttributeError):
                                        continue
        
        # Fallback: create a minimal response structure
        duration_ms = int((time.time() - start_time) * 1000)
        
        # Extract the first meaningful output
        output_data = None
        if task_result:
            for run_output in task_result:
                for output in run_output.outputs:
                    if output and hasattr(output, "results"):
                        ...
                        if isinstance(results, dict) and results:
                            ...
                            for key, value in results.items():
                                if hasattr(value, "text"):
                                    output_data = {"text": value.text}
                                    break
                                elif hasattr(value, "data"):
                                    output_data = {"data": value.data}
                                    break
                                else:
        ...
        if output_data is None:
            output_data = {"message": "No output generated"}
...
async def workflow_run_flow(
    *,
    ...
    Args:
        ...
        flow (FlowRead | None): The flow to execute, loaded via dependency
        input_request (SimplifiedAPIRequest | None): Input parameters for the flow
        stream (bool): Whether to stream the response (not supported in workflow mode)
        api_key_user (UserRead): Authenticated user from API key
        context (dict | None): Optional context to pass to the flow
        http_request (Request): The incoming HTTP request for extracting global variables
    ...
    Notes:
        ...
        - Streaming is not supported - use /run endpoint for streaming
        - Extracts global variables from HTTP headers with prefix X-LANGFLOW-GLOBAL-VAR-*
    """
    telemetry_service = get_telemetry_service()

    # Streaming is not supported in workflow mode
    if stream:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Streaming is not supported in workflow mode. Use /run endpoint for streaming.")
