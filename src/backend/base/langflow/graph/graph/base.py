...
class Graph:
    ...
    async def arun(
        ...
        *,
        ...
        logger.warning(
            f"[BUG LOG] Starting graph execution with {len(inputs)} input(s), {len(outputs or [])} output(s), stream={stream}"
        )
        logger.warning(
            f"[BUG LOG] Graph details: flow_id={self.flow_id}, session_id={session_id}, vertices={len(self.vertices)}"
        )
        ...
        for i, (run_inputs, components, input_type) in enumerate(zip(inputs, inputs_components, types, strict=True)):
            ...
            try:
                ...
                logger.warning(
                    f"[BUG LOG] Run {i + 1} completed in {elapsed_time:.2f} seconds with {len(run_outputs)} output(s)"
                )
                ...
                elapsed_time = time.time() - start_time
                logger.warning(f"[BUG LOG] Run {i + 1} failed after {elapsed_time:.2f} seconds: {e!s}", exc_info=True)
