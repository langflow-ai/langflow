...
class Component(CustomComponent):
    ...
    def _filter_tools_by_status(self, tools: list[Tool], metadata: pd.DataFrame | None) -> list[Tool]:
        ...
        """
        # Return tools directly if metadata is None or empty DataFrame
        if metadata is None or metadata.empty:
            enabled = self.enabled_tools
            if enabled is None:
                return tools
            enabled_set = set(enabled)
            return [tool for tool in tools if enabled_set.intersection({tool.name, *tool.tags})]

        # Convert metadata DataFrame to a dictionary with name as key and status as value
        tool_status = pd.Series(metadata.status.values, index=metadata.name).to_dict()

        # Create the list of allowed tools based on their status
