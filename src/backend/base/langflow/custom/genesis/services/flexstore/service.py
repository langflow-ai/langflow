...
class FlexStoreService(Service):
    ...
    def set_ready(self) -> None:
        ...
        if not self.settings.is_configured():
            raise ValueError("FlexStore settings are not properly configured")
        self._ready = True
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            logger.warning(f"FlexStore health check failed: {e!s}")
            return False
    ...
    async def get_containers(self, storage_account: str) -> list[str]:
        ...
        try:
            ...
            logger.error(f"Error fetching containers: {e!s}")
            return []

    async def get_files(self, storage_account: str, container_name: str) -> list[str]:
        ...
        try:
            ...
            logger.error(f"Error fetching files: {e!s}")
            return []

    async def get_signed_url(
        self, storage_account: str, container_name: str, file_name: str
    ) -> str | None:
        ...
        try:
            ...
            logger.error(f"Error getting signed URL: {e!s}")
            return None
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(
                        f"Request failed (attempt {attempt + 1}/{max_retries}), retrying in {wait_time:.2f}s: {e!s}"
                    )
                    await asyncio.sleep(wait_time)
                else:
                    logger.error(f"Request failed after {max_retries} attempts: {e!s}")
                    raise
    ...
    async def get_signed_url_upload(
        self, storage_account: str | None, container_name: str | None, file_name: str
    ) -> str | None:
        ...
        try:
            ...
            logger.error(f"Error getting upload signed URL: {e!s}")
            return None
