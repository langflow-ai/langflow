...
def upgrade() -> None:
    ...
    if not migration.table_exists("evaluation", conn):
        op.create_table(
            ...
            sa.Column("total_flow_tokens", sa.Integer(), nullable=True),
            sa.Column("total_llm_judge_tokens", sa.Integer(), nullable=True),
            sa.Column("llm_judge_prompt", sqlmodel.sql.sqltypes.AutoString(), nullable=True),
            sa.Column("llm_judge_model", sa.JSON(), nullable=True),
    ...
    if not migration.table_exists("evaluationresult", conn):
        op.create_table(
            ...
            sa.Column("flow_tokens", sa.Integer(), nullable=True),
            sa.Column("llm_judge_tokens", sa.Integer(), nullable=True),
            sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=True),
