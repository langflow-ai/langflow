...
def upgrade() -> None:
    ...
    if not migration.table_exists(conn=conn, name="subscription"):
        ...
        if "subscription" in table_names:
            indexes = inspector.get_indexes("subscription")
            index_names = [index["name"] for index in indexes]

            with op.batch_alter_table("subscription", schema=None) as batch_op:
                if "ix_subscription_flow_id" not in index_names:
    ...
    if not migration.table_exists(conn=conn, name="task"):
        ...
        if "task" in table_names:
            ...
            with op.batch_alter_table("task", schema=None) as batch_op:
                if "ix_task_assignee_id" not in index_names:
...
def downgrade() -> None:
    ...
    if "task" in table_names:
        ...
        with op.batch_alter_table("task", schema=None) as batch_op:
            ...
            if "ix_task_assignee_id" in index_names:
    ...
    if "subscription" in table_names:
        ...
        with op.batch_alter_table("subscription", schema=None) as batch_op:
            if "ix_subscription_flow_id" in index_names:
