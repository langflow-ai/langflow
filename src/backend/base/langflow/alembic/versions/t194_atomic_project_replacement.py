...
revision: str = "f194a1b2c3d4"
down_revision: str | None = "e6f9a2b4c8d1"
...
def upgrade() -> None:
    ...
    if not migration.table_exists("project_replacement_operation", conn):
        ...
        missing_columns = {
            column for column in expected_columns if not migration.column_exists("project_replacement_operation", column, conn)
        }
        if missing_columns:
            missing = ", ".join(sorted(missing_columns))
            raise RuntimeError(f"Existing 'project_replacement_operation' table is missing columns: {missing}")
