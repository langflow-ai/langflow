...
def update_input_types(build_config: dotdict) -> dotdict:
    ...
    input_types_key = "input_types"

    for key, value in build_config.items():
        if isinstance(value, dict):
            if input_types_key not in value:
                value[input_types_key] = []
        elif hasattr(value, input_types_key):
            if getattr(value, input_types_key) is None:
                setattr(value, input_types_key, [])
