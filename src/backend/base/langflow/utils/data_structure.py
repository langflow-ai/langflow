...
def get_type_str(value: Any) -> str:
    ...
    if value is None:
        ...
        if any(date_pattern in value.lower() for date_pattern in 
               ["date", "time", "yyyy", "mm/dd", "dd/mm", "yyyy-mm"]):
...
def analyze_value(value: Any, max_depth: int = 10, current_depth: int = 0, 
                 path: str = "", size_hints: bool = True, 
                 include_samples: bool = True) -> Union[str, dict]:
    """
    ...
    try:
        if isinstance(value, (list, tuple, set)):
            ...
            if include_samples and length > 0 and isinstance(value[0], (dict, list)) and current_depth < max_depth - 1:
                sample = analyze_value(value[0], max_depth, current_depth + 1, 
                                    f"{path}[0]", size_hints, include_samples)
            ...
            for k, v in value.items():
                ...
                try:
                    result[k] = analyze_value(v, max_depth, current_depth + 1, 
                                           new_path, size_hints, include_samples)
...
def get_data_structure(data_obj: Union[Data, dict], 
                      max_depth: int = 10,
                      size_hints: bool = True,
                      include_sample_values: bool = False,
                      include_sample_structure: bool = True,
                      max_sample_size: int = 3) -> dict:
    """
    ...
    result = {
        "structure": analyze_value(data, max_depth=max_depth, size_hints=size_hints, 
