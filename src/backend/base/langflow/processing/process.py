...
def apply_tweaks(node: dict[str, Any], node_tweaks: dict[str, Any]) -> None:
    ...
    for tweak_name, tweak_value in node_tweaks.items():
        ...
        field_entry = template_data[tweak_name]
        field_type = field_entry.get("type", "")
        ...
        if field_type == "NestedDict":
            ...
            if len(tweak_value) == 1 and "value" in tweak_value:
                field_entry["value"] = tweak_value["value"]
            else:
                field_entry["value"] = tweak_value
