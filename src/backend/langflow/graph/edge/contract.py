...
from langflow.graph.schema import Payload
...
class ContractEdge(Edge):
    ...
    async def process_runnable(self, message: Payload, **kwargs):
        """
        ...
        result, extra = await self.run_source(message, **kwargs)
        ...
        # of that param
        if not isinstance(self.target.params.get("root_field"), str):
            self.target.params["root_field"] = message.result_pairs[-1].result
        # ------------------ RUNNABLE EDGE ------------------
        self.is_fulfilled = True

    async def run_source(self, message: Payload, **kwargs):
        """
        ...
        if isinstance(self.source, LangChainVertex):
            ...
            result = runnable_object(last_result_pair.result)
            extra = None
        return result, extra
    ...
    def __repr__(self) -> str:
        param = "Run" if self.is_runnable else self._target_param_key
        return f"{self.source.vertex_type} -[{param}]-> {self.target.vertex_type}"
