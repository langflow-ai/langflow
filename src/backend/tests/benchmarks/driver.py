...
the MEAS-01..MEAS-07 scenario set. Produces:
  ...
  - `{baseline_dir}/baseline-YYYY-MM-DD.json`: machine-readable sidecar with
    `measurement_mode: "bytecode_compile_delta"` at the top level (D-11a).
...
differentiator per D-12a (dep-install is not the MEAS-07 signal; bytecode
compile cost is).
...
# Measurement-mode tag (D-11a). The value is fixed for this driver version; Phase 6 diffs
...
# ---------------------------------------------------------------------------
# Image tag mapping. Option A1 per D-11a/D-12a.
# ---------------------------------------------------------------------------
...
def run_hyperfine_local(scenario: Scenario, *, output_dir: Path, warmup: int, min_runs: int, max_runs: int) -> dict:
    ...
    Non-authoritative per D-10: the baseline document marks any local-mode results as smoke only.
    """
    export_path = output_dir / f"{scenario.name}.json"
    export_path.parent.mkdir(parents=True, exist_ok=True)
...
def _write_regression_comment(*, rows: list[_VerifyRow], thresholds: dict, output_path: Path) -> None:
    ...
    lines.append("")
    lines.append(
        "Per D-16: to merge anyway, apply the `benchmarks:override` label AND document the "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="driver",
        description="Cold-start benchmark driver (MEAS-01..MEAS-07). Produces baseline-YYYY-MM-DD artifacts.",
    )
