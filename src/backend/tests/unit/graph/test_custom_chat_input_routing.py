...
def test_set_inputs_routes_chat_input_to_custom_input_vertex():
    ...
    vertex.is_input = True
    vertex.data = {"node": {"template": {"input_value": {}}}}

    graph.get_vertex = MagicMock(return_value=vertex)

    graph._set_inputs(
        input_components=[],
        inputs={"input_value": "hello from api"},
        input_type="chat",
    )

    vertex.update_raw_params.assert_called_once_with(
        {"input_value": "hello from api"},
        overwrite=True,
    )
