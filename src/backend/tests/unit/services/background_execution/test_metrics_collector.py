...
async def test_queue_metrics_count_only_workflow_jobs():
    """Knowledge-base ingestion shares the job table but is never claimed here.

    ``claim_next_queued_lease`` only claims WORKFLOW rows, so counting every job
    type reported a backlog this queue would never drain: a stuck ingestion row
    would inflate the depth gauge and hold the oldest-queued age up forever.
    """
