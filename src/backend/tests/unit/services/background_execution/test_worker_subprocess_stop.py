...
An ACTUAL ``langflow worker`` OS subprocess runs a slowish multi-vertex real
flow; the API-side facade calls ``stop_job`` (the durable STOP signal, the
single source of truth); the SEPARATE worker process honors the stop at its
next vertex boundary and the job ends CANCELLED with the STOP signal consumed.
The database is the only shared channel — no broker anywhere.

Requires real Postgres (LANGFLOW_TEST_DATABASE_URI). Skips otherwise. Every
wait is bounded.
"""
...
async def test_real_worker_subprocess_honors_durable_stop():
    ...
    try:
        ...
        backend = DBBackgroundQueue(job_service=harness.job_service, owner="api:stop")
        ...
        leftover = [s for s in await harness.job_service.unconsumed_signals(job_id) if s.signal_type == SignalType.STOP]
