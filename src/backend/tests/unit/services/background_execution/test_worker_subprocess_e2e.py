...
An ACTUAL ``subprocess.Popen(["uv", "run", "langflow", "worker"])`` lease-claims
a job off the shared Postgres (the database IS the queue — no broker), builds a
REAL ChatInput->ChatOutput graph, runs it to COMPLETED, and the API-side facade
reattaches and replays the durable milestones the worker persisted. Then a
second job is killed with ``kill -9`` mid-flight and the lease watchdog
reconciles the stranded job.

Requires LANGFLOW_TEST_DATABASE_URI (real Postgres). Skips otherwise. Every
wait is bounded.
"""
...
async def test_real_worker_subprocess_runs_job_and_facade_reattaches():
    ...
    pg = _require_real_pg()
    harness = await setup_worker_harness(pg)
    try:
        ...
        # Sanity: the API process did NOT run it — the QUEUED row is the queue entry.
        ...
        # API-side facade (scaled DB backend, fresh instance) reattaches and
        # replays the durable milestones the worker wrote — cross-process, off
        # the shared database alone.
        backend_b = DBBackgroundQueue(job_service=harness.job_service, owner="api:b", poll_interval_s=0.05)
...
async def test_real_worker_kill9_midjob_reconciled_by_watchdog():
    ...
    try:
        ...
        # The durable row is now a stranded IN_PROGRESS orphan whose lease was
        # last stamped by the dead worker.
        ...
        backend = DBBackgroundQueue(job_service=harness.job_service, owner="worker:reconciler")
