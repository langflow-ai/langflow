"""DB backend: facade stop_job (scaled) lands a STOP the worker honors -> CANCELLED.

The facade's scaled stop_job writes the durable ExecutionSignal(STOP), the single
source of truth (no pub/sub fast-path anywhere). The worker's JobRunner polls
unconsumed_signals at vertex boundaries and cooperatively cancels. This proves
the control path end-to-end: stop -> worker sees STOP -> job CANCELLED, with the
STOP signal stamped consumed. Stop latency is one vertex-boundary poll, bounded
by the run's durable-frame cadence.
"""
...
def _slow_source_factory(in_flight: asyncio.Event):
    ...
    def _factory(**_kw):
        async def _source(**_kwargs) -> AsyncIterator[tuple[bytes, str]]:
            # Emit a durable frame, signal the test that the run is in-flight,
            # then keep emitting durable frames slowly so the JobRunner polls
            # the STOP signal at a boundary and cancels before the natural end.
...
async def test_scaled_stop_cancels_the_running_job(active_user):
    ...
    async def drive_stop_then_wait():
        ...
        # Wait until the durable row reaches a terminal state, then end the loop.
        for _ in range(200):
