...
class TestLangfuseConcurrency:
    ...
    async def test_parallel_tool_start_without_lock_fails(self):
        ...
        await asyncio.gather(
            tool_start("weather_tool"),
            tool_start("search_tool")
        )
    ...
    def test_concurrent_parent_creation_with_threads(self):
        ...
        threads = [
            threading.Thread(target=tool_start, args=(i,))
            for i in range(10)
        ]
    ...
    async def test_agent_with_parallel_tools_scenario(self):
        ...
        await asyncio.gather(
            execute_tool("weather_api", 0.01),
            execute_tool("search_api", 0.02),
            execute_tool("calculator", 0.01)
        )
    ...
    async def test_streaming_concurrent_tokens(self):
        ...
        await asyncio.gather(*[
            process_token(token, llm_run_id)
            for token in tokens
        ])
