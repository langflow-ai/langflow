...
class TestLangfuseTracer:
    ...
    def test_get_langchain_callback_uses_trace_context(self, mock_callback_class, mock_langfuse_class):
        """Test get_langchain_callback passes trace_context with hex ID.

        Verifies that the callback handler receives the trace_id in
        the v3 trace_context format.
        """
        from langflow.services.tracing.langfuse import LangFuseTracer
        ...
        # Verify CallbackHandler was called with trace_context
        mock_callback_class.assert_called_once_with(trace_context={"trace_id": trace_id_hex})
