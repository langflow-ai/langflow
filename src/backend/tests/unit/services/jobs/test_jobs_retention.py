"""Retention: age-based purge of terminal jobs and their child rows.

Every flow run writes a job row and nothing else deletes it, so the table grows
forever without a purge, and job_events adds a row per durable milestone of a
background run. The child tables carry job_id as a plain indexed column with NO
foreign key, so nothing cascades and a purge that forgets them leaves orphans
no query will ever reach again.
"""
