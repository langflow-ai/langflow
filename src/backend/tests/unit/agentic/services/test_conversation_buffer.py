...
class TestPushAndGetRecent:
    ...
    def test_get_recent_should_return_turns_in_oldest_first_order(self):
        ...
        assert buf.get_recent("s1") == [t1, t2, t3]

    def test_get_recent_with_explicit_limit_should_return_only_last_n(self):
        buf = ConversationBuffer()
        for i in range(5):
            buf.push("s1", ConversationTurn(user=f"u{i}", assistant=f"a{i}"))

        # Last 2 turns, oldest-first.
        recent = buf.get_recent("s1", limit=2)
...
class TestSessionIsolation:
    def test_get_recent_should_isolate_sessions(self):
        buf = ConversationBuffer()
        buf.push("alice", ConversationTurn(user="hi-a", assistant="ack-a"))
        buf.push("bob", ConversationTurn(user="hi-b", assistant="ack-b"))

        assert [t.user for t in buf.get_recent("alice")] == ["hi-a"]
        assert [t.user for t in buf.get_recent("bob")] == ["hi-b"]
...
class TestLRUEviction:
    def test_push_should_evict_oldest_session_when_max_sessions_exceeded(self):
        ...
        for i in range(MAX_SESSIONS + 1):
            buf.push(f"s{i}", ConversationTurn(user=f"u{i}", assistant=f"a{i}"))

        assert buf.get_recent("s0") == [], "Oldest session must be evicted"
        # The most recent session still has its turn.
        assert len(buf.get_recent(f"s{MAX_SESSIONS}")) == 1
    ...
    def test_push_should_refresh_session_on_use_so_recently_used_stays(self):
        ...
        buf.push("s0", ConversationTurn(user="u0-fresh", assistant="a0-fresh"))

        # Now adding one more new session should evict s1 (the new oldest),
        # not s0 which we just refreshed.
        buf.push("s-new", ConversationTurn(user="u-new", assistant="a-new"))

        assert buf.get_recent("s1") == []
        assert len(buf.get_recent("s0")) == 2  # original + fresh
        assert len(buf.get_recent("s-new")) == 1
