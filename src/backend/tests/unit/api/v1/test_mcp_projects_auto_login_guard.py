...
def _db_yielding(project, *, user=None) -> MagicMock:
...
async def test_verify_project_auth_public_project_keeps_caller_anonymous():
    ...
    try:
        with patch(f"{MODULE}.get_settings_service", return_value=_settings(auto_login=True, skip_auth=False)):
            result = await verify_project_auth(
                _db_yielding(project, user=owner), project.id, query_param=None, header_param=None
            )
