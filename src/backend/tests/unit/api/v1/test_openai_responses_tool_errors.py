...
``Message`` shape and pin the failure contract: the call is emitted once, with
``status="failed"`` and the error text, for both the plain ``function_call`` form and the
``tool_call.results`` form. The non-streaming path shares the contract.
"""
...
async def _stream(
    monkeypatch: pytest.MonkeyPatch, frames: list[Message], *, include: list[str] | None = None
) -> list[dict]:
    owner_id = uuid4()
    flow = _flow(owner_id)
...
async def test_stream_emits_failed_tool_call_as_function_call(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert events[-1]["event"] == "response.completed" or any(e["event"] == "response.completed" for e in events)
...
async def test_stream_successful_tool_call_is_unchanged(monkeypatch: pytest.MonkeyPatch) -> None:
    succeeded = ToolContent(name=TOOL_NAME, tool_input=TOOL_INPUT, output="3 documents", error=None, duration=5)
...
async def test_non_stream_marks_failed_tool_call(monkeypatch: pytest.MonkeyPatch, include: list[str] | None) -> None:
    owner_id = uuid4()
    flow = _flow(owner_id)
    message = _agent_message(_failed_tool_block(), state="complete", text="The flow failed.")
