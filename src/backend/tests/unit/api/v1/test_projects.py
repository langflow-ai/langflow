...
async def test_create_project_with_cyrillic_name(client: AsyncClient, logged_in_headers):
    cyrillic_case = {
        "name": CYRILLIC_NAME,
    ...
    response = await client.post("api/v1/projects/", json=cyrillic_case, headers=logged_in_headers)
    result = response.json()

    assert response.status_code == status.HTTP_201_CREATED
    assert result["name"] == CYRILLIC_NAME
    assert result["description"] == CYRILLIC_DESC


async def test_update_project_with_cyrillic_name(client: AsyncClient, logged_in_headers, basic_case):
    # Create a project first
    response_ = await client.post("api/v1/projects/", json=basic_case, headers=logged_in_headers)
    id_ = response_.json()["id"]

    # Update with Cyrillic
    update_case = {"name": CYRILLIC_NAME, "description": CYRILLIC_DESC}
    response = await client.patch(f"api/v1/projects/{id_}", json=update_case, headers=logged_in_headers)
    result = response.json()
...
async def test_download_project(client: AsyncClient, logged_in_headers, basic_case):
    # Create a project first
    response_ = await client.post("api/v1/projects/", json=basic_case, headers=logged_in_headers)
    id_ = response_.json()["id"]

    # Download the project
    response = await client.get(f"api/v1/projects/download/{id_}", headers=logged_in_headers)
...
async def test_upload_project(client: AsyncClient, logged_in_headers, basic_case):
    # Create a project first
    response_ = await client.post("api/v1/projects/", json=basic_case, headers=logged_in_headers)
    id_ = response_.json()["id"]

    # Download the project to get a valid zip
    download_response = await client.get(f"api/v1/projects/download/{id_}", headers=logged_in_headers)
    zip_content = download_response.content

    # Upload the project
    files = {"file": ("project.zip", zip_content, "application/x-zip-compressed")}
...
async def test_list_starter_projects(client: AsyncClient, logged_in_headers):
    response = await client.get("api/v1/projects/starter-projects/", headers=logged_in_headers)
    result = response.json()

    assert response.status_code == status.HTTP_200_OK
    assert isinstance(result, list)
    # Starter projects should exist
    assert len(result) > 0


async def test_create_project_from_starter(client: AsyncClient, logged_in_headers):
    # Get list of starter projects
    list_response = await client.get("api/v1/projects/starter-projects/", headers=logged_in_headers)
    starter_projects = list_response.json()

    if len(starter_projects) > 0:
        # Use the first starter project
        starter_project = starter_projects[0]
        project_name = starter_project.get("name", "Test Starter Project")

        # Create project from starter
        create_data = {
            "name": f"Test {project_name}",
            "description": "Created from starter",
            "flows_list": [],
            "components_list": [],
        }

        response = await client.post("api/v1/projects/", json=create_data, headers=logged_in_headers)
        assert response.status_code == status.HTTP_201_CREATED


async def test_project_flows_relationship(client: AsyncClient, logged_in_headers, basic_case):
    # Create a project
    ...
    flow_data = {
        "name": "Test Flow",
        "description": "Test flow in project",
        "data": {},
    ...
    # Verify the flow is associated with the project
    flow_id = flow_response.json()["id"]
    get_flow_response = await client.get(f"api/v1/flows/{flow_id}", headers=logged_in_headers)
    flow_result = get_flow_response.json()
