...
@pytest.mark.asyncio
async def test_cached_session_graph_is_rebuilt_when_admin_only_applies(monkeypatch):
    """A cached graph predating the policy is rebuilt from trusted component source."""
    from langflow.api.v1 import endpoints
    ...
    monkeypatch.setattr(endpoints, "prepare_flow_build_for_user", AsyncMock(return_value=sanitized))
    monkeypatch.setattr(endpoints, "_graph_executes_as_actor", lambda *_args, **_kwargs: True)
    ...
    assert captured["runtime_graph"] is not cached_graph
    assert captured["rebuilt_from_payload"] == sanitized
