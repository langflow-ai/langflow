...
async def test_a_refused_save_is_recorded_with_its_reason(client: AsyncClient, logged_in_headers, audit_on):  # noqa: ARG001
    flow = await _create_flow(client, logged_in_headers)
    stale_token = flow["version_token"]

    accepted = await client.patch(
        f"api/v1/flows/{flow['id']}",
        json={"data": _graph("first-writer")},
        headers={**logged_in_headers, "If-Match": stale_token},
    )
    assert accepted.status_code == status.HTTP_200_OK

    refused = await client.patch(
        f"api/v1/flows/{flow['id']}",
        json={"data": _graph("second-writer")},
        headers={**logged_in_headers, "If-Match": stale_token},
    )
    ...
    assert len(denied) == 1
    assert denied[0]["payload"]["reason"] == "version_conflict"


async def test_only_one_of_many_racing_writers_is_recorded_as_succeeding(
    client: AsyncClient,
    logged_in_headers,
    audit_on,  # noqa: ARG001
):
    """The log would lie if two concurrent writers both looked accepted."""
    flow = await _create_flow(client, logged_in_headers)
    token = flow["version_token"]

    async def save(label: str):
        return await client.patch(
            f"api/v1/flows/{flow['id']}",
            json={"data": _graph(label)},
            headers={**logged_in_headers, "If-Match": token},
        )

    responses = await asyncio.gather(*(save(f"writer-{i}") for i in range(8)), return_exceptions=True)
    accepted = [r for r in responses if not isinstance(r, Exception) and r.status_code == status.HTTP_200_OK]

    entries = await _trail(client, logged_in_headers, flow["id"])
    updates = [e for e in entries if e["event"] == "langflow.audit.flow.updated"]

    assert len(accepted) == 1, "multi-edit must elect exactly one writer"
    assert len(updates) == len(accepted)
