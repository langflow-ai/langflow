...
def test_get_embeddings_falls_back_when_metadata_stripped(mock_get_class, mock_get_api_key):
    """Selections persisted via the generic ``/models`` catalog lack the
    enriched ``embedding_class`` and ``param_mapping`` metadata. Instantiation
    should derive both from ``EMBEDDING_PROVIDER_CLASS_MAPPING`` /
    ``EMBEDDING_PARAM_MAPPINGS`` rather than failing — otherwise KB
