...
async def test_webhook_endpoint_requires_api_key_when_auto_login_false(client, added_webhook_test):
    """Test that webhook endpoint requires API key when AUTO_LOGIN=false (default test configuration)."""
    ...
    # Should fail without API key when auto_login=false (default test config)
    response = await client.post(endpoint, json=payload)
    assert response.status_code == 403
    assert "API key required when auto login is disabled" in response.json()["detail"]
...
async def test_webhook_with_auto_login_enabled(client, added_webhook_test):
    """Test webhook behavior when AUTO_LOGIN=true - should work without API key."""
    # Mock the settings service to return AUTO_LOGIN=true
    from unittest.mock import patch

    with patch("langflow.services.auth.utils.get_settings_service") as mock_settings:
        mock_auth_settings = type("AuthSettings", (), {"AUTO_LOGIN": True})()
        ...
        # Should work without API key when AUTO_LOGIN=true
        response = await client.post(endpoint, json=payload)
        assert response.status_code == 202
