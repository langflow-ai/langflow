...
class TestXquikComponent(ComponentTestBaseWithoutClient):
    ...
    @patch("lfx.components.data_source.xquik.requests.get")
    def test_run_text_handles_non_json_response(self, mock_get, component_class, default_kwargs):
        component = component_class(**default_kwargs)
        response = Mock()
        response.json.side_effect = ValueError("not json")
        response.raise_for_status.return_value = None
        ...
        assert '"text": "plain response"' in result.text
        assert result.data["operation"] == XquikComponent.SEARCH_TWEETS
    ...
    def test_missing_path_identifier_raises(
        ...
        component = component_class(**{**default_kwargs, "operation": operation, field_name: ""})

        with pytest.raises(ValueError, match=expected_message):
            component._build_url(operation)

    def test_path_identifier_is_url_encoded(self, component_class, default_kwargs):
        component = component_class(
            **{
                **default_kwargs,
                "operation": XquikComponent.GET_USER,
                "user_identifier": "name/with space",
            }
        )
    ...
    @pytest.mark.parametrize(("configured", "expected"), [(0, 30), (-1, 1), (999, 300)])
    @patch("lfx.components.data_source.xquik.requests.get")
    def test_timeout_is_bounded(self, mock_get, component_class, default_kwargs, configured, expected):
        component = component_class(**{**default_kwargs, "timeout": configured})
        response = Mock()
        ...
        assert mock_get.call_args.kwargs["timeout"] == expected
