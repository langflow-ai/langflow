...
async def test_handle_on_tool_start_with_stream_tool_updates_true():
    ...
    send_message = AsyncMock(
        side_effect=lambda message, skip_db_update=False: message  # noqa: ARG005
    )
    tool_blocks_map = {}
    agent_message = Message(
        sender=MESSAGE_SENDER_AI,
    ...
    # Call with stream_tool_updates=True (keyword argument)
    updated_message, _start_time = await handle_on_tool_start(
        event, agent_message, tool_blocks_map, send_message, 0.0, stream_tool_updates=True
    )

    # Verify send_message was called with skip_db_update=False (immediate DB write)
    send_message.assert_called_once()
    call_args = send_message.call_args
    assert call_args.kwargs["skip_db_update"] is False
    assert isinstance(updated_message.content_blocks[0].contents[-1], ToolContent)
...
async def test_handle_on_tool_start_with_stream_tool_updates_false():
    ...
    # Call with stream_tool_updates=False (keyword argument)
    updated_message, _start_time = await handle_on_tool_start(
        event, agent_message, tool_blocks_map, send_message, 0.0, stream_tool_updates=False
    )

    # Verify send_message was called with skip_db_update=True (batched DB write)
    send_message.assert_called_once()
    call_args = send_message.call_args
    assert call_args.kwargs["skip_db_update"] is True
    assert isinstance(updated_message.content_blocks[0].contents[-1], ToolContent)
...
async def test_handle_on_tool_start_keyword_only_parameter():
    ...
    # Attempting to pass stream_tool_updates as positional should raise TypeError
    with pytest.raises(TypeError, match="takes 5 positional arguments but 6 were given"):
        await handle_on_tool_start(event, agent_message, tool_blocks_map, send_message, 0.0, True)  # noqa: FBT003
...
async def test_handle_on_tool_end_with_stream_tool_updates():
    ...
    # First create a tool with on_tool_start
    start_event = {
        "event": "on_tool_start",
        "name": "test_tool",
        "run_id": "test_run",
    ...
    agent_message, _ = await handle_on_tool_start(
        start_event, agent_message, tool_blocks_map, send_message, 0.0, stream_tool_updates=True
    )

    # Reset mock to check on_tool_end call
    send_message.reset_mock()

    # Now test on_tool_end with stream_tool_updates=True
    end_event = {"event": "on_tool_end", "name": "test_tool", "run_id": "test_run", "data": {"output": "result"}}

    _updated_message, _start_time = await handle_on_tool_end(
        end_event, agent_message, tool_blocks_map, send_message, 0.0, stream_tool_updates=True
    )

    # Verify send_message was called with skip_db_update=False
    send_message.assert_called_once()
    call_args = send_message.call_args
    assert call_args.kwargs["skip_db_update"] is False
...
async def test_handle_on_tool_error_with_stream_tool_updates():
    ...
    agent_message, _ = await handle_on_tool_start(
        start_event, agent_message, tool_blocks_map, send_message, 0.0, stream_tool_updates=False
    )

    # Reset mock to check on_tool_error call
    send_message.reset_mock()

    # Now test on_tool_error with stream_tool_updates=False
    error_event = {
        "event": "on_tool_error",
        "name": "test_tool",
        "run_id": "test_run",
    ...
    _updated_message, _start_time = await handle_on_tool_error(
        error_event, agent_message, tool_blocks_map, send_message, 0.0, stream_tool_updates=False
    )

    # Verify send_message was called with skip_db_update=True
    send_message.assert_called_once()
    call_args = send_message.call_args
    assert call_args.kwargs["skip_db_update"] is True
...
async def test_process_agent_events_with_stream_tool_updates():
    ...
    events = [
        ...
        {"event": "on_tool_end", "name": "test_tool", "run_id": "test_run", "data": {"output": "result"}},
        {
            "event": "on_chain_end",
            "name": "agent",
            "data": {"output": AgentFinish(return_values={"output": "Done"}, log="complete")},
        },
    ]

    # Call with stream_tool_updates=True
    result = await process_agent_events(
        create_event_iterator(events), agent_message, send_message, stream_tool_updates=True
    )

    # Verify multiple send_message calls were made (immediate updates)
    assert send_message.call_count >= 2
    # Check that at least one call had skip_db_update=False (immediate write)
    immediate_writes = [call for call in send_message.call_args_list if not call.kwargs.get("skip_db_update", True)]
    assert len(immediate_writes) > 0
    assert result.properties.state == "complete"
