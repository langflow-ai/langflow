...
def test_local_db_collection_name_is_scoped_by_user(tmp_path: Path) -> None:
    """Two different user ids must produce distinct local Chroma collection names."""
...
def test_local_db_falls_back_to_original_name_without_user(tmp_path: Path) -> None:
    """A missing runtime user id falls back to the original collection name (fail-open, documented)."""
    mock_chroma = MagicMock()
    mock_chroma.get.return_value = {"ids": [], "documents": [], "metadatas": []}

    with patch("langchain_chroma.Chroma", return_value=mock_chroma) as mock_chroma_class:
        component = LocalDBComponent().set(
            collection_name="shared_collection",
            persist_directory=str(tmp_path / "shared"),
            embedding=None,
            ingest_data=[],
    ...
    assert mock_chroma_class.call_args.kwargs["collection_name"] == "shared_collection"


def test_local_db_same_apparent_namespace_isolated_by_user(tmp_path: Path) -> None:
    """Two users sharing a persist directory and collection name cannot read each other's data."""
