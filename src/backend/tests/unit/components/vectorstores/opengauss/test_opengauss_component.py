...
class TestOpenGaussVectorStoreComponent(ComponentTestBaseWithoutClient):
    @pytest.fixture
    ...
    def default_kwargs(self) -> dict[str, Any]:
        return {
            "connection_string": "postgresql://gaussdb:openGauss@123@localhost:5432/postgres",
    ...
    def test_init(self, component_class: type[Any]) -> None:
        component = component_class()
        assert component.display_name == "openGauss"
        assert component.name == "opengauss"
        assert component.icon == "openGauss"

    @patch("lfx.components.opengauss.opengauss.OpenGaussVectorStore.from_documents")
    @patch("psycopg2.pool.SimpleConnectionPool")
    def test_build_vector_store_with_data(
        self,
        mock_pool: MagicMock,
        mock_from_docs: MagicMock,
        ...
        component = component_class().set(**default_kwargs, ingest_data=[Data(text="test")])
        component.build_vector_store()
        mock_from_docs.assert_called_once()

    @patch("lfx.components.opengauss.opengauss.OpenGaussVectorStore._create_table")
    @patch("psycopg2.pool.SimpleConnectionPool")
    def test_build_vector_store_without_data(
        self,
        mock_pool: MagicMock,
    ...
    @patch("lfx.components.opengauss.opengauss.OpenGaussVectorStore._create_table")
    @patch("lfx.components.opengauss.opengauss.OpenGaussVectorStore.similarity_search")
    @patch("psycopg2.pool.SimpleConnectionPool")
    def test_search_documents(
        ...
        component = component_class().set(**default_kwargs, search_query="test query", number_of_results=3)
        results = component.search_documents()
        assert len(results) == 1
        assert results[0].text == "result"
        mock_search.assert_called_once_with(query="test query", k=3)
    ...
    def test_search_documents_empty_query(
        ...
        component = component_class().set(**default_kwargs, search_query="")
        results = component.search_documents()
        assert results == []

    def test_extract_text_common_keys(self, component_class: type[Any]) -> None:
        from lfx.schema.data import Data
    ...
    def test_extract_text_empty(self, component_class: type[Any]) -> None:
        from lfx.schema.data import Data
    ...
    def test_connection_string_parsing(self, component_class: type[Any]) -> None:
    ...
    def test_connection_string_defaults(self, component_class: type[Any]) -> None:
        component = component_class().set(
    ...
    def test_inputs_defined(self, component_class: type[Any]) -> None:
        component = component_class()
        ...
        assert "embedding_dimension" in input_names
    ...
    def test_add_texts_batching(self, mock_pool: MagicMock, mock_exec: MagicMock) -> None:
        ...
        assert len(ids) == 30
        assert mock_exec.call_count == 2  # 30 texts / 20 batch_size = 2 batches
