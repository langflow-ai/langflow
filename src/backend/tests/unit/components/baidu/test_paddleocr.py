...
class TestPaddleOCRComponent(ComponentTestBaseWithoutClient):
    @pytest.fixture
    def component_class(self):
        return PaddleOCRComponent
    ...
    def _mock_paddleocr_api(self, monkeypatch):
        def fake_submit(_self, **_kwargs):
            return "ocr-job-id"
    ...
    def test_paddleocr_ocr_mode_processes_text(self, monkeypatch, tmp_path):
        file_path = tmp_path / "sample.png"
        file_path.write_bytes(b"fake image")
    ...
    def test_paddleocr_document_parsing_processes_markdown(self, monkeypatch, tmp_path):
    ...
    def test_paddleocr_document_parsing_falls_back_to_ocr_results(self, tmp_path):
        file_path = tmp_path / "sample.pdf"
        file_path.write_bytes(b"fake pdf")
    ...
    def test_paddleocr_requires_access_token(self, tmp_path):
        file_path = tmp_path / "sample.png"
        file_path.write_bytes(b"fake image")
    ...
    def test_paddleocr_formats_auth_error(self):
        component = PaddleOCRComponent()
    ...
    def test_task_type_updates_model_options(self):
        component = PaddleOCRComponent()
        build_config = {
