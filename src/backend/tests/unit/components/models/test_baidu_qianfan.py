...
@pytest.mark.api_key_required
def test_custom_endpoint(qianfan_credentials):
    ...
    assert model.endpoint == "https://custom.endpoint.example"


@pytest.mark.api_key_required
@pytest.mark.parametrize("temp", [0.0, 0.5, 1.0])
def test_temperature_settings(qianfan_credentials, temp):
...
@pytest.mark.api_key_required
def test_model_parameters(qianfan_credentials):
    """Test all model parameters are correctly set"""
    test_params = {"model": "ERNIE-Bot-turbo", "temperature": 0.8, "top_p": 0.9, "penalty_score": 1.2}

    component = QianfanChatEndpointComponent(
        qianfan_ak=qianfan_credentials["ak"], qianfan_sk=qianfan_credentials["sk"], **test_params
    )
