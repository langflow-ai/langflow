...
class TestRunFlowBaseComponentFlowRetrieval:
    ...
    @pytest.mark.asyncio
    async def test_get_graph_uses_cache_when_available(self):
        """Test that get_graph returns cached graph when available."""
        component = RunFlowBaseComponent()
        component._user_id = str(uuid4())
        ...
        component._flow_cache_call = Mock(return_value=mock_graph)
...
class TestRunFlowBaseComponentFlowCaching:
    ...
    def test_build_flow_cache_key_with_name_and_id(self):
        """Test building cache key with both name and ID."""
        component = RunFlowBaseComponent()
        component._user_id = str(uuid4())
        flow_name = "test_flow"
        flow_id = str(uuid4())

        key = component._build_flow_cache_key(flow_name=flow_name, flow_id=flow_id)

        assert f"run_flow:{component._user_id}:{flow_id}:{flow_name}" == key
    ...
    def test_build_flow_cache_key_without_user_id_raises_error(self, mock_user_id):  # noqa: ARG002
        ...
        with pytest.raises(ValueError, match="Flow name or id, and user ID are required"):
            component._build_flow_cache_key(flow_name="test")

    def test_build_flow_cache_key_without_name_or_id_raises_error(self):
        """Test that building cache key without name or ID raises ValueError."""
        component = RunFlowBaseComponent()
        component._user_id = str(uuid4())

        with pytest.raises(ValueError, match="Flow name or id, and user ID are required"):
            component._build_flow_cache_key(flow_name=None, flow_id=None)
    ...
    def test_get_cached_flow_returns_none_on_cache_miss(self):
        ...
        with patch.object(component, "_build_flow_cache_key") as mock_build_key:
            ...
            result = component._get_cached_flow(flow_name="test", flow_id=None)
