...
def test_published_images_enable_multi_tenant_hardening(dockerfile: str, target: str | None) -> None:
...
def test_core_and_full_targets_share_one_runtime_hardening_contract() -> None:
    ...
    assert {key: stage_envs["core"].get(key) for key in EXPECTED_RUNTIME_ENV} == EXPECTED_RUNTIME_ENV
    assert {key: stage_envs["full"].get(key) for key in EXPECTED_RUNTIME_ENV} == EXPECTED_RUNTIME_ENV

    source = dockerfile.read_text(encoding="utf-8")
    for key in EXPECTED_RUNTIME_ENV:
        assert source.count(f"{key}=") == 1, f"{key} must be declared only in the shared runtime stage"
