...
async def test_openai_streaming_format_comparison(
    ...
    if len(openai_actual_tool_events) > 0:
        ...
        if len(our_function_events) > 0:
            print(f"✅ Our API also produced function call events")
            print(f"✅ Both APIs support function call streaming")
        else:
            print(f"❌ Our API did not produce function call events")
            pytest.fail("Our API should produce function call events when OpenAI does")
    else:
        print(f"ℹ️  No function calls were made by OpenAI")
