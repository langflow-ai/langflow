...
from langflow.components.helpers import MergeDataComponent
...
async def test_from_data():
    outputs = await run_single_component(ParseJSONDataComponent, inputs={
    ...
    outputs = await run_single_component(ParseJSONDataComponent, inputs={
        "input_value": ComponentInputHandle(
            clazz=TextToData, inputs={"text_data": ['{"key":[{"field1": 1, "field2": 2}]}'], "is_json": True}, output_name="from_text"
        ),
...
async def test_from_message():
    outputs = await run_single_component(ParseJSONDataComponent, inputs={
        "input_value": ComponentInputHandle(
                clazz=ChatInput, inputs={}, output_name="message"
            ),
        "query": ".[0].key"
