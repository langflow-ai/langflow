...
async def test_get_server_info(mcp_client):
    ...
    # Catch specific expected exceptions for optional tools
    try:
        from mcp.types import JSONRPCError

        expected_exceptions = (JSONRPCError, ValueError, KeyError, AttributeError)
    except ImportError:
        expected_exceptions = (ValueError, KeyError, AttributeError)

    try:
        result = await client.run_tool("get_server_info", {})
    except expected_exceptions as e:
        # Some implementations might not have get_server_info, so we'll skip for expected errors
...
async def test_client_reconnection(mcp_client):
    ...
    if transport == "stdio":
        ...
        pytest.skip(f"Reconnection test not implemented for {transport}")
    elif transport == "sse":
