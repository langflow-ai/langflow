...
async def superuser(
    ...
    """Creates a superuser for admin access tests."""
    user_id = uuid4()
    async with session_scope() as session:
        user = User(
            id=user_id,
            username=f"test_superuser_{user_id}",
    ...
    async with session_scope() as session:
        ...
        keys_to_delete = (await session.exec(stmt)).all()
        for key in keys_to_delete:
            await session.delete(key)
...
@pytest.fixture
async def created_superuser_api_key(superuser: User) -> AsyncGenerator[ApiKey, None]:
    """Creates and yields an API key for the superuser."""
    raw_key = f"superuser-key-{uuid4()}"
    hashed_key = get_password_hash(raw_key)
    api_key = ApiKey(
        user_id=superuser.id,
        name="Test API Key for Superuser",
        api_key=raw_key,
        hashed_api_key=hashed_key,
    )
...
@pytest.fixture
def superuser_api_key(created_superuser_api_key: ApiKey) -> str:
    """Provides the API key string for the superuser."""
    return created_superuser_api_key.api_key
