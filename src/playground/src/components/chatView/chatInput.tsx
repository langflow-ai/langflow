...
import { ChatInputType, FilePreviewType } from "./types";
import useFileSizeValidator from "src/hooks/use-file-size-validator";
import { ALLOWED_IMAGE_INPUT_EXTENSIONS } from "src/constants";
import TextAreaWrapper from "./textAreaWrapper";
import useFocusOnUnlock from "src/hooks/use-focus-unlock";
import useAutoResizeTextArea from "src/hooks/use-auto-resize-text-area";
...
export default function ChatInput({
  ...
  const onFileUploadError = usePlaygroundStore((state) => state.onFileUploadError);
  const { validateFileSize } = useFileSizeValidator(onFileUploadError);
  ...
  const handleFileChange = async (
    ...
    if (file) {
      ...
      if (
        !fileExtension ||
        !ALLOWED_IMAGE_INPUT_EXTENSIONS.has(fileExtension)
      ) {
        onFileUploadError?.("Error uploading file");
        return;
      }
      ...
      mutate(
        { file, id: currentFlowId },
        {
          ...
          onError: (error) => {
            ...
            onFileUploadError?.(error.response?.data?.detail);
          },
        },
      );
    }
