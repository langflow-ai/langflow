...
class JobService(Service):
    ...
    async def update_job_status(
        ...
        async with session_scope() as session:
            ...
            return await _jobs_crud().update_job_status(session, job_id, status, finished_timestamp=finished_at)
    ...
    async def get_latest_jobs_by_asset_ids(self, asset_ids: Sequence[UUID | str]) -> dict[UUID, Job]:
        ...
        async with session_scope() as session:
            return await _jobs_crud().get_latest_jobs_by_asset_ids(session, uuid_asset_ids)
