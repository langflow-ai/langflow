...
if TYPE_CHECKING:
    ...
    from langflow_services.tracing.base import BaseTracer
    from langflow_services.tracing.schema import Log


def _get_langsmith_tracer():
    from langflow_services.tracing.langsmith import LangSmithTracer

    return LangSmithTracer


def _get_langwatch_tracer():
    from langflow_services.tracing.langwatch import LangWatchTracer

    return LangWatchTracer


def _get_langfuse_tracer():
    from langflow_services.tracing.langfuse import LangFuseTracer

    return LangFuseTracer


def _get_arize_phoenix_tracer():
    from langflow_services.tracing.arize_phoenix import ArizePhoenixTracer

    return ArizePhoenixTracer


def _get_opik_tracer():
    from langflow_services.tracing.opik import OpikTracer

    return OpikTracer


def _get_traceloop_tracer():
    from langflow_services.tracing.traceloop import TraceloopTracer

    return TraceloopTracer


def _get_native_tracer():
    from langflow_services.tracing.native import NativeTracer

    return NativeTracer


def _get_openlayer_tracer():
    from langflow_services.tracing.openlayer import OpenlayerTracer
