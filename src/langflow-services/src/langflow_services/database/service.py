...
from langflow_services.database import models
from langflow_services.database.constants import (
    MIN_POSTGRESQL_MAJOR_VERSION,
    POSTGRESQL_VERSION_REQUIRED_MESSAGE,
)
from langflow_services.deps import session_scope
from langflow_services.providers import get_hook

if TYPE_CHECKING:
    from lfx.services.settings.service import SettingsService


def configure_windows_postgres_event_loop(*, source: str) -> None:
    """Invoke the host-registered Windows/Postgres event-loop hook when present.

    Host bootstrap registers the hook via ``langflow.services.utils``. When the
    hook is absent (isolated package use, partial tests), this is a no-op and
    never discovers ``langflow.*``.
    """
    configure = get_hook("configure_windows_postgres_event_loop")
...
class DatabaseService(Service):
    ...
    def __init__(
        ...
        *,
        ...
        # Preserve the historical ``DatabaseService(settings_service)`` signature.
        # When paths are omitted, resolve them from the host-registered provider
        # (set by langflow-base during ``register_all_service_factories``).
        if script_location is None or alembic_cfg_path is None:
            from langflow_services.database.factory import _resolve_alembic_paths

            resolved_script, resolved_cfg = _resolve_alembic_paths()
            script_location = script_location or resolved_script
            alembic_cfg_path = alembic_cfg_path or resolved_cfg
    ...
    async def teardown(self) -> None:
        ...
        try:
            from langflow_services.deps import get_settings_service, session_scope
            from langflow_services.providers import get_hook

            teardown_superuser = get_hook("teardown_superuser")
            if teardown_superuser is not None:
                # Host-owned AUTO_LOGIN cleanup (registered by langflow-base).
