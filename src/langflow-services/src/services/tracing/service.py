...
def _get_langsmith_tracer():
    from services.tracing.langsmith import LangSmithTracer

    return LangSmithTracer


def _get_langwatch_tracer():
    from services.tracing.langwatch import LangWatchTracer

    return LangWatchTracer


def _get_langfuse_tracer():
    from services.tracing.langfuse import LangFuseTracer

    return LangFuseTracer


def _get_arize_phoenix_tracer():
    from services.tracing.arize_phoenix import ArizePhoenixTracer

    return ArizePhoenixTracer


def _get_opik_tracer():
    from services.tracing.opik import OpikTracer

    return OpikTracer


def _get_traceloop_tracer():
    from services.tracing.traceloop import TraceloopTracer

    return TraceloopTracer


def _get_native_tracer():
    from services.tracing.native import NativeTracer

    return NativeTracer


def _get_openlayer_tracer():
    from services.tracing.openlayer import OpenlayerTracer
