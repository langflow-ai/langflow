...
from services.providers import get_hook
...
def configure_windows_postgres_event_loop(*, source: str) -> None:
    """Compat wrapper so callers/tests can patch this module attribute.

    Prefers the host-registered hook; falls back to the langflow helper when
    present so direct ``DatabaseService(...)`` construction in tests matches
    the pre-extraction always-call behavior.
    """
    ...
    try:
        helper = importlib.import_module("langflow.helpers.windows_postgres_helper")
    except ModuleNotFoundError:
        return
    helper.configure_windows_postgres_event_loop(source=source)
    ...
    def __init__(
        ...
        *,
        ...
        if script_location is None or alembic_cfg_path is None:
            from services.database.factory import _resolve_alembic_paths
