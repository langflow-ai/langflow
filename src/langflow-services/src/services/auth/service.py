...
from services.auth.constants import AUTO_LOGIN_ERROR, AUTO_LOGIN_SESSION_WARNING, AUTO_LOGIN_WARNING
...
from services.auth.external import (
    ExternalIdentity,
...
# Host hooks (registered by langflow-base during bootstrap).
_jit_user_defaults_hook = None
_get_user_by_flow_hook = None


def set_jit_user_defaults_hook(hook) -> None:
    """Register a host callback for post-JIT user folder/variable setup."""
    global _jit_user_defaults_hook
    _jit_user_defaults_hook = hook


def set_get_user_by_flow_id_hook(hook) -> None:
    """Register host helper ``get_user_by_flow_id_or_endpoint_name``."""
    global _get_user_by_flow_hook
    _get_user_by_flow_hook = hook


async def authenticate_api_key(session, api_key: str):
    return await get_crud("api_key").authenticate_api_key(session, api_key)
...
async def get_user_by_username(session, username: str):
    return await get_crud("user").get_user_by_username(session, username)


async def update_user_last_login_at(user_id, session):
    return await get_crud("user").update_user_last_login_at(user_id, session)


async def get_all_superusers(session):
    return await get_crud("user").get_all_superusers(session)


async def get_user_by_flow_id_or_endpoint_name(*args, **kwargs):
    if _get_user_by_flow_hook is None:
        msg = "get_user_by_flow_id hook is not registered"
        raise RuntimeError(msg)
    return await _get_user_by_flow_hook(*args, **kwargs)
...
class AuthService(BaseAuthService):
    ...
    async def _authenticate_with_token(self, token: str, db: AsyncSession) -> User:
        ...
        from services.auth.utils import ACCESS_TOKEN_TYPE, get_jwt_verification_key
    ...
    async def _initialize_jit_user_defaults(user: User, db: AsyncSession) -> None:
        if _jit_user_defaults_hook is None:
            msg = "JIT user defaults hook is not registered"
            raise RuntimeError(msg)
        await _jit_user_defaults_hook(user, db)
    ...
    def create_token(self, data: dict, expires_delta: timedelta):
        from services.auth.utils import get_jwt_signing_key
    ...
    async def create_refresh_token(self, refresh_token: str, db: AsyncSession):
        from services.auth.utils import get_jwt_verification_key
    ...
    def _get_fernet(self) -> Fernet:
        from services.auth.utils import ensure_fernet_key
